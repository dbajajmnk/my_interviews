...
**Integer promotions** is promotion of small integer types before arithmetic.

# 2. Why

Interviewers use **Integer promotions** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: promotion of small integer types before arithmetic. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

char/short often promote to int/unsigned int before operators
...
Use **Integer promotions** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Integer promotions**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
