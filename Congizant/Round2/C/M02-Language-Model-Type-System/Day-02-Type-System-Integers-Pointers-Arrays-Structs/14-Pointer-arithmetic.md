...
**Pointer arithmetic** is movement within one array object in units of element type.

# 2. Why

Interviewers use **Pointer arithmetic** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: movement within one array object in units of element type. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

only same-array and one-past arithmetic is defined
...
Use **Pointer arithmetic** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Pointer arithmetic**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
