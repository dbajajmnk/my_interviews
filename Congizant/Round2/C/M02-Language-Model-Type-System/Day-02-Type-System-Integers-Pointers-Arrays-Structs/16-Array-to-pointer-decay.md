# Topic 16 — Array-to-pointer decay
...
**Array-to-pointer decay** is implicit array expression conversion to first-element pointer.

# 2. Why

Interviewers use **Array-to-pointer decay** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: implicit array expression conversion to first-element pointer. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

function parameters lose array extent unless length is passed separately
...
Use **Array-to-pointer decay** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Array-to-pointer decay**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
