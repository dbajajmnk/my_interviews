...
**Opaque structs** is hiding representation behind incomplete public type.

# 2. Why

Interviewers use **Opaque structs** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: hiding representation behind incomplete public type. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

header declares handle, source file defines fields
...
Use **Opaque structs** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Opaque structs**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
