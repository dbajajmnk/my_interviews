# Topic 07 — Composition with structs
...
**Composition with structs** is the C engineering concept **Composition with structs**.

# 2. Why

Interviewers use **Composition with structs** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: the C engineering concept **Composition with structs**. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.
...
Use **Composition with structs** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
1. What is **Composition with structs**?
...
Write the smallest runnable example for **Composition with structs**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
