# Topic 11 — Dependency injection with function pointers
...
**Dependency injection with function pointers** is address of a compatible function used for callbacks/dispatch.

# 2. Why

Interviewers use **Dependency injection with function pointers** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: address of a compatible function used for callbacks/dispatch. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

exact compatible signatures are required
...
Use **Dependency injection with function pointers** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
1. What is **Dependency injection with function pointers**?
...
Write the smallest runnable example for **Dependency injection with function pointers**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
