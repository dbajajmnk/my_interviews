# Topic 16 — Discriminated union pattern
...
**Discriminated union pattern** is the C engineering concept **Discriminated union pattern**.

# 2. Why

Interviewers use **Discriminated union pattern** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: the C engineering concept **Discriminated union pattern**. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.
...
Use **Discriminated union pattern** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
1. What is **Discriminated union pattern**?
...
Write the smallest runnable example for **Discriminated union pattern**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
