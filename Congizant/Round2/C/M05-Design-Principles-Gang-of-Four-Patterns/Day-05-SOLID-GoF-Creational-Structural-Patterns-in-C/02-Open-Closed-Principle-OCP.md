...
**Open/Closed Principle (OCP)** is support extension at real variation points.

# 2. Why

Interviewers use **Open/Closed Principle (OCP)** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: support extension at real variation points. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

function tables/callbacks isolate expected variation
...
Use **Open/Closed Principle (OCP)** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Open/Closed Principle (OCP)**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
