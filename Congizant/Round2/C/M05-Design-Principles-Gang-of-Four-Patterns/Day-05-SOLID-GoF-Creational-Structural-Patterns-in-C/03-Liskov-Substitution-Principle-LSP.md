...
**Liskov Substitution Principle (LSP)** is interchangeable implementations preserve behavior.

# 2. Why

Interviewers use **Liskov Substitution Principle (LSP)** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: interchangeable implementations preserve behavior. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

function-table implementations must honor identical contracts
...
Use **Liskov Substitution Principle (LSP)** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Liskov Substitution Principle (LSP)**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
