...
**Interface Segregation Principle (ISP)** is clients depend on small capabilities.

# 2. Why

Interviewers use **Interface Segregation Principle (ISP)** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: clients depend on small capabilities. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

prefer focused callback tables/modules
...
Use **Interface Segregation Principle (ISP)** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Interface Segregation Principle (ISP)**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
