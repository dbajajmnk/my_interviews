...
**Dependency Inversion Principle (DIP)** is policy depends on supplied capabilities.

# 2. Why

Interviewers use **Dependency Inversion Principle (DIP)** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: policy depends on supplied capabilities. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

inject function pointers/context handles
...
Use **Dependency Inversion Principle (DIP)** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Dependency Inversion Principle (DIP)**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
