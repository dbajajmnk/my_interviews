...
**Factory Method** is centralizes creation behind a stable function.

# 2. Why

Interviewers use **Factory Method** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: centralizes creation behind a stable function. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

factory functions can return opaque handles or configured structs
...
```c
parser_t *parser_create(parser_kind_t kind);
```
...
Use **Factory Method** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Factory Method**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
