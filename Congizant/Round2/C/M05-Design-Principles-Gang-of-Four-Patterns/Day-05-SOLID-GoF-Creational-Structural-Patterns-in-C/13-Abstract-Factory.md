...
**Abstract Factory** is creates compatible families of components.

# 2. Why

Interviewers use **Abstract Factory** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: creates compatible families of components. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

a struct of creation callbacks can represent a provider factory
...
```c
typedef struct { storage_t *(*storage_create)(void); } factory_t;
```
...
Use **Abstract Factory** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Abstract Factory**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
