...
**Builder** is constructs complex state step-by-step.

# 2. Why

Interviewers use **Builder** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: constructs complex state step-by-step. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

config structs plus setter functions work when staged validation is useful
...
```c
request_builder_t b = request_builder_default();
```
...
Use **Builder** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Builder**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
