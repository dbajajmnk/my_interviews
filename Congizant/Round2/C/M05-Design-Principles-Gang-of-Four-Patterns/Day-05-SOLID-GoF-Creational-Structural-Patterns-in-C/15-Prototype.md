...
**Prototype** is creates a new object by explicit copying/cloning.

# 2. Why

Interviewers use **Prototype** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: creates a new object by explicit copying/cloning. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

clone functions must document deep/shallow ownership
...
```c
widget_t *widget_clone(const widget_t *src);
```
...
Use **Prototype** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Prototype**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
