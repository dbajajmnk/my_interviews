...
**Adapter** is translates one API into another contract.

# 2. Why

Interviewers use **Adapter** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: translates one API into another contract. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

wrapper functions map calls/data/errors
...
```c
int payment_adapter_charge(void *ctx, int cents);
```
...
Use **Adapter** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Adapter**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
