...
**Decorator** is adds behavior around an existing contract.

# 2. Why

Interviewers use **Decorator** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: adds behavior around an existing contract. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

wrap an inner handle/function table and delegate
...
```c
repo_t metrics_repo_wrap(repo_t inner);
```
...
Use **Decorator** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Decorator**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
