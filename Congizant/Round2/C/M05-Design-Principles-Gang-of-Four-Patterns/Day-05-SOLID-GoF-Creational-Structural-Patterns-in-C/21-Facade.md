...
**Facade** is offers one simple API over a subsystem.

# 2. Why

Interviewers use **Facade** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: offers one simple API over a subsystem. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

module-level façade coordinates internal collaborators
...
```c
int checkout_run(checkout_t *c,const order_t *o);
```
...
Use **Facade** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Facade**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
