...
**Flyweight** is shares immutable intrinsic state.

# 2. Why

Interviewers use **Flyweight** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: shares immutable intrinsic state. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

canonical metadata may be referenced by many objects when memory profiling justifies it
...
```c
const product_type_t *product_type_get(const char *code);
```
...
Use **Flyweight** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Flyweight**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
