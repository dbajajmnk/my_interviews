...
**Proxy** is controls access to another operation.

# 2. Why

Interviewers use **Proxy** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: controls access to another operation. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

function-table wrapper can authorize/log before delegation
...
```c
repo_t secured_repo_wrap(repo_t inner);
```
...
Use **Proxy** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Proxy**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
