...
Interviewers use **Chain of Responsibility** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: passes work through ordered handlers. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

handler array/list invokes each until handled/rejected
...
```c
typedef int (*handler_fn)(request_t*,void*);
```
...
Use **Chain of Responsibility** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Chain of Responsibility**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
