...
**Interpreter** is represents/evaluates a small grammar.

# 2. Why

Interviewers use **Interpreter** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: represents/evaluates a small grammar. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

tagged-union AST plus evaluator is natural in C
...
```c
bool expr_eval(const expr_t *e,const context_t *c);
```
...
Use **Interpreter** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Interpreter**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
