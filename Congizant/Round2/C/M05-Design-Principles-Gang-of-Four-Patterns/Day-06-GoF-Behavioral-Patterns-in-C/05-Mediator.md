...
**Mediator** is centralizes collaboration.

# 2. Why

Interviewers use **Mediator** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: centralizes collaboration. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

a context/module coordinates multiple dependencies
...
```c
int checkout_mediator_run(mediator_t *m,order_t *o);
```
...
Use **Mediator** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Mediator**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
