...
**Observer** is notifies subscribers.

# 2. Why

Interviewers use **Observer** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: notifies subscribers. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

callback list plus explicit unsubscribe/lifetime rules implements it
...
```c
typedef void (*event_cb)(const event_t*,void*);
```
...
Use **Observer** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Observer**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
