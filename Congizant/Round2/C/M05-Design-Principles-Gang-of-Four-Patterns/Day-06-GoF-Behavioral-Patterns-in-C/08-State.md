...
**State** is models behavior by explicit state.

# 2. Why

Interviewers use **State** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: models behavior by explicit state. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

enum+switch or per-state function table works well
...
```c
typedef enum { PENDING,PAID,CANCELLED } order_state_t;
```
...
Use **State** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **State**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
