...
**Template Method** is fixes algorithm steps while allowing callbacks.

# 2. Why

Interviewers use **Template Method** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: fixes algorithm steps while allowing callbacks. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

pass an ops table instead of inheritance
...
```c
int import_run(const import_ops_t *ops,void *ctx);
```
...
Use **Template Method** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Template Method**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
