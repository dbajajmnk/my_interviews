...
**Visitor** is adds operations over tagged structures.

# 2. Why

Interviewers use **Visitor** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: adds operations over tagged structures. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

tagged union + switch/callback table implements the intent
...
```c
void node_visit(const node_t *n,const visitor_t *v,void *ctx);
```
...
Use **Visitor** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Visitor**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
