...
**snprintf** is bounded formatted output.

# 2. Why

Interviewers use **snprintf** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: bounded formatted output. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

writes at most capacity and reports required size; truncation must be checked
...
```c
int n=snprintf(buf,cap,"%s",input);
```
...
Use **snprintf** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **snprintf**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
