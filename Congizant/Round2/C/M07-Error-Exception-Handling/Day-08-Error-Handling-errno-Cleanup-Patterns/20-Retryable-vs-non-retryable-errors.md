...
**Retryable vs non-retryable errors** is bounded repeat for transient failure.

# 2. Why

Interviewers use **Retryable vs non-retryable errors** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: bounded repeat for transient failure. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

requires idempotency, backoff/jitter and time budget
...
Use **Retryable vs non-retryable errors** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Retryable vs non-retryable errors**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
