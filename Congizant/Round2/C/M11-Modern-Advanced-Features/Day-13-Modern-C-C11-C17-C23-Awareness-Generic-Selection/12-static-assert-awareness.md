# Topic 12 — static_assert awareness
...
**static_assert awareness** is compile-time invariant check.

# 2. Why

Interviewers use **static_assert awareness** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: compile-time invariant check. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

translation fails when constant condition is false
...
Use **static_assert awareness** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
1. What is **static_assert awareness**?
...
Write the smallest runnable example for **static_assert awareness**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
