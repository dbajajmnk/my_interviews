...
**_Generic** is compile-time selection by expression type.

# 2. Why

Interviewers use **_Generic** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: compile-time selection by expression type. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

enables limited generic macros without runtime dispatch
...
```c
#define kind(x) _Generic((x),int:"int",default:"other")
```
...
Use **_Generic** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **_Generic**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
