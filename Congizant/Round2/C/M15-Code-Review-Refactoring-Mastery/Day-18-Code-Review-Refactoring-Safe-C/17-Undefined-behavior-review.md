# Topic 17 — Undefined behavior review
...
**Undefined behavior review** is operation outside C's defined semantics.

# 2. Why

Interviewers use **Undefined behavior review** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: operation outside C's defined semantics. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

compiler may optimize assuming it never occurs
...
Use **Undefined behavior review** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Undefined behavior review**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
