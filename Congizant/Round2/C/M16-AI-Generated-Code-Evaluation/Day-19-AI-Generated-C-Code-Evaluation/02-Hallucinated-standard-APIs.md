# Topic 02 — Hallucinated standard APIs
...
**Hallucinated standard APIs** is the C engineering concept **Hallucinated standard APIs**.

# 2. Why

Interviewers use **Hallucinated standard APIs** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: the C engineering concept **Hallucinated standard APIs**. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.
...
Use **Hallucinated standard APIs** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
1. What is **Hallucinated standard APIs**?
...
Write the smallest runnable example for **Hallucinated standard APIs**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
