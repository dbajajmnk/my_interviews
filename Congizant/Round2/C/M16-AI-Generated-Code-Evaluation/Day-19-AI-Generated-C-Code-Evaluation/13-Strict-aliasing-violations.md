# Topic 13 — Strict-aliasing violations
...
**Strict-aliasing violations** is the C engineering concept **Strict-aliasing violations**.

# 2. Why

Interviewers use **Strict-aliasing violations** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: the C engineering concept **Strict-aliasing violations**. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.
...
Use **Strict-aliasing violations** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
1. What is **Strict-aliasing violations**?
...
Write the smallest runnable example for **Strict-aliasing violations**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
