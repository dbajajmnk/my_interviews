# Topic 14 — Incorrect volatile use
...
**Incorrect volatile use** is access qualifier, not thread synchronization.

# 2. Why

Interviewers use **Incorrect volatile use** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: access qualifier, not thread synchronization. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

ensures observable accesses for special objects but not atomicity/happens-before
...
Use **Incorrect volatile use** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
1. What is **Incorrect volatile use**?
...
Write the smallest runnable example for **Incorrect volatile use**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
