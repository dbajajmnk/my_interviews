...
**Prepared statements** is SQL with values bound separately from syntax.

# 2. Why

Interviewers use **Prepared statements** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: SQL with values bound separately from syntax. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

prevents injection and gives explicit statement lifecycle
...
Use **Prepared statements** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Prepared statements**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
