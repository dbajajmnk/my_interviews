# Topic 09 — Format string vulnerabilities
...
**Format string vulnerabilities** is printf-family format interpreted as code-like directives.

# 2. Why

Interviewers use **Format string vulnerabilities** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: printf-family format interpreted as code-like directives. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

never let attacker data become the format string
...
Use **Format string vulnerabilities** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
1. What is **Format string vulnerabilities**?
...
Write the smallest runnable example for **Format string vulnerabilities**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
