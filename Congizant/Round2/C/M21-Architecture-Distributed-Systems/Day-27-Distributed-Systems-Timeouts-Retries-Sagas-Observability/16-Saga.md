...
**Saga** is distributed workflow with compensation.

# 2. Why

Interviewers use **Saga** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: distributed workflow with compensation. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

explicit state machine handles partial success
...
```c
typedef enum {SAGA_RESERVED,SAGA_CHARGED,SAGA_FAILED} saga_state_t;
```
...
Use **Saga** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
Write the smallest runnable example for **Saga**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
