# Topic 10 — Buffer overflow incident
...
**Buffer overflow incident** is write beyond object bounds.

# 2. Why

Interviewers use **Buffer overflow incident** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: write beyond object bounds. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

validate capacity and arithmetic before copy/write
...
```c
if (len >= cap) return ERR_TOO_LARGE;
```
...
Use **Buffer overflow incident** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
1. What is **Buffer overflow incident**?
...
Write the smallest runnable example for **Buffer overflow incident**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
