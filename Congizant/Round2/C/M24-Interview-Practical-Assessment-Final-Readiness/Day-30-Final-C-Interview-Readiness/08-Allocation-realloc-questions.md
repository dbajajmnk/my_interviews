# Topic 08 — Allocation/realloc questions
...
**Allocation/realloc questions** is resize allocation, potentially moving it.

# 2. Why

Interviewers use **Allocation/realloc questions** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: resize allocation, potentially moving it. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.

# 4. Engineering Explanation

on failure original remains valid; assign through temporary
...
```c
void *tmp=realloc(p,new_size); if(tmp) p=tmp;
```
...
Use **Allocation/realloc questions** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
1. What is **Allocation/realloc questions**?
...
Write the smallest runnable example for **Allocation/realloc questions**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
