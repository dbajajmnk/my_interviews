# Topic 10 — Undefined-behavior questions
...
**Undefined-behavior questions** is the C engineering concept **Undefined-behavior questions**.

# 2. Why

Interviewers use **Undefined-behavior questions** to test C semantics, bounds, integer/pointer behavior, ownership, cleanup, concurrency, security, and system reasoning.

# 3. Plain English

Plain English: the C engineering concept **Undefined-behavior questions**. In C, every buffer needs a known capacity, every pointer needs a valid lifetime, and every resource needs an owner.
...
Use **Undefined-behavior questions** only where it solves a concrete systems, embedded, networking, database, performance or safety requirement. **Evidence First:** do not turn course examples into invented project claims.
...
1. What is **Undefined-behavior questions**?
...
Write the smallest runnable example for **Undefined-behavior questions**, introduce one realistic bug, reproduce it with evidence, repair it, and state the review rule.
