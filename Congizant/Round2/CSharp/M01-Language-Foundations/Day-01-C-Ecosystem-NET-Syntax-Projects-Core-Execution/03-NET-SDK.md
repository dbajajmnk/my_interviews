...
**.NET SDK** is the C#/.NET engineering concept **.NET SDK**.

---

# 2. Why

Interviewers use **.NET SDK** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **.NET SDK** is about the C#/.NET engineering concept **.NET SDK**. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **.NET SDK** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **.NET SDK**.
...
Create the smallest runnable C# example demonstrating **.NET SDK**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **.NET SDK** in plain English.
