# Topic 02 — Type inference with var
...
**Type inference with var** is the C#/.NET engineering concept **Type inference with var**.

---

# 2. Why

Interviewers use **Type inference with var** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Type inference with var** is about the C#/.NET engineering concept **Type inference with var**. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Type inference with var** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Type inference with var**.
...
1. What is **Type inference with var**?
...
Create the smallest runnable C# example demonstrating **Type inference with var**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Type inference with var** in plain English.
