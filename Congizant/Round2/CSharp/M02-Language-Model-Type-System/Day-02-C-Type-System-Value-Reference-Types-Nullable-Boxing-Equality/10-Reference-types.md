...
**Reference types** is type whose variable contains a managed reference to an object.

---

# 2. Why

Interviewers use **Reference types** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Reference types** is about type whose variable contains a managed reference to an object. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Reference types** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Reference types**.
...
Create the smallest runnable C# example demonstrating **Reference types**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
