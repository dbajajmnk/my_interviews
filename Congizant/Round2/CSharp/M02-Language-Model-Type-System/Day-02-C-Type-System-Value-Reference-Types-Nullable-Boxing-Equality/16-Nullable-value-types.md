...
**Nullable value types** is type whose variable contains the value representation directly according to CLI semantics.

---

# 2. Why

Interviewers use **Nullable value types** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Nullable value types** is about type whose variable contains the value representation directly according to CLI semantics. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Nullable value types** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Nullable value types**.
...
Create the smallest runnable C# example demonstrating **Nullable value types**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Nullable value types** in plain English.
