# Topic 17 — Nullable reference types
...
**Nullable reference types** is compiler analysis of possible null references.

---

# 2. Why

Interviewers use **Nullable reference types** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Nullable reference types** is about compiler analysis of possible null references. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

nullable annotations/warnings help at compile time but do not change CLR reference representation
...
In a production C#/.NET system, **Nullable reference types** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Nullable reference types**.
...
Create the smallest runnable C# example demonstrating **Nullable reference types**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Nullable reference types** in plain English.
