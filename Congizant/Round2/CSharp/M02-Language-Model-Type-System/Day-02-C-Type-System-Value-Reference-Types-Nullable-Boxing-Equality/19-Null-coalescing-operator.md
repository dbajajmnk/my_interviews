# Topic 19 — Null-coalescing operator
...
**Null-coalescing operator** is the C#/.NET engineering concept **Null-coalescing operator**.

---

# 2. Why

Interviewers use **Null-coalescing operator** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Null-coalescing operator** is about the C#/.NET engineering concept **Null-coalescing operator**. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Null-coalescing operator** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Null-coalescing operator**.
...
1. What is **Null-coalescing operator**?
...
Create the smallest runnable C# example demonstrating **Null-coalescing operator**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Null-coalescing operator** in plain English.
