...
**Extension methods** is static method callable with instance syntax when in scope.

---

# 2. Why

Interviewers use **Extension methods** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Extension methods** is about static method callable with instance syntax when in scope. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

compile-time syntax sugar; it does not modify or override the target type
...
```csharp
public static string Slug(this string s) => s.ToLowerInvariant();
```
...
In a production C#/.NET system, **Extension methods** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Extension methods**.
...
Create the smallest runnable C# example demonstrating **Extension methods**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Extension methods** in plain English.
