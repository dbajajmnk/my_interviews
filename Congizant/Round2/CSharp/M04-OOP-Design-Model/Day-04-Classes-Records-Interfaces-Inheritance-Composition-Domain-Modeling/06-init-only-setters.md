...
**init-only setters** is the C#/.NET engineering concept **init-only setters**.

---

# 2. Why

Interviewers use **init-only setters** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **init-only setters** is about the C#/.NET engineering concept **init-only setters**. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **init-only setters** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **init-only setters**.
...
Create the smallest runnable C# example demonstrating **init-only setters**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **init-only setters** in plain English.
