...
**Record classes** is value-oriented reference type with generated equality and non-destructive mutation support.

---

# 2. Why

Interviewers use **Record classes** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Record classes** is about value-oriented reference type with generated equality and non-destructive mutation support. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Record classes** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Record classes**.
...
Create the smallest runnable C# example demonstrating **Record classes**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Record classes** in plain English.
