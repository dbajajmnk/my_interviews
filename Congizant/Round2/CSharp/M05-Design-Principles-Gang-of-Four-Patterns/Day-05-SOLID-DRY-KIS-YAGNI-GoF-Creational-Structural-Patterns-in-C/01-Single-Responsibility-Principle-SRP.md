...
Interviewers use **Single Responsibility Principle (SRP)** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Single Responsibility Principle (SRP)** is about one primary reason to change. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

separate transport, policy, persistence, formatting and unrelated change axes
...
In a production C#/.NET system, **Single Responsibility Principle (SRP)** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Single Responsibility Principle (SRP)**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
