...
Interviewers use **Open/Closed Principle (OCP)** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Open/Closed Principle (OCP)** is about support expected extension without repeated invasive edits. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

use stable interfaces/delegates/composition at real variation points
...
In a production C#/.NET system, **Open/Closed Principle (OCP)** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Open/Closed Principle (OCP)**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
