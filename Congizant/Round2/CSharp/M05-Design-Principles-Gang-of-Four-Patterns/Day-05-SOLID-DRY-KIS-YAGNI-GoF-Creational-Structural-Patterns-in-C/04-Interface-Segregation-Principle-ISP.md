...
**Interface Segregation Principle (ISP)** is clients depend on small cohesive capabilities.

---

# 2. Why

Interviewers use **Interface Segregation Principle (ISP)** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Interface Segregation Principle (ISP)** is about clients depend on small cohesive capabilities. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

split broad service interfaces to reduce coupling and mocking burden
...
In a production C#/.NET system, **Interface Segregation Principle (ISP)** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Interface Segregation Principle (ISP)**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
