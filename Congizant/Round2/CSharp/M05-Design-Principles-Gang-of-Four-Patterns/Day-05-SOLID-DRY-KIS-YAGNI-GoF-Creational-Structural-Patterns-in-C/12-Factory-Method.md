...
**Factory Method** is delegates creation behind a stable abstraction.

---

# 2. Why

Interviewers use **Factory Method** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Factory Method** is about delegates creation behind a stable abstraction. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

construct through a method/factory while callers depend on product contract
...
```csharp
public interface IParser { object Parse(string s); }
```
...
In a production C#/.NET system, **Factory Method** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Factory Method**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
