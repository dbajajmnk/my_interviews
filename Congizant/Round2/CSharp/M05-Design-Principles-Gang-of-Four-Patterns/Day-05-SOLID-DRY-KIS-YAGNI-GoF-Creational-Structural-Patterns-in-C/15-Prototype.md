...
**Prototype** is creates new objects from an existing configured instance.

---

# 2. Why

Interviewers use **Prototype** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Prototype** is about creates new objects from an existing configured instance. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

records with `with`, MemberwiseClone awareness, or explicit copy logic can implement the intent
...
```csharp
var copy = original with { TimeoutMs = 5000 };
```
...
In a production C#/.NET system, **Prototype** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Prototype**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
