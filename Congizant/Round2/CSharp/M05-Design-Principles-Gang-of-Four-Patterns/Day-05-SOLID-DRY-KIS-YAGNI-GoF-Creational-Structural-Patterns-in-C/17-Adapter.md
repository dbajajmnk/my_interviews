...
**Adapter** is translates an incompatible dependency to your application contract.

---

# 2. Why

Interviewers use **Adapter** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Adapter** is about translates an incompatible dependency to your application contract. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

wrap vendor SDK calls and map arguments/results/errors
...
```csharp
public sealed class LegacyAdapter : IPaymentGateway { }
```
...
In a production C#/.NET system, **Adapter** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Adapter**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
