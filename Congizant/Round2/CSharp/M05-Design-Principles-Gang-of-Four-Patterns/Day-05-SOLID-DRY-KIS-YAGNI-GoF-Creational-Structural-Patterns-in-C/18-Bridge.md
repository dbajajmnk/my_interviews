...
Interviewers use **Bridge** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Bridge** is about separates abstraction from implementation dimension. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

composition plus interfaces lets both sides vary independently
...
```csharp
public abstract class Notification { protected readonly ISender Sender; }
```
...
In a production C#/.NET system, **Bridge** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Bridge**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
