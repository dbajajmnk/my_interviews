...
**Facade** is provides one cohesive entry point over a subsystem.

---

# 2. Why

Interviewers use **Facade** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Facade** is about provides one cohesive entry point over a subsystem. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

application service coordinates multiple lower-level collaborators
...
In a production C#/.NET system, **Facade** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Facade**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
