...
**Flyweight** is shares immutable intrinsic state to reduce memory cost.

---

# 2. Why

Interviewers use **Flyweight** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Flyweight** is about shares immutable intrinsic state to reduce memory cost. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

canonical shared metadata can help only after measurement proves repetition cost
...
```csharp
private static readonly ConcurrentDictionary<string, ProductType> Cache = new();
```
...
In a production C#/.NET system, **Flyweight** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Flyweight**.
...
Create the smallest runnable C# example demonstrating **Flyweight**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
