...
Interviewers use **Interpreter** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Interpreter** is about models/evaluates a small expression language. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

discriminated class/record hierarchies plus pattern matching can implement a small AST
...
In a production C#/.NET system, **Interpreter** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Interpreter**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
