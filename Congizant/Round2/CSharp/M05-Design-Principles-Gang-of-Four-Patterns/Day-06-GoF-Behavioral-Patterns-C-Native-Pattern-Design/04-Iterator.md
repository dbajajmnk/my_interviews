...
**Iterator** is provides sequential traversal without exposing representation.

---

# 2. Why

Interviewers use **Iterator** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Iterator** is about provides sequential traversal without exposing representation. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

IEnumerable<T>/yield implement the iterator pattern directly
...
```csharp
IEnumerable<int> Range(){ yield return 1; }
```
...
In a production C#/.NET system, **Iterator** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Iterator**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
