...
**Template Method** is defines algorithm skeleton with overridable steps.

---

# 2. Why

Interviewers use **Template Method** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Template Method** is about defines algorithm skeleton with overridable steps. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

abstract base classes implement this classically, but composition can be simpler
...
```csharp
public abstract class ImportJob { public async Task RunAsync(){ await ReadAsync(); } protected abstract Task ReadAsync(); }
```
...
In a production C#/.NET system, **Template Method** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Template Method**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
