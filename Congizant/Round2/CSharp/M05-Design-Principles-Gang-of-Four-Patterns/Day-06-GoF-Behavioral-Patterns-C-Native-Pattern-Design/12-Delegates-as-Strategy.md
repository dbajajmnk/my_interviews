# Topic 12 — Delegates as Strategy
...
**Delegates as Strategy** is type-safe callable reference to one or more methods.

---

# 2. Why

Interviewers use **Delegates as Strategy** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Delegates as Strategy** is about type-safe callable reference to one or more methods. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

delegates store target/method invocation data and support multicast invocation lists
...
In a production C#/.NET system, **Delegates as Strategy** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Delegates as Strategy**.
...
1. What is **Delegates as Strategy**?
...
Create the smallest runnable C# example demonstrating **Delegates as Strategy**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Delegates as Strategy** in plain English.
