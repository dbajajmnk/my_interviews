...
**IEnumerable<T>** is pull-based enumeration abstraction.

---

# 2. Why

Interviewers use **IEnumerable<T>** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **IEnumerable<T>** is about pull-based enumeration abstraction. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

GetEnumerator/MoveNext drives lazy traversal; LINQ composes deferred iterators
...
```csharp
IEnumerable<int> Query() { yield return 1; }
```
...
In a production C#/.NET system, **IEnumerable<T>** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **IEnumerable<T>**.
...
Create the smallest runnable C# example demonstrating **IEnumerable<T>**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **IEnumerable<T>** in plain English.
