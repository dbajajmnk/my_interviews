...
**Deferred execution** is query execution delayed until enumeration.

---

# 2. Why

Interviewers use **Deferred execution** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Deferred execution** is about query execution delayed until enumeration. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

re-enumeration can repeat database/network/computation work unless materialized
...
In a production C#/.NET system, **Deferred execution** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Deferred execution**.
...
Create the smallest runnable C# example demonstrating **Deferred execution**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Deferred execution** in plain English.
