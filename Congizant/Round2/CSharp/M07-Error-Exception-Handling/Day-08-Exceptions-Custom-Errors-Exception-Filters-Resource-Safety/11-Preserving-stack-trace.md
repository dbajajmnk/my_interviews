# Topic 11 — Preserving stack trace
...
**Preserving stack trace** is the C#/.NET engineering concept **Preserving stack trace**.

---

# 2. Why

Interviewers use **Preserving stack trace** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Preserving stack trace** is about the C#/.NET engineering concept **Preserving stack trace**. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Preserving stack trace** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Preserving stack trace**.
...
1. What is **Preserving stack trace**?
...
Create the smallest runnable C# example demonstrating **Preserving stack trace**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Preserving stack trace** in plain English.
