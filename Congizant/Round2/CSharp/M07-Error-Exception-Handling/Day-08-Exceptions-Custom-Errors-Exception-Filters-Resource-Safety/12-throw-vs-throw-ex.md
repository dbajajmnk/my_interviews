...
**throw vs throw ex** is preserving current exception stack on rethrow.

---

# 2. Why

Interviewers use **throw vs throw ex** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **throw vs throw ex** is about preserving current exception stack on rethrow. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

`throw;` rethrows current exception preserving stack context; `throw ex;` resets stack origin
...
In a production C#/.NET system, **throw vs throw ex** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **throw vs throw ex**.
...
Create the smallest runnable C# example demonstrating **throw vs throw ex**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **throw vs throw ex** in plain English.
