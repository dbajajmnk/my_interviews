# Topic 02 — Namespaces vs assemblies
...
**Namespaces vs assemblies** is the C#/.NET engineering concept **Namespaces vs assemblies**.

---

# 2. Why

Interviewers use **Namespaces vs assemblies** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Namespaces vs assemblies** is about the C#/.NET engineering concept **Namespaces vs assemblies**. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Namespaces vs assemblies** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Namespaces vs assemblies**.
...
1. What is **Namespaces vs assemblies**?
...
Create the smallest runnable C# example demonstrating **Namespaces vs assemblies**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Namespaces vs assemblies** in plain English.
