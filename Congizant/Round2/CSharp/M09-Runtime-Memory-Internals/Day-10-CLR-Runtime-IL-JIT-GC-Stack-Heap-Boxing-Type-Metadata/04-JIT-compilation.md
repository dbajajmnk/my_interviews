...
**JIT compilation** is runtime compilation from IL to native code.

---

# 2. Why

Interviewers use **JIT compilation** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **JIT compilation** is about runtime compilation from IL to native code. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

tiered compilation and runtime profiling may optimize hot methods; details are implementation-specific
...
```csharp
// inspect with diagnostics/disassembly
```
...
In a production C#/.NET system, **JIT compilation** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **JIT compilation**.
...
Create the smallest runnable C# example demonstrating **JIT compilation**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **JIT compilation** in plain English.
