# Topic 11 — Value type representation
...
**Value type representation** is type whose variable contains the value representation directly according to CLI semantics.

---

# 2. Why

Interviewers use **Value type representation** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Value type representation** is about type whose variable contains the value representation directly according to CLI semantics. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

struct/primitive values are copied by value unless passed by ref; placement is not simply 'always stack'
...
In a production C#/.NET system, **Value type representation** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Value type representation**.
...
1. What is **Value type representation**?
...
Create the smallest runnable C# example demonstrating **Value type representation**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Value type representation** in plain English.
