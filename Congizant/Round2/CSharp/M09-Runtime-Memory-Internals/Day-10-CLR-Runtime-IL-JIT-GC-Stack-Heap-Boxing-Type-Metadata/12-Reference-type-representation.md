# Topic 12 — Reference type representation
...
**Reference type representation** is type whose variable contains a managed reference to an object.

---

# 2. Why

Interviewers use **Reference type representation** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Reference type representation** is about type whose variable contains a managed reference to an object. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

object lifetime/storage is managed by CLR; assignment copies the reference value
...
In a production C#/.NET system, **Reference type representation** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Reference type representation**.
...
1. What is **Reference type representation**?
...
Create the smallest runnable C# example demonstrating **Reference type representation**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Reference type representation** in plain English.
