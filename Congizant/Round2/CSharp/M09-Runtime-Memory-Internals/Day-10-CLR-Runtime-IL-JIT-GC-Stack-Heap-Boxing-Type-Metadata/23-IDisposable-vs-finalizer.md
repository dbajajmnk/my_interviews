# Topic 23 — IDisposable vs finalizer
...
**IDisposable vs finalizer** is deterministic resource cleanup contract.

---

# 2. Why

Interviewers use **IDisposable vs finalizer** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **IDisposable vs finalizer** is about deterministic resource cleanup contract. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

Dispose releases unmanaged/owned resources independently of GC timing; using compiles to try/finally-like cleanup
...
In a production C#/.NET system, **IDisposable vs finalizer** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **IDisposable vs finalizer**.
...
1. What is **IDisposable vs finalizer**?
...
Create the smallest runnable C# example demonstrating **IDisposable vs finalizer**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **IDisposable vs finalizer** in plain English.
