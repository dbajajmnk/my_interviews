...
**Task.WhenAll** is the C#/.NET engineering concept **Task.WhenAll**.

---

# 2. Why

Interviewers use **Task.WhenAll** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Task.WhenAll** is about the C#/.NET engineering concept **Task.WhenAll**. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Task.WhenAll** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Task.WhenAll**.
...
Create the smallest runnable C# example demonstrating **Task.WhenAll**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Task.WhenAll** in plain English.
