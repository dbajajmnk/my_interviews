# Topic 16 — CancellationTokenSource
...
**CancellationTokenSource** is cooperative cancellation signal.

---

# 2. Why

Interviewers use **CancellationTokenSource** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **CancellationTokenSource** is about cooperative cancellation signal. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **CancellationTokenSource** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **CancellationTokenSource**.
...
1. What is **CancellationTokenSource**?
...
Create the smallest runnable C# example demonstrating **CancellationTokenSource**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **CancellationTokenSource** in plain English.
