# Topic 21 — Deadlock from sync-over-async awareness
...
**Deadlock from sync-over-async awareness** is compiler-transformed asynchronous method.

---

# 2. Why

Interviewers use **Deadlock from sync-over-async awareness** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Deadlock from sync-over-async awareness** is about compiler-transformed asynchronous method. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

async methods use state-machine machinery around awaits and return Task/Task<T>/ValueTask forms
...
```csharp
public async Task<User> LoadAsync(){ return await repo.LoadAsync(); }
```
...
In a production C#/.NET system, **Deadlock from sync-over-async awareness** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Deadlock from sync-over-async awareness**.
...
1. What is **Deadlock from sync-over-async awareness**?
...
Create the smallest runnable C# example demonstrating **Deadlock from sync-over-async awareness**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Deadlock from sync-over-async awareness** in plain English.
