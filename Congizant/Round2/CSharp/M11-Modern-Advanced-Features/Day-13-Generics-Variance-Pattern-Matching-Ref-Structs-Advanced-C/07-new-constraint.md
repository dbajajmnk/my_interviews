...
**new() constraint** is the C#/.NET engineering concept **new() constraint**.

---

# 2. Why

Interviewers use **new() constraint** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **new() constraint** is about the C#/.NET engineering concept **new() constraint**. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **new() constraint** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **new() constraint**.
...
Create the smallest runnable C# example demonstrating **new() constraint**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **new() constraint** in plain English.
