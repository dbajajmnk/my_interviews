...
**out covariance** is generic interface/delegate assignability direction.

---

# 2. Why

Interviewers use **out covariance** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **out covariance** is about generic interface/delegate assignability direction. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **out covariance** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **out covariance**.
...
Create the smallest runnable C# example demonstrating **out covariance**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **out covariance** in plain English.
