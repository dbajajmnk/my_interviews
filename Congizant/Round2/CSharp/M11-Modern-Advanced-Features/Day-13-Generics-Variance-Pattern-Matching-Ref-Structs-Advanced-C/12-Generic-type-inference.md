# Topic 12 — Generic type inference
...
**Generic type inference** is compile-time parameterized type/method.

---

# 2. Why

Interviewers use **Generic type inference** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Generic type inference** is about compile-time parameterized type/method. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Generic type inference** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Generic type inference**.
...
1. What is **Generic type inference**?
...
Create the smallest runnable C# example demonstrating **Generic type inference**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Generic type inference** in plain English.
