...
**Record structs** is value-oriented reference type with generated equality and non-destructive mutation support.

---

# 2. Why

Interviewers use **Record structs** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Record structs** is about value-oriented reference type with generated equality and non-destructive mutation support. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Record structs** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Record structs**.
...
Create the smallest runnable C# example demonstrating **Record structs**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Record structs** in plain English.
