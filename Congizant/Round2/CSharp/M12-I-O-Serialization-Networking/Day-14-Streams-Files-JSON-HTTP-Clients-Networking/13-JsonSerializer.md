...
**JsonSerializer** is the C#/.NET engineering concept **JsonSerializer**.

---

# 2. Why

Interviewers use **JsonSerializer** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **JsonSerializer** is about the C#/.NET engineering concept **JsonSerializer**. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **JsonSerializer** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **JsonSerializer**.
...
Create the smallest runnable C# example demonstrating **JsonSerializer**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **JsonSerializer** in plain English.
