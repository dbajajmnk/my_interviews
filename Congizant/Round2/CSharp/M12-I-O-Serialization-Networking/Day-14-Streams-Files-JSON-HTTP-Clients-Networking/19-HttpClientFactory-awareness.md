# Topic 19 — HttpClientFactory awareness
...
**HttpClientFactory awareness** is reusable HTTP abstraction.

---

# 2. Why

Interviewers use **HttpClientFactory awareness** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **HttpClientFactory awareness** is about reusable HTTP abstraction. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

underlying handlers pool connections; creating clients/handlers incorrectly can cause socket/DNS/lifetime problems
...
In a production C#/.NET system, **HttpClientFactory awareness** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **HttpClientFactory awareness**.
...
1. What is **HttpClientFactory awareness**?
...
Create the smallest runnable C# example demonstrating **HttpClientFactory awareness**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **HttpClientFactory awareness** in plain English.
