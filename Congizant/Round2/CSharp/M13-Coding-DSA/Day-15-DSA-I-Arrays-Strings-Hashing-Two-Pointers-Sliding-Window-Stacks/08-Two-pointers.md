...
Interviewers use **Two pointers** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Two pointers** is about two-index algorithm pattern. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

monotonic movement can reduce repeated scans to O(n)
...
```csharp
for (int l=0,r=a.Length-1; l<r;) { break; }
```
...
In a production C#/.NET system, **Two pointers** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Two pointers**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
