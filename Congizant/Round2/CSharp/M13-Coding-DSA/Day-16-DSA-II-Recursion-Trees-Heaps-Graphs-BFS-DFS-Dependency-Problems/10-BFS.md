...
**BFS** is breadth-first traversal using queue.

---

# 2. Why

Interviewers use **BFS** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **BFS** is about breadth-first traversal using queue. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

visited tracking plus adjacency lists gives O(V+E) traversal
...
In a production C#/.NET system, **BFS** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **BFS**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
