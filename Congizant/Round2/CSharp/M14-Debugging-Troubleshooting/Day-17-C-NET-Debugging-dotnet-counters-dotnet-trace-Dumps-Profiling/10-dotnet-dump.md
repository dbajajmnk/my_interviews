...
**dotnet-dump** is managed dump collection/analysis tool.

---

# 2. Why

Interviewers use **dotnet-dump** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **dotnet-dump** is about managed dump collection/analysis tool. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

captures process memory for postmortem managed stack/heap analysis
...
In a production C#/.NET system, **dotnet-dump** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **dotnet-dump**.
...
Create the smallest runnable C# example demonstrating **dotnet-dump**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **dotnet-dump** in plain English.
