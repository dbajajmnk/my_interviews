...
**Memory leak diagnosis** is the C#/.NET engineering concept **Memory leak diagnosis**.

---

# 2. Why

Interviewers use **Memory leak diagnosis** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Memory leak diagnosis** is about the C#/.NET engineering concept **Memory leak diagnosis**. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Memory leak diagnosis** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Memory leak diagnosis**.
...
Create the smallest runnable C# example demonstrating **Memory leak diagnosis**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
