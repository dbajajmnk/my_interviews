# Topic 15 — ThreadPool starvation diagnosis
...
**ThreadPool starvation diagnosis** is shared pool of worker threads used by Tasks and runtime infrastructure.

---

# 2. Why

Interviewers use **ThreadPool starvation diagnosis** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **ThreadPool starvation diagnosis** is about shared pool of worker threads used by Tasks and runtime infrastructure. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

blocking many worker threads can cause starvation and latency spikes
...
In a production C#/.NET system, **ThreadPool starvation diagnosis** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **ThreadPool starvation diagnosis**.
...
1. What is **ThreadPool starvation diagnosis**?
...
Create the smallest runnable C# example demonstrating **ThreadPool starvation diagnosis**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **ThreadPool starvation diagnosis** in plain English.
