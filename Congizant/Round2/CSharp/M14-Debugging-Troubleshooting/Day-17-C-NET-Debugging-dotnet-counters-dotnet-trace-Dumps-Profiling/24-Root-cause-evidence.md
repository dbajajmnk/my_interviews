...
**Root-cause evidence** is the C#/.NET engineering concept **Root-cause evidence**.

---

# 2. Why

Interviewers use **Root-cause evidence** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Root-cause evidence** is about the C#/.NET engineering concept **Root-cause evidence**. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Root-cause evidence** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Root-cause evidence**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
