...
**Async correctness** is compiler-transformed asynchronous method.

---

# 2. Why

Interviewers use **Async correctness** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Async correctness** is about compiler-transformed asynchronous method. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Async correctness** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Async correctness**.
...
Create the smallest runnable C# example demonstrating **Async correctness**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
