...
**Static mutable state** is the C#/.NET engineering concept **Static mutable state**.

---

# 2. Why

Interviewers use **Static mutable state** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Static mutable state** is about the C#/.NET engineering concept **Static mutable state**. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Static mutable state** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Static mutable state**.
...
1. What is **Static mutable state**?
...
Create the smallest runnable C# example demonstrating **Static mutable state**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Static mutable state** in plain English.
