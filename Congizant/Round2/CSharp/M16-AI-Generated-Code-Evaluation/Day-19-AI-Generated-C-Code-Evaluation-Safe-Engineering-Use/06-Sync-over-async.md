...
**Sync-over-async** is compiler-transformed asynchronous method.

---

# 2. Why

Interviewers use **Sync-over-async** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Sync-over-async** is about compiler-transformed asynchronous method. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Sync-over-async** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Sync-over-async**.
...
Create the smallest runnable C# example demonstrating **Sync-over-async**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Sync-over-async** in plain English.
