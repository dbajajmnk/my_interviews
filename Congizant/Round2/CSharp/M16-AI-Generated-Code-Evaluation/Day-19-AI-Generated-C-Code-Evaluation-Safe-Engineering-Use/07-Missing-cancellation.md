...
**Missing cancellation** is the C#/.NET engineering concept **Missing cancellation**.

---

# 2. Why

Interviewers use **Missing cancellation** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Missing cancellation** is about the C#/.NET engineering concept **Missing cancellation**. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Missing cancellation** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Missing cancellation**.
...
1. What is **Missing cancellation**?
...
Create the smallest runnable C# example demonstrating **Missing cancellation**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Missing cancellation** in plain English.
