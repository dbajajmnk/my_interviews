...
**xUnit awareness** is popular .NET test framework.

---

# 2. Why

Interviewers use **xUnit awareness** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **xUnit awareness** is about popular .NET test framework. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

Facts/Theories and fixtures structure unit/integration tests; async tests return Task
...
```csharp
[Fact] public async Task Works() { await Task.CompletedTask; }
```
...
In a production C#/.NET system, **xUnit awareness** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **xUnit awareness**.
...
Create the smallest runnable C# example demonstrating **xUnit awareness**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **xUnit awareness** in plain English.
