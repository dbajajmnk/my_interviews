...
**Cancellation tests** is the C#/.NET engineering concept **Cancellation tests**.

---

# 2. Why

Interviewers use **Cancellation tests** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Cancellation tests** is about the C#/.NET engineering concept **Cancellation tests**. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Cancellation tests** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Cancellation tests**.
...
Create the smallest runnable C# example demonstrating **Cancellation tests**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Cancellation tests** in plain English.
