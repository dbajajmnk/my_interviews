...
**DbContext** is EF Core unit-of-work/change-tracking session.

---

# 2. Why

Interviewers use **DbContext** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **DbContext** is about EF Core unit-of-work/change-tracking session. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

normally scoped per request/use case; not thread-safe and should not be singleton
...
In a production C#/.NET system, **DbContext** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **DbContext**.
...
Create the smallest runnable C# example demonstrating **DbContext**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **DbContext** in plain English.
