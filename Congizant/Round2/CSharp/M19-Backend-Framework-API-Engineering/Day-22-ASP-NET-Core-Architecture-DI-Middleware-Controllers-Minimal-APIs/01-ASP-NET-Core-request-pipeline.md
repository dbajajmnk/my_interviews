# Topic 01 — ASP.NET Core request pipeline
...
**ASP.NET Core request pipeline** is cross-platform web framework on .NET.

---

# 2. Why

Interviewers use **ASP.NET Core request pipeline** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **ASP.NET Core request pipeline** is about cross-platform web framework on .NET. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **ASP.NET Core request pipeline** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **ASP.NET Core request pipeline**.
...
Create the smallest runnable C# example demonstrating **ASP.NET Core request pipeline**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **ASP.NET Core request pipeline** in plain English.
