# Topic 07 — Dependency injection container
...
**Dependency injection container** is built-in service registration/resolution model.

---

# 2. Why

Interviewers use **Dependency injection container** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Dependency injection container** is about built-in service registration/resolution model. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

lifetimes control sharing/disposal; captive dependency mistakes violate scope assumptions
...
In a production C#/.NET system, **Dependency injection container** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Dependency injection container**.
...
1. What is **Dependency injection container**?
...
Create the smallest runnable C# example demonstrating **Dependency injection container**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Dependency injection container** in plain English.
