...
Interviewers use **Authentication vs authorization** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Authentication vs authorization** is about runtime identity verification. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

handlers validate credentials/token/session and establish ClaimsPrincipal
...
In a production C#/.NET system, **Authentication vs authorization** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Authentication vs authorization**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
