...
**SQL injection** is untrusted data changing SQL structure.

---

# 2. Why

Interviewers use **SQL injection** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **SQL injection** is about untrusted data changing SQL structure. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

parameterized commands/EF parameters keep values separate from SQL syntax
...
In a production C#/.NET system, **SQL injection** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **SQL injection**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
