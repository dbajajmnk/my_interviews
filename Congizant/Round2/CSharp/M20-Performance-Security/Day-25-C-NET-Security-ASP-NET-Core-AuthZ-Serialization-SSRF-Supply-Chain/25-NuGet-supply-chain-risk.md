# Topic 25 — NuGet supply-chain risk
...
**NuGet supply-chain risk** is package manager/ecosystem for .NET.

---

# 2. Why

Interviewers use **NuGet supply-chain risk** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **NuGet supply-chain risk** is about package manager/ecosystem for .NET. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **NuGet supply-chain risk** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **NuGet supply-chain risk**.
...
1. What is **NuGet supply-chain risk**?
...
Create the smallest runnable C# example demonstrating **NuGet supply-chain risk**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **NuGet supply-chain risk** in plain English.
