...
Interviewers use **Outbox pattern** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Outbox pattern** is about reliable event publication intent tied to DB commit. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

business state and outbox record commit in one local transaction; publisher retries later
...
```csharp
// transaction: Save order + OutboxMessage
```
...
In a production C#/.NET system, **Outbox pattern** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Outbox pattern**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
