...
**Load shedding** is the C#/.NET engineering concept **Load shedding**.

---

# 2. Why

Interviewers use **Load shedding** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Load shedding** is about the C#/.NET engineering concept **Load shedding**. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Load shedding** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Load shedding**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
