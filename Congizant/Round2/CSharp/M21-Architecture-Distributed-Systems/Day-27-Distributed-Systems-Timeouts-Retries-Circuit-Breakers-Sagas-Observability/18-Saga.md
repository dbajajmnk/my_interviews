...
**Saga** is distributed workflow of local transactions and compensation.

---

# 2. Why

Interviewers use **Saga** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Saga** is about distributed workflow of local transactions and compensation. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

explicit state/compensation handles partial success without global distributed transaction
...
```csharp
public sealed record SagaState(string Name);
```
...
In a production C#/.NET system, **Saga** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Saga**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
