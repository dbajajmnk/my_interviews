...
**Docker** is containerized .NET deployment.

---

# 2. Why

Interviewers use **Docker** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Docker** is about containerized .NET deployment. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

publish output and runtime/container image must align with framework/RID/certs/resources
...
In a production C#/.NET system, **Docker** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Docker**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
