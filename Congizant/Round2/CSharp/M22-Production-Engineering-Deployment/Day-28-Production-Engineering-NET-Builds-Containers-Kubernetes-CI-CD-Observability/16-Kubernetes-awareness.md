...
**Kubernetes awareness** is orchestration for replicas/lifecycle/traffic.

---

# 2. Why

Interviewers use **Kubernetes awareness** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Kubernetes awareness** is about orchestration for replicas/lifecycle/traffic. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

readiness/liveness/resources/rollout must align with ASP.NET graceful shutdown and runtime behavior
...
In a production C#/.NET system, **Kubernetes awareness** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Kubernetes awareness**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
