...
**Backward-compatible migrations** is incremental modernization.

---

# 2. Why

Interviewers use **Backward-compatible migrations** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Backward-compatible migrations** is about incremental modernization. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Backward-compatible migrations** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Backward-compatible migrations**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
