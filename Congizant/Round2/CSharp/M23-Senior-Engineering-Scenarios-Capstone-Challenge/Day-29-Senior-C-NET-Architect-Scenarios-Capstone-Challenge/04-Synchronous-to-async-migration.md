# Topic 04 — Synchronous to async migration
...
**Synchronous to async migration** is compiler-transformed asynchronous method.

---

# 2. Why

Interviewers use **Synchronous to async migration** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Synchronous to async migration** is about compiler-transformed asynchronous method. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Synchronous to async migration** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **Synchronous to async migration**.
...
1. What is **Synchronous to async migration**?
...
Create the smallest runnable C# example demonstrating **Synchronous to async migration**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Synchronous to async migration** in plain English.
