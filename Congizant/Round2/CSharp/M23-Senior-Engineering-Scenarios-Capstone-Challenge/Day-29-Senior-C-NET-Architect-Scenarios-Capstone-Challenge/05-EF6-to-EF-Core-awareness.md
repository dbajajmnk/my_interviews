# Topic 05 — EF6 to EF Core awareness
...
**EF6 to EF Core awareness** is modern .NET ORM.

---

# 2. Why

Interviewers use **EF6 to EF Core awareness** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **EF6 to EF Core awareness** is about modern .NET ORM. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.

---

# 4. Engineering Explanation

DbContext tracks entities and translates LINQ expression trees to provider SQL; inspect generated SQL and lifecycle
...
```csharp
var order = await db.Orders.AsNoTracking().SingleAsync(x => x.Id == id);
```
...
In a production C#/.NET system, **EF6 to EF Core awareness** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
- Reproduce the smallest failure involving **EF6 to EF Core awareness**.
...
1. What is **EF6 to EF Core awareness**?
...
Create the smallest runnable C# example demonstrating **EF6 to EF Core awareness**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **EF6 to EF Core awareness** in plain English.
