...
**Database bottleneck** is the C#/.NET engineering concept **Database bottleneck**.

---

# 2. Why

Interviewers use **Database bottleneck** to test whether you understand C# semantics, CLR/runtime behavior, async/resource ownership, framework behavior, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Database bottleneck** is about the C#/.NET engineering concept **Database bottleneck**. Use the language/runtime feature only when it makes the contract safer, clearer, or more efficient.
...
In a production C#/.NET system, **Database bottleneck** should connect to a concrete CLR, async, ASP.NET Core, EF Core, API, data, security, or architecture requirement.
...
Create the smallest runnable C# example demonstrating **Database bottleneck**. Introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
