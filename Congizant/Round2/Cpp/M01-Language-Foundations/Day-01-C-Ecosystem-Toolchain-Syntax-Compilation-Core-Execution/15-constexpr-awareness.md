...
**constexpr awareness** is allows compile-time evaluation when inputs/context permit.

---

# 2. Why

Interviewers use **constexpr awareness** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **constexpr awareness** is about allows compile-time evaluation when inputs/context permit. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

constexpr functions/objects may also execute at runtime unless immediate evaluation required
...
In a production C++ system, **constexpr awareness** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **constexpr awareness**.
...
Create the smallest runnable C++ example demonstrating **constexpr awareness**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **constexpr awareness** in plain English.
