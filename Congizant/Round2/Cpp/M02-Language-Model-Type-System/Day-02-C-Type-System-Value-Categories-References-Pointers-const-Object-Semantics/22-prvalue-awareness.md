...
**prvalue awareness** is temporary/expiring value used for move-aware overloads.

---

# 2. Why

Interviewers use **prvalue awareness** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **prvalue awareness** is about temporary/expiring value used for move-aware overloads. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **prvalue awareness** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **prvalue awareness**.
...
Create the smallest runnable C++ example demonstrating **prvalue awareness**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **prvalue awareness** in plain English.
