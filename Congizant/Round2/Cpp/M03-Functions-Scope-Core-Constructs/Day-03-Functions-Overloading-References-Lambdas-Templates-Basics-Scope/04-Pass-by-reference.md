...
**Pass by reference** is the C++ engineering concept **Pass by reference**.

---

# 2. Why

Interviewers use **Pass by reference** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Pass by reference** is about the C++ engineering concept **Pass by reference**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Pass by reference** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Pass by reference**.
...
Create the smallest runnable C++ example demonstrating **Pass by reference**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Pass by reference** in plain English.
