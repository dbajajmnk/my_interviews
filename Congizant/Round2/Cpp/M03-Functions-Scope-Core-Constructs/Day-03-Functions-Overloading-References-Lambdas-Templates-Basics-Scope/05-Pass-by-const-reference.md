# Topic 05 — Pass by const reference
...
**Pass by const reference** is the C++ engineering concept **Pass by const reference**.

---

# 2. Why

Interviewers use **Pass by const reference** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Pass by const reference** is about the C++ engineering concept **Pass by const reference**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Pass by const reference** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Pass by const reference**.
...
1. What is **Pass by const reference**?
...
Create the smallest runnable C++ example demonstrating **Pass by const reference**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Pass by const reference** in plain English.
