...
**Pass by pointer** is the C++ engineering concept **Pass by pointer**.

---

# 2. Why

Interviewers use **Pass by pointer** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Pass by pointer** is about the C++ engineering concept **Pass by pointer**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Pass by pointer** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Pass by pointer**.
...
Create the smallest runnable C++ example demonstrating **Pass by pointer**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Pass by pointer** in plain English.
