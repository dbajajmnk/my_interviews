...
**Return by value** is the C++ engineering concept **Return by value**.

---

# 2. Why

Interviewers use **Return by value** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Return by value** is about the C++ engineering concept **Return by value**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Return by value** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Return by value**.
...
Create the smallest runnable C++ example demonstrating **Return by value**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Return by value** in plain English.
