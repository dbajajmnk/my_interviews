...
**Default arguments** is the C++ engineering concept **Default arguments**.

---

# 2. Why

Interviewers use **Default arguments** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Default arguments** is about the C++ engineering concept **Default arguments**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Default arguments** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Default arguments**.
...
Create the smallest runnable C++ example demonstrating **Default arguments**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Default arguments** in plain English.
