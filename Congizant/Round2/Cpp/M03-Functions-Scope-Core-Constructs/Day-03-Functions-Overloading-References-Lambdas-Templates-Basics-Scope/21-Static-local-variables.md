# Topic 21 — Static local variables
...
**Static local variables** is the C++ engineering concept **Static local variables**.

---

# 2. Why

Interviewers use **Static local variables** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Static local variables** is about the C++ engineering concept **Static local variables**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Static local variables** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Static local variables**.
...
1. What is **Static local variables**?
...
Create the smallest runnable C++ example demonstrating **Static local variables**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Static local variables** in plain English.
