...
**Virtual functions** is the C++ engineering concept **Virtual functions**.

---

# 2. Why

Interviewers use **Virtual functions** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Virtual functions** is about the C++ engineering concept **Virtual functions**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Virtual functions** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Virtual functions**.
...
Create the smallest runnable C++ example demonstrating **Virtual functions**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Virtual functions** in plain English.
