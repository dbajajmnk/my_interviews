# Topic 22 — Pure virtual functions
...
**Pure virtual functions** is the C++ engineering concept **Pure virtual functions**.

---

# 2. Why

Interviewers use **Pure virtual functions** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Pure virtual functions** is about the C++ engineering concept **Pure virtual functions**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Pure virtual functions** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Pure virtual functions**.
...
Create the smallest runnable C++ example demonstrating **Pure virtual functions**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Pure virtual functions** in plain English.
