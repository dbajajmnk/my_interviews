...
**Composition over inheritance** is the C++ engineering concept **Composition over inheritance**.

---

# 2. Why

Interviewers use **Composition over inheritance** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Composition over inheritance** is about the C++ engineering concept **Composition over inheritance**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Composition over inheritance** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Composition over inheritance**.
...
Create the smallest runnable C++ example demonstrating **Composition over inheritance**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
