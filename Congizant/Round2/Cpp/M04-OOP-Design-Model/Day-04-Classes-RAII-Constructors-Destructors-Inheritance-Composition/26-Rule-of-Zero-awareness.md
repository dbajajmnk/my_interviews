# Topic 26 — Rule of Zero awareness
...
**Rule of Zero awareness** is delegating ownership to RAII members.

---

# 2. Why

Interviewers use **Rule of Zero awareness** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Rule of Zero awareness** is about delegating ownership to RAII members. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

use vector/string/unique_ptr etc. so compiler-generated special members are correct
...
```cpp
struct User { std::string name; std::vector<int> values; };
```
...
In a production C++ system, **Rule of Zero awareness** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Rule of Zero awareness**.
...
1. What is **Rule of Zero awareness**?
...
Create the smallest runnable C++ example demonstrating **Rule of Zero awareness**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Rule of Zero awareness** in plain English.
