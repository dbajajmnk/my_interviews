...
**Rule of Five** is special member functions needed when manually owning resources.

---

# 2. Why

Interviewers use **Rule of Five** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Rule of Five** is about special member functions needed when manually owning resources. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Rule of Five** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Rule of Five**.
...
Create the smallest runnable C++ example demonstrating **Rule of Five**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Rule of Five** in plain English.
