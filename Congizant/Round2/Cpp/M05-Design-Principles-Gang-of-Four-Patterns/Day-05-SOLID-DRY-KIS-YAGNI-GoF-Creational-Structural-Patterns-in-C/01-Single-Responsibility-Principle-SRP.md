...
**Single Responsibility Principle (SRP)** is one primary reason to change.

---

# 2. Why

Interviewers use **Single Responsibility Principle (SRP)** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Single Responsibility Principle (SRP)** is about one primary reason to change. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

separate resource ownership, policy, I/O, persistence and formatting
...
In a production C++ system, **Single Responsibility Principle (SRP)** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Single Responsibility Principle (SRP)**.
...
Create the smallest runnable C++ example demonstrating **Single Responsibility Principle (SRP)**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
