...
**Open/Closed Principle (OCP)** is support expected extension without invasive edits.

---

# 2. Why

Interviewers use **Open/Closed Principle (OCP)** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Open/Closed Principle (OCP)** is about support expected extension without invasive edits. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

use stable interfaces/templates/functions at real variation points
...
In a production C++ system, **Open/Closed Principle (OCP)** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Open/Closed Principle (OCP)**.
...
Create the smallest runnable C++ example demonstrating **Open/Closed Principle (OCP)**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
