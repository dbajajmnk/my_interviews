...
**Liskov Substitution Principle (LSP)** is subtypes preserve behavioral contracts.

---

# 2. Why

Interviewers use **Liskov Substitution Principle (LSP)** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Liskov Substitution Principle (LSP)** is about subtypes preserve behavioral contracts. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

public inheritance should model substitutability, not code reuse
...
In a production C++ system, **Liskov Substitution Principle (LSP)** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
Create the smallest runnable C++ example demonstrating **Liskov Substitution Principle (LSP)**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
