...
Interviewers use **Interface Segregation Principle (ISP)** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Interface Segregation Principle (ISP)** is about clients depend on narrow cohesive contracts. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

avoid fat abstract base classes
...
In a production C++ system, **Interface Segregation Principle (ISP)** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
Create the smallest runnable C++ example demonstrating **Interface Segregation Principle (ISP)**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
