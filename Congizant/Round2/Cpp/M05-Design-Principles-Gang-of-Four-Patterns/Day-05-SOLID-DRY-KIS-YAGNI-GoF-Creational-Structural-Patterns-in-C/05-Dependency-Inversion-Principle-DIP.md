...
**Dependency Inversion Principle (DIP)** is high-level policy depends on abstractions.

---

# 2. Why

Interviewers use **Dependency Inversion Principle (DIP)** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Dependency Inversion Principle (DIP)** is about high-level policy depends on abstractions. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

inject interfaces/functions/templates and keep infrastructure at boundaries
...
```cpp
class OrderService { std::unique_ptr<OrderRepository> repo_; };
```
...
In a production C++ system, **Dependency Inversion Principle (DIP)** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Dependency Inversion Principle (DIP)**.
...
Create the smallest runnable C++ example demonstrating **Dependency Inversion Principle (DIP)**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
