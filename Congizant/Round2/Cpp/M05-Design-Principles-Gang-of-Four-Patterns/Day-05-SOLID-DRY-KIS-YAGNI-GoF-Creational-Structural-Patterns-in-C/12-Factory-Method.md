...
**Factory Method** is delegates object creation through a stable creation point.

---

# 2. Why

Interviewers use **Factory Method** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Factory Method** is about delegates object creation through a stable creation point. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

use a function/static factory/virtual creation hook only when construction varies
...
In a production C++ system, **Factory Method** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
Create the smallest runnable C++ example demonstrating **Factory Method**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
