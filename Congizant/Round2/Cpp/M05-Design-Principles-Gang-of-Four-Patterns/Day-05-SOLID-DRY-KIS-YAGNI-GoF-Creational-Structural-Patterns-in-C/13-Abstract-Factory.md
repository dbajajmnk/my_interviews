...
**Abstract Factory** is creates families of related objects.

---

# 2. Why

Interviewers use **Abstract Factory** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Abstract Factory** is about creates families of related objects. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

a factory interface can produce compatible provider-specific components
...
```cpp
struct CloudFactory { virtual std::unique_ptr<Storage> storage() = 0; };
```
...
In a production C++ system, **Abstract Factory** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
Create the smallest runnable C++ example demonstrating **Abstract Factory**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
