...
**Builder** is constructs a complex object step by step.

---

# 2. Why

Interviewers use **Builder** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Builder** is about constructs a complex object step by step. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

use when staged configuration is clearer than constructors/designated values
...
```cpp
auto request = RequestBuilder{}.url(url).timeout(3s).build();
```
...
In a production C++ system, **Builder** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
Create the smallest runnable C++ example demonstrating **Builder**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
