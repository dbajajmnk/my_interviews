...
**Prototype** is creates objects by copying/cloning an existing object.

---

# 2. Why

Interviewers use **Prototype** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Prototype** is about creates objects by copying/cloning an existing object. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

copy construction or virtual clone can implement this; Rule of Zero is preferred where possible
...
In a production C++ system, **Prototype** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
Create the smallest runnable C++ example demonstrating **Prototype**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
