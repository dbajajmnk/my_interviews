...
**Adapter** is translates an incompatible API into the contract your code expects.

---

# 2. Why

Interviewers use **Adapter** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Adapter** is about translates an incompatible API into the contract your code expects. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

wrap legacy/vendor types and map ownership/errors at the boundary
...
```cpp
class LegacyAdapter : public PaymentPort { /*...*/ };
```
...
In a production C++ system, **Adapter** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
Create the smallest runnable C++ example demonstrating **Adapter**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
