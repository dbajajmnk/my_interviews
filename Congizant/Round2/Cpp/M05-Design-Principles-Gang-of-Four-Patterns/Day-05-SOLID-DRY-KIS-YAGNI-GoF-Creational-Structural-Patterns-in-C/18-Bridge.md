...
**Bridge** is separates abstraction from implementation.

---

# 2. Why

Interviewers use **Bridge** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Bridge** is about separates abstraction from implementation. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

composition through interfaces/templates avoids subclass explosion
...
```cpp
class Notification { std::unique_ptr<Sender> sender_; };
```
...
In a production C++ system, **Bridge** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
Create the smallest runnable C++ example demonstrating **Bridge**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
