...
**Composite** is treats leaves and groups through one common interface.

---

# 2. Why

Interviewers use **Composite** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Composite** is about treats leaves and groups through one common interface. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

a composite owns/delegates to children recursively
...
```cpp
struct Rule { virtual bool valid(Context const&) const = 0; };
```
...
In a production C++ system, **Composite** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
Create the smallest runnable C++ example demonstrating **Composite**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
