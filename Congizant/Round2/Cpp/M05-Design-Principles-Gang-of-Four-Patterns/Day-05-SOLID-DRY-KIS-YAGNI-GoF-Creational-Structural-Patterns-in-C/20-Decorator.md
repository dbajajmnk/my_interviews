...
**Decorator** is adds behavior while preserving the same contract.

---

# 2. Why

Interviewers use **Decorator** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Decorator** is about adds behavior while preserving the same contract. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

own or reference the wrapped component explicitly; clarify lifetime
...
```cpp
class MetricsRepo : public Repo { std::unique_ptr<Repo> inner_; };
```
...
In a production C++ system, **Decorator** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Decorator**.
...
Create the smallest runnable C++ example demonstrating **Decorator**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
