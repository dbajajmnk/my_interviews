...
**Flyweight** is shares reusable intrinsic state.

---

# 2. Why

Interviewers use **Flyweight** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Flyweight** is about shares reusable intrinsic state. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

shared immutable metadata can reduce memory when profiling proves repetition cost
...
```cpp
std::unordered_map<std::string,std::shared_ptr<const ProductType>> cache;
```
...
In a production C++ system, **Flyweight** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
Create the smallest runnable C++ example demonstrating **Flyweight**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
