...
**Proxy** is controls access to another component while preserving a compatible interface.

---

# 2. Why

Interviewers use **Proxy** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Proxy** is about controls access to another component while preserving a compatible interface. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

explicit proxy wrappers make ownership/security visible
...
In a production C++ system, **Proxy** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
Create the smallest runnable C++ example demonstrating **Proxy**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
