...
**Interpreter** is models/evaluates a small expression language.

---

# 2. Why

Interviewers use **Interpreter** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Interpreter** is about models/evaluates a small expression language. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

use variant/class AST and visitor/pattern dispatch for small DSLs
...
```cpp
using Expr = std::variant<Eq, And, Or>;
```
...
In a production C++ system, **Interpreter** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
Create the smallest runnable C++ example demonstrating **Interpreter**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
