...
**State** is models behavior that changes by state.

---

# 2. Why

Interviewers use **State** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **State** is about models behavior that changes by state. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

classes or std::variant + std::visit can model explicit states
...
```cpp
using OrderState = std::variant<Pending,Paid,Cancelled>;
```
...
In a production C++ system, **State** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **State**.
...
Create the smallest runnable C++ example demonstrating **State**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
