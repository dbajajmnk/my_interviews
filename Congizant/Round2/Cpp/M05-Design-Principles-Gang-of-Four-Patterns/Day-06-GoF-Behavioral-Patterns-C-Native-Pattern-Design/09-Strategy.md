...
**Strategy** is encapsulates interchangeable algorithms.

---

# 2. Why

Interviewers use **Strategy** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Strategy** is about encapsulates interchangeable algorithms. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

std::function, function objects or templates can make Strategy lightweight
...
```cpp
using PricingStrategy = std::function<Money(Order const&)>;
```
...
In a production C++ system, **Strategy** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Strategy**.
...
Create the smallest runnable C++ example demonstrating **Strategy**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
