...
**Template Method** is defines an algorithm skeleton with overridable steps.

---

# 2. Why

Interviewers use **Template Method** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Template Method** is about defines an algorithm skeleton with overridable steps. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

classical virtual-hook design works, though composition may be simpler
...
```cpp
class ImportJob { public: void run(){ read(); validate(); } protected: virtual void read()=0; virtual void validate()=0; };
```
...
In a production C++ system, **Template Method** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
Create the smallest runnable C++ example demonstrating **Template Method**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
