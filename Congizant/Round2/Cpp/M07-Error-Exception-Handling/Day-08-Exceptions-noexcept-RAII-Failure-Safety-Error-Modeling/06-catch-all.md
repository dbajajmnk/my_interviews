...
**catch-all** is the C++ engineering concept **catch-all**.

---

# 2. Why

Interviewers use **catch-all** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **catch-all** is about the C++ engineering concept **catch-all**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **catch-all** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **catch-all**.
...
Create the smallest runnable C++ example demonstrating **catch-all**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **catch-all** in plain English.
