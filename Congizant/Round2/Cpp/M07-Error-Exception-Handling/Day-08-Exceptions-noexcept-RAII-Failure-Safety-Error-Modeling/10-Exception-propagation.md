# Topic 10 — Exception propagation
...
**Exception propagation** is the C++ engineering concept **Exception propagation**.

---

# 2. Why

Interviewers use **Exception propagation** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Exception propagation** is about the C++ engineering concept **Exception propagation**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Exception propagation** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Exception propagation**.
...
Create the smallest runnable C++ example demonstrating **Exception propagation**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Exception propagation** in plain English.
