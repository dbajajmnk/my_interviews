# Topic 22 — std::optional as absence
...
**std::optional as absence** is value-or-absence wrapper without dynamic allocation requirement.

---

# 2. Why

Interviewers use **std::optional as absence** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **std::optional as absence** is about value-or-absence wrapper without dynamic allocation requirement. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

avoids sentinel values for optional return/state
...
In a production C++ system, **std::optional as absence** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **std::optional as absence**.
...
1. What is **std::optional as absence**?
...
Create the smallest runnable C++ example demonstrating **std::optional as absence**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **std::optional as absence** in plain English.
