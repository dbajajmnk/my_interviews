...
**One Definition Rule** is program-wide consistency requirement for definitions.

---

# 2. Why

Interviewers use **One Definition Rule** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **One Definition Rule** is about program-wide consistency requirement for definitions. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

violations can cause compile/link errors or undefined behavior, especially with headers/templates/inline entities
...
In a production C++ system, **One Definition Rule** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **One Definition Rule**.
...
Create the smallest runnable C++ example demonstrating **One Definition Rule**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **One Definition Rule** in plain English.
