...
**ABI awareness** is binary contract between separately compiled components.

---

# 2. Why

Interviewers use **ABI awareness** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **ABI awareness** is about binary contract between separately compiled components. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **ABI awareness** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **ABI awareness**.
...
Create the smallest runnable C++ example demonstrating **ABI awareness**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **ABI awareness** in plain English.
