# Topic 03 — Thread storage duration awareness
...
**Thread storage duration awareness** is OS-thread-backed std::thread execution.

---

# 2. Why

Interviewers use **Thread storage duration awareness** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Thread storage duration awareness** is about OS-thread-backed std::thread execution. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

thread must be joined or detached before destruction; jthread improves lifetime/cancellation
...
In a production C++ system, **Thread storage duration awareness** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Thread storage duration awareness**.
...
1. What is **Thread storage duration awareness**?
...
Create the smallest runnable C++ example demonstrating **Thread storage duration awareness**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Thread storage duration awareness** in plain English.
