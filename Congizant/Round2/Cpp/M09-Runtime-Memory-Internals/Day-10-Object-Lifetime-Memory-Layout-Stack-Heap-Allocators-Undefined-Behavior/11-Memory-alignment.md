...
**Memory alignment** is address constraints for object representation.

---

# 2. Why

Interviewers use **Memory alignment** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Memory alignment** is about address constraints for object representation. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

alignof and padding ensure members/objects satisfy alignment; packing can harm correctness/performance
...
```cpp
alignas(64) struct Counter { std::atomic<int> v; };
```
...
In a production C++ system, **Memory alignment** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Memory alignment**.
...
Create the smallest runnable C++ example demonstrating **Memory alignment**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Memory alignment** in plain English.
