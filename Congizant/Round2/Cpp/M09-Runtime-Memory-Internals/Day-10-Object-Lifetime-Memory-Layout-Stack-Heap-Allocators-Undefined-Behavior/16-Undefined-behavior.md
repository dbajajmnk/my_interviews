...
**Undefined behavior** is execution for which the C++ standard imposes no requirements.

---

# 2. Why

Interviewers use **Undefined behavior** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Undefined behavior** is about execution for which the C++ standard imposes no requirements. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Undefined behavior** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Undefined behavior**.
...
Create the smallest runnable C++ example demonstrating **Undefined behavior**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Undefined behavior** in plain English.
