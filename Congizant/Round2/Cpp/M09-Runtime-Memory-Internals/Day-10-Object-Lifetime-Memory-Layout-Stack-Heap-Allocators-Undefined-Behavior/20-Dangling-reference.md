...
**Dangling reference** is the C++ engineering concept **Dangling reference**.

---

# 2. Why

Interviewers use **Dangling reference** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Dangling reference** is about the C++ engineering concept **Dangling reference**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Dangling reference** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Dangling reference**.
...
Create the smallest runnable C++ example demonstrating **Dangling reference**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Dangling reference** in plain English.
