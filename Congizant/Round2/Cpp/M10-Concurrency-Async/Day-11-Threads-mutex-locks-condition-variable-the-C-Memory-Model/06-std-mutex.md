...
**std::mutex** is mutual exclusion around shared invariants.

---

# 2. Why

Interviewers use **std::mutex** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **std::mutex** is about mutual exclusion around shared invariants. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **std::mutex** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **std::mutex**.
...
Create the smallest runnable C++ example demonstrating **std::mutex**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **std::mutex** in plain English.
