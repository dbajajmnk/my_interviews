# Topic 14 — std::condition_variable
...
**std::condition_variable** is blocking wait/notify coordination.

---

# 2. Why

Interviewers use **std::condition_variable** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **std::condition_variable** is about blocking wait/notify coordination. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

wait must use a predicate due to spurious wakeups and lost timing assumptions
...
In a production C++ system, **std::condition_variable** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **std::condition_variable**.
...
1. What is **std::condition_variable**?
...
Create the smallest runnable C++ example demonstrating **std::condition_variable**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **std::condition_variable** in plain English.
