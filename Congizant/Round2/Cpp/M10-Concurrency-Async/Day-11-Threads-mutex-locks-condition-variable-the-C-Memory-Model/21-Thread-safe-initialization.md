# Topic 21 — Thread-safe initialization
...
**Thread-safe initialization** is OS-thread-backed std::thread execution.

---

# 2. Why

Interviewers use **Thread-safe initialization** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Thread-safe initialization** is about OS-thread-backed std::thread execution. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Thread-safe initialization** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Thread-safe initialization**.
...
1. What is **Thread-safe initialization**?
...
Create the smallest runnable C++ example demonstrating **Thread-safe initialization**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Thread-safe initialization** in plain English.
