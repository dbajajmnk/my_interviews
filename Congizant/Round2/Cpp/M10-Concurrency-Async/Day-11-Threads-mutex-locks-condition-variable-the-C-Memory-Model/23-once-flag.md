...
**once_flag** is the C++ engineering concept **once_flag**.

---

# 2. Why

Interviewers use **once_flag** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **once_flag** is about the C++ engineering concept **once_flag**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **once_flag** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **once_flag**.
...
Create the smallest runnable C++ example demonstrating **once_flag**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **once_flag** in plain English.
