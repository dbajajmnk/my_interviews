...
**Producer-consumer** is the C++ engineering concept **Producer-consumer**.

---

# 2. Why

Interviewers use **Producer-consumer** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Producer-consumer** is about the C++ engineering concept **Producer-consumer**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Producer-consumer** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Producer-consumer**.
...
Create the smallest runnable C++ example demonstrating **Producer-consumer**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Producer-consumer** in plain English.
