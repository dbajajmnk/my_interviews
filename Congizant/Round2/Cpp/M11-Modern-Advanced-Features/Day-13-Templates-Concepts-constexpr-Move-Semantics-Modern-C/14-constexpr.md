...
**constexpr** is allows compile-time evaluation when inputs/context permit.

---

# 2. Why

Interviewers use **constexpr** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **constexpr** is about allows compile-time evaluation when inputs/context permit. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **constexpr** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **constexpr**.
...
Create the smallest runnable C++ example demonstrating **constexpr**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **constexpr** in plain English.
