...
**std::move** is cast to an xvalue enabling move overload resolution.

---

# 2. Why

Interviewers use **std::move** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **std::move** is about cast to an xvalue enabling move overload resolution. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

std::move itself moves nothing; the selected move constructor/assignment decides what happens
...
In a production C++ system, **std::move** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **std::move**.
...
Create the smallest runnable C++ example demonstrating **std::move**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **std::move** in plain English.
