...
**std::optional** is value-or-absence wrapper without dynamic allocation requirement.

---

# 2. Why

Interviewers use **std::optional** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **std::optional** is about value-or-absence wrapper without dynamic allocation requirement. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **std::optional** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **std::optional**.
...
Create the smallest runnable C++ example demonstrating **std::optional**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **std::optional** in plain English.
