...
**std::unique_ptr** is exclusive RAII ownership.

---

# 2. Why

Interviewers use **std::unique_ptr** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **std::unique_ptr** is about exclusive RAII ownership. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

move-only smart pointer deletes owned object automatically
...
In a production C++ system, **std::unique_ptr** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **std::unique_ptr**.
...
Create the smallest runnable C++ example demonstrating **std::unique_ptr**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **std::unique_ptr** in plain English.
