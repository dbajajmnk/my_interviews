# Topic 21 — JSON library awareness
...
**JSON library awareness** is the C++ engineering concept **JSON library awareness**.

---

# 2. Why

Interviewers use **JSON library awareness** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **JSON library awareness** is about the C++ engineering concept **JSON library awareness**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **JSON library awareness** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **JSON library awareness**.
...
1. What is **JSON library awareness**?
...
Create the smallest runnable C++ example demonstrating **JSON library awareness**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **JSON library awareness** in plain English.
