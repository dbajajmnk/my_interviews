...
**Two pointers** is two-index algorithm pattern.

---

# 2. Why

Interviewers use **Two pointers** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Two pointers** is about two-index algorithm pattern. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

monotonic pointer movement can reduce repeated scans
...
```cpp
for(std::size_t l=0,r=a.size()-1;l<r;){break;}
```
...
In a production C++ system, **Two pointers** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Two pointers**.
...
Create the smallest runnable C++ example demonstrating **Two pointers**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
