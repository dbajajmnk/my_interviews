...
**Topological sort** is dependency ordering for DAGs.

---

# 2. Why

Interviewers use **Topological sort** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Topological sort** is about dependency ordering for DAGs. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

Kahn/DFS detects cycles and emits O(V+E) ordering
...
In a production C++ system, **Topological sort** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
Create the smallest runnable C++ example demonstrating **Topological sort**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
