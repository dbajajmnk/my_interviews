...
**GDB awareness** is native debugger.

---

# 2. Why

Interviewers use **GDB awareness** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **GDB awareness** is about native debugger. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

breakpoints/backtraces/watchpoints inspect compiled process state; optimized code may reorder/elide variables
...
In a production C++ system, **GDB awareness** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **GDB awareness**.
...
Create the smallest runnable C++ example demonstrating **GDB awareness**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **GDB awareness** in plain English.
