...
**AddressSanitizer** is runtime instrumentation for memory errors.

---

# 2. Why

Interviewers use **AddressSanitizer** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **AddressSanitizer** is about runtime instrumentation for memory errors. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

ASan detects many use-after-free/out-of-bounds issues with overhead
...
In a production C++ system, **AddressSanitizer** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **AddressSanitizer**.
...
Create the smallest runnable C++ example demonstrating **AddressSanitizer**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **AddressSanitizer** in plain English.
