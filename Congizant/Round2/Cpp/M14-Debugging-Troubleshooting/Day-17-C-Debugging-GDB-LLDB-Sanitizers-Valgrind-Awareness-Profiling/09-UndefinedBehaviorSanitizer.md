# Topic 09 — UndefinedBehaviorSanitizer
...
**UndefinedBehaviorSanitizer** is runtime checks for selected undefined behaviors.

---

# 2. Why

Interviewers use **UndefinedBehaviorSanitizer** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **UndefinedBehaviorSanitizer** is about runtime checks for selected undefined behaviors. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

UBSan detects categories such as invalid shifts/casts/overflow cases depending on flags
...
In a production C++ system, **UndefinedBehaviorSanitizer** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **UndefinedBehaviorSanitizer**.
...
1. What is **UndefinedBehaviorSanitizer**?
...
Create the smallest runnable C++ example demonstrating **UndefinedBehaviorSanitizer**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **UndefinedBehaviorSanitizer** in plain English.
