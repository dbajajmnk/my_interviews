...
**CPU profiling** is the C++ engineering concept **CPU profiling**.

---

# 2. Why

Interviewers use **CPU profiling** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **CPU profiling** is about the C++ engineering concept **CPU profiling**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **CPU profiling** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **CPU profiling**.
...
Create the smallest runnable C++ example demonstrating **CPU profiling**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
