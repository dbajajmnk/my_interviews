...
**Root-cause evidence** is the C++ engineering concept **Root-cause evidence**.

---

# 2. Why

Interviewers use **Root-cause evidence** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Root-cause evidence** is about the C++ engineering concept **Root-cause evidence**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Root-cause evidence** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Root-cause evidence**.
...
Create the smallest runnable C++ example demonstrating **Root-cause evidence**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Root-cause evidence** in plain English.
