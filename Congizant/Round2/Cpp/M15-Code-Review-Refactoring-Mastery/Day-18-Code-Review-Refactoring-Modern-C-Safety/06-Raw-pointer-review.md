...
**Raw pointer review** is the C++ engineering concept **Raw pointer review**.

---

# 2. Why

Interviewers use **Raw pointer review** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Raw pointer review** is about the C++ engineering concept **Raw pointer review**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Raw pointer review** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Raw pointer review**.
...
Create the smallest runnable C++ example demonstrating **Raw pointer review**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Raw pointer review** in plain English.
