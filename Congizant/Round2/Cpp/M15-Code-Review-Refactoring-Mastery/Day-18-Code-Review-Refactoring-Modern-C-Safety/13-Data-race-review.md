...
**Data race review** is unsynchronized conflicting memory access with at least one write.

---

# 2. Why

Interviewers use **Data race review** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Data race review** is about unsynchronized conflicting memory access with at least one write. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Data race review** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Data race review**.
...
Create the smallest runnable C++ example demonstrating **Data race review**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Data race review** in plain English.
