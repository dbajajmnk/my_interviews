...
**Const-correctness** is the C++ engineering concept **Const-correctness**.

---

# 2. Why

Interviewers use **Const-correctness** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Const-correctness** is about the C++ engineering concept **Const-correctness**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Const-correctness** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Const-correctness**.
...
Create the smallest runnable C++ example demonstrating **Const-correctness**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Const-correctness** in plain English.
