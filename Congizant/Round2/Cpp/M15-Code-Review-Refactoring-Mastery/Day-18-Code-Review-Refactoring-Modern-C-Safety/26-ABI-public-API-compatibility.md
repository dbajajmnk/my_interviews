# Topic 26 — ABI/public API compatibility
...
**ABI/public API compatibility** is binary contract between separately compiled components.

---

# 2. Why

Interviewers use **ABI/public API compatibility** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **ABI/public API compatibility** is about binary contract between separately compiled components. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

layout, name mangling, calling convention, standard-library/runtime choices and compiler flags can break compatibility
...
In a production C++ system, **ABI/public API compatibility** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **ABI/public API compatibility**.
...
1. What is **ABI/public API compatibility**?
...
Create the smallest runnable C++ example demonstrating **ABI/public API compatibility**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **ABI/public API compatibility** in plain English.
