...
**AI code as untrusted proposal** is the C++ engineering concept **AI code as untrusted proposal**.

---

# 2. Why

Interviewers use **AI code as untrusted proposal** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **AI code as untrusted proposal** is about the C++ engineering concept **AI code as untrusted proposal**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **AI code as untrusted proposal** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **AI code as untrusted proposal**.
...
Create the smallest runnable C++ example demonstrating **AI code as untrusted proposal**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
