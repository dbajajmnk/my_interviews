...
**Parameterized tests** is the C++ engineering concept **Parameterized tests**.

---

# 2. Why

Interviewers use **Parameterized tests** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Parameterized tests** is about the C++ engineering concept **Parameterized tests**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Parameterized tests** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Parameterized tests**.
...
Create the smallest runnable C++ example demonstrating **Parameterized tests**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
