...
**Integration tests** is the C++ engineering concept **Integration tests**.

---

# 2. Why

Interviewers use **Integration tests** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Integration tests** is about the C++ engineering concept **Integration tests**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Integration tests** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Integration tests**.
...
Create the smallest runnable C++ example demonstrating **Integration tests**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
