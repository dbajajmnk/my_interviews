...
**Sanitizers in CI** is the C++ engineering concept **Sanitizers in CI**.

---

# 2. Why

Interviewers use **Sanitizers in CI** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Sanitizers in CI** is about the C++ engineering concept **Sanitizers in CI**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Sanitizers in CI** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Sanitizers in CI**.
...
Create the smallest runnable C++ example demonstrating **Sanitizers in CI**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Sanitizers in CI** in plain English.
