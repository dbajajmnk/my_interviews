# Topic 17 — ThreadSanitizer tests
...
**ThreadSanitizer tests** is OS-thread-backed std::thread execution.

---

# 2. Why

Interviewers use **ThreadSanitizer tests** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **ThreadSanitizer tests** is about OS-thread-backed std::thread execution. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **ThreadSanitizer tests** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **ThreadSanitizer tests**.
...
1. What is **ThreadSanitizer tests**?
...
Create the smallest runnable C++ example demonstrating **ThreadSanitizer tests**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **ThreadSanitizer tests** in plain English.
