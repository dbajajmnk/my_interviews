...
**REST resource design** is resource-oriented HTTP API contract.

---

# 2. Why

Interviewers use **REST resource design** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **REST resource design** is about resource-oriented HTTP API contract. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

language/framework choice does not change method/status/idempotency/auth semantics
...
In a production C++ system, **REST resource design** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
Create the smallest runnable C++ example demonstrating **REST resource design**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
