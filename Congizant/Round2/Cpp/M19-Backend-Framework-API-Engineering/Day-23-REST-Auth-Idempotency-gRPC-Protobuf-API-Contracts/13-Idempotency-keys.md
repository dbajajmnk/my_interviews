...
**Idempotency keys** is duplicate-safe state-changing operation.

---

# 2. Why

Interviewers use **Idempotency keys** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Idempotency keys** is about duplicate-safe state-changing operation. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

persisted key/state prevents repeated effects across retries/processes
...
```cpp
auto key = request.header("Idempotency-Key");
```
...
In a production C++ system, **Idempotency keys** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
Create the smallest runnable C++ example demonstrating **Idempotency keys**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
