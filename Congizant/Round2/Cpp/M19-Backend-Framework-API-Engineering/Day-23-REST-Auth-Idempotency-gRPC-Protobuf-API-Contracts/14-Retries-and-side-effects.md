# Topic 14 — Retries and side effects
...
**Retries and side effects** is the C++ engineering concept **Retries and side effects**.

---

# 2. Why

Interviewers use **Retries and side effects** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Retries and side effects** is about the C++ engineering concept **Retries and side effects**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Retries and side effects** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Retries and side effects**.
...
Create the smallest runnable C++ example demonstrating **Retries and side effects**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
