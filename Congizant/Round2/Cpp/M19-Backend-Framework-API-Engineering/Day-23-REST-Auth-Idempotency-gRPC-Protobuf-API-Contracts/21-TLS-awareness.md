...
**TLS awareness** is the C++ engineering concept **TLS awareness**.

---

# 2. Why

Interviewers use **TLS awareness** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **TLS awareness** is about the C++ engineering concept **TLS awareness**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **TLS awareness** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **TLS awareness**.
...
Create the smallest runnable C++ example demonstrating **TLS awareness**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **TLS awareness** in plain English.
