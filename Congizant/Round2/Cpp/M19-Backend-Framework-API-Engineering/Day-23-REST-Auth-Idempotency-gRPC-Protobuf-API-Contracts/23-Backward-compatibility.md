# Topic 23 — Backward compatibility
...
**Backward compatibility** is the C++ engineering concept **Backward compatibility**.

---

# 2. Why

Interviewers use **Backward compatibility** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Backward compatibility** is about the C++ engineering concept **Backward compatibility**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Backward compatibility** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Backward compatibility**.
...
Create the smallest runnable C++ example demonstrating **Backward compatibility**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
