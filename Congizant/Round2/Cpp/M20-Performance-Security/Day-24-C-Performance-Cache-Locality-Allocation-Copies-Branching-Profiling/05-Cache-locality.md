...
**Cache locality** is keeping frequently accessed data close in memory.

---

# 2. Why

Interviewers use **Cache locality** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Cache locality** is about keeping frequently accessed data close in memory. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

contiguous data often improves cache behavior compared with pointer-heavy structures
...
In a production C++ system, **Cache locality** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Cache locality**.
...
Create the smallest runnable C++ example demonstrating **Cache locality**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Cache locality** in plain English.
