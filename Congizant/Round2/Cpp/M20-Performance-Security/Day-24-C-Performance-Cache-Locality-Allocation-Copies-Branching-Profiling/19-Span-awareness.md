...
**Span awareness** is non-owning view over contiguous elements.

---

# 2. Why

Interviewers use **Span awareness** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Span awareness** is about non-owning view over contiguous elements. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

does not own or extend lifetime and carries size information
...
In a production C++ system, **Span awareness** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Span awareness**.
...
Create the smallest runnable C++ example demonstrating **Span awareness**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Span awareness** in plain English.
