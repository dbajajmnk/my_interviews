...
**Buffer overflow** is writing beyond buffer bounds.

---

# 2. Why

Interviewers use **Buffer overflow** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Buffer overflow** is about writing beyond buffer bounds. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Buffer overflow** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Buffer overflow**.
...
Create the smallest runnable C++ example demonstrating **Buffer overflow**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Buffer overflow** in plain English.
