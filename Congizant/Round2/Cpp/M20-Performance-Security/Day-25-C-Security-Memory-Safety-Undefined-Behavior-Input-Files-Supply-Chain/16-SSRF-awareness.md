...
**SSRF awareness** is server-side request forgery.

---

# 2. Why

Interviewers use **SSRF awareness** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **SSRF awareness** is about server-side request forgery. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

validate destination and restrict egress/redirect/DNS resolution when fetching attacker-influenced URLs
...
In a production C++ system, **SSRF awareness** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **SSRF awareness**.
...
Create the smallest runnable C++ example demonstrating **SSRF awareness**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **SSRF awareness** in plain English.
