...
**Sanitizers** is the C++ engineering concept **Sanitizers**.

---

# 2. Why

Interviewers use **Sanitizers** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Sanitizers** is about the C++ engineering concept **Sanitizers**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Sanitizers** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Sanitizers**.
...
Create the smallest runnable C++ example demonstrating **Sanitizers**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Sanitizers** in plain English.
