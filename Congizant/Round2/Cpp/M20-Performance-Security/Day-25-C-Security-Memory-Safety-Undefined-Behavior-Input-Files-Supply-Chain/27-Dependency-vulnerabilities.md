# Topic 27 — Dependency vulnerabilities
...
**Dependency vulnerabilities** is binary contract between separately compiled components.

---

# 2. Why

Interviewers use **Dependency vulnerabilities** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Dependency vulnerabilities** is about binary contract between separately compiled components. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Dependency vulnerabilities** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Dependency vulnerabilities**.
...
1. What is **Dependency vulnerabilities**?
...
Create the smallest runnable C++ example demonstrating **Dependency vulnerabilities**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Dependency vulnerabilities** in plain English.
