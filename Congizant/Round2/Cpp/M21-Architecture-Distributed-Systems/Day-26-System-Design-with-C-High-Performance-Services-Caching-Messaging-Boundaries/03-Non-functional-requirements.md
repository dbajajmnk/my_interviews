...
**Non-functional requirements** is the C++ engineering concept **Non-functional requirements**.

---

# 2. Why

Interviewers use **Non-functional requirements** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Non-functional requirements** is about the C++ engineering concept **Non-functional requirements**. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Non-functional requirements** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Non-functional requirements**.
...
Create the smallest runnable C++ example demonstrating **Non-functional requirements**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
