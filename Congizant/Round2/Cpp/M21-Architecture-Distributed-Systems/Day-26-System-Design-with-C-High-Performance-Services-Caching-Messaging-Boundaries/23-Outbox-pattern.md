...
**Outbox pattern** is reliable event publication tied to local database transaction.

---

# 2. Why

Interviewers use **Outbox pattern** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Outbox pattern** is about reliable event publication tied to local database transaction. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

business row and outbox record commit atomically; publisher retries later
...
In a production C++ system, **Outbox pattern** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
Create the smallest runnable C++ example demonstrating **Outbox pattern**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
