...
**Outbox** is reliable event publication tied to local database transaction.

---

# 2. Why

Interviewers use **Outbox** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Outbox** is about reliable event publication tied to local database transaction. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Outbox** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Outbox**.
...
Create the smallest runnable C++ example demonstrating **Outbox**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
