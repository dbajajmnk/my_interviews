...
**Saga** is distributed workflow of local transactions and compensations.

---

# 2. Why

Interviewers use **Saga** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Saga** is about distributed workflow of local transactions and compensations. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.

---

# 4. Engineering Explanation

explicit state handles partial failure without global transaction
...
```cpp
enum class SagaState { Reserved, Charged, Failed };
```
...
In a production C++ system, **Saga** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Saga**.
...
Create the smallest runnable C++ example demonstrating **Saga**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
