# Topic 03 — Raw pointer to RAII migration
...
**Raw pointer to RAII migration** is resource lifetime bound to object lifetime.

---

# 2. Why

Interviewers use **Raw pointer to RAII migration** to test whether you understand C++ semantics, ownership, object lifetime, undefined behavior, concurrency, resource safety, and performance trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Raw pointer to RAII migration** is about resource lifetime bound to object lifetime. In C++, every low-level optimization must still preserve lifetime, ownership and defined behavior.
...
In a production C++ system, **Raw pointer to RAII migration** should connect to a concrete ownership, performance, concurrency, API, data, systems, or architecture requirement.
...
- Reproduce the smallest failure involving **Raw pointer to RAII migration**.
...
1. What is **Raw pointer to RAII migration**?
...
Create the smallest runnable C++ example demonstrating **Raw pointer to RAII migration**. Then introduce one realistic misuse, reproduce it using compiler/sanitizer/debugger evidence, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Raw pointer to RAII migration** in plain English.
