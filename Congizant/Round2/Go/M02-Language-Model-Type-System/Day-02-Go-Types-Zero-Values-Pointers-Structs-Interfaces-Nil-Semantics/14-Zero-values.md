...
**Zero values** is the automatically initialized default value of a Go type.

---

# 2. Why

Interviewers use **Zero values** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Zero values** is about the automatically initialized default value of a Go type. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

Go deliberately makes zero values usable for many types; nil applies to pointers, slices, maps, channels, functions and interfaces
...
In a production Go system, **Zero values** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Zero values**.
...
Create the smallest runnable Go example demonstrating **Zero values**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Zero values** in plain English.
