# Topic 20 — Implicit interface satisfaction
...
**Implicit interface satisfaction** is a behavior contract satisfied implicitly by method set.

---

# 2. Why

Interviewers use **Implicit interface satisfaction** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Implicit interface satisfaction** is about a behavior contract satisfied implicitly by method set. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **Implicit interface satisfaction** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Implicit interface satisfaction**.
...
1. What is **Implicit interface satisfaction**?
...
Create the smallest runnable Go example demonstrating **Implicit interface satisfaction**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Implicit interface satisfaction** in plain English.
