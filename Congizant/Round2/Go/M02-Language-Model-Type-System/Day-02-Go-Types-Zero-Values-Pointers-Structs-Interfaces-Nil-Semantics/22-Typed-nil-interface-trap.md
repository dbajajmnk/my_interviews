# Topic 22 — Typed nil interface trap
...
**Typed nil interface trap** is the interface case where dynamic type is non-nil but dynamic value is nil.

---

# 2. Why

Interviewers use **Typed nil interface trap** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Typed nil interface trap** is about the interface case where dynamic type is non-nil but dynamic value is nil. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **Typed nil interface trap** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Typed nil interface trap**.
...
1. What is **Typed nil interface trap**?
...
Create the smallest runnable Go example demonstrating **Typed nil interface trap**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Typed nil interface trap** in plain English.
