# Topic 04 — Multiple return values
...
**Multiple return values** is the Go engineering concept **Multiple return values**.

---

# 2. Why

Interviewers use **Multiple return values** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Multiple return values** is about the Go engineering concept **Multiple return values**. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **Multiple return values** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Multiple return values**.
...
1. What is **Multiple return values**?
...
Create the smallest runnable Go example demonstrating **Multiple return values**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Multiple return values** in plain English.
