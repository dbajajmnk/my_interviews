...
**Closures** is a function value capturing variables from lexical scope.

---

# 2. Why

Interviewers use **Closures** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Closures** is about a function value capturing variables from lexical scope. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

captured variables may escape depending on lifetime/escape analysis
...
In a production Go system, **Closures** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Closures**.
...
Create the smallest runnable Go example demonstrating **Closures**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
