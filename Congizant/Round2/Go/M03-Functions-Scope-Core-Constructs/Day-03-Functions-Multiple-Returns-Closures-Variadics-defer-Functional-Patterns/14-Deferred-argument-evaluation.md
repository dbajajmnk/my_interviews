# Topic 14 — Deferred argument evaluation
...
**Deferred argument evaluation** is scheduling a function call for surrounding function exit.

---

# 2. Why

Interviewers use **Deferred argument evaluation** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Deferred argument evaluation** is about scheduling a function call for surrounding function exit. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **Deferred argument evaluation** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Deferred argument evaluation**.
...
1. What is **Deferred argument evaluation**?
...
Create the smallest runnable Go example demonstrating **Deferred argument evaluation**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Deferred argument evaluation** in plain English.
