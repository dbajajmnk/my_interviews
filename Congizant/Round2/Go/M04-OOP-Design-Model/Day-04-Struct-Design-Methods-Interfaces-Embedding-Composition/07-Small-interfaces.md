...
**Small interfaces** is a behavior contract satisfied implicitly by method set.

---

# 2. Why

Interviewers use **Small interfaces** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Small interfaces** is about a behavior contract satisfied implicitly by method set. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **Small interfaces** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Small interfaces**.
...
Create the smallest runnable Go example demonstrating **Small interfaces**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Small interfaces** in plain English.
