...
Interviewers use **Single Responsibility Principle (SRP)** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Single Responsibility Principle (SRP)** is about one primary reason to change. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

separate transport, business policy, persistence and unrelated responsibilities
...
In a production Go system, **Single Responsibility Principle (SRP)** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Single Responsibility Principle (SRP)**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
