...
**Open/Closed Principle (OCP)** is support expected extension without repeated invasive edits.

---

# 2. Why

Interviewers use **Open/Closed Principle (OCP)** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Open/Closed Principle (OCP)** is about support expected extension without repeated invasive edits. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

use small interfaces/functions only at real variation points
...
In a production Go system, **Open/Closed Principle (OCP)** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Open/Closed Principle (OCP)**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
