...
**Liskov Substitution Principle (LSP)** is implementations preserve interface behavioral expectations.

---

# 2. Why

Interviewers use **Liskov Substitution Principle (LSP)** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Liskov Substitution Principle (LSP)** is about implementations preserve interface behavioral expectations. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

implicit satisfaction still requires semantic substitutability
...
In a production Go system, **Liskov Substitution Principle (LSP)** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Liskov Substitution Principle (LSP)**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
