...
**Interface Segregation Principle (ISP)** is consumers depend on narrow capabilities.

---

# 2. Why

Interviewers use **Interface Segregation Principle (ISP)** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Interface Segregation Principle (ISP)** is about consumers depend on narrow capabilities. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

Go strongly favors small consumer-owned interfaces
...
In a production Go system, **Interface Segregation Principle (ISP)** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Interface Segregation Principle (ISP)**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
