...
**Factory Method** is delegates creation behind a stable function/interface boundary.

---

# 2. Why

Interviewers use **Factory Method** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Factory Method** is about delegates creation behind a stable function/interface boundary. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

Go commonly uses constructor/factory functions rather than subclass creation hooks
...
```go
func NewParser(kind string) (Parser, error) { return nil, nil }
```
...
In a production Go system, **Factory Method** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Factory Method**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
