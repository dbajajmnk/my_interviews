...
**Builder** is constructs complex configuration incrementally.

---

# 2. Why

Interviewers use **Builder** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Builder** is about constructs complex configuration incrementally. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

Go usually prefers constructor + functional options unless staged construction is truly required
...
```go
client := NewClient(WithTimeout(time.Second), WithRetries(2))
```
...
In a production Go system, **Builder** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Builder**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
