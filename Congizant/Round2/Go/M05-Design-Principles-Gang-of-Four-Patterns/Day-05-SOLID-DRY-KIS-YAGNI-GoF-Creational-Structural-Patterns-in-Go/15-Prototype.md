...
**Prototype** is creates variants from an existing value/template.

---

# 2. Why

Interviewers use **Prototype** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Prototype** is about creates variants from an existing value/template. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

struct assignment plus explicit deep-copy rules are usually clearer than clone hierarchies
...
```go
next := base; next.Timeout = 5*time.Second
```
...
In a production Go system, **Prototype** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Prototype**.
...
Create the smallest runnable Go example demonstrating **Prototype**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
