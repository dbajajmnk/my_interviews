...
**Singleton** is provides one shared process-level instance.

---

# 2. Why

Interviewers use **Singleton** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Singleton** is about provides one shared process-level instance. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

package state or sync.Once can implement it, but explicit dependency injection is often easier to test
...
In a production Go system, **Singleton** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Singleton**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
