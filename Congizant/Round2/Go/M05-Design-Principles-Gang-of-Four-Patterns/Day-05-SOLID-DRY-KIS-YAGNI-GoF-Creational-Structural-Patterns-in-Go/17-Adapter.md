...
**Adapter** is translates an incompatible dependency into the interface your code expects.

---

# 2. Why

Interviewers use **Adapter** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Adapter** is about translates an incompatible dependency into the interface your code expects. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

wrap vendor types and map methods/results/errors at the boundary
...
```go
type LegacyAdapter struct{ c *LegacyClient }
```
...
In a production Go system, **Adapter** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Adapter**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
