...
**Composite** is treats leaf and group through the same interface.

---

# 2. Why

Interviewers use **Composite** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Composite** is about treats leaf and group through the same interface. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

group recursively delegates the operation to children
...
```go
type Rule interface { Valid(Context) bool }
```
...
In a production Go system, **Composite** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Composite**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
