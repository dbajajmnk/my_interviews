...
**Decorator** is adds behavior by wrapping the same interface/function contract.

---

# 2. Why

Interviewers use **Decorator** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Decorator** is about adds behavior by wrapping the same interface/function contract. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

wrappers are natural for metrics, tracing, retry, cache and auth
...
```go
func WithMetrics(next Handler) Handler { return next }
```
...
In a production Go system, **Decorator** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Decorator**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
