...
**Flyweight** is shares immutable intrinsic state when repeated instances are costly.

---

# 2. Why

Interviewers use **Flyweight** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Flyweight** is about shares immutable intrinsic state when repeated instances are costly. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

canonical metadata caches help only when measurement justifies them
...
In a production Go system, **Flyweight** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Flyweight**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
