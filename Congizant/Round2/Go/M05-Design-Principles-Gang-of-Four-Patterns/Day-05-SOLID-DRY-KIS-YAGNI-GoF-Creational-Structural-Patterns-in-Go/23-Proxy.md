...
Interviewers use **Proxy** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Proxy** is about controls access while preserving the service contract. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

explicit wrappers are usually clearer than reflection-heavy dynamic proxies
...
```go
type SecuredRepo struct{ next Repo }
```
...
In a production Go system, **Proxy** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Proxy**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
