...
**Command** is represents an action/request as a value.

---

# 2. Why

Interviewers use **Command** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Command** is about represents an action/request as a value. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

struct commands can be queued, logged, audited and dispatched
...
```go
type CreateOrder struct{ CustomerID string }
```
...
In a production Go system, **Command** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Command**.
...
Create the smallest runnable Go example demonstrating **Command**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
