...
**Iterator** is provides sequential traversal.

---

# 2. Why

Interviewers use **Iterator** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Iterator** is about provides sequential traversal. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

range handles native collections; custom iterator APIs should justify their complexity
...
In a production Go system, **Iterator** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Iterator**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
