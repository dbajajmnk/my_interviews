...
**Memento** is captures restorable state snapshots.

---

# 2. Why

Interviewers use **Memento** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Memento** is about captures restorable state snapshots. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

copyable structs are natural immutable-by-convention snapshots
...
```go
type Snapshot struct{ Text string; Cursor int }
```
...
In a production Go system, **Memento** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Memento**.
...
Create the smallest runnable Go example demonstrating **Memento**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
