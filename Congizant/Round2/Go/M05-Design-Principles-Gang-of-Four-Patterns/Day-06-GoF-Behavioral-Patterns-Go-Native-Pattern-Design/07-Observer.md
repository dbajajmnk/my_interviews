...
**Observer** is notifies subscribers about events.

---

# 2. Why

Interviewers use **Observer** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Observer** is about notifies subscribers about events. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

channels or explicit subscriber sets can implement it; lifecycle/backpressure must be defined
...
```go
type EventBus struct{ subs []chan Event }
```
...
In a production Go system, **Observer** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Observer**.
...
Create the smallest runnable Go example demonstrating **Observer**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
