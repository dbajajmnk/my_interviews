...
**State** is encapsulates state-dependent behavior.

---

# 2. Why

Interviewers use **State** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **State** is about encapsulates state-dependent behavior. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

explicit state values plus switch are often simpler than state classes
...
In a production Go system, **State** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **State**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
