...
**Template Method** is fixes an algorithm skeleton with customizable steps.

---

# 2. Why

Interviewers use **Template Method** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Template Method** is about fixes an algorithm skeleton with customizable steps. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

Go usually favors composition/function injection over inheritance templates
...
```go
func RunImport(read, validate, persist func() error) error { return nil }
```
...
In a production Go system, **Template Method** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Template Method**.
...
Create the smallest runnable Go example demonstrating **Template Method**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
