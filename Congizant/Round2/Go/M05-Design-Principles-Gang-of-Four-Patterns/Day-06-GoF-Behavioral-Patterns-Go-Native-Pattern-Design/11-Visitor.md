...
**Visitor** is adds operations over a stable heterogeneous structure.

---

# 2. Why

Interviewers use **Visitor** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Visitor** is about adds operations over a stable heterogeneous structure. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

type switches are often simpler than classical double dispatch
...
```go
switch n := node.(type) { case TextNode: _ = n }
```
...
In a production Go system, **Visitor** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Visitor**.
...
Create the smallest runnable Go example demonstrating **Visitor**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
