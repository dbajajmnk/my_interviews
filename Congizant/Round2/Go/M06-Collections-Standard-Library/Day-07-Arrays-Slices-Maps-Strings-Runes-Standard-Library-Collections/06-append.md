...
**append** is adding elements to a slice.

---

# 2. Why

Interviewers use **append** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **append** is about adding elements to a slice. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

append may mutate shared backing storage or allocate/copy depending on capacity
...
In a production Go system, **append** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **append**.
...
Create the smallest runnable Go example demonstrating **append**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **append** in plain English.
