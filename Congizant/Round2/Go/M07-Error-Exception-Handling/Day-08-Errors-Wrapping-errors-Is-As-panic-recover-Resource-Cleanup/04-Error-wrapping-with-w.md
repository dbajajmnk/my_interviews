# Topic 04 — Error wrapping with %w
...
**Error wrapping with %w** is preserving cause while adding context.

---

# 2. Why

Interviewers use **Error wrapping with %w** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Error wrapping with %w** is about preserving cause while adding context. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

fmt.Errorf with %w makes the underlying cause discoverable by Is/As
...
In a production Go system, **Error wrapping with %w** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Error wrapping with %w**.
...
1. What is **Error wrapping with %w**?
...
Create the smallest runnable Go example demonstrating **Error wrapping with %w**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Error wrapping with %w** in plain English.
