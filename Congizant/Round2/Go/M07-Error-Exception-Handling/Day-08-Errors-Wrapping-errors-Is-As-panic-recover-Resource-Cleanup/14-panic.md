...
**panic** is abnormal stack-unwinding control flow.

---

# 2. Why

Interviewers use **panic** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **panic** is about abnormal stack-unwinding control flow. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **panic** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **panic**.
...
Create the smallest runnable Go example demonstrating **panic**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **panic** in plain English.
