# Topic 20 — Retryable vs non-retryable errors
...
**Retryable vs non-retryable errors** is bounded repeated attempt for transient failure.

---

# 2. Why

Interviewers use **Retryable vs non-retryable errors** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Retryable vs non-retryable errors** is about bounded repeated attempt for transient failure. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

retries require deadline budget, idempotency, backoff and jitter
...
In a production Go system, **Retryable vs non-retryable errors** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Retryable vs non-retryable errors**.
...
Create the smallest runnable Go example demonstrating **Retryable vs non-retryable errors**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
