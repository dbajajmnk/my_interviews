...
**Garbage collector** is runtime reachability-based heap reclamation.

---

# 2. Why

Interviewers use **Garbage collector** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Garbage collector** is about runtime reachability-based heap reclamation. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

GC cost depends on live heap/allocation/workload; tune only after profiles/metrics
...
In a production Go system, **Garbage collector** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Garbage collector**.
...
Create the smallest runnable Go example demonstrating **Garbage collector**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Garbage collector** in plain English.
