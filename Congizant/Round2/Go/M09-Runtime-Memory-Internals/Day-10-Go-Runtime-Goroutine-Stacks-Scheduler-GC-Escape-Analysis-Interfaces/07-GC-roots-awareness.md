...
**GC roots awareness** is the Go engineering concept **GC roots awareness**.

---

# 2. Why

Interviewers use **GC roots awareness** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **GC roots awareness** is about the Go engineering concept **GC roots awareness**. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **GC roots awareness** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **GC roots awareness**.
...
Create the smallest runnable Go example demonstrating **GC roots awareness**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **GC roots awareness** in plain English.
