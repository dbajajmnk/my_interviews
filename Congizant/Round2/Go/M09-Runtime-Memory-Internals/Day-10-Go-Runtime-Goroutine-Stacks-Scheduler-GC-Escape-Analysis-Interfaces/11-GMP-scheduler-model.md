...
**GMP scheduler model** is runtime scheduling of goroutines on OS threads.

---

# 2. Why

Interviewers use **GMP scheduler model** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **GMP scheduler model** is about runtime scheduling of goroutines on OS threads. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **GMP scheduler model** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **GMP scheduler model**.
...
Create the smallest runnable Go example demonstrating **GMP scheduler model**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **GMP scheduler model** in plain English.
