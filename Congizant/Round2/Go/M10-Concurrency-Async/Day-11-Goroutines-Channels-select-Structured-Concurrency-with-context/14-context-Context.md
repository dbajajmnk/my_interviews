...
**context.Context** is request-scoped cancellation/deadline propagation mechanism.

---

# 2. Why

Interviewers use **context.Context** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **context.Context** is about request-scoped cancellation/deadline propagation mechanism. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **context.Context** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **context.Context**.
...
Create the smallest runnable Go example demonstrating **context.Context**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **context.Context** in plain English.
