# Topic 19 — Context values caution
...
**Context values caution** is request-scoped cancellation/deadline propagation mechanism.

---

# 2. Why

Interviewers use **Context values caution** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Context values caution** is about request-scoped cancellation/deadline propagation mechanism. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **Context values caution** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Context values caution**.
...
1. What is **Context values caution**?
...
Create the smallest runnable Go example demonstrating **Context values caution**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Context values caution** in plain English.
