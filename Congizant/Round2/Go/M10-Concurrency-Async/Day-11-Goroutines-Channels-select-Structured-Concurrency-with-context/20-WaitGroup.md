...
**WaitGroup** is counting synchronization for goroutine completion.

---

# 2. Why

Interviewers use **WaitGroup** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **WaitGroup** is about counting synchronization for goroutine completion. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **WaitGroup** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **WaitGroup**.
...
Create the smallest runnable Go example demonstrating **WaitGroup**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **WaitGroup** in plain English.
