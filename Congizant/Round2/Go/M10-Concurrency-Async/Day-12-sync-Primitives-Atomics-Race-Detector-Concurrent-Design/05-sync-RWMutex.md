...
**sync.RWMutex** is mutual exclusion for shared invariants.

---

# 2. Why

Interviewers use **sync.RWMutex** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **sync.RWMutex** is about mutual exclusion for shared invariants. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **sync.RWMutex** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **sync.RWMutex**.
...
Create the smallest runnable Go example demonstrating **sync.RWMutex**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **sync.RWMutex** in plain English.
