...
**sync.Once** is the Go engineering concept **sync.Once**.

---

# 2. Why

Interviewers use **sync.Once** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **sync.Once** is about the Go engineering concept **sync.Once**. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **sync.Once** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **sync.Once**.
...
Create the smallest runnable Go example demonstrating **sync.Once**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **sync.Once** in plain English.
