...
**WaitGroup misuse** is counting synchronization for goroutine completion.

---

# 2. Why

Interviewers use **WaitGroup misuse** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **WaitGroup misuse** is about counting synchronization for goroutine completion. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

Add/Done/Wait coordinate completion; misuse around copies/order can race or panic
...
In a production Go system, **WaitGroup misuse** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **WaitGroup misuse**.
...
Create the smallest runnable Go example demonstrating **WaitGroup misuse**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **WaitGroup misuse** in plain English.
