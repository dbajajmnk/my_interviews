...
**Generic types** is type-parameterized compile-time abstraction.

---

# 2. Why

Interviewers use **Generic types** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Generic types** is about type-parameterized compile-time abstraction. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **Generic types** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Generic types**.
...
Create the smallest runnable Go example demonstrating **Generic types**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Generic types** in plain English.
