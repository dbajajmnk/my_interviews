# Topic 12 — When not to use generics
...
**When not to use generics** is type-parameterized compile-time abstraction.

---

# 2. Why

Interviewers use **When not to use generics** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **When not to use generics** is about type-parameterized compile-time abstraction. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **When not to use generics** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **When not to use generics**.
...
1. What is **When not to use generics**?
...
Create the smallest runnable Go example demonstrating **When not to use generics**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **When not to use generics** in plain English.
