# Topic 19 — Compile-time interface assertions
...
**Compile-time interface assertions** is a behavior contract satisfied implicitly by method set.

---

# 2. Why

Interviewers use **Compile-time interface assertions** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Compile-time interface assertions** is about a behavior contract satisfied implicitly by method set. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **Compile-time interface assertions** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Compile-time interface assertions**.
...
1. What is **Compile-time interface assertions**?
...
Create the smallest runnable Go example demonstrating **Compile-time interface assertions**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Compile-time interface assertions** in plain English.
