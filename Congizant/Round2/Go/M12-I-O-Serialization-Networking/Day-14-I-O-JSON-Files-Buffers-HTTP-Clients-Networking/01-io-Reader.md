...
**io.Reader** is minimal streaming input interface.

---

# 2. Why

Interviewers use **io.Reader** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **io.Reader** is about minimal streaming input interface. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **io.Reader** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **io.Reader**.
...
Create the smallest runnable Go example demonstrating **io.Reader**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **io.Reader** in plain English.
