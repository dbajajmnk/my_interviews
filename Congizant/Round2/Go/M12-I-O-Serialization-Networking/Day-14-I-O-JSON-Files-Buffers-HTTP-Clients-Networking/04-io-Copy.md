...
**io.Copy** is the Go engineering concept **io.Copy**.

---

# 2. Why

Interviewers use **io.Copy** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **io.Copy** is about the Go engineering concept **io.Copy**. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **io.Copy** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **io.Copy**.
...
Create the smallest runnable Go example demonstrating **io.Copy**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **io.Copy** in plain English.
