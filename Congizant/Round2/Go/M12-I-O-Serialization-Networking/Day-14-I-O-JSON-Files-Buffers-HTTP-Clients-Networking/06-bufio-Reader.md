...
**bufio.Reader** is minimal streaming input interface.

---

# 2. Why

Interviewers use **bufio.Reader** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **bufio.Reader** is about minimal streaming input interface. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

Read fills a caller-provided buffer, enabling composition across files/network/buffers
...
In a production Go system, **bufio.Reader** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **bufio.Reader**.
...
Create the smallest runnable Go example demonstrating **bufio.Reader**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **bufio.Reader** in plain English.
