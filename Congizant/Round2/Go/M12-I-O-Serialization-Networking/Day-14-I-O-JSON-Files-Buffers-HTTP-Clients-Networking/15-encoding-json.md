...
**encoding/json** is encoding/decoding between Go values and JSON.

---

# 2. Why

Interviewers use **encoding/json** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **encoding/json** is about encoding/decoding between Go values and JSON. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **encoding/json** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **encoding/json**.
...
Create the smallest runnable Go example demonstrating **encoding/json**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **encoding/json** in plain English.
