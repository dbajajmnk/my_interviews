...
**JSON tags** is encoding/decoding between Go values and JSON.

---

# 2. Why

Interviewers use **JSON tags** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **JSON tags** is about encoding/decoding between Go values and JSON. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **JSON tags** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **JSON tags**.
...
Create the smallest runnable Go example demonstrating **JSON tags**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **JSON tags** in plain English.
