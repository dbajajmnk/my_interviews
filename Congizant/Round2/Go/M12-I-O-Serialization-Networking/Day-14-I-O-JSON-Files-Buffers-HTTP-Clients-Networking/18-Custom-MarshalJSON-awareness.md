# Topic 18 — Custom MarshalJSON awareness
...
**Custom MarshalJSON awareness** is encoding/decoding between Go values and JSON.

---

# 2. Why

Interviewers use **Custom MarshalJSON awareness** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Custom MarshalJSON awareness** is about encoding/decoding between Go values and JSON. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

runtime reflection/custom marshalers use exported fields/tags; decoding is not full business validation
...
In a production Go system, **Custom MarshalJSON awareness** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Custom MarshalJSON awareness**.
...
1. What is **Custom MarshalJSON awareness**?
...
Create the smallest runnable Go example demonstrating **Custom MarshalJSON awareness**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Custom MarshalJSON awareness** in plain English.
