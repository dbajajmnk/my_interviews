...
**HTTP client** is reusable HTTP client abstraction.

---

# 2. Why

Interviewers use **HTTP client** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **HTTP client** is about reusable HTTP client abstraction. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

Client/Transport manage deadlines, redirects and connection pooling; reuse transports and close bodies
...
In a production Go system, **HTTP client** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **HTTP client**.
...
Create the smallest runnable Go example demonstrating **HTTP client**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
