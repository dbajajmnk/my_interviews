...
**Sliding window** is incremental contiguous-range state.

---

# 2. Why

Interviewers use **Sliding window** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Sliding window** is about incremental contiguous-range state. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

update counts/sum as boundaries move rather than recomputing each interval
...
In a production Go system, **Sliding window** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Sliding window**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
