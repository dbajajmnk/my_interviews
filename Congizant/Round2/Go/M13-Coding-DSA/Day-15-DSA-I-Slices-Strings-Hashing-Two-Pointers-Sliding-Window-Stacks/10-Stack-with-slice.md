...
**Stack with slice** is a descriptor over a backing array.

---

# 2. Why

Interviewers use **Stack with slice** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Stack with slice** is about a descriptor over a backing array. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **Stack with slice** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Stack with slice**.
...
Create the smallest runnable Go example demonstrating **Stack with slice**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Stack with slice** in plain English.
