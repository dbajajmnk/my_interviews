...
**Race detector** is runtime instrumentation that detects unsynchronized conflicting memory access.

---

# 2. Why

Interviewers use **Race detector** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Race detector** is about runtime instrumentation that detects unsynchronized conflicting memory access. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

go test -race is essential evidence for concurrent code but cannot prove absence of all concurrency bugs
...
In a production Go system, **Race detector** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Race detector**.
...
Create the smallest runnable Go example demonstrating **Race detector**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Race detector** in plain English.
