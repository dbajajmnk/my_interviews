...
**HTTP trace awareness** is runtime execution trace.

---

# 2. Why

Interviewers use **HTTP trace awareness** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **HTTP trace awareness** is about runtime execution trace. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **HTTP trace awareness** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **HTTP trace awareness**.
...
1. What is **HTTP trace awareness**?
...
Create the smallest runnable Go example demonstrating **HTTP trace awareness**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **HTTP trace awareness** in plain English.
