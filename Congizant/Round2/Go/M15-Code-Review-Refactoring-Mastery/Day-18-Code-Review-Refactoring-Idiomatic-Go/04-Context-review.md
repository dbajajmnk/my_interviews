...
**Context review** is request-scoped cancellation/deadline propagation mechanism.

---

# 2. Why

Interviewers use **Context review** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Context review** is about request-scoped cancellation/deadline propagation mechanism. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **Context review** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Context review**.
...
Create the smallest runnable Go example demonstrating **Context review**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Context review** in plain English.
