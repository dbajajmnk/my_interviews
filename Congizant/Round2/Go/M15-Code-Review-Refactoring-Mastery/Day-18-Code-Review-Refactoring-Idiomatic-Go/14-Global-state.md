...
**Global state** is the Go engineering concept **Global state**.

---

# 2. Why

Interviewers use **Global state** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Global state** is about the Go engineering concept **Global state**. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **Global state** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Global state**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
