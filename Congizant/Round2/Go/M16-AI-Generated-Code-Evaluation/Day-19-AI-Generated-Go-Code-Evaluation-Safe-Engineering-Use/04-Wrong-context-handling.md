# Topic 04 — Wrong context handling
...
**Wrong context handling** is request-scoped cancellation/deadline propagation mechanism.

---

# 2. Why

Interviewers use **Wrong context handling** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Wrong context handling** is about request-scoped cancellation/deadline propagation mechanism. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **Wrong context handling** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Wrong context handling**.
...
1. What is **Wrong context handling**?
...
Create the smallest runnable Go example demonstrating **Wrong context handling**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Wrong context handling** in plain English.
