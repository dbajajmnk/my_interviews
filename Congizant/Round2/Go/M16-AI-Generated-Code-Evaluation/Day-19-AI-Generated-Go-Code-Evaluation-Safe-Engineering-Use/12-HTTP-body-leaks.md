...
**HTTP body leaks** is the Go engineering concept **HTTP body leaks**.

---

# 2. Why

Interviewers use **HTTP body leaks** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **HTTP body leaks** is about the Go engineering concept **HTTP body leaks**. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **HTTP body leaks** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **HTTP body leaks**.
...
Create the smallest runnable Go example demonstrating **HTTP body leaks**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **HTTP body leaks** in plain English.
