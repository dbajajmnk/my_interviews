...
**Fuzz testing** is input exploration through built-in fuzz targets.

---

# 2. Why

Interviewers use **Fuzz testing** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Fuzz testing** is about input exploration through built-in fuzz targets. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **Fuzz testing** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Fuzz testing**.
...
Create the smallest runnable Go example demonstrating **Fuzz testing**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Fuzz testing** in plain English.
