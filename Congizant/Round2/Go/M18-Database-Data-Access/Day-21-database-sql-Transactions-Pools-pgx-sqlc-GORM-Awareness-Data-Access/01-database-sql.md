...
**database/sql** is database abstraction and connection pool.

---

# 2. Why

Interviewers use **database/sql** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **database/sql** is about database abstraction and connection pool. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

sql.DB is a long-lived pool; context and pool settings affect saturation and latency
...
In a production Go system, **database/sql** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **database/sql**.
...
Create the smallest runnable Go example demonstrating **database/sql**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **database/sql** in plain English.
