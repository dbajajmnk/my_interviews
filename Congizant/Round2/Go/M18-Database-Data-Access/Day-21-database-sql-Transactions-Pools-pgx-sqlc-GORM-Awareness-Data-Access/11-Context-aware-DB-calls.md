# Topic 11 — Context-aware DB calls
...
**Context-aware DB calls** is request-scoped cancellation/deadline propagation mechanism.

---

# 2. Why

Interviewers use **Context-aware DB calls** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Context-aware DB calls** is about request-scoped cancellation/deadline propagation mechanism. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **Context-aware DB calls** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Context-aware DB calls**.
...
1. What is **Context-aware DB calls**?
...
Create the smallest runnable Go example demonstrating **Context-aware DB calls**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Context-aware DB calls** in plain English.
