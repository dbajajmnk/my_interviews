...
**Middleware pattern** is http.Handler wrapper pipeline.

---

# 2. Why

Interviewers use **Middleware pattern** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Middleware pattern** is about http.Handler wrapper pipeline. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

middleware executes cross-cutting behavior around next handler and must preserve cancellation/error/response semantics
...
In a production Go system, **Middleware pattern** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Middleware pattern**.
...
Create the smallest runnable Go example demonstrating **Middleware pattern**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Middleware pattern** in plain English.
