...
**Performance methodology** is measure-first optimization discipline.

---

# 2. Why

Interviewers use **Performance methodology** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Performance methodology** is about measure-first optimization discipline. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

pprof/trace/benchmarks/pool metrics distinguish CPU, allocation, lock, scheduler, I/O and downstream bottlenecks
...
In a production Go system, **Performance methodology** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Performance methodology**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
