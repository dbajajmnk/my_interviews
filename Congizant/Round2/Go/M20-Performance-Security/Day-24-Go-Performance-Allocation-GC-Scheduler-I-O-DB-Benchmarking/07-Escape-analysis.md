...
**Escape analysis** is compiler analysis deciding whether storage can remain on a goroutine stack.

---

# 2. Why

Interviewers use **Escape analysis** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Escape analysis** is about compiler analysis deciding whether storage can remain on a goroutine stack. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

use -gcflags=-m to inspect compiler decisions rather than guessing
...
In a production Go system, **Escape analysis** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Escape analysis**.
...
Create the smallest runnable Go example demonstrating **Escape analysis**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Escape analysis** in plain English.
