...
**Too many goroutines** is lightweight concurrent function managed by Go runtime.

---

# 2. Why

Interviewers use **Too many goroutines** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Too many goroutines** is about lightweight concurrent function managed by Go runtime. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **Too many goroutines** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Too many goroutines**.
...
Create the smallest runnable Go example demonstrating **Too many goroutines**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Too many goroutines** in plain English.
