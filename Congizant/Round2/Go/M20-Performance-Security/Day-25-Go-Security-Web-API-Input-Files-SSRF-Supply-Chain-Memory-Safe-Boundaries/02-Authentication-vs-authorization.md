...
**Authentication vs authorization** is runtime identity verification.

---

# 2. Why

Interviewers use **Authentication vs authorization** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Authentication vs authorization** is about runtime identity verification. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

credentials are validated and turned into a trusted principal; types alone prove nothing about caller identity
...
```go
principal,err:=auth.Verify(ctx,token)
```
...
In a production Go system, **Authentication vs authorization** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Authentication vs authorization**.
...
Create the smallest runnable Go example demonstrating **Authentication vs authorization**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
