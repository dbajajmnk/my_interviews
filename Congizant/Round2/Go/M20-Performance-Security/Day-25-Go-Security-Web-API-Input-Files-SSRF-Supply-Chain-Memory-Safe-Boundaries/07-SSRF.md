...
Interviewers use **SSRF** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **SSRF** is about server-side request forgery through attacker-controlled destinations. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

validate scheme/host/IP, redirects and enforce egress controls
...
In a production Go system, **SSRF** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **SSRF**.
...
Create the smallest runnable Go example demonstrating **SSRF**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
