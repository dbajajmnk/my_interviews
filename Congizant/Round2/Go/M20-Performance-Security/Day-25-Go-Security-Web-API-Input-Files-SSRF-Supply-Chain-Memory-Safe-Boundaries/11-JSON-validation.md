...
**JSON validation** is encoding/decoding between Go values and JSON.

---

# 2. Why

Interviewers use **JSON validation** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **JSON validation** is about encoding/decoding between Go values and JSON. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **JSON validation** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **JSON validation**.
...
Create the smallest runnable Go example demonstrating **JSON validation**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **JSON validation** in plain English.
