...
**Non-functional requirements** is the Go engineering concept **Non-functional requirements**.

---

# 2. Why

Interviewers use **Non-functional requirements** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Non-functional requirements** is about the Go engineering concept **Non-functional requirements**. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **Non-functional requirements** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Non-functional requirements**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
