...
**Outbox pattern** is reliable event-publication intent tied to a local DB transaction.

---

# 2. Why

Interviewers use **Outbox pattern** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Outbox pattern** is about reliable event-publication intent tied to a local DB transaction. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

business row and outbox record commit together; publisher retries later
...
In a production Go system, **Outbox pattern** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Outbox pattern**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
