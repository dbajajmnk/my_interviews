...
**Circuit breaker** is failure-isolation state machine.

---

# 2. Why

Interviewers use **Circuit breaker** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Circuit breaker** is about failure-isolation state machine. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

opens after repeated failure, rejects while unhealthy, probes recovery in half-open state
...
In a production Go system, **Circuit breaker** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Circuit breaker**.
...
Create the smallest runnable Go example demonstrating **Circuit breaker**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
