...
**Saga** is distributed business workflow using local transactions and compensation.

---

# 2. Why

Interviewers use **Saga** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Saga** is about distributed business workflow using local transactions and compensation. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

explicit states/compensation handle partial success without one global transaction
...
In a production Go system, **Saga** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Saga**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
