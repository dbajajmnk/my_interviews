# Topic 26 — Context cancellation across service calls
...
**Context cancellation across service calls** is request-scoped cancellation/deadline propagation mechanism.

---

# 2. Why

Interviewers use **Context cancellation across service calls** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Context cancellation across service calls** is about request-scoped cancellation/deadline propagation mechanism. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

Done signals cancellation and deadlines bound work; Context should be passed explicitly, not stored casually
...
In a production Go system, **Context cancellation across service calls** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Context cancellation across service calls**.
...
1. What is **Context cancellation across service calls**?
...
Create the smallest runnable Go example demonstrating **Context cancellation across service calls**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Context cancellation across service calls** in plain English.
