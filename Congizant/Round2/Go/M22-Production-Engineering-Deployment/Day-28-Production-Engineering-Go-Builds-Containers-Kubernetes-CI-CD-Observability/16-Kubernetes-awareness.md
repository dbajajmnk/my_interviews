...
**Kubernetes awareness** is orchestration of service replicas and traffic.

---

# 2. Why

Interviewers use **Kubernetes awareness** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Kubernetes awareness** is about orchestration of service replicas and traffic. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.

---

# 4. Engineering Explanation

probes/resources/rollouts must align with graceful shutdown and runtime behavior
...
In a production Go system, **Kubernetes awareness** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
Create the smallest runnable Go example demonstrating **Kubernetes awareness**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
