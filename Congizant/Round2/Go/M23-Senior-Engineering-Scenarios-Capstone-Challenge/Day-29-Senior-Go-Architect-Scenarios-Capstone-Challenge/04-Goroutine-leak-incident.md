# Topic 04 — Goroutine leak incident
...
**Goroutine leak incident** is lightweight concurrent function managed by Go runtime.

---

# 2. Why

Interviewers use **Goroutine leak incident** to test whether you understand Go semantics, ownership, runtime/resource behavior, concurrency correctness, failure handling, and production trade-offs—not merely syntax.

---

# 3. Plain English

Plain English: **Goroutine leak incident** is about lightweight concurrent function managed by Go runtime. In Go, prefer the simplest explicit mechanism that preserves correctness, ownership, and operability.
...
In a production Go system, **Goroutine leak incident** should connect to a concrete package, API, concurrency, data, networking, runtime, or architecture requirement.
...
- Reproduce the smallest failure involving **Goroutine leak incident**.
...
1. What is **Goroutine leak incident**?
...
Create the smallest runnable Go example demonstrating **Goroutine leak incident**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should have caught it.
...
- [ ] I can define **Goroutine leak incident** in plain English.
