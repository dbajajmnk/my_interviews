# Topic 03 — compile → bytecode → execute
...
**compile → bytecode → execute** is Java compilation and execution pipeline.

---

# 2. Why

Interviewers ask **compile → bytecode → execute** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **compile → bytecode → execute** is about Java compilation and execution pipeline. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

compiler emits bytecode; JVM loads/verifies/interprets/JIT-compiles execution paths. Within this day, the closest engineering anchor is **JDK / JRE / JVM**: JDK is the development kit, JRE is the runtime concept, and the JVM executes bytecode. Modern JDK distributions include the runtime needed to execute applications.
...
- compiler emits bytecode; JVM loads/verifies/interprets/JIT-compiles execution paths
...
- Use **compile → bytecode → execute** only when its semantics fit the requirement.
...
1. Memorizing the definition of **compile → bytecode → execute** without understanding runtime behavior.
...
- First reproduce the symptom involving **compile → bytecode → execute**.
- Primary focus: separate compile, linkage, and runtime failures.
...
- Does **compile → bytecode → execute** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **compile → bytecode → execute** correctly for the project's Java version.
...
1. What is **compile → bytecode → execute** in Java?
2. Why would you use **compile → bytecode → execute**?
3. How does **compile → bytecode → execute** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **compile → bytecode → execute**?
5. Where would **compile → bytecode → execute** appear in a real production system?
...
Create the smallest Java example that demonstrates **compile → bytecode → execute** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **compile → bytecode → execute** in plain English.
