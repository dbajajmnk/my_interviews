...
**type inference** is compile-time parameterized type safety.

---

# 2. Why

Interviewers ask **type inference** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **type inference** is about compile-time parameterized type safety. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

generic types are implemented mainly with erasure; wildcard bounds control variance at API boundaries. Within this day, the closest engineering anchor is **Primitive vs Reference**: Primitive variables contain primitive values; reference variables contain references to objects. Java passes both kinds of variable values by value.
...
- Use **type inference** only when its semantics fit the requirement.
...
1. Memorizing the definition of **type inference** without understanding runtime behavior.
...
- First reproduce the symptom involving **type inference**.
...
- Does **type inference** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **type inference** correctly for the project's Java version.
...
2. Why would you use **type inference**?
3. How does **type inference** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **type inference**?
5. Where would **type inference** appear in a real production system?
...
Create the smallest Java example that demonstrates **type inference** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
