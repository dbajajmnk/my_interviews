...
**overloading** is compile-time selection among methods with the same name.

---

# 2. Why

Interviewers ask **overloading** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **overloading** is about compile-time selection among methods with the same name. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

compiler selects an applicable most-specific signature; return type alone cannot overload. Within this day, the closest engineering anchor is **Method Overloading**: Overloading is resolved at compile time using method signatures and applicable conversions.
...
```java
void print(int x) {} void print(String x) {}
```
...
- compiler selects an applicable most-specific signature; return type alone cannot overload
...
- Use **overloading** only when its semantics fit the requirement.
...
1. Memorizing the definition of **overloading** without understanding runtime behavior.
...
- First reproduce the symptom involving **overloading**.
- Primary focus: reduce ambiguous wrapper/null overload sets.
...
- Does **overloading** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **overloading** correctly for the project's Java version.
...
2. Why would you use **overloading**?
3. How does **overloading** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **overloading**?
5. Where would **overloading** appear in a real production system?
...
Create the smallest Java example that demonstrates **overloading** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **overloading** in plain English.
