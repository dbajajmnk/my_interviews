# Topic 08 — functional interfaces
...
**functional interfaces** is behavior values targeting single-abstract-method interfaces.

---

# 2. Why

Interviewers ask **functional interfaces** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **functional interfaces** is about behavior values targeting single-abstract-method interfaces. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

lambda body is compiled to an invokedynamic-based runtime form rather than a simple anonymous-class rewrite. Within this day, the closest engineering anchor is **Functional Interface**: An interface with one abstract method can be a lambda target; default/static methods do not change that single-abstract-method rule.
...
- lambda body is compiled to an invokedynamic-based runtime form rather than a simple anonymous-class rewrite
...
- Use **functional interfaces** only when its semantics fit the requirement.
...
1. Memorizing the definition of **functional interfaces** without understanding runtime behavior.
...
- First reproduce the symptom involving **functional interfaces**.
- Primary focus: check target typing, captures, side effects, and checked exceptions.
...
- Does **functional interfaces** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **functional interfaces** correctly for the project's Java version.
...
1. What is **functional interfaces** in Java?
2. Why would you use **functional interfaces**?
3. How does **functional interfaces** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **functional interfaces**?
5. Where would **functional interfaces** appear in a real production system?
...
Create the smallest Java example that demonstrates **functional interfaces** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **functional interfaces** in plain English.
