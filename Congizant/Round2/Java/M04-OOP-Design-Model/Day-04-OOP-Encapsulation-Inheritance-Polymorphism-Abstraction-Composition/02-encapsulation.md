...
**encapsulation** is protecting state and invariants behind controlled operations.

---

# 2. Why

Interviewers ask **encapsulation** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **encapsulation** is about protecting state and invariants behind controlled operations. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

private state plus methods prevents arbitrary external mutation. Within this day, the closest engineering anchor is **Encapsulation**: Protect invariants by controlling state transitions rather than exposing fields directly.
...
```java
private BigDecimal balance; public void deposit(BigDecimal amount) { /* validate */ }
```
...
- private state plus methods prevents arbitrary external mutation
...
- Use **encapsulation** only when its semantics fit the requirement.
...
1. Memorizing the definition of **encapsulation** without understanding runtime behavior.
...
- First reproduce the symptom involving **encapsulation**.
- Primary focus: find setters/public fields that bypass invariants.
...
- Does **encapsulation** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **encapsulation** correctly for the project's Java version.
...
2. Why would you use **encapsulation**?
3. How does **encapsulation** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **encapsulation**?
5. Where would **encapsulation** appear in a real production system?
...
Create the smallest Java example that demonstrates **encapsulation** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
