# Topic 09 — composition over inheritance
...
**composition over inheritance** is subtype relationship with inherited behavior/state.

---

# 2. Why

Interviewers ask **composition over inheritance** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **composition over inheritance** is about subtype relationship with inherited behavior/state. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

runtime polymorphism dispatches overrides through a base reference. Within this day, the closest engineering anchor is **Inheritance**: Useful for true substitutable IS-A relationships, but deep hierarchies increase coupling.
...
```java
class PremiumCustomer extends Customer { @Override Money discount() { return null; } }
```
...
- runtime polymorphism dispatches overrides through a base reference
...
In a Java/Spring production system, **composition over inheritance** appears in the context of **OOP, Encapsulation, Inheritance, Polymorphism, Abstraction & Composition**. Connect it to the day's project concern: Model a notification system using an interface plus Email/SMS implementations. Then redesign an inheritance-heavy version using composition and compare extensibility/testability.
...
- Use **composition over inheritance** only when its semantics fit the requirement.
...
1. Memorizing the definition of **composition over inheritance** without understanding runtime behavior.
...
- First reproduce the symptom involving **composition over inheritance**.
- Primary focus: check substitutability and fragile hierarchy coupling.
...
- Does **composition over inheritance** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **composition over inheritance** correctly for the project's Java version.
...
1. What is **composition over inheritance** in Java?
2. Why would you use **composition over inheritance**?
3. How does **composition over inheritance** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **composition over inheritance**?
5. Where would **composition over inheritance** appear in a real production system?
...
Create the smallest Java example that demonstrates **composition over inheritance** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **composition over inheritance** in plain English.
