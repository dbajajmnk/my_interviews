# Topic 01 — Single Responsibility Principle (SRP)
...
**Single Responsibility Principle (SRP)** is a cohesive unit should have one primary reason to change.

---

# 2. Why

Interviewers ask **Single Responsibility Principle (SRP)** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **Single Responsibility Principle (SRP)** is about a cohesive unit should have one primary reason to change. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

split policy, persistence, transport, formatting, and unrelated change axes when they evolve independently. Within this day, the closest engineering anchor is **SRP**: A module should have one cohesive reason to change—not merely one method.
...
```java
final class InvoiceCalculator { Money total(Invoice invoice) { /* policy */ return null; } }
final class InvoiceRepository { void save(Invoice invoice) { /* persistence */ } }
```
...
- split policy, persistence, transport, formatting, and unrelated change axes when they evolve independently
...
In production Java code, **Single Responsibility Principle (SRP)** is useful when it reduces change propagation, makes contracts safer, or improves testability around a real change axis.
...
- Use **Single Responsibility Principle (SRP)** only when its semantics fit the requirement.
...
1. Memorizing the definition of **Single Responsibility Principle (SRP)** without understanding runtime behavior.
...
- First reproduce the symptom involving **Single Responsibility Principle (SRP)**.
- Primary focus: review change reasons/contracts/coupling and write a focused test.
...
- Does **Single Responsibility Principle (SRP)** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **Single Responsibility Principle (SRP)** correctly for the project's Java version.
...
1. What is **Single Responsibility Principle (SRP)** in Java?
2. Why would you use **Single Responsibility Principle (SRP)**?
3. How does **Single Responsibility Principle (SRP)** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **Single Responsibility Principle (SRP)**?
5. Where would **Single Responsibility Principle (SRP)** appear in a real production system?
...
Create the smallest Java example that demonstrates **Single Responsibility Principle (SRP)** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **Single Responsibility Principle (SRP)** in plain English.
