# Topic 02 — Open/Closed Principle (OCP)
...
**Open/Closed Principle (OCP)** is software should support expected extensions without repeated invasive modification.

---

# 2. Why

Interviewers ask **Open/Closed Principle (OCP)** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **Open/Closed Principle (OCP)** is about software should support expected extensions without repeated invasive modification. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

stable abstractions and composition isolate common variation points. Within this day, the closest engineering anchor is **OCP**: Design stable abstractions so common extensions need fewer invasive changes.
...
- stable abstractions and composition isolate common variation points
...
In production Java code, **Open/Closed Principle (OCP)** is useful when it reduces change propagation, makes contracts safer, or improves testability around a real change axis.
...
- Use **Open/Closed Principle (OCP)** only when its semantics fit the requirement.
...
1. Memorizing the definition of **Open/Closed Principle (OCP)** without understanding runtime behavior.
...
- First reproduce the symptom involving **Open/Closed Principle (OCP)**.
...
- Does **Open/Closed Principle (OCP)** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **Open/Closed Principle (OCP)** correctly for the project's Java version.
...
1. What is **Open/Closed Principle (OCP)** in Java?
2. Why would you use **Open/Closed Principle (OCP)**?
3. How does **Open/Closed Principle (OCP)** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **Open/Closed Principle (OCP)**?
5. Where would **Open/Closed Principle (OCP)** appear in a real production system?
...
Create the smallest Java example that demonstrates **Open/Closed Principle (OCP)** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **Open/Closed Principle (OCP)** in plain English.
