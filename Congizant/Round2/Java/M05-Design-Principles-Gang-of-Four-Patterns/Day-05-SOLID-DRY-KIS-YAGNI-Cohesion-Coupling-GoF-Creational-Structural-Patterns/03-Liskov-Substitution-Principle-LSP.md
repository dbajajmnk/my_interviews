# Topic 03 — Liskov Substitution Principle (LSP)
...
**Liskov Substitution Principle (LSP)** is a subtype must preserve the behavioral expectations of its abstraction.

---

# 2. Why

Interviewers ask **Liskov Substitution Principle (LSP)** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **Liskov Substitution Principle (LSP)** is about a subtype must preserve the behavioral expectations of its abstraction. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

subtypes cannot strengthen preconditions, break postconditions, or surprise clients with unsupported behavior. Within this day, the closest engineering anchor is **LSP**: Subtypes must preserve expectations of their abstraction.
...
```java
interface Storage { void save(byte[] data); } // every implementation must honor the same contract
```
...
- subtypes cannot strengthen preconditions, break postconditions, or surprise clients with unsupported behavior
...
In production Java code, **Liskov Substitution Principle (LSP)** is useful when it reduces change propagation, makes contracts safer, or improves testability around a real change axis.
...
- Use **Liskov Substitution Principle (LSP)** only when its semantics fit the requirement.
...
1. Memorizing the definition of **Liskov Substitution Principle (LSP)** without understanding runtime behavior.
...
- First reproduce the symptom involving **Liskov Substitution Principle (LSP)**.
...
- Does **Liskov Substitution Principle (LSP)** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **Liskov Substitution Principle (LSP)** correctly for the project's Java version.
...
1. What is **Liskov Substitution Principle (LSP)** in Java?
2. Why would you use **Liskov Substitution Principle (LSP)**?
3. How does **Liskov Substitution Principle (LSP)** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **Liskov Substitution Principle (LSP)**?
5. Where would **Liskov Substitution Principle (LSP)** appear in a real production system?
...
Create the smallest Java example that demonstrates **Liskov Substitution Principle (LSP)** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **Liskov Substitution Principle (LSP)** in plain English.
