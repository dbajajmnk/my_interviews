...
**Interface Segregation Principle (ISP)** is contracts and partial abstractions.

---

# 2. Why

Interviewers ask **Interface Segregation Principle (ISP)** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **Interface Segregation Principle (ISP)** is about contracts and partial abstractions. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

interfaces define capabilities; abstract classes may hold state/shared implementation. Within this day, the closest engineering anchor is **ISP**: Consumers should not depend on methods they do not need.
...
In production Java code, **Interface Segregation Principle (ISP)** is useful when it reduces change propagation, makes contracts safer, or improves testability around a real change axis.
...
- Use **Interface Segregation Principle (ISP)** only when its semantics fit the requirement.
...
1. Memorizing the definition of **Interface Segregation Principle (ISP)** without understanding runtime behavior.
...
- First reproduce the symptom involving **Interface Segregation Principle (ISP)**.
...
- Does **Interface Segregation Principle (ISP)** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **Interface Segregation Principle (ISP)** correctly for the project's Java version.
...
1. What is **Interface Segregation Principle (ISP)** in Java?
2. Why would you use **Interface Segregation Principle (ISP)**?
3. How does **Interface Segregation Principle (ISP)** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **Interface Segregation Principle (ISP)**?
5. Where would **Interface Segregation Principle (ISP)** appear in a real production system?
...
Create the smallest Java example that demonstrates **Interface Segregation Principle (ISP)** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
