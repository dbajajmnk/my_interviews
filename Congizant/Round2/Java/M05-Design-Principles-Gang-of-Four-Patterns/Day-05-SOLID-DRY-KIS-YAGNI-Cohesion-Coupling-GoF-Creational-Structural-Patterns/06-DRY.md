...
**DRY** is avoiding duplicated knowledge/rules.

---

# 2. Why

Interviewers ask **DRY** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **DRY** is about avoiding duplicated knowledge/rules. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

DRY targets duplicated decisions, not every repeated line. Within this day, the closest engineering anchor is **SRP**: A module should have one cohesive reason to change—not merely one method.
...
```java
Money tax(Money amount) { /* one policy source */ return null; }
```
...
- DRY targets duplicated decisions, not every repeated line
...
- Use **DRY** only when its semantics fit the requirement.
...
1. Memorizing the definition of **DRY** without understanding runtime behavior.
...
- First reproduce the symptom involving **DRY**.
- Primary focus: confirm duplication is truly the same change reason before abstracting.
...
- Does **DRY** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **DRY** correctly for the project's Java version.
...
1. What is **DRY** in Java?
2. Why would you use **DRY**?
3. How does **DRY** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **DRY**?
5. Where would **DRY** appear in a real production system?
...
Create the smallest Java example that demonstrates **DRY** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **DRY** in plain English.
