...
**cohesion/coupling** is how strongly responsibilities belong together and how dependent components are.

---

# 2. Why

Interviewers ask **cohesion/coupling** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **cohesion/coupling** is about how strongly responsibilities belong together and how dependent components are. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

high cohesion and deliberate low coupling reduce change propagation. Within this day, the closest engineering anchor is **SRP**: A module should have one cohesive reason to change—not merely one method.
...
```java
interface OrderRepository { Optional<Order> find(Id id); }
```
...
- high cohesion and deliberate low coupling reduce change propagation
...
In a Java/Spring production system, **cohesion/coupling** appears in the context of **SOLID, DRY, KIS, YAGNI, Cohesion/Coupling & GoF Creational/Structural Patterns**. Connect it to the day's project concern: Take a service that directly constructs DB, cache, and HTTP clients. Refactor using DIP. Then identify one real use case each for Builder, Adapter, Decorator, Facade, and Proxy.
...
- Use **cohesion/coupling** only when its semantics fit the requirement.
...
1. Memorizing the definition of **cohesion/coupling** without understanding runtime behavior.
...
- First reproduce the symptom involving **cohesion/coupling**.
- Primary focus: map reasons to change and dependency edges.
...
- Does **cohesion/coupling** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **cohesion/coupling** correctly for the project's Java version.
...
1. What is **cohesion/coupling** in Java?
2. Why would you use **cohesion/coupling**?
3. How does **cohesion/coupling** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **cohesion/coupling**?
5. Where would **cohesion/coupling** appear in a real production system?
...
Create the smallest Java example that demonstrates **cohesion/coupling** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **cohesion/coupling** in plain English.
