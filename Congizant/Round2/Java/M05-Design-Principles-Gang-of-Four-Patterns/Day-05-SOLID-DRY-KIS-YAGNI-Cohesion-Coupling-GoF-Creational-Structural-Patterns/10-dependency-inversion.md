...
**dependency inversion** is high-level policy depending on abstractions.

---

# 2. Why

Interviewers ask **dependency inversion** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **dependency inversion** is about high-level policy depending on abstractions. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

business code owns/depends on ports while infrastructure implements them. Within this day, the closest engineering anchor is **SRP**: A module should have one cohesive reason to change—not merely one method.
...
```java
final class Service { Service(Storage storage) {} }
```
...
- business code owns/depends on ports while infrastructure implements them
...
- Use **dependency inversion** only when its semantics fit the requirement.
...
1. Memorizing the definition of **dependency inversion** without understanding runtime behavior.
...
- First reproduce the symptom involving **dependency inversion**.
- Primary focus: inspect direct `new` of low-level clients inside policy code.
...
- Does **dependency inversion** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **dependency inversion** correctly for the project's Java version.
...
1. What is **dependency inversion** in Java?
2. Why would you use **dependency inversion**?
3. How does **dependency inversion** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **dependency inversion**?
5. Where would **dependency inversion** appear in a real production system?
...
Create the smallest Java example that demonstrates **dependency inversion** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **dependency inversion** in plain English.
