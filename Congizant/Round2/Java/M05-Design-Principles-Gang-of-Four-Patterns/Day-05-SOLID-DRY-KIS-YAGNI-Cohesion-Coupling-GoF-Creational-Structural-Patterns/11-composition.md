...
**composition** is building behavior by collaborating objects rather than inheriting implementation.

---

# 2. Why

Interviewers ask **composition** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **composition** is about building behavior by collaborating objects rather than inheriting implementation. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

an object delegates responsibilities to contained dependencies. Within this day, the closest engineering anchor is **SRP**: A module should have one cohesive reason to change—not merely one method.
...
```java
final class Service { private final Repository repo; Service(Repository repo){ this.repo=repo; } }
```
...
- an object delegates responsibilities to contained dependencies
...
- Use **composition** only when its semantics fit the requirement.
...
1. Memorizing the definition of **composition** without understanding runtime behavior.
...
- First reproduce the symptom involving **composition**.
- Primary focus: inspect dependency direction and object lifecycle.
...
- Does **composition** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **composition** correctly for the project's Java version.
...
2. Why would you use **composition**?
3. How does **composition** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **composition**?
5. Where would **composition** appear in a real production system?
...
Create the smallest Java example that demonstrates **composition** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **composition** in plain English.
