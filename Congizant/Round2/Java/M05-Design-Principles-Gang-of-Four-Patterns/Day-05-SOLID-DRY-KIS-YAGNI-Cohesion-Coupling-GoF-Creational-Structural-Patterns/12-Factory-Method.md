...
**Factory Method** is defines a creation operation that lets subclasses/implementations decide the concrete product.

---

# 2. Why

Interviewers ask **Factory Method** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **Factory Method** is about defines a creation operation that lets subclasses/implementations decide the concrete product. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

creator delegates object construction rather than hard-coding a concrete class. Within this day, the closest engineering anchor is **SRP**: A module should have one cohesive reason to change—not merely one method.
...
abstract class ParserFactory {
    ...
    Object parse(String input) { return create().parse(input); }
}
```
...
- creator delegates object construction rather than hard-coding a concrete class
...
In a real Java system, use **Factory Method** for provider-specific parser/client creation. Do not use the pattern merely to increase abstraction.
...
- Use **Factory Method** only when its semantics fit the requirement.
...
1. Memorizing the definition of **Factory Method** without understanding runtime behavior.
...
- First reproduce the symptom involving **Factory Method**.
- Primary focus: verify the pattern solves provider-specific parser/client creation; remove it if it only adds ceremony.
...
- Does **Factory Method** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **Factory Method** correctly for the project's Java version.
...
1. What is **Factory Method** in Java?
2. Why would you use **Factory Method**?
3. How does **Factory Method** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **Factory Method**?
5. Where would **Factory Method** appear in a real production system?
...
Create the smallest Java example that demonstrates **Factory Method** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **Factory Method** in plain English.
