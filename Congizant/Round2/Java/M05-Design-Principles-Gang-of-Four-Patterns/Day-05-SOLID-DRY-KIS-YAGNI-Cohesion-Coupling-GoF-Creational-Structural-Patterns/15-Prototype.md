...
**Prototype** is creates new objects by copying an existing configured object.

---

# 2. Why

Interviewers ask **Prototype** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **Prototype** is about creates new objects by copying an existing configured object. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

copy operation duplicates prototype state, with shallow/deep-copy semantics chosen deliberately. Within this day, the closest engineering anchor is **SRP**: A module should have one cohesive reason to change—not merely one method.
...
```java
record ReportConfig(String format, List<String> columns) {
    ReportConfig copyWithFormat(String newFormat) {
        return new ReportConfig(newFormat, List.copyOf(columns));
    }
}
```
...
- copy operation duplicates prototype state, with shallow/deep-copy semantics chosen deliberately
...
In a real Java system, use **Prototype** for cloning templates/configuration snapshots when copy semantics are clear. Do not use the pattern merely to increase abstraction.
...
- Use **Prototype** only when its semantics fit the requirement.
...
1. Memorizing the definition of **Prototype** without understanding runtime behavior.
...
- First reproduce the symptom involving **Prototype**.
- Primary focus: verify the pattern solves cloning templates/configuration snapshots when copy semantics are clear; remove it if it only adds ceremony.
...
- Does **Prototype** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **Prototype** correctly for the project's Java version.
...
1. What is **Prototype** in Java?
2. Why would you use **Prototype**?
3. How does **Prototype** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **Prototype**?
5. Where would **Prototype** appear in a real production system?
...
Create the smallest Java example that demonstrates **Prototype** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **Prototype** in plain English.
