...
**Adapter** is translates one interface into another interface expected by the client.

---

# 2. Why

Interviewers ask **Adapter** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **Adapter** is about translates one interface into another interface expected by the client. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

adapter wraps an incompatible dependency and maps calls/data. Within this day, the closest engineering anchor is **SRP**: A module should have one cohesive reason to change—not merely one method.
...
final class LegacyPaymentAdapter implements PaymentGateway {
    ...
    public Receipt pay(Money m) {
        return map(client.charge(m.amount()));
    }
}
```
...
- adapter wraps an incompatible dependency and maps calls/data
...
In a real Java system, use **Adapter** for third-party SDK or legacy-system integration. Do not use the pattern merely to increase abstraction.
...
- Use **Adapter** only when its semantics fit the requirement.
...
1. Memorizing the definition of **Adapter** without understanding runtime behavior.
...
- First reproduce the symptom involving **Adapter**.
- Primary focus: verify the pattern solves third-party SDK or legacy-system integration; remove it if it only adds ceremony.
...
- Does **Adapter** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **Adapter** correctly for the project's Java version.
...
1. What is **Adapter** in Java?
2. Why would you use **Adapter**?
3. How does **Adapter** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **Adapter**?
5. Where would **Adapter** appear in a real production system?
...
Create the smallest Java example that demonstrates **Adapter** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **Adapter** in plain English.
