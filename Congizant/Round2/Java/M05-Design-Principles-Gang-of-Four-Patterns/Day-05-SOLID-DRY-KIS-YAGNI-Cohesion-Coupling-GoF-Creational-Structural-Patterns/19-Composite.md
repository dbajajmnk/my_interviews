...
**Composite** is treats individual objects and groups uniformly through a common interface.

---

# 2. Why

Interviewers ask **Composite** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **Composite** is about treats individual objects and groups uniformly through a common interface. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

composite recursively delegates an operation to child components. Within this day, the closest engineering anchor is **SRP**: A module should have one cohesive reason to change—not merely one method.
...
```java
interface Rule { boolean valid(Context c); }
record AllRules(List<Rule> children) implements Rule {
    public boolean valid(Context c) {
        return children.stream().allMatch(r -> r.valid(c));
    }
}
```
...
- composite recursively delegates an operation to child components
...
In a real Java system, use **Composite** for hierarchical permissions, UI/tree structures, validation groups. Do not use the pattern merely to increase abstraction.
...
- Use **Composite** only when its semantics fit the requirement.
...
1. Memorizing the definition of **Composite** without understanding runtime behavior.
...
- First reproduce the symptom involving **Composite**.
- Primary focus: verify the pattern solves hierarchical permissions, UI/tree structures, validation groups; remove it if it only adds ceremony.
...
- Does **Composite** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **Composite** correctly for the project's Java version.
...
1. What is **Composite** in Java?
2. Why would you use **Composite**?
3. How does **Composite** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **Composite**?
5. Where would **Composite** appear in a real production system?
...
Create the smallest Java example that demonstrates **Composite** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **Composite** in plain English.
