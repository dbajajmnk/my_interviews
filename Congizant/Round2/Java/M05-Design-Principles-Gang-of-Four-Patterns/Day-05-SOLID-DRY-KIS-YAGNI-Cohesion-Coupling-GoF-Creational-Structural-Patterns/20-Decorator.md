...
**Decorator** is adds behavior by wrapping an object while preserving the same contract.

---

# 2. Why

Interviewers ask **Decorator** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **Decorator** is about adds behavior by wrapping an object while preserving the same contract. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

calls are intercepted before/after delegation to the wrapped component. Within this day, the closest engineering anchor is **SRP**: A module should have one cohesive reason to change—not merely one method.
...
```java
final class MetricsStorage implements Storage {
    private final Storage delegate;
    public void save(byte[] data) {
        timer.record(() -> delegate.save(data));
    }
}
```
...
- calls are intercepted before/after delegation to the wrapped component
...
In a real Java system, use **Decorator** for logging, metrics, retry, caching around a service. Do not use the pattern merely to increase abstraction.
...
- Use **Decorator** only when its semantics fit the requirement.
...
1. Memorizing the definition of **Decorator** without understanding runtime behavior.
...
- First reproduce the symptom involving **Decorator**.
- Primary focus: verify the pattern solves logging, metrics, retry, caching around a service; remove it if it only adds ceremony.
...
- Does **Decorator** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **Decorator** correctly for the project's Java version.
...
1. What is **Decorator** in Java?
2. Why would you use **Decorator**?
3. How does **Decorator** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **Decorator**?
5. Where would **Decorator** appear in a real production system?
...
Create the smallest Java example that demonstrates **Decorator** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **Decorator** in plain English.
