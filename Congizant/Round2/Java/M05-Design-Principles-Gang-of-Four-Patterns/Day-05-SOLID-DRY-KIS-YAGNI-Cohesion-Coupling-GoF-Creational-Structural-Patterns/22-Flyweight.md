...
**Flyweight** is shares reusable intrinsic state across many logical objects to reduce memory cost.

---

# 2. Why

Interviewers ask **Flyweight** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **Flyweight** is about shares reusable intrinsic state across many logical objects to reduce memory cost. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

shared immutable state is separated from per-use extrinsic state. Within this day, the closest engineering anchor is **SRP**: A module should have one cohesive reason to change—not merely one method.
...
```java
record ProductType(String code, String description) {}
// Reuse canonical ProductType instances from a cache.
```
...
- shared immutable state is separated from per-use extrinsic state
...
In a real Java system, use **Flyweight** for large numbers of repeated metadata/style/token objects when memory evidence justifies it. Do not use the pattern merely to increase abstraction.
...
- Use **Flyweight** only when its semantics fit the requirement.
...
1. Memorizing the definition of **Flyweight** without understanding runtime behavior.
...
- First reproduce the symptom involving **Flyweight**.
- Primary focus: verify the pattern solves large numbers of repeated metadata/style/token objects when memory evidence justifies it; remove it if it only adds ceremony.
...
- Does **Flyweight** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **Flyweight** correctly for the project's Java version.
...
1. What is **Flyweight** in Java?
2. Why would you use **Flyweight**?
3. How does **Flyweight** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **Flyweight**?
5. Where would **Flyweight** appear in a real production system?
...
Create the smallest Java example that demonstrates **Flyweight** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **Flyweight** in plain English.
