...
**Proxy** is controls access to another object while presenting a compatible interface.

---

# 2. Why

Interviewers ask **Proxy** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **Proxy** is about controls access to another object while presenting a compatible interface. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

proxy intercepts calls for authorization, lazy access, remote invocation, caching, or instrumentation. Within this day, the closest engineering anchor is **SRP**: A module should have one cohesive reason to change—not merely one method.
...
```java
final class SecuredReportService implements ReportService {
    private final ReportService delegate;
    public Report load(User u, long id) {
        authorize(u, id);
        return delegate.load(u, id);
    }
}
```
...
- proxy intercepts calls for authorization, lazy access, remote invocation, caching, or instrumentation
...
In a real Java system, use **Proxy** for Spring proxies for transactions/security; remote-service proxies. Do not use the pattern merely to increase abstraction.
...
- Use **Proxy** only when its semantics fit the requirement.
...
1. Memorizing the definition of **Proxy** without understanding runtime behavior.
...
- First reproduce the symptom involving **Proxy**.
- Primary focus: verify the pattern solves Spring proxies for transactions/security; remote-service proxies; remove it if it only adds ceremony.
...
- Does **Proxy** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **Proxy** correctly for the project's Java version.
...
1. What is **Proxy** in Java?
2. Why would you use **Proxy**?
3. How does **Proxy** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **Proxy**?
5. Where would **Proxy** appear in a real production system?
...
Create the smallest Java example that demonstrates **Proxy** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **Proxy** in plain English.
