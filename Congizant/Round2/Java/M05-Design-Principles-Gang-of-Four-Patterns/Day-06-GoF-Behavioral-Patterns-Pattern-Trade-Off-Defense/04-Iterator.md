...
**Iterator** is provides sequential traversal without exposing collection representation.

---

# 2. Why

Interviewers ask **Iterator** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **Iterator** is about provides sequential traversal without exposing collection representation. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

iterator maintains traversal position/state while the aggregate remains encapsulated. Within this day, the closest engineering anchor is **Strategy**: Encapsulates interchangeable algorithms behind one contract.
...
- iterator maintains traversal position/state while the aggregate remains encapsulated
...
In a real Java system, use **Iterator** for custom aggregate traversal; Java collections already provide Iterator. Do not use the pattern merely to increase abstraction.
...
- Use **Iterator** only when its semantics fit the requirement.
...
1. Memorizing the definition of **Iterator** without understanding runtime behavior.
...
- First reproduce the symptom involving **Iterator**.
- Primary focus: verify the pattern solves custom aggregate traversal; Java collections already provide Iterator; remove it if it only adds ceremony.
...
- Does **Iterator** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **Iterator** correctly for the project's Java version.
...
2. Why would you use **Iterator**?
3. How does **Iterator** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **Iterator**?
5. Where would **Iterator** appear in a real production system?
...
Create the smallest Java example that demonstrates **Iterator** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **Iterator** in plain English.
