...
**Mediator** is centralizes complex communication among collaborating objects.

---

# 2. Why

Interviewers ask **Mediator** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **Mediator** is about centralizes complex communication among collaborating objects. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

participants communicate through a mediator rather than many pairwise dependencies. Within this day, the closest engineering anchor is **Strategy**: Encapsulates interchangeable algorithms behind one contract.
...
- participants communicate through a mediator rather than many pairwise dependencies
...
In a real Java system, use **Mediator** for workflow coordination where peer-to-peer coupling is exploding. Do not use the pattern merely to increase abstraction.
...
- Use **Mediator** only when its semantics fit the requirement.
...
1. Memorizing the definition of **Mediator** without understanding runtime behavior.
...
- First reproduce the symptom involving **Mediator**.
- Primary focus: verify the pattern solves workflow coordination where peer-to-peer coupling is exploding; remove it if it only adds ceremony.
...
- Does **Mediator** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **Mediator** correctly for the project's Java version.
...
1. What is **Mediator** in Java?
2. Why would you use **Mediator**?
3. How does **Mediator** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **Mediator**?
5. Where would **Mediator** appear in a real production system?
...
Create the smallest Java example that demonstrates **Mediator** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **Mediator** in plain English.
