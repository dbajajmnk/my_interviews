...
**Visitor** is adds operations across a stable object structure without modifying every element type.

---

# 2. Why

Interviewers ask **Visitor** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **Visitor** is about adds operations across a stable object structure without modifying every element type. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

double dispatch routes element + visitor combination to the correct operation. Within this day, the closest engineering anchor is **Strategy**: Encapsulates interchangeable algorithms behind one contract.
...
```java
interface Visitor { void visit(TextNode n); void visit(ImageNode n); }
```
...
- double dispatch routes element + visitor combination to the correct operation
...
In a real Java system, use **Visitor** for compiler/AST or stable hierarchy with frequently added operations. Do not use the pattern merely to increase abstraction.
...
- Use **Visitor** only when its semantics fit the requirement.
...
1. Memorizing the definition of **Visitor** without understanding runtime behavior.
...
- First reproduce the symptom involving **Visitor**.
- Primary focus: verify the pattern solves compiler/AST or stable hierarchy with frequently added operations; remove it if it only adds ceremony.
...
- Does **Visitor** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **Visitor** correctly for the project's Java version.
...
1. What is **Visitor** in Java?
2. Why would you use **Visitor**?
3. How does **Visitor** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **Visitor**?
5. Where would **Visitor** appear in a real production system?
...
Create the smallest Java example that demonstrates **Visitor** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **Visitor** in plain English.
