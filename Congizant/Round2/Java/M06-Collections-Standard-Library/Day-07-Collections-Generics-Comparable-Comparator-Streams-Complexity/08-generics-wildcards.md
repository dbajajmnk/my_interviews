...
**generics/wildcards** is compile-time parameterized type safety.

---

# 2. Why

Interviewers ask **generics/wildcards** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **generics/wildcards** is about compile-time parameterized type safety. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

generic types are implemented mainly with erasure; wildcard bounds control variance at API boundaries. Within this day, the closest engineering anchor is **Collection Choice**: Choose from access pattern: indexed access, uniqueness, key lookup, ordering, priority, FIFO/LIFO.
...
- generic types are implemented mainly with erasure; wildcard bounds control variance at API boundaries
...
In a Java/Spring production system, **generics/wildcards** appears in the context of **Collections, Generics, Comparable/Comparator, Streams & Complexity**. Connect it to the day's project concern: Implement word frequency, LRU-like access reasoning, top-K with PriorityQueue, deduplication with Set, and custom sorting with Comparator. State time/space complexity.
...
- Use **generics/wildcards** only when its semantics fit the requirement.
...
1. Memorizing the definition of **generics/wildcards** without understanding runtime behavior.
...
- First reproduce the symptom involving **generics/wildcards**.
- Primary focus: enable warnings and remove raw/unchecked assumptions.
...
- Does **generics/wildcards** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **generics/wildcards** correctly for the project's Java version.
...
1. What is **generics/wildcards** in Java?
2. Why would you use **generics/wildcards**?
3. How does **generics/wildcards** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **generics/wildcards**?
5. Where would **generics/wildcards** appear in a real production system?
...
Create the smallest Java example that demonstrates **generics/wildcards** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **generics/wildcards** in plain English.
