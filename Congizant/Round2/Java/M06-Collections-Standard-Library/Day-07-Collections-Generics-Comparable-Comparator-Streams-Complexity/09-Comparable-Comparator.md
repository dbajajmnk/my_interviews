# Topic 09 — Comparable/Comparator
...
**Comparable/Comparator** is natural/external ordering contracts.

---

# 2. Why

Interviewers ask **Comparable/Comparator** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **Comparable/Comparator** is about natural/external ordering contracts. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

Comparable belongs to type's natural order; Comparator is external/composable. Within this day, the closest engineering anchor is **Collection Choice**: Choose from access pattern: indexed access, uniqueness, key lookup, ordering, priority, FIFO/LIFO.
...
- Comparable belongs to type's natural order; Comparator is external/composable
...
In a Java/Spring production system, **Comparable/Comparator** appears in the context of **Collections, Generics, Comparable/Comparator, Streams & Complexity**. Connect it to the day's project concern: Implement word frequency, LRU-like access reasoning, top-K with PriorityQueue, deduplication with Set, and custom sorting with Comparator. State time/space complexity.
...
- Use **Comparable/Comparator** only when its semantics fit the requirement.
...
1. Memorizing the definition of **Comparable/Comparator** without understanding runtime behavior.
...
- First reproduce the symptom involving **Comparable/Comparator**.
- Primary focus: verify comparator consistency and overflow-prone subtraction.
...
- Does **Comparable/Comparator** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **Comparable/Comparator** correctly for the project's Java version.
...
1. What is **Comparable/Comparator** in Java?
2. Why would you use **Comparable/Comparator**?
3. How does **Comparable/Comparator** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **Comparable/Comparator**?
5. Where would **Comparable/Comparator** appear in a real production system?
...
Create the smallest Java example that demonstrates **Comparable/Comparator** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **Comparable/Comparator** in plain English.
