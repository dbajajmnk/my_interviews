...
intermediate operations are lazy; terminal operation drives traversal. Within this day, the closest engineering anchor is **Streams**: A pipeline describes transformations; streams are not data structures and should not be used when a simple loop is clearer.
...
In a Java/Spring production system, **streams** appears in the context of **Collections, Generics, Comparable/Comparator, Streams & Complexity**. Connect it to the day's project concern: Implement word frequency, LRU-like access reasoning, top-K with PriorityQueue, deduplication with Set, and custom sorting with Comparator. State time/space complexity.
