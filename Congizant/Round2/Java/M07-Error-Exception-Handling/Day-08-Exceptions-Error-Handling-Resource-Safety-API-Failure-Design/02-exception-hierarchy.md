...
**exception hierarchy** is Java throwable taxonomy and recovery contracts.

---

# 2. Why

Interviewers ask **exception hierarchy** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **exception hierarchy** is about Java throwable taxonomy and recovery contracts. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

checked exceptions participate in compile-time handling; runtime exceptions do not require declaration. Within this day, the closest engineering anchor is **Exception Translation**: Infrastructure exceptions should often be translated into domain/application exceptions at boundaries without losing root cause.
...
- checked exceptions participate in compile-time handling; runtime exceptions do not require declaration
...
- Use **exception hierarchy** only when its semantics fit the requirement.
...
1. Memorizing the definition of **exception hierarchy** without understanding runtime behavior.
...
- First reproduce the symptom involving **exception hierarchy**.
- Primary focus: preserve causes and avoid blanket catch.
...
- Does **exception hierarchy** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **exception hierarchy** correctly for the project's Java version.
...
1. What is **exception hierarchy** in Java?
2. Why would you use **exception hierarchy**?
3. How does **exception hierarchy** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **exception hierarchy**?
5. Where would **exception hierarchy** appear in a real production system?
...
Create the smallest Java example that demonstrates **exception hierarchy** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **exception hierarchy** in plain English.
