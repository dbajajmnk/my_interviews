...
**error contracts** is stable failure communication and operational evidence.

---

# 2. Why

Interviewers ask **error contracts** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **error contracts** is about stable failure communication and operational evidence. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

internal exception details are mapped to non-sensitive external codes while logs preserve trace context. Within this day, the closest engineering anchor is **API Error Contract**: External APIs need stable, non-sensitive error codes/messages rather than raw stack traces.
...
- Use **error contracts** only when its semantics fit the requirement.
...
1. Memorizing the definition of **error contracts** without understanding runtime behavior.
...
- First reproduce the symptom involving **error contracts**.
...
- Does **error contracts** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **error contracts** correctly for the project's Java version.
...
1. What is **error contracts** in Java?
2. Why would you use **error contracts**?
3. How does **error contracts** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **error contracts**?
5. Where would **error contracts** appear in a real production system?
...
Create the smallest Java example that demonstrates **error contracts** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **error contracts** in plain English.
