...
**dependency scopes** is lexical visibility and lifetime of names.

---

# 2. Why

Interviewers ask **dependency scopes** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **dependency scopes** is about lexical visibility and lifetime of names. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

locals/parameters/fields/class members follow different scopes; lambdas capture lexical context. Within this day, the closest engineering anchor is **Dependency Management**: Transitive dependencies and version convergence matter for security and runtime compatibility.
...
- Use **dependency scopes** only when its semantics fit the requirement.
...
1. Memorizing the definition of **dependency scopes** without understanding runtime behavior.
...
- First reproduce the symptom involving **dependency scopes**.
...
- Does **dependency scopes** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **dependency scopes** correctly for the project's Java version.
...
1. What is **dependency scopes** in Java?
2. Why would you use **dependency scopes**?
3. How does **dependency scopes** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **dependency scopes**?
5. Where would **dependency scopes** appear in a real production system?
...
Create the smallest Java example that demonstrates **dependency scopes** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **dependency scopes** in plain English.
