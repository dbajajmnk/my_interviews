...
**generational GC** is automatic reachability-based memory reclamation.

---

# 2. Why

Interviewers ask **generational GC** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **generational GC** is about automatic reachability-based memory reclamation. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

collectors identify unreachable objects from GC roots with collector-specific pause/throughput behavior. Within this day, the closest engineering anchor is **GC**: Garbage collectors reclaim objects unreachable from GC roots. Different collectors optimize throughput, pause, and footprint trade-offs.
...
```java
System.gc(); // generally not an application-level fix
```
...
- collectors identify unreachable objects from GC roots with collector-specific pause/throughput behavior
...
- Use **generational GC** only when its semantics fit the requirement.
...
1. Memorizing the definition of **generational GC** without understanding runtime behavior.
...
- First reproduce the symptom involving **generational GC**.
- Primary focus: measure allocation, live set, pause and collector behavior before tuning.
...
- Does **generational GC** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **generational GC** correctly for the project's Java version.
...
1. What is **generational GC** in Java?
2. Why would you use **generational GC**?
3. How does **generational GC** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **generational GC**?
5. Where would **generational GC** appear in a real production system?
...
Create the smallest Java example that demonstrates **generational GC** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **generational GC** in plain English.
