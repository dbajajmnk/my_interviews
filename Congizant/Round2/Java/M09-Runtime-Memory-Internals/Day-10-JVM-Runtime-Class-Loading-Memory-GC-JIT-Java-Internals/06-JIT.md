...
**JIT** is runtime optimization based on observed execution.

---

# 2. Why

Interviewers ask **JIT** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **JIT** is about runtime optimization based on observed execution. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

hot bytecode may be compiled/optimized; escape analysis can eliminate some allocations/locks. Within this day, the closest engineering anchor is **JIT**: Frequently executed bytecode can be compiled/optimized at runtime based on profiling.
...
- Use **JIT** only when its semantics fit the requirement.
...
1. Memorizing the definition of **JIT** without understanding runtime behavior.
...
- First reproduce the symptom involving **JIT**.
...
- Does **JIT** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **JIT** correctly for the project's Java version.
...
2. Why would you use **JIT**?
3. How does **JIT** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **JIT**?
5. Where would **JIT** appear in a real production system?
...
Create the smallest Java example that demonstrates **JIT** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **JIT** in plain English.
