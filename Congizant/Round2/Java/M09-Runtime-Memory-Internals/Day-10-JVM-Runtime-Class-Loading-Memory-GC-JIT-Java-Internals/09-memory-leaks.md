...
**memory leaks** is JVM memory ownership and reachability.

---

# 2. Why

Interviewers ask **memory leaks** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **memory leaks** is about JVM memory ownership and reachability. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

thread stacks hold frames; heap holds most objects; metaspace stores class metadata; GC follows reachability. Within this day, the closest engineering anchor is **Java Memory Leak**: GC cannot reclaim objects that remain reachable accidentally—e.g., unbounded caches/listeners/static collections.
...
- Use **memory leaks** only when its semantics fit the requirement.
...
1. Memorizing the definition of **memory leaks** without understanding runtime behavior.
...
- First reproduce the symptom involving **memory leaks**.
...
- Does **memory leaks** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **memory leaks** correctly for the project's Java version.
...
2. Why would you use **memory leaks**?
3. How does **memory leaks** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **memory leaks**?
5. Where would **memory leaks** appear in a real production system?
...
Create the smallest Java example that demonstrates **memory leaks** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
