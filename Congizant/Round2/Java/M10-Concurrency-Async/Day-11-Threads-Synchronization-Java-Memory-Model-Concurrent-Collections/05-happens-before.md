...
**happens-before** is formal visibility/order relationship in the Java Memory Model.

---

# 2. Why

Interviewers ask **happens-before** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **happens-before** is about formal visibility/order relationship in the Java Memory Model. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

specified actions such as lock/unlock and volatile write/read establish visibility guarantees. Within this day, the closest engineering anchor is **Happens-Before**: Java Memory Model relation defining when writes are guaranteed visible to reads.
...
```java
// unlock(lock) happens-before later lock(lock)
```
...
- specified actions such as lock/unlock and volatile write/read establish visibility guarantees
...
- Use **happens-before** only when its semantics fit the requirement.
...
1. Memorizing the definition of **happens-before** without understanding runtime behavior.
...
- First reproduce the symptom involving **happens-before**.
- Primary focus: reason from guarantees rather than observed timing.
...
- Does **happens-before** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **happens-before** correctly for the project's Java version.
...
1. What is **happens-before** in Java?
2. Why would you use **happens-before**?
3. How does **happens-before** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **happens-before**?
5. Where would **happens-before** appear in a real production system?
...
Create the smallest Java example that demonstrates **happens-before** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **happens-before** in plain English.
