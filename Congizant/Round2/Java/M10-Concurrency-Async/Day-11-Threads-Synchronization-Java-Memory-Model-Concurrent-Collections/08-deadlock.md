...
**deadlock** is cycle of threads waiting forever for locks/resources.

---

# 2. Why

Interviewers ask **deadlock** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **deadlock** is about cycle of threads waiting forever for locks/resources. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

inconsistent lock ordering can create a wait-for cycle. Within this day, the closest engineering anchor is **Race Condition**: Correctness depends on unpredictable interleaving of concurrent operations.
...
- inconsistent lock ordering can create a wait-for cycle
...
- Use **deadlock** only when its semantics fit the requirement.
...
1. Memorizing the definition of **deadlock** without understanding runtime behavior.
...
- First reproduce the symptom involving **deadlock**.
- Primary focus: capture thread dump and enforce lock ordering.
...
- Does **deadlock** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **deadlock** correctly for the project's Java version.
...
1. What is **deadlock** in Java?
2. Why would you use **deadlock**?
3. How does **deadlock** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **deadlock**?
5. Where would **deadlock** appear in a real production system?
...
Create the smallest Java example that demonstrates **deadlock** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **deadlock** in plain English.
