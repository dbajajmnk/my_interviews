...
**backpressure** is limiting work when downstream cannot keep up.

---

# 2. Why

Interviewers ask **backpressure** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **backpressure** is about limiting work when downstream cannot keep up. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

bounded queues/semaphores/rate limits prevent unbounded in-flight growth. Within this day, the closest engineering anchor is **Backpressure**: Concurrency must be bounded against DB/provider capacity even when threads are cheap.
...
- bounded queues/semaphores/rate limits prevent unbounded in-flight growth
...
- Use **backpressure** only when its semantics fit the requirement.
...
1. Memorizing the definition of **backpressure** without understanding runtime behavior.
...
- First reproduce the symptom involving **backpressure**.
- Primary focus: monitor saturation and rejection/queue age.
...
- Does **backpressure** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **backpressure** correctly for the project's Java version.
...
1. What is **backpressure** in Java?
2. Why would you use **backpressure**?
3. How does **backpressure** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **backpressure**?
5. Where would **backpressure** appear in a real production system?
...
Create the smallest Java example that demonstrates **backpressure** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **backpressure** in plain English.
