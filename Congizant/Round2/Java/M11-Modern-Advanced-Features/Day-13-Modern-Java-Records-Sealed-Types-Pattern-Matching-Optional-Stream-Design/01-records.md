...
**records** is concise nominal data carrier.

---

# 2. Why

Interviewers ask **records** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **records** is about concise nominal data carrier. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

compiler generates final component fields, accessors, equality/hashCode/toString; component objects may still be mutable. Within this day, the closest engineering anchor is **Records**: Concise nominal data carriers with generated accessors, equality, hashCode, and toString. They are shallowly immutable with final component references.
...
- Use **records** only when its semantics fit the requirement.
...
1. Memorizing the definition of **records** without understanding runtime behavior.
...
- First reproduce the symptom involving **records**.
...
- Does **records** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **records** correctly for the project's Java version.
...
1. What is **records** in Java?
2. Why would you use **records**?
3. How does **records** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **records**?
5. Where would **records** appear in a real production system?
...
Create the smallest Java example that demonstrates **records** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **records** in plain English.
