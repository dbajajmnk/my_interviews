...
**sealed classes** is closed subtype hierarchy.

---

# 2. Why

Interviewers ask **sealed classes** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **sealed classes** is about closed subtype hierarchy. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

compiler restricts permitted subclasses and can help exhaustive pattern matching. Within this day, the closest engineering anchor is **Sealed Types**: Restrict permitted implementations and make closed domain hierarchies explicit.
...
- compiler restricts permitted subclasses and can help exhaustive pattern matching
...
- Use **sealed classes** only when its semantics fit the requirement.
...
1. Memorizing the definition of **sealed classes** without understanding runtime behavior.
...
- First reproduce the symptom involving **sealed classes**.
- Primary focus: check intended extensibility and module/package rules.
...
- Does **sealed classes** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **sealed classes** correctly for the project's Java version.
...
1. What is **sealed classes** in Java?
2. Why would you use **sealed classes**?
3. How does **sealed classes** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **sealed classes**?
5. Where would **sealed classes** appear in a real production system?
...
Create the smallest Java example that demonstrates **sealed classes** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **sealed classes** in plain English.
