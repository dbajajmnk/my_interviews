...
**pattern matching** is type/value-oriented modern control flow.

---

# 2. Why

Interviewers ask **pattern matching** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **pattern matching** is about type/value-oriented modern control flow. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

compiler performs pattern compatibility/exhaustiveness checks depending on construct/hierarchy. Within this day, the closest engineering anchor is **Pattern Matching**: Reduces boilerplate when testing/deconstructing values, especially with sealed hierarchies.
...
- Use **pattern matching** only when its semantics fit the requirement.
...
1. Memorizing the definition of **pattern matching** without understanding runtime behavior.
...
- First reproduce the symptom involving **pattern matching**.
...
- Does **pattern matching** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **pattern matching** correctly for the project's Java version.
...
2. Why would you use **pattern matching**?
3. How does **pattern matching** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **pattern matching**?
5. Where would **pattern matching** appear in a real production system?
...
Create the smallest Java example that demonstrates **pattern matching** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **pattern matching** in plain English.
