...
Interviewers ask **var** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **var** is about local-variable type inference. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

compiler infers the static type at compile time; Java remains statically typed. Within this day, the closest engineering anchor is **Records**: Concise nominal data carriers with generated accessors, equality, hashCode, and toString. They are shallowly immutable with final component references.
...
- compiler infers the static type at compile time; Java remains statically typed
...
- Use **var** only when its semantics fit the requirement.
...
1. Memorizing the definition of **var** without understanding runtime behavior.
...
- First reproduce the symptom involving **var**.
- Primary focus: avoid when inferred type is non-obvious.
...
- Does **var** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **var** correctly for the project's Java version.
...
2. Why would you use **var**?
3. How does **var** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **var**?
5. Where would **var** appear in a real production system?
...
Create the smallest Java example that demonstrates **var** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **var** in plain English.
