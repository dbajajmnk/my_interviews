...
**Files/Path** is modern filesystem API.

---

# 2. Why

Interviewers ask **Files/Path** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **Files/Path** is about modern filesystem API. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

`Path` models filesystem paths and `Files` performs operations with stream/resource semantics. Within this day, the closest engineering anchor is **Byte vs Character I/O**: Streams work with bytes; readers/writers handle characters with encodings.
...
```java
try (var lines = Files.lines(path)) { lines.forEach(System.out::println); }
```
...
- `Path` models filesystem paths and `Files` performs operations with stream/resource semantics
...
- Use **Files/Path** only when its semantics fit the requirement.
...
1. Memorizing the definition of **Files/Path** without understanding runtime behavior.
...
- First reproduce the symptom involving **Files/Path**.
- Primary focus: avoid loading huge files fully when streaming works.
...
- Does **Files/Path** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **Files/Path** correctly for the project's Java version.
...
1. What is **Files/Path** in Java?
2. Why would you use **Files/Path**?
3. How does **Files/Path** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **Files/Path**?
5. Where would **Files/Path** appear in a real production system?
...
Create the smallest Java example that demonstrates **Files/Path** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **Files/Path** in plain English.
