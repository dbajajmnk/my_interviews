...
**HTTP client** is bounded remote request/response communication.

---

# 2. Why

Interviewers ask **HTTP client** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **HTTP client** is about bounded remote request/response communication. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

connection establishment, request timeout, response streaming, DNS/TLS and remote service behavior contribute to latency/failure. Within this day, the closest engineering anchor is **HTTP Client**: Production calls need timeout, status/error handling, bounded response size, retry policy, and observability.
...
- Use **HTTP client** only when its semantics fit the requirement.
...
1. Memorizing the definition of **HTTP client** without understanding runtime behavior.
...
- First reproduce the symptom involving **HTTP client**.
...
- Does **HTTP client** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **HTTP client** correctly for the project's Java version.
...
1. What is **HTTP client** in Java?
2. Why would you use **HTTP client**?
3. How does **HTTP client** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **HTTP client**?
5. Where would **HTTP client** appear in a real production system?
...
Create the smallest Java example that demonstrates **HTTP client** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **HTTP client** in plain English.
