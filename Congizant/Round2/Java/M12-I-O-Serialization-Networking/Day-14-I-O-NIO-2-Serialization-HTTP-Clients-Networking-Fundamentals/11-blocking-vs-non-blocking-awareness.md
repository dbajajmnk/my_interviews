# Topic 11 — blocking vs non-blocking awareness
...
**blocking vs non-blocking awareness** is Java packaging formats.

---

# 2. Why

Interviewers ask **blocking vs non-blocking awareness** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **blocking vs non-blocking awareness** is about Java packaging formats. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

JAR packages classes/resources; WAR follows web-application packaging conventions. Within this day, the closest engineering anchor is **Byte vs Character I/O**: Streams work with bytes; readers/writers handle characters with encodings.
...
- JAR packages classes/resources; WAR follows web-application packaging conventions
...
- Use **blocking vs non-blocking awareness** only when its semantics fit the requirement.
...
1. Memorizing the definition of **blocking vs non-blocking awareness** without understanding runtime behavior.
...
- First reproduce the symptom involving **blocking vs non-blocking awareness**.
- Primary focus: inspect archive contents/manifest/runtime launch model.
...
- Does **blocking vs non-blocking awareness** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **blocking vs non-blocking awareness** correctly for the project's Java version.
...
1. What is **blocking vs non-blocking awareness** in Java?
2. Why would you use **blocking vs non-blocking awareness**?
3. How does **blocking vs non-blocking awareness** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **blocking vs non-blocking awareness**?
5. Where would **blocking vs non-blocking awareness** appear in a real production system?
...
Create the smallest Java example that demonstrates **blocking vs non-blocking awareness** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **blocking vs non-blocking awareness** in plain English.
