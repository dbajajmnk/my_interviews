...
**strings** is immutable character-sequence values.

---

# 2. Why

Interviewers ask **strings** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **strings** is about immutable character-sequence values. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

String literals may be interned; operations create new logical values. Within this day, the closest engineering anchor is **Interview Flow**: Clarify → examples → brute force → optimize → code → test → complexity → follow-up.
...
- Use **strings** only when its semantics fit the requirement.
...
1. Memorizing the definition of **strings** without understanding runtime behavior.
...
- First reproduce the symptom involving **strings**.
...
- Does **strings** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **strings** correctly for the project's Java version.
...
2. Why would you use **strings**?
3. How does **strings** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **strings**?
5. Where would **strings** appear in a real production system?
...
Create the smallest Java example that demonstrates **strings** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **strings** in plain English.
