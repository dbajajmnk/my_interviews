...
**two pointers** is two moving indices that reduce repeated scanning.

---

# 2. Why

Interviewers ask **two pointers** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **two pointers** is about two moving indices that reduce repeated scanning. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

monotonic pointer movement can turn nested search into linear traversal. Within this day, the closest engineering anchor is **Two Pointers**: Useful when indexes can move monotonically to avoid nested scans.
...
```java
for (int l=0,r=a.length-1; l<r; ) { /* move l/r */ }
```
...
- monotonic pointer movement can turn nested search into linear traversal
...
- Use **two pointers** only when its semantics fit the requirement.
...
1. Memorizing the definition of **two pointers** without understanding runtime behavior.
...
- First reproduce the symptom involving **two pointers**.
- Primary focus: write invariant and test pointer boundaries.
...
- Does **two pointers** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **two pointers** correctly for the project's Java version.
...
1. What is **two pointers** in Java?
2. Why would you use **two pointers**?
3. How does **two pointers** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **two pointers**?
5. Where would **two pointers** appear in a real production system?
...
Create the smallest Java example that demonstrates **two pointers** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **two pointers** in plain English.
