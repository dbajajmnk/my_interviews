...
**sliding window** is incremental state over a contiguous range.

---

# 2. Why

Interviewers ask **sliding window** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **sliding window** is about incremental state over a contiguous range. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

expand/contract window while updating counts/sum rather than recomputing. Within this day, the closest engineering anchor is **Sliding Window**: Maintains incremental state for contiguous ranges.
...
```java
for (int right=0; right<s.length(); right++) { /* expand */ }
```
...
- expand/contract window while updating counts/sum rather than recomputing
...
- Use **sliding window** only when its semantics fit the requirement.
...
1. Memorizing the definition of **sliding window** without understanding runtime behavior.
...
- First reproduce the symptom involving **sliding window**.
- Primary focus: trace invariant on duplicate/empty cases.
...
- Does **sliding window** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **sliding window** correctly for the project's Java version.
...
1. What is **sliding window** in Java?
2. Why would you use **sliding window**?
3. How does **sliding window** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **sliding window**?
5. Where would **sliding window** appear in a real production system?
...
Create the smallest Java example that demonstrates **sliding window** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **sliding window** in plain English.
