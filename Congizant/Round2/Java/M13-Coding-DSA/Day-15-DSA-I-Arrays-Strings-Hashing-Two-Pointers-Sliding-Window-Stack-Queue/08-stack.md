...
**stack** is JVM memory ownership and reachability.

---

# 2. Why

Interviewers ask **stack** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **stack** is about JVM memory ownership and reachability. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

thread stacks hold frames; heap holds most objects; metaspace stores class metadata; GC follows reachability. Within this day, the closest engineering anchor is **Stack/Deque**: Model LIFO/FIFO and monotonic patterns; `ArrayDeque` is usually preferable to legacy Stack.
...
- Use **stack** only when its semantics fit the requirement.
...
1. Memorizing the definition of **stack** without understanding runtime behavior.
...
- First reproduce the symptom involving **stack**.
...
- Does **stack** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **stack** correctly for the project's Java version.
...
2. Why would you use **stack**?
3. How does **stack** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **stack**?
5. Where would **stack** appear in a real production system?
...
Create the smallest Java example that demonstrates **stack** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **stack** in plain English.
