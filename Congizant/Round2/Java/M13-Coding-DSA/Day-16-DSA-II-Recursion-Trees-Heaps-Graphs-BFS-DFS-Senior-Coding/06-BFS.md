...
**BFS** is breadth-first graph/tree traversal.

---

# 2. Why

Interviewers ask **BFS** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **BFS** is about breadth-first graph/tree traversal. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

queue processes nodes layer by layer; visited prevents repeated cycles. Within this day, the closest engineering anchor is **Recursion**: Needs base case, progress, and stack-depth awareness.
...
- queue processes nodes layer by layer; visited prevents repeated cycles
...
In a Java/Spring production system, **BFS** appears in the context of **DSA II: Recursion, Trees, Heaps, Graphs, BFS/DFS & Senior Coding**. Connect it to the day's project concern: Implement tree depth, level-order traversal, top-K frequent elements, graph reachability, cycle detection, and dependency ordering. State time/space complexity.
...
- Use **BFS** only when its semantics fit the requirement.
...
1. Memorizing the definition of **BFS** without understanding runtime behavior.
...
- First reproduce the symptom involving **BFS**.
- Primary focus: check visited timing and disconnected components.
...
- Does **BFS** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **BFS** correctly for the project's Java version.
...
1. What is **BFS** in Java?
2. Why would you use **BFS**?
3. How does **BFS** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **BFS**?
5. Where would **BFS** appear in a real production system?
...
Create the smallest Java example that demonstrates **BFS** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **BFS** in plain English.
