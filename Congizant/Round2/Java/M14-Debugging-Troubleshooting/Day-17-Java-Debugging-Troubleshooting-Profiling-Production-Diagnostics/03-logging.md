...
internal exception details are mapped to non-sensitive external codes while logs preserve trace context. Within this day, the closest engineering anchor is **Debug Flow**: Reproduce → capture evidence → isolate → hypothesize → test → fix → regression → monitor.
...
In a Java/Spring production system, **logging** appears in the context of **Java Debugging, Troubleshooting, Profiling & Production Diagnostics**. Connect it to the day's project concern: Given symptoms—100% CPU, deadlock, memory leak, GC pauses, slow DB calls—write the first three pieces of evidence you would collect before changing code.
