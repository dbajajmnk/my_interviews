...
**JFR/JMC awareness** is Java packaging formats.

---

# 2. Why

Interviewers ask **JFR/JMC awareness** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **JFR/JMC awareness** is about Java packaging formats. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

JAR packages classes/resources; WAR follows web-application packaging conventions. Within this day, the closest engineering anchor is **Debug Flow**: Reproduce → capture evidence → isolate → hypothesize → test → fix → regression → monitor.
...
In a Java/Spring production system, **JFR/JMC awareness** appears in the context of **Java Debugging, Troubleshooting, Profiling & Production Diagnostics**. Connect it to the day's project concern: Given symptoms—100% CPU, deadlock, memory leak, GC pauses, slow DB calls—write the first three pieces of evidence you would collect before changing code.
...
- Use **JFR/JMC awareness** only when its semantics fit the requirement.
...
1. Memorizing the definition of **JFR/JMC awareness** without understanding runtime behavior.
...
- First reproduce the symptom involving **JFR/JMC awareness**.
...
- Does **JFR/JMC awareness** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **JFR/JMC awareness** correctly for the project's Java version.
...
1. What is **JFR/JMC awareness** in Java?
2. Why would you use **JFR/JMC awareness**?
3. How does **JFR/JMC awareness** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **JFR/JMC awareness**?
5. Where would **JFR/JMC awareness** appear in a real production system?
...
Create the smallest Java example that demonstrates **JFR/JMC awareness** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **JFR/JMC awareness** in plain English.
