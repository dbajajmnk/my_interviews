...
**performance** is the Java engineering concept **performance** within Code Review, Refactoring, Clean Java & Maintainability.

---

# 2. Why

Interviewers ask **performance** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **performance** is about the Java engineering concept **performance** within Code Review, Refactoring, Clean Java & Maintainability. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

Requirement/correctness → security → data/concurrency → errors → performance → architecture → tests → readability/style.. Within this day, the closest engineering anchor is **Review Order**: Requirement/correctness → security → data/concurrency → errors → performance → architecture → tests → readability/style.
...
```java
// Smell: boolean flags create unclear call sites.
process(order, true, false);

// Prefer explicit intent:
process(order, ProcessingMode.EXPRESS);
```
...
- Requirement/correctness → security → data/concurrency → errors → performance → architecture → tests → readability/style.
...
- Use **performance** only when its semantics fit the requirement.
...
1. Memorizing the definition of **performance** without understanding runtime behavior.
...
- First reproduce the symptom involving **performance**.
- Primary focus: reproduce failures involving performance, capture runtime/compiler/test evidence, and isolate the smallest failing assumption.
...
- Does **performance** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **performance** correctly for the project's Java version.
...
1. What is **performance** in Java?
2. Why would you use **performance**?
3. How does **performance** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **performance**?
5. Where would **performance** appear in a real production system?
...
Create the smallest Java example that demonstrates **performance** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **performance** in plain English.
