...
**contract tests** is verification of producer/consumer interface compatibility.

---

# 2. Why

Interviewers ask **contract tests** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **contract tests** is about verification of producer/consumer interface compatibility. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

tests encode agreed request/response/message contracts across independently evolving components. Within this day, the closest engineering anchor is **Contract Test**: Protects API/message compatibility between independently evolving components.
...
```java
// Verify API/message schema against consumer expectations.
```
...
- tests encode agreed request/response/message contracts across independently evolving components
...
- Use **contract tests** only when its semantics fit the requirement.
...
1. Memorizing the definition of **contract tests** without understanding runtime behavior.
...
- First reproduce the symptom involving **contract tests**.
- Primary focus: version contracts and run in CI.
...
- Does **contract tests** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **contract tests** correctly for the project's Java version.
...
1. What is **contract tests** in Java?
2. Why would you use **contract tests**?
3. How does **contract tests** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **contract tests**?
5. Where would **contract tests** appear in a real production system?
...
Create the smallest Java example that demonstrates **contract tests** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **contract tests** in plain English.
