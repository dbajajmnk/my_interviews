...
**coverage limits** is test-suite evidence metrics.

---

# 2. Why

Interviewers ask **coverage limits** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **coverage limits** is about test-suite evidence metrics. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

coverage measures execution; mutation testing probes assertion strength by changing code. Within this day, the closest engineering anchor is **Coverage**: Execution coverage is evidence of exercised code, not proof of correctness.
...
```java
// Prefer meaningful assertions and risk-based coverage.
```
...
- coverage measures execution; mutation testing probes assertion strength by changing code
...
- Use **coverage limits** only when its semantics fit the requirement.
...
1. Memorizing the definition of **coverage limits** without understanding runtime behavior.
...
- First reproduce the symptom involving **coverage limits**.
- Primary focus: do not equate high coverage with correctness.
...
- Does **coverage limits** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **coverage limits** correctly for the project's Java version.
...
1. What is **coverage limits** in Java?
2. Why would you use **coverage limits**?
3. How does **coverage limits** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **coverage limits**?
5. Where would **coverage limits** appear in a real production system?
...
Create the smallest Java example that demonstrates **coverage limits** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **coverage limits** in plain English.
