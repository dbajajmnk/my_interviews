...
**connection pooling** is bounded reusable database connections.

---

# 2. Why

Interviewers ask **connection pooling** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **connection pooling** is about bounded reusable database connections. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

requests borrow/hold/return connections; pool wait rises when demand exceeds capacity. Within this day, the closest engineering anchor is **JDBC**: Low-level Java database API; prepared statements provide parameterization and safer execution.
...
```java
// pool capacity must align with database capacity
```
...
- requests borrow/hold/return connections; pool wait rises when demand exceeds capacity
...
In a Java/Spring production system, **connection pooling** appears in the context of **JDBC, SQL, JPA/Hibernate, Transactions, ORM Pitfalls & Data Access**. Connect it to the day's project concern: Implement repository operation with JDBC prepared statement and JPA equivalent. Reproduce N+1, inspect queries, fix with fetch join/entity graph/query projection.
...
- Use **connection pooling** only when its semantics fit the requirement.
...
1. Memorizing the definition of **connection pooling** without understanding runtime behavior.
...
- First reproduce the symptom involving **connection pooling**.
- Primary focus: measure wait/active/max and transaction duration.
...
- Does **connection pooling** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **connection pooling** correctly for the project's Java version.
...
2. Why would you use **connection pooling**?
3. How does **connection pooling** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **connection pooling**?
5. Where would **connection pooling** appear in a real production system?
...
Create the smallest Java example that demonstrates **connection pooling** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **connection pooling** in plain English.
