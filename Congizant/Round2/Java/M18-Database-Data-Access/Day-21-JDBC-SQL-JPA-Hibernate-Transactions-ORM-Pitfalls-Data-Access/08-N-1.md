...
**N+1** is association fetching strategy and query-count behavior.

---

# 2. Why

Interviewers ask **N+1** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **N+1** is about association fetching strategy and query-count behavior. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

lazy access can trigger additional queries; eager can overfetch; fetch joins/projections shape SQL deliberately. Within this day, the closest engineering anchor is **N+1**: Loading collection/association per row causes many queries; solve with deliberate fetch strategy.
...
- Use **N+1** only when its semantics fit the requirement.
...
1. Memorizing the definition of **N+1** without understanding runtime behavior.
...
- First reproduce the symptom involving **N+1**.
...
- Does **N+1** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **N+1** correctly for the project's Java version.
...
1. What is **N+1** in Java?
2. Why would you use **N+1**?
3. How does **N+1** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **N+1**?
5. Where would **N+1** appear in a real production system?
...
Create the smallest Java example that demonstrates **N+1** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **N+1** in plain English.
