...
**locking** is concurrent database update control.

---

# 2. Why

Interviewers ask **locking** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **locking** is about concurrent database update control. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.
...
- Use **locking** only when its semantics fit the requirement.
...
1. Memorizing the definition of **locking** without understanding runtime behavior.
...
- First reproduce the symptom involving **locking**.
...
- Does **locking** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **locking** correctly for the project's Java version.
...
2. Why would you use **locking**?
3. How does **locking** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **locking**?
5. Where would **locking** appear in a real production system?
...
Create the smallest Java example that demonstrates **locking** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **locking** in plain English.
