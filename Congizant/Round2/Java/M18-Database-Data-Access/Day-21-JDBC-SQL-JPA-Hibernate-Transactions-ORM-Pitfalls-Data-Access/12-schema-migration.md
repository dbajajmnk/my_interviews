...
**schema migration** is controlled schema evolution.

---

# 2. Why

Interviewers ask **schema migration** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **schema migration** is about controlled schema evolution. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

versioned migrations change DB structure/data with compatibility/rollback considerations. Within this day, the closest engineering anchor is **JDBC**: Low-level Java database API; prepared statements provide parameterization and safer execution.
...
```java
// Add column → deploy compatible code → backfill → remove old later.
```
...
- versioned migrations change DB structure/data with compatibility/rollback considerations
...
- Use **schema migration** only when its semantics fit the requirement.
...
1. Memorizing the definition of **schema migration** without understanding runtime behavior.
...
- First reproduce the symptom involving **schema migration**.
- Primary focus: use expand/contract for zero-downtime releases.
...
- Does **schema migration** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **schema migration** correctly for the project's Java version.
...
1. What is **schema migration** in Java?
2. Why would you use **schema migration**?
3. How does **schema migration** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **schema migration**?
5. Where would **schema migration** appear in a real production system?
...
Create the smallest Java example that demonstrates **schema migration** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **schema migration** in plain English.
