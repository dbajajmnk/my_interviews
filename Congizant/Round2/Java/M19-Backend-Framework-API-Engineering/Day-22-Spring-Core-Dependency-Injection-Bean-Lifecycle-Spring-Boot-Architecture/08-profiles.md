...
**profiles** is modern filesystem API.

---

# 2. Why

Interviewers ask **profiles** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **profiles** is about modern filesystem API. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

`Path` models filesystem paths and `Files` performs operations with stream/resource semantics. Within this day, the closest engineering anchor is **IoC / DI**: Application receives dependencies from container rather than constructing infrastructure directly.
...
- Use **profiles** only when its semantics fit the requirement.
...
1. Memorizing the definition of **profiles** without understanding runtime behavior.
...
- First reproduce the symptom involving **profiles**.
...
- Does **profiles** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **profiles** correctly for the project's Java version.
...
1. What is **profiles** in Java?
2. Why would you use **profiles**?
3. How does **profiles** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **profiles**?
5. Where would **profiles** appear in a real production system?
...
Create the smallest Java example that demonstrates **profiles** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **profiles** in plain English.
