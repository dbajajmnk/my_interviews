...
**REST controllers** is HTTP contract mapping into application use cases.

---

# 2. Why

Interviewers ask **REST controllers** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **REST controllers** is about HTTP contract mapping into application use cases. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

Spring MVC maps requests, binds/validates DTOs and dispatches handlers. Within this day, the closest engineering anchor is **Controller Contract**: Controller maps HTTP contract to application use case; domain logic remains elsewhere.
...
```java
@PostMapping ResponseEntity<OrderResponse> create(@Valid @RequestBody CreateOrderRequest req) { return null; }
```
...
- Spring MVC maps requests, binds/validates DTOs and dispatches handlers
...
- Use **REST controllers** only when its semantics fit the requirement.
...
1. Memorizing the definition of **REST controllers** without understanding runtime behavior.
...
- First reproduce the symptom involving **REST controllers**.
- Primary focus: separate transport DTOs from entities/domain.
...
- Does **REST controllers** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **REST controllers** correctly for the project's Java version.
...
1. What is **REST controllers** in Java?
2. Why would you use **REST controllers**?
3. How does **REST controllers** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **REST controllers**?
5. Where would **REST controllers** appear in a real production system?
...
Create the smallest Java example that demonstrates **REST controllers** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **REST controllers** in plain English.
