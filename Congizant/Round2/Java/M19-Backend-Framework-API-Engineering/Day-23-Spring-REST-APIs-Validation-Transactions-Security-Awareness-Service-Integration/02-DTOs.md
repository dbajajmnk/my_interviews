...
**DTOs** is HTTP contract mapping into application use cases.

---

# 2. Why

Interviewers ask **DTOs** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **DTOs** is about HTTP contract mapping into application use cases. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.
...
- Use **DTOs** only when its semantics fit the requirement.
...
1. Memorizing the definition of **DTOs** without understanding runtime behavior.
...
- First reproduce the symptom involving **DTOs**.
...
- Does **DTOs** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **DTOs** correctly for the project's Java version.
...
1. What is **DTOs** in Java?
2. Why would you use **DTOs**?
3. How does **DTOs** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **DTOs**?
5. Where would **DTOs** appear in a real production system?
...
Create the smallest Java example that demonstrates **DTOs** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **DTOs** in plain English.
