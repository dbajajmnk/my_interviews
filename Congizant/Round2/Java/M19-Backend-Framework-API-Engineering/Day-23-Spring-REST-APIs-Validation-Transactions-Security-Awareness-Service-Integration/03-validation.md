...
**validation** is HTTP contract mapping into application use cases.

---

# 2. Why

Interviewers ask **validation** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **validation** is about HTTP contract mapping into application use cases. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

Spring MVC maps requests, binds/validates DTOs and dispatches handlers. Within this day, the closest engineering anchor is **Validation**: Separate syntactic/schema validation from business rules.
...
- Use **validation** only when its semantics fit the requirement.
...
1. Memorizing the definition of **validation** without understanding runtime behavior.
...
- First reproduce the symptom involving **validation**.
...
- Does **validation** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **validation** correctly for the project's Java version.
...
1. What is **validation** in Java?
2. Why would you use **validation**?
3. How does **validation** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **validation**?
5. Where would **validation** appear in a real production system?
...
Create the smallest Java example that demonstrates **validation** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **validation** in plain English.
