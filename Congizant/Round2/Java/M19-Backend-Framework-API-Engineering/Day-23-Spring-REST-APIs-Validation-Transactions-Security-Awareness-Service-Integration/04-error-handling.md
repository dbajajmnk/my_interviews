...
**error handling** is stable HTTP success/failure semantics.

---

# 2. Why

Interviewers ask **error handling** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **error handling** is about stable HTTP success/failure semantics. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

application exceptions are translated into explicit status/error bodies. Within this day, the closest engineering anchor is **Controller Contract**: Controller maps HTTP contract to application use case; domain logic remains elsewhere.
...
```java
@RestControllerAdvice class ApiErrors { /* handlers */ }
```
...
- application exceptions are translated into explicit status/error bodies
...
- Use **error handling** only when its semantics fit the requirement.
...
1. Memorizing the definition of **error handling** without understanding runtime behavior.
...
- First reproduce the symptom involving **error handling**.
- Primary focus: test error contract and avoid stack-trace leakage.
...
- Does **error handling** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **error handling** correctly for the project's Java version.
...
1. What is **error handling** in Java?
2. Why would you use **error handling**?
3. How does **error handling** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **error handling**?
5. Where would **error handling** appear in a real production system?
...
Create the smallest Java example that demonstrates **error handling** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **error handling** in plain English.
