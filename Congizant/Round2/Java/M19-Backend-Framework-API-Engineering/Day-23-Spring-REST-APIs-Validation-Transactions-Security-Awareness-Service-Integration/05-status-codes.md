...
**status codes** is stable HTTP success/failure semantics.

---

# 2. Why

Interviewers ask **status codes** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **status codes** is about stable HTTP success/failure semantics. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.
...
- Use **status codes** only when its semantics fit the requirement.
...
1. Memorizing the definition of **status codes** without understanding runtime behavior.
...
- First reproduce the symptom involving **status codes**.
...
- Does **status codes** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **status codes** correctly for the project's Java version.
...
1. What is **status codes** in Java?
2. Why would you use **status codes**?
3. How does **status codes** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **status codes**?
5. Where would **status codes** appear in a real production system?
...
Create the smallest Java example that demonstrates **status codes** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **status codes** in plain English.
