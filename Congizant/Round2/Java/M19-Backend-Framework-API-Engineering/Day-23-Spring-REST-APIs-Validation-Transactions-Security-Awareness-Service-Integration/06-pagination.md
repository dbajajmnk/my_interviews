...
**pagination** is bounded retrieval of large result sets.

---

# 2. Why

Interviewers ask **pagination** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **pagination** is about bounded retrieval of large result sets. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

offset pagination scans/skips increasingly; keyset pagination uses stable ordering cursor. Within this day, the closest engineering anchor is **Controller Contract**: Controller maps HTTP contract to application use case; domain logic remains elsewhere.
...
- offset pagination scans/skips increasingly; keyset pagination uses stable ordering cursor
...
- Use **pagination** only when its semantics fit the requirement.
...
1. Memorizing the definition of **pagination** without understanding runtime behavior.
...
- First reproduce the symptom involving **pagination**.
- Primary focus: test ordering and page consistency.
...
- Does **pagination** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **pagination** correctly for the project's Java version.
...
1. What is **pagination** in Java?
2. Why would you use **pagination**?
3. How does **pagination** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **pagination**?
5. Where would **pagination** appear in a real production system?
...
Create the smallest Java example that demonstrates **pagination** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **pagination** in plain English.
