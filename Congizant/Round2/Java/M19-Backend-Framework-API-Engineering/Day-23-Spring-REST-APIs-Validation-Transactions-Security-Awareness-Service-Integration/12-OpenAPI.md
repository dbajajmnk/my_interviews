...
**OpenAPI** is machine-readable HTTP API contract.

---

# 2. Why

Interviewers ask **OpenAPI** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **OpenAPI** is about machine-readable HTTP API contract. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

schema describes operations/types/statuses and supports tooling/compatibility review. Within this day, the closest engineering anchor is **Controller Contract**: Controller maps HTTP contract to application use case; domain logic remains elsewhere.
...
```java
// Define request/response schemas and error responses explicitly.
```
...
- schema describes operations/types/statuses and supports tooling/compatibility review
...
- Use **OpenAPI** only when its semantics fit the requirement.
...
1. Memorizing the definition of **OpenAPI** without understanding runtime behavior.
...
- First reproduce the symptom involving **OpenAPI**.
- Primary focus: treat generated docs as contract to test, not decoration.
...
- Does **OpenAPI** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **OpenAPI** correctly for the project's Java version.
...
1. What is **OpenAPI** in Java?
2. Why would you use **OpenAPI**?
3. How does **OpenAPI** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **OpenAPI**?
5. Where would **OpenAPI** appear in a real production system?
...
Create the smallest Java example that demonstrates **OpenAPI** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **OpenAPI** in plain English.
