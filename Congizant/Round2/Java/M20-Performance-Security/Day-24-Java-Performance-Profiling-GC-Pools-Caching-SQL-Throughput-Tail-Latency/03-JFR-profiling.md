...
**JFR/profiling** is JVM event recording and analysis.

---

# 2. Why

Interviewers ask **JFR/profiling** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **JFR/profiling** is about JVM event recording and analysis. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

Flight Recorder captures runtime events such as CPU, allocation, locks, GC with low overhead. Within this day, the closest engineering anchor is **Measure First**: Profile representative workload before optimization.
...
- Flight Recorder captures runtime events such as CPU, allocation, locks, GC with low overhead
...
In a Java/Spring production system, **JFR/profiling** appears in the context of **Java Performance: Profiling, GC, Pools, Caching, SQL, Throughput & Tail Latency**. Connect it to the day's project concern: Given profiles/metrics, rank fixes for N+1 queries, high allocation, lock contention, DB pool wait, cache miss, and slow model provider. Explain evidence needed.
...
- Use **JFR/profiling** only when its semantics fit the requirement.
...
1. Memorizing the definition of **JFR/profiling** without understanding runtime behavior.
...
- First reproduce the symptom involving **JFR/profiling**.
- Primary focus: record representative window and correlate with SLO breach.
...
- Does **JFR/profiling** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **JFR/profiling** correctly for the project's Java version.
...
1. What is **JFR/profiling** in Java?
2. Why would you use **JFR/profiling**?
3. How does **JFR/profiling** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **JFR/profiling**?
5. Where would **JFR/profiling** appear in a real production system?
...
Create the smallest Java example that demonstrates **JFR/profiling** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **JFR/profiling** in plain English.
