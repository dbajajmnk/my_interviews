...
**GC** is automatic reachability-based memory reclamation.

---

# 2. Why

Interviewers ask **GC** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **GC** is about automatic reachability-based memory reclamation. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

collectors identify unreachable objects from GC roots with collector-specific pause/throughput behavior. Within this day, the closest engineering anchor is **Measure First**: Profile representative workload before optimization.
...
In a Java/Spring production system, **GC** appears in the context of **Java Performance: Profiling, GC, Pools, Caching, SQL, Throughput & Tail Latency**. Connect it to the day's project concern: Given profiles/metrics, rank fixes for N+1 queries, high allocation, lock contention, DB pool wait, cache miss, and slow model provider. Explain evidence needed.
...
- Use **GC** only when its semantics fit the requirement.
...
1. Memorizing the definition of **GC** without understanding runtime behavior.
...
- First reproduce the symptom involving **GC**.
...
- Does **GC** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **GC** correctly for the project's Java version.
...
1. What is **GC** in Java?
2. Why would you use **GC**?
3. How does **GC** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **GC**?
5. Where would **GC** appear in a real production system?
...
Create the smallest Java example that demonstrates **GC** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **GC** in plain English.
