...
lazy access can trigger additional queries; eager can overfetch; fetch joins/projections shape SQL deliberately. Within this day, the closest engineering anchor is **Measure First**: Profile representative workload before optimization.
...
In a Java/Spring production system, **N+1** appears in the context of **Java Performance: Profiling, GC, Pools, Caching, SQL, Throughput & Tail Latency**. Connect it to the day's project concern: Given profiles/metrics, rank fixes for N+1 queries, high allocation, lock contention, DB pool wait, cache miss, and slow model provider. Explain evidence needed.
