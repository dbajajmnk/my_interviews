...
during network partition some operations must favor consistency or availability according to requirements. Within this day, the closest engineering anchor is **Measure First**: Profile representative workload before optimization.
...
In a Java/Spring production system, **capacity** appears in the context of **Java Performance: Profiling, GC, Pools, Caching, SQL, Throughput & Tail Latency**. Connect it to the day's project concern: Given profiles/metrics, rank fixes for N+1 queries, high allocation, lock contention, DB pool wait, cache miss, and slow model provider. Explain evidence needed.
