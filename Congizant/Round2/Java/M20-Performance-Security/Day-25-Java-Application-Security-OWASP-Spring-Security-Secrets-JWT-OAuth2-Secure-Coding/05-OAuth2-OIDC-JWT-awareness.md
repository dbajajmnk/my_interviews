# Topic 05 — OAuth2/OIDC/JWT awareness
...
**OAuth2/OIDC/JWT awareness** is Java packaging formats.

---

# 2. Why

Interviewers ask **OAuth2/OIDC/JWT awareness** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **OAuth2/OIDC/JWT awareness** is about Java packaging formats. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

JAR packages classes/resources; WAR follows web-application packaging conventions. Within this day, the closest engineering anchor is **OAuth2/OIDC**: OAuth2 is authorization framework; OIDC adds identity layer. JWT is a token format, not an authentication protocol by itself.
...
In a Java/Spring production system, **OAuth2/OIDC/JWT awareness** appears in the context of **Java/Application Security: OWASP, Spring Security, Secrets, JWT/OAuth2 & Secure Coding**. Connect it to the day's project concern: Threat-model a Spring REST API + React frontend. Cover login, token/session, BOLA, SQL injection, SSRF, file upload, secrets, dependency CVEs, logging, and tenant isolation.
...
- Use **OAuth2/OIDC/JWT awareness** only when its semantics fit the requirement.
...
1. Memorizing the definition of **OAuth2/OIDC/JWT awareness** without understanding runtime behavior.
...
- First reproduce the symptom involving **OAuth2/OIDC/JWT awareness**.
...
- Does **OAuth2/OIDC/JWT awareness** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **OAuth2/OIDC/JWT awareness** correctly for the project's Java version.
...
1. What is **OAuth2/OIDC/JWT awareness** in Java?
2. Why would you use **OAuth2/OIDC/JWT awareness**?
3. How does **OAuth2/OIDC/JWT awareness** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **OAuth2/OIDC/JWT awareness**?
5. Where would **OAuth2/OIDC/JWT awareness** appear in a real production system?
...
Create the smallest Java example that demonstrates **OAuth2/OIDC/JWT awareness** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **OAuth2/OIDC/JWT awareness** in plain English.
