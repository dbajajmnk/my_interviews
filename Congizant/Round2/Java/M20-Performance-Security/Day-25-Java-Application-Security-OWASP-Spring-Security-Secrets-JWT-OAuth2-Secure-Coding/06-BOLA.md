...
**BOLA** is broken object-level authorization.

---

# 2. Why

Interviewers ask **BOLA** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **BOLA** is about broken object-level authorization. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

authenticated identity accesses object without server checking permission on that object. Within this day, the closest engineering anchor is **Trust Boundary**: Every user/service/data/tool boundary needs validation and authorization.
...
```java
// Query resource using principal tenant/ownership, not user-supplied tenant alone.
```
...
- authenticated identity accesses object without server checking permission on that object
...
In a Java/Spring production system, **BOLA** appears in the context of **Java/Application Security: OWASP, Spring Security, Secrets, JWT/OAuth2 & Secure Coding**. Connect it to the day's project concern: Threat-model a Spring REST API + React frontend. Cover login, token/session, BOLA, SQL injection, SSRF, file upload, secrets, dependency CVEs, logging, and tenant isolation.
...
- Use **BOLA** only when its semantics fit the requirement.
...
1. Memorizing the definition of **BOLA** without understanding runtime behavior.
...
- First reproduce the symptom involving **BOLA**.
- Primary focus: write cross-user/tenant negative tests.
...
- Does **BOLA** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **BOLA** correctly for the project's Java version.
...
1. What is **BOLA** in Java?
2. Why would you use **BOLA**?
3. How does **BOLA** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **BOLA**?
5. Where would **BOLA** appear in a real production system?
...
Create the smallest Java example that demonstrates **BOLA** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **BOLA** in plain English.
