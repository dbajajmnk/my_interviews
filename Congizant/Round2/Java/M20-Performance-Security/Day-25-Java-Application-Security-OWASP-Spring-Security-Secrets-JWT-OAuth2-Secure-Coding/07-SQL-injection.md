...
**SQL injection** is untrusted input changing SQL structure.

---

# 2. Why

Interviewers ask **SQL injection** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **SQL injection** is about untrusted input changing SQL structure. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

string concatenation lets data become executable syntax; prepared parameters separate them. Within this day, the closest engineering anchor is **Trust Boundary**: Every user/service/data/tool boundary needs validation and authorization.
...
- string concatenation lets data become executable syntax; prepared parameters separate them
...
In a Java/Spring production system, **SQL injection** appears in the context of **Java/Application Security: OWASP, Spring Security, Secrets, JWT/OAuth2 & Secure Coding**. Connect it to the day's project concern: Threat-model a Spring REST API + React frontend. Cover login, token/session, BOLA, SQL injection, SSRF, file upload, secrets, dependency CVEs, logging, and tenant isolation.
...
- Use **SQL injection** only when its semantics fit the requirement.
...
1. Memorizing the definition of **SQL injection** without understanding runtime behavior.
...
- First reproduce the symptom involving **SQL injection**.
- Primary focus: test malicious input and inspect query construction.
...
- Does **SQL injection** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **SQL injection** correctly for the project's Java version.
...
1. What is **SQL injection** in Java?
2. Why would you use **SQL injection**?
3. How does **SQL injection** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **SQL injection**?
5. Where would **SQL injection** appear in a real production system?
...
Create the smallest Java example that demonstrates **SQL injection** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **SQL injection** in plain English.
