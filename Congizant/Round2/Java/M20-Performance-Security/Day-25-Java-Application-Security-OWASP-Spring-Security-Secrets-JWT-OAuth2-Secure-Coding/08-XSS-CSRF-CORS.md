...
**XSS/CSRF/CORS** is untrusted content executing in browser context.

---

# 2. Why

Interviewers ask **XSS/CSRF/CORS** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **XSS/CSRF/CORS** is about untrusted content executing in browser context. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

unsafe HTML/DOM insertion turns data into script/markup. Within this day, the closest engineering anchor is **CSRF/CORS**: CSRF concerns state-changing requests with automatically sent credentials; CORS is browser cross-origin policy, not auth.
...
```java
// React escapes text by default; avoid unsafe HTML injection.
```
...
- unsafe HTML/DOM insertion turns data into script/markup
...
In a Java/Spring production system, **XSS/CSRF/CORS** appears in the context of **Java/Application Security: OWASP, Spring Security, Secrets, JWT/OAuth2 & Secure Coding**. Connect it to the day's project concern: Threat-model a Spring REST API + React frontend. Cover login, token/session, BOLA, SQL injection, SSRF, file upload, secrets, dependency CVEs, logging, and tenant isolation.
...
- Use **XSS/CSRF/CORS** only when its semantics fit the requirement.
...
1. Memorizing the definition of **XSS/CSRF/CORS** without understanding runtime behavior.
...
- First reproduce the symptom involving **XSS/CSRF/CORS**.
- Primary focus: encode/sanitize at output and use CSP where appropriate.
...
- Does **XSS/CSRF/CORS** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **XSS/CSRF/CORS** correctly for the project's Java version.
...
1. What is **XSS/CSRF/CORS** in Java?
2. Why would you use **XSS/CSRF/CORS**?
3. How does **XSS/CSRF/CORS** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **XSS/CSRF/CORS**?
5. Where would **XSS/CSRF/CORS** appear in a real production system?
...
Create the smallest Java example that demonstrates **XSS/CSRF/CORS** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **XSS/CSRF/CORS** in plain English.
