...
**SSRF** is server-side request forgery through attacker-controlled destinations.

---

# 2. Why

Interviewers ask **SSRF** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **SSRF** is about server-side request forgery through attacker-controlled destinations. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

server fetches internal/link-local/private resources on attacker behalf. Within this day, the closest engineering anchor is **Trust Boundary**: Every user/service/data/tool boundary needs validation and authorization.
...
```java
// Never expose arbitrary fetch_url without destination controls.
```
...
- server fetches internal/link-local/private resources on attacker behalf
...
In a Java/Spring production system, **SSRF** appears in the context of **Java/Application Security: OWASP, Spring Security, Secrets, JWT/OAuth2 & Secure Coding**. Connect it to the day's project concern: Threat-model a Spring REST API + React frontend. Cover login, token/session, BOLA, SQL injection, SSRF, file upload, secrets, dependency CVEs, logging, and tenant isolation.
...
- Use **SSRF** only when its semantics fit the requirement.
...
1. Memorizing the definition of **SSRF** without understanding runtime behavior.
...
- First reproduce the symptom involving **SSRF**.
- Primary focus: allowlist destinations and enforce egress network policy.
...
- Does **SSRF** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **SSRF** correctly for the project's Java version.
...
1. What is **SSRF** in Java?
2. Why would you use **SSRF**?
3. How does **SSRF** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **SSRF**?
5. Where would **SSRF** appear in a real production system?
...
Create the smallest Java example that demonstrates **SSRF** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **SSRF** in plain English.
