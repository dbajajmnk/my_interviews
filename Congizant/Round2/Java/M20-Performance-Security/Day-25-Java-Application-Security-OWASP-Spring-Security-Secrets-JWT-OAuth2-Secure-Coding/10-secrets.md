...
**secrets** is sensitive credentials/configuration requiring controlled storage and access.

---

# 2. Why

Interviewers ask **secrets** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **secrets** is about sensitive credentials/configuration requiring controlled storage and access. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

runtime retrieves secret through a managed store/identity rather than source code. Within this day, the closest engineering anchor is **Secrets**: Never hard-code or log secrets; use managed secret stores and least privilege.
...
In a Java/Spring production system, **secrets** appears in the context of **Java/Application Security: OWASP, Spring Security, Secrets, JWT/OAuth2 & Secure Coding**. Connect it to the day's project concern: Threat-model a Spring REST API + React frontend. Cover login, token/session, BOLA, SQL injection, SSRF, file upload, secrets, dependency CVEs, logging, and tenant isolation.
...
- Use **secrets** only when its semantics fit the requirement.
...
1. Memorizing the definition of **secrets** without understanding runtime behavior.
...
- First reproduce the symptom involving **secrets**.
...
- Does **secrets** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **secrets** correctly for the project's Java version.
...
1. What is **secrets** in Java?
2. Why would you use **secrets**?
3. How does **secrets** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **secrets**?
5. Where would **secrets** appear in a real production system?
...
Create the smallest Java example that demonstrates **secrets** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **secrets** in plain English.
