...
**dependency risk** is third-party software supply-chain exposure.

---

# 2. Why

Interviewers ask **dependency risk** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **dependency risk** is about third-party software supply-chain exposure. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

application inherits vulnerabilities/behavior from dependencies and build artifacts. Within this day, the closest engineering anchor is **Trust Boundary**: Every user/service/data/tool boundary needs validation and authorization.
...
- application inherits vulnerabilities/behavior from dependencies and build artifacts
...
- Use **dependency risk** only when its semantics fit the requirement.
...
1. Memorizing the definition of **dependency risk** without understanding runtime behavior.
...
- First reproduce the symptom involving **dependency risk**.
- Primary focus: scan, pin/manage, patch, verify provenance.
...
- Does **dependency risk** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **dependency risk** correctly for the project's Java version.
...
1. What is **dependency risk** in Java?
2. Why would you use **dependency risk**?
3. How does **dependency risk** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **dependency risk**?
5. Where would **dependency risk** appear in a real production system?
...
Create the smallest Java example that demonstrates **dependency risk** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
