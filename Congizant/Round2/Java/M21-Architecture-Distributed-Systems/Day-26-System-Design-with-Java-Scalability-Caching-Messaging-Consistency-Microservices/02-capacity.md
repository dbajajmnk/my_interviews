...
**capacity** is partition-time trade-off reasoning in distributed systems.

---

# 2. Why

Interviewers ask **capacity** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **capacity** is about partition-time trade-off reasoning in distributed systems. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

during network partition some operations must favor consistency or availability according to requirements. Within this day, the closest engineering anchor is **Progressive Design**: Start with simplest system meeting requirements; add distributed components only for identified needs.
...
In a Java/Spring production system, **capacity** appears in the context of **System Design with Java: Scalability, Caching, Messaging, Consistency & Microservices**. Connect it to the day's project concern: Design URL shortener and order service using the same 12-step method: requirements → estimates → API/data → baseline → bottleneck → scale/failure/security.
...
- Use **capacity** only when its semantics fit the requirement.
...
1. Memorizing the definition of **capacity** without understanding runtime behavior.
...
- First reproduce the symptom involving **capacity**.
...
- Does **capacity** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **capacity** correctly for the project's Java version.
...
1. What is **capacity** in Java?
2. Why would you use **capacity**?
3. How does **capacity** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **capacity**?
5. Where would **capacity** appear in a real production system?
...
Create the smallest Java example that demonstrates **capacity** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **capacity** in plain English.
