...
**load balancing** is horizontal request distribution across interchangeable instances.

---

# 2. Why

Interviewers ask **load balancing** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **load balancing** is about horizontal request distribution across interchangeable instances. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

load balancer routes to ready instances; shared/durable state lives outside process. Within this day, the closest engineering anchor is **Progressive Design**: Start with simplest system meeting requirements; add distributed components only for identified needs.
...
In a Java/Spring production system, **load balancing** appears in the context of **System Design with Java: Scalability, Caching, Messaging, Consistency & Microservices**. Connect it to the day's project concern: Design URL shortener and order service using the same 12-step method: requirements → estimates → API/data → baseline → bottleneck → scale/failure/security.
...
- Use **load balancing** only when its semantics fit the requirement.
...
1. Memorizing the definition of **load balancing** without understanding runtime behavior.
...
- First reproduce the symptom involving **load balancing**.
...
- Does **load balancing** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **load balancing** correctly for the project's Java version.
...
1. What is **load balancing** in Java?
2. Why would you use **load balancing**?
3. How does **load balancing** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **load balancing**?
5. Where would **load balancing** appear in a real production system?
...
Create the smallest Java example that demonstrates **load balancing** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **load balancing** in plain English.
