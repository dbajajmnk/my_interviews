...
**microservices** is deployment/ownership boundary around cohesive business capability.

---

# 2. Why

Interviewers ask **microservices** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **microservices** is about deployment/ownership boundary around cohesive business capability. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.
...
In a Java/Spring production system, **microservices** appears in the context of **System Design with Java: Scalability, Caching, Messaging, Consistency & Microservices**. Connect it to the day's project concern: Design URL shortener and order service using the same 12-step method: requirements → estimates → API/data → baseline → bottleneck → scale/failure/security.
...
- Use **microservices** only when its semantics fit the requirement.
...
1. Memorizing the definition of **microservices** without understanding runtime behavior.
...
- First reproduce the symptom involving **microservices**.
...
- Does **microservices** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **microservices** correctly for the project's Java version.
...
2. Why would you use **microservices**?
3. How does **microservices** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **microservices**?
5. Where would **microservices** appear in a real production system?
...
Create the smallest Java example that demonstrates **microservices** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **microservices** in plain English.
