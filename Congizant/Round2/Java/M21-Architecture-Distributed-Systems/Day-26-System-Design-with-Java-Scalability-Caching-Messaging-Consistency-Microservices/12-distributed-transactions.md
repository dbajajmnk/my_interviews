...
**distributed transactions** is atomic consistency boundary.

---

# 2. Why

Interviewers ask **distributed transactions** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **distributed transactions** is about atomic consistency boundary. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

DB isolation/locking/MVCC and commit/rollback preserve specified invariants. Within this day, the closest engineering anchor is **Progressive Design**: Start with simplest system meeting requirements; add distributed components only for identified needs.
...
```java
@Transactional public void transfer(...) { /* DB work */ }
```
...
- DB isolation/locking/MVCC and commit/rollback preserve specified invariants
...
- Use **distributed transactions** only when its semantics fit the requirement.
...
1. Memorizing the definition of **distributed transactions** without understanding runtime behavior.
...
- First reproduce the symptom involving **distributed transactions**.
- Primary focus: keep transaction short and avoid remote waits inside.
...
- Does **distributed transactions** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **distributed transactions** correctly for the project's Java version.
...
1. What is **distributed transactions** in Java?
2. Why would you use **distributed transactions**?
3. How does **distributed transactions** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **distributed transactions**?
5. Where would **distributed transactions** appear in a real production system?
...
Create the smallest Java example that demonstrates **distributed transactions** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **distributed transactions** in plain English.
