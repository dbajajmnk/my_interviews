...
**circuit breaker** is failure-isolation state machine around an unhealthy dependency.

---

# 2. Why

Interviewers ask **circuit breaker** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **circuit breaker** is about failure-isolation state machine around an unhealthy dependency. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

opens after failure threshold, rejects calls, probes recovery in half-open state. Within this day, the closest engineering anchor is **Circuit Breaker**: Stops repeated calls to unhealthy dependency and protects caller resources.
...
- opens after failure threshold, rejects calls, probes recovery in half-open state
...
- Use **circuit breaker** only when its semantics fit the requirement.
...
1. Memorizing the definition of **circuit breaker** without understanding runtime behavior.
...
- First reproduce the symptom involving **circuit breaker**.
- Primary focus: monitor states and avoid hiding persistent errors.
...
- Does **circuit breaker** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **circuit breaker** correctly for the project's Java version.
...
1. What is **circuit breaker** in Java?
2. Why would you use **circuit breaker**?
3. How does **circuit breaker** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **circuit breaker**?
5. Where would **circuit breaker** appear in a real production system?
...
Create the smallest Java example that demonstrates **circuit breaker** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **circuit breaker** in plain English.
