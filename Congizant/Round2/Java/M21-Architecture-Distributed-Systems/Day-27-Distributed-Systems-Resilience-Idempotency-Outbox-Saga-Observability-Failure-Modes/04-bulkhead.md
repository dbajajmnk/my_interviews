...
**bulkhead** is resource isolation between workloads/dependencies.

---

# 2. Why

Interviewers ask **bulkhead** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **bulkhead** is about resource isolation between workloads/dependencies. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

separate pools/semaphores prevent one dependency from consuming all capacity. Within this day, the closest engineering anchor is **Timeout**: Every remote call needs bounded waiting.
...
```java
// separate semaphore/pool for payment vs search
```
...
- separate pools/semaphores prevent one dependency from consuming all capacity
...
- Use **bulkhead** only when its semantics fit the requirement.
...
1. Memorizing the definition of **bulkhead** without understanding runtime behavior.
...
- First reproduce the symptom involving **bulkhead**.
- Primary focus: size from SLO/capacity and monitor saturation.
...
- Does **bulkhead** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **bulkhead** correctly for the project's Java version.
...
1. What is **bulkhead** in Java?
2. Why would you use **bulkhead**?
3. How does **bulkhead** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **bulkhead**?
5. Where would **bulkhead** appear in a real production system?
...
Create the smallest Java example that demonstrates **bulkhead** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **bulkhead** in plain English.
