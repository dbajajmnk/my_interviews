...
**outbox** is reliable publication intent tied to a local DB transaction.

---

# 2. Why

Interviewers ask **outbox** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **outbox** is about reliable publication intent tied to a local DB transaction. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

business row and outbox row commit atomically; background publisher sends/retries event. Within this day, the closest engineering anchor is **Outbox**: Solves DB commit + event publish dual-write gap using one local transaction.
...
```java
// transaction: save order + save outbox event
```
...
- business row and outbox row commit atomically; background publisher sends/retries event
...
- Use **outbox** only when its semantics fit the requirement.
...
1. Memorizing the definition of **outbox** without understanding runtime behavior.
...
- First reproduce the symptom involving **outbox**.
- Primary focus: monitor outbox lag and idempotent consumers.
...
- Does **outbox** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **outbox** correctly for the project's Java version.
...
1. What is **outbox** in Java?
2. Why would you use **outbox**?
3. How does **outbox** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **outbox**?
5. Where would **outbox** appear in a real production system?
...
Create the smallest Java example that demonstrates **outbox** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **outbox** in plain English.
