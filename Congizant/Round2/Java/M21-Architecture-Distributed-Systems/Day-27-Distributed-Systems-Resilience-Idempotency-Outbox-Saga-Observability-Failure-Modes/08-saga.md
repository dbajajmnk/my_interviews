...
**saga** is distributed business transaction using local steps plus compensation.

---

# 2. Why

Interviewers ask **saga** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **saga** is about distributed business transaction using local steps plus compensation. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

orchestrated/choreographed steps commit independently and compensate on later failure. Within this day, the closest engineering anchor is **Saga**: Coordinates/compensates distributed business steps when one ACID transaction is unavailable.
...
```java
// reserve → charge → ship; compensate reserve/charge on failure
```
...
- orchestrated/choreographed steps commit independently and compensate on later failure
...
- Use **saga** only when its semantics fit the requirement.
...
1. Memorizing the definition of **saga** without understanding runtime behavior.
...
- First reproduce the symptom involving **saga**.
- Primary focus: model irreversible steps and reconciliation.
...
- Does **saga** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **saga** correctly for the project's Java version.
...
1. What is **saga** in Java?
2. Why would you use **saga**?
3. How does **saga** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **saga**?
5. Where would **saga** appear in a real production system?
...
Create the smallest Java example that demonstrates **saga** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **saga** in plain English.
