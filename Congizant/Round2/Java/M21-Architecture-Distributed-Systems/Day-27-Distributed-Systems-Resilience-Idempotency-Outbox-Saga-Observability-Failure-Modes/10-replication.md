...
**replication** is maintaining copies of data for availability/read scale.

---

# 2. Why

Interviewers ask **replication** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **replication** is about maintaining copies of data for availability/read scale. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

replicas apply changes with possible lag/failover semantics. Within this day, the closest engineering anchor is **Timeout**: Every remote call needs bounded waiting.
...
- replicas apply changes with possible lag/failover semantics
...
- Use **replication** only when its semantics fit the requirement.
...
1. Memorizing the definition of **replication** without understanding runtime behavior.
...
- First reproduce the symptom involving **replication**.
- Primary focus: measure lag and route freshness-sensitive reads appropriately.
...
- Does **replication** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **replication** correctly for the project's Java version.
...
1. What is **replication** in Java?
2. Why would you use **replication**?
3. How does **replication** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **replication**?
5. Where would **replication** appear in a real production system?
...
Create the smallest Java example that demonstrates **replication** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **replication** in plain English.
