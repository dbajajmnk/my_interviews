...
**Kubernetes concepts** is orchestrated replica lifecycle and traffic readiness.

---

# 2. Why

Interviewers ask **Kubernetes concepts** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **Kubernetes concepts** is about orchestrated replica lifecycle and traffic readiness. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

controllers maintain replicas; readiness gates traffic, liveness can trigger restart. Within this day, the closest engineering anchor is **Kubernetes Awareness**: Deployment manages replicas; Service provides discovery; probes control lifecycle/traffic; ConfigMap/Secret separate config.
...
```java
// readiness: can serve traffic; liveness: process should restart
```
...
- controllers maintain replicas; readiness gates traffic, liveness can trigger restart
...
- Use **Kubernetes concepts** only when its semantics fit the requirement.
...
1. Memorizing the definition of **Kubernetes concepts** without understanding runtime behavior.
...
- First reproduce the symptom involving **Kubernetes concepts**.
- Primary focus: avoid probes that create dependency cascades.
...
- Does **Kubernetes concepts** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **Kubernetes concepts** correctly for the project's Java version.
...
1. What is **Kubernetes concepts** in Java?
2. Why would you use **Kubernetes concepts**?
3. How does **Kubernetes concepts** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **Kubernetes concepts**?
5. Where would **Kubernetes concepts** appear in a real production system?
...
Create the smallest Java example that demonstrates **Kubernetes concepts** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **Kubernetes concepts** in plain English.
