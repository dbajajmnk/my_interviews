# Topic 06 — configuration/secrets
...
**configuration/secrets** is sensitive credentials/configuration requiring controlled storage and access.

---

# 2. Why

Interviewers ask **configuration/secrets** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **configuration/secrets** is about sensitive credentials/configuration requiring controlled storage and access. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

runtime retrieves secret through a managed store/identity rather than source code. Within this day, the closest engineering anchor is **Container**: Packages app/runtime/dependencies with resource boundaries; JVM must respect container CPU/memory constraints.
...
```java
// Inject secret reference at runtime; never hard-code.
```
...
- runtime retrieves secret through a managed store/identity rather than source code
...
- Use **configuration/secrets** only when its semantics fit the requirement.
...
1. Memorizing the definition of **configuration/secrets** without understanding runtime behavior.
...
- First reproduce the symptom involving **configuration/secrets**.
- Primary focus: rotate, audit, and avoid logs.
...
- Does **configuration/secrets** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **configuration/secrets** correctly for the project's Java version.
...
1. What is **configuration/secrets** in Java?
2. Why would you use **configuration/secrets**?
3. How does **configuration/secrets** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **configuration/secrets**?
5. Where would **configuration/secrets** appear in a real production system?
...
Create the smallest Java example that demonstrates **configuration/secrets** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **configuration/secrets** in plain English.
