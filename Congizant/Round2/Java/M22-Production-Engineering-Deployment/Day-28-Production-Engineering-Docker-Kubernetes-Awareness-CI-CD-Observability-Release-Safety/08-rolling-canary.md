...
**rolling/canary** is controlled promotion of a tested artifact into production.

---

# 2. Why

Interviewers ask **rolling/canary** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **rolling/canary** is about controlled promotion of a tested artifact into production. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

immutable artifacts pass gates; staged rollout limits blast radius; rollback requires compatibility. Within this day, the closest engineering anchor is **Canary**: Gradually exposes new version and uses metrics to decide promotion/rollback.
...
- Use **rolling/canary** only when its semantics fit the requirement.
...
1. Memorizing the definition of **rolling/canary** without understanding runtime behavior.
...
- First reproduce the symptom involving **rolling/canary**.
...
- Does **rolling/canary** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **rolling/canary** correctly for the project's Java version.
...
1. What is **rolling/canary** in Java?
2. Why would you use **rolling/canary**?
3. How does **rolling/canary** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **rolling/canary**?
5. Where would **rolling/canary** appear in a real production system?
...
Create the smallest Java example that demonstrates **rolling/canary** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **rolling/canary** in plain English.
