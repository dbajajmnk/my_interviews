# Topic 07 — microservice decision
...
**microservice decision** is deployment/ownership boundary around cohesive business capability.

---

# 2. Why

Interviewers ask **microservice decision** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **microservice decision** is about deployment/ownership boundary around cohesive business capability. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

network boundaries add failure/latency/consistency cost compared with in-process modules. Within this day, the closest engineering anchor is **Scenario Structure**: Clarify → evidence → baseline → trade-off → recommendation → validation.
...
- Use **microservice decision** only when its semantics fit the requirement.
...
1. Memorizing the definition of **microservice decision** without understanding runtime behavior.
...
- First reproduce the symptom involving **microservice decision**.
...
- Does **microservice decision** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **microservice decision** correctly for the project's Java version.
...
1. What is **microservice decision** in Java?
2. Why would you use **microservice decision**?
3. How does **microservice decision** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **microservice decision**?
5. Where would **microservice decision** appear in a real production system?
...
Create the smallest Java example that demonstrates **microservice decision** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **microservice decision** in plain English.
