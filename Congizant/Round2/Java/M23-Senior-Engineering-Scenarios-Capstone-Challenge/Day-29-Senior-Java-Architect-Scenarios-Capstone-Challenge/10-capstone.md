...
**capstone** is partition-time trade-off reasoning in distributed systems.

---

# 2. Why

Interviewers ask **capstone** to test whether you understand its Java semantics, internal behavior, failure modes, and when its trade-offs are justified—not only its definition.

---

# 3. Plain English

Plain English: **capstone** is about partition-time trade-off reasoning in distributed systems. Use it when the requirement matches that problem; otherwise prefer the simpler Java design.

---

# 4. Engineering Explanation

during network partition some operations must favor consistency or availability according to requirements. Within this day, the closest engineering anchor is **Scenario Structure**: Clarify → evidence → baseline → trade-off → recommendation → validation.
...
- Use **capstone** only when its semantics fit the requirement.
...
1. Memorizing the definition of **capstone** without understanding runtime behavior.
...
- First reproduce the symptom involving **capstone**.
...
- Does **capstone** solve a real requirement or add accidental complexity?
...
- Ask whether AI-generated code uses **capstone** correctly for the project's Java version.
...
1. What is **capstone** in Java?
2. Why would you use **capstone**?
3. How does **capstone** work internally at the level relevant to a Java engineer?
4. What is a common mistake with **capstone**?
5. Where would **capstone** appear in a real production system?
...
Create the smallest Java example that demonstrates **capstone** correctly. Then deliberately introduce one realistic bug or misuse, reproduce it, fix it, and explain the code-review rule that would have caught it.
...
- [ ] I can define **capstone** in plain English.
