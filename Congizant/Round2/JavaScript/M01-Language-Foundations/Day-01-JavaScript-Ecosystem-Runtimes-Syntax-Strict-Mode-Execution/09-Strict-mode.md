...
**Strict mode** is enabling stricter language semantics for legacy script contexts.

---

# 2. Why

Interviewers use **Strict mode** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Strict mode means enabling stricter language semantics for legacy script contexts. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

strict mode changes selected error/binding semantics; ES modules are strict automatically A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- strict mode changes selected error/binding semantics; ES modules are strict automatically
...
In real projects, this matters for safer legacy script execution.
...
- Use **Strict mode** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Strict mode** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Strict mode**.
...
- Does **Strict mode** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Strict mode** is valid for the target runtime and module system.
...
2. Why would you use **Strict mode** in JavaScript?
3. How does **Strict mode** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Strict mode**?
5. Where does **Strict mode** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Strict mode**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Strict mode** in plain English.
