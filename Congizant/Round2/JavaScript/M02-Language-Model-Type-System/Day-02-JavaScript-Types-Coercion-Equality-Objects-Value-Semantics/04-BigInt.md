...
**BigInt** is arbitrary-precision integer values.

---

# 2. Why

Interviewers use **BigInt** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: BigInt means arbitrary-precision integer values. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

BigInt is distinct from Number and cannot be mixed in arithmetic without explicit conversion A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- BigInt is distinct from Number and cannot be mixed in arithmetic without explicit conversion
...
In real projects, this matters for large integer identifiers/counters.
...
- Use **BigInt** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **BigInt** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **BigInt**.
...
- Does **BigInt** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **BigInt** is valid for the target runtime and module system.
...
1. What is **BigInt**?
2. Why would you use **BigInt** in JavaScript?
3. How does **BigInt** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **BigInt**?
5. Where does **BigInt** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **BigInt**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **BigInt** in plain English.
