...
**Type coercion** is implicit or explicit type conversion.

---

# 2. Why

Interviewers use **Type coercion** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Type coercion means implicit or explicit type conversion. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
- Use **Type coercion** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Type coercion** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Type coercion**.
...
- Does **Type coercion** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Type coercion** is valid for the target runtime and module system.
...
2. Why would you use **Type coercion** in JavaScript?
3. How does **Type coercion** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Type coercion**?
5. Where does **Type coercion** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Type coercion**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Type coercion** in plain English.
