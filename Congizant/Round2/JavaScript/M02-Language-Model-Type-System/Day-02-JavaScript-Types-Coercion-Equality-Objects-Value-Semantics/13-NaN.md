...
**NaN** is special numeric not-a-number value.

---

# 2. Why

Interviewers use **NaN** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: NaN means special numeric not-a-number value. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

NaN is not equal to itself under `===`; use Number.isNaN for intentional checks A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- NaN is not equal to itself under `===`; use Number.isNaN for intentional checks
...
In real projects, this matters for numeric parsing/validation.
...
- Use **NaN** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **NaN** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **NaN**.
...
- Does **NaN** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **NaN** is valid for the target runtime and module system.
...
1. What is **NaN**?
2. Why would you use **NaN** in JavaScript?
3. How does **NaN** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **NaN**?
5. Where does **NaN** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **NaN**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **NaN** in plain English.
