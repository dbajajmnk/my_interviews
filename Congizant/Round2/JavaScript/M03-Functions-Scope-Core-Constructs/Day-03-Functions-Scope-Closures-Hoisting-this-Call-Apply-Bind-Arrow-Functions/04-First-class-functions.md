# Topic 04 — First-class functions
...
**First-class functions** is treating functions as values.

---

# 2. Why

Interviewers use **First-class functions** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: First-class functions means treating functions as values. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

functions can be stored, passed, returned, and composed A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
const withLog = fn => (...args) => (console.log(args), fn(...args));
```
...
In real projects, this matters for higher-order design.
...
- Use **First-class functions** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **First-class functions** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **First-class functions**.
...
- Does **First-class functions** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **First-class functions** is valid for the target runtime and module system.
...
2. Why would you use **First-class functions** in JavaScript?
3. How does **First-class functions** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **First-class functions**?
5. Where does **First-class functions** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **First-class functions**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **First-class functions** in plain English.
