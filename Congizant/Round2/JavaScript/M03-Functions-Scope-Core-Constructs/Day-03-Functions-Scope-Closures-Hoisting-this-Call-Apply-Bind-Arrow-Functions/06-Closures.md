...
**Closures** is a function retaining access to its lexical environment.

---

# 2. Why

Interviewers use **Closures** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Closures means a function retaining access to its lexical environment. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
- Use **Closures** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Closures** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Closures**.
...
- Does **Closures** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Closures** is valid for the target runtime and module system.
...
1. What is **Closures**?
2. Why would you use **Closures** in JavaScript?
3. How does **Closures** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Closures**?
5. Where does **Closures** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Closures**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Closures** in plain English.
