...
**Hoisting** is pre-execution binding instantiation.

---

# 2. Why

Interviewers use **Hoisting** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Hoisting means pre-execution binding instantiation. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

function declarations and `var`/lexical declarations are created before line-by-line execution with different initialization rules A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- function declarations and `var`/lexical declarations are created before line-by-line execution with different initialization rules
...
In real projects, this matters for output-prediction/debugging.
...
- Use **Hoisting** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Hoisting** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Hoisting**.
...
- Does **Hoisting** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Hoisting** is valid for the target runtime and module system.
...
1. What is **Hoisting**?
2. Why would you use **Hoisting** in JavaScript?
3. How does **Hoisting** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Hoisting**?
5. Where does **Hoisting** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Hoisting**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Hoisting** in plain English.
