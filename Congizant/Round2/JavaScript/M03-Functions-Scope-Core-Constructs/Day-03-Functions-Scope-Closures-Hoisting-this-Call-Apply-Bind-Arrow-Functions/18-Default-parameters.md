...
**Default parameters** is parameter initialization when argument is undefined.

---

# 2. Why

Interviewers use **Default parameters** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Default parameters means parameter initialization when argument is undefined. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

default expressions run at call time in parameter scope A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- default expressions run at call time in parameter scope
...
In real projects, this matters for API defaults.
...
- Use **Default parameters** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Default parameters** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Default parameters**.
...
- Does **Default parameters** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Default parameters** is valid for the target runtime and module system.
...
1. What is **Default parameters**?
2. Why would you use **Default parameters** in JavaScript?
3. How does **Default parameters** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Default parameters**?
5. Where does **Default parameters** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Default parameters**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Default parameters** in plain English.
