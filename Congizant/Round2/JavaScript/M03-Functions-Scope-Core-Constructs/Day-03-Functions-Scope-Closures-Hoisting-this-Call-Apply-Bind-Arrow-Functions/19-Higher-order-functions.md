# Topic 19 — Higher-order functions
...
**Higher-order functions** is functions accepting/returning functions.

---

# 2. Why

Interviewers use **Higher-order functions** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Higher-order functions means functions accepting/returning functions. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

behavior is injected/composed rather than hard-coded A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- behavior is injected/composed rather than hard-coded
...
In real projects, this matters for functional composition.
...
- Use **Higher-order functions** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Higher-order functions** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Higher-order functions**.
...
- Does **Higher-order functions** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Higher-order functions** is valid for the target runtime and module system.
...
2. Why would you use **Higher-order functions** in JavaScript?
3. How does **Higher-order functions** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Higher-order functions**?
5. Where does **Higher-order functions** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Higher-order functions**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Higher-order functions** in plain English.
