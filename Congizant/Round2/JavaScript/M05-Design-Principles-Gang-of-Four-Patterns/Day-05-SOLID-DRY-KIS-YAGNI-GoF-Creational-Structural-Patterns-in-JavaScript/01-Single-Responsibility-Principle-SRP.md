...
**Single Responsibility Principle (SRP)** is one cohesive reason to change.

---

# 2. Why

Interviewers use **Single Responsibility Principle (SRP)** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Single Responsibility Principle (SRP) means one cohesive reason to change. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

separate policy, transport, persistence, formatting, and unrelated change axes A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- separate policy, transport, persistence, formatting, and unrelated change axes
...
Split a handler that validates, queries SQL, sends email, and formats HTML.
...
- Use **Single Responsibility Principle (SRP)** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Single Responsibility Principle (SRP)** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Single Responsibility Principle (SRP)**.
...
- Does **Single Responsibility Principle (SRP)** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Single Responsibility Principle (SRP)** is valid for the target runtime and module system.
...
2. Why would you use **Single Responsibility Principle (SRP)** in JavaScript?
3. How does **Single Responsibility Principle (SRP)** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Single Responsibility Principle (SRP)**?
5. Where does **Single Responsibility Principle (SRP)** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Single Responsibility Principle (SRP)**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
