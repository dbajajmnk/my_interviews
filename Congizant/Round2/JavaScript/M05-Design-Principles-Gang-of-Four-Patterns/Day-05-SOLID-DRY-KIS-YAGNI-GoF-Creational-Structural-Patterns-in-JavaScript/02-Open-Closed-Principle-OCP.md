...
**Open/Closed Principle (OCP)** is support expected extension through stable contracts/composition.

---

# 2. Why

Interviewers use **Open/Closed Principle (OCP)** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Open/Closed Principle (OCP) means support expected extension through stable contracts/composition. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

functions/objects can be injected rather than editing giant conditionals A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- functions/objects can be injected rather than editing giant conditionals
...
Use a strategy map for pricing policies.
...
- Use **Open/Closed Principle (OCP)** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Open/Closed Principle (OCP)** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Open/Closed Principle (OCP)**.
...
- Does **Open/Closed Principle (OCP)** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Open/Closed Principle (OCP)** is valid for the target runtime and module system.
...
2. Why would you use **Open/Closed Principle (OCP)** in JavaScript?
3. How does **Open/Closed Principle (OCP)** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Open/Closed Principle (OCP)**?
5. Where does **Open/Closed Principle (OCP)** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Open/Closed Principle (OCP)**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
