...
**Liskov Substitution Principle (LSP)** is substitutable implementations preserve the abstraction's behavioral expectations.

---

# 2. Why

Interviewers use **Liskov Substitution Principle (LSP)** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Liskov Substitution Principle (LSP) means substitutable implementations preserve the abstraction's behavioral expectations. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

duck typing makes contract discipline more important, not less A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- duck typing makes contract discipline more important, not less
...
Every storage adapter must honor save/load semantics.
...
- Use **Liskov Substitution Principle (LSP)** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Liskov Substitution Principle (LSP)** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Liskov Substitution Principle (LSP)**.
...
- Does **Liskov Substitution Principle (LSP)** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Liskov Substitution Principle (LSP)** is valid for the target runtime and module system.
...
2. Why would you use **Liskov Substitution Principle (LSP)** in JavaScript?
3. How does **Liskov Substitution Principle (LSP)** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Liskov Substitution Principle (LSP)**?
5. Where does **Liskov Substitution Principle (LSP)** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Liskov Substitution Principle (LSP)**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
