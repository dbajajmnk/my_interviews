...
**Factory Method** is delegates creation through a factory operation so callers do not hard-code concrete products.

---

# 2. Why

Interviewers use **Factory Method** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Factory Method means delegates creation through a factory operation so callers do not hard-code concrete products. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

creation decision is separated from use; in JavaScript the factory may simply be a function A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- creation decision is separated from use; in JavaScript the factory may simply be a function
...
Use it for provider/parser/client creation when construction varies; reject it when a simpler function/object/module solves the same problem.
...
- Use **Factory Method** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Factory Method** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Factory Method**.
...
- Does **Factory Method** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Factory Method** is valid for the target runtime and module system.
...
2. Why would you use **Factory Method** in JavaScript?
3. How does **Factory Method** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Factory Method**?
5. Where does **Factory Method** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Factory Method**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
