...
**Abstract Factory** is creates a family of related objects through one family-level factory.

---

# 2. Why

Interviewers use **Abstract Factory** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Abstract Factory means creates a family of related objects through one family-level factory. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

a factory object/function returns compatible collaborators for one environment A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- a factory object/function returns compatible collaborators for one environment
...
Use it for environment/provider-specific infrastructure families; reject it when a simpler function/object/module solves the same problem.
...
- Use **Abstract Factory** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Abstract Factory** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Abstract Factory**.
...
- Does **Abstract Factory** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Abstract Factory** is valid for the target runtime and module system.
...
2. Why would you use **Abstract Factory** in JavaScript?
3. How does **Abstract Factory** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Abstract Factory**?
5. Where does **Abstract Factory** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Abstract Factory**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
