...
**Prototype** is creates new objects by cloning/delegating from a prototype or template.

---

# 2. Why

Interviewers use **Prototype** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Prototype means creates new objects by cloning/delegating from a prototype or template. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

JavaScript has native prototype delegation; object copying still requires clear shallow/deep semantics A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
const template = { retries: 2, timeout: 3000 };
const config = { ...template, timeout: 5000 };
```
...
- JavaScript has native prototype delegation; object copying still requires clear shallow/deep semantics
...
Use it for configuration templates and object delegation; reject it when a simpler function/object/module solves the same problem.
...
- Use **Prototype** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Prototype** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Prototype**.
...
- Does **Prototype** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Prototype** is valid for the target runtime and module system.
...
1. What is **Prototype**?
2. Why would you use **Prototype** in JavaScript?
3. How does **Prototype** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Prototype**?
5. Where does **Prototype** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Prototype**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
