...
**Adapter** is translates an incompatible interface into the interface your application expects.

---

# 2. Why

Interviewers use **Adapter** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Adapter means translates an incompatible interface into the interface your application expects. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

wrapper maps methods, arguments, results, and errors A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
const paymentGateway = legacyClient => ({
  async charge({ amount }) {
    const r = await legacyClient.makePayment(amount);
    return { id: r.txnId };
  }
});
```
...
- wrapper maps methods, arguments, results, and errors
...
Use it for legacy or third-party SDK integration; reject it when a simpler function/object/module solves the same problem.
...
- Use **Adapter** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Adapter** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Adapter**.
...
- Does **Adapter** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Adapter** is valid for the target runtime and module system.
...
1. What is **Adapter**?
2. Why would you use **Adapter** in JavaScript?
3. How does **Adapter** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Adapter**?
5. Where does **Adapter** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Adapter**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
