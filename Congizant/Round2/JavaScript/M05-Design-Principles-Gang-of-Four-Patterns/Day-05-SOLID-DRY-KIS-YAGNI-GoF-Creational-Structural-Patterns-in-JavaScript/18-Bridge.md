...
**Bridge** is separates an abstraction from an implementation dimension so each can vary independently.

---

# 2. Why

Interviewers use **Bridge** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Bridge means separates an abstraction from an implementation dimension so each can vary independently. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

high-level behavior delegates the implementation detail through a collaborator A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
const makeNotifier = sender => ({
  welcome: user => sender.send(`Welcome ${user.name}`)
});
```
...
- high-level behavior delegates the implementation detail through a collaborator
...
Use it for notification type independent from delivery provider; reject it when a simpler function/object/module solves the same problem.
...
- Use **Bridge** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Bridge** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Bridge**.
...
- Does **Bridge** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Bridge** is valid for the target runtime and module system.
...
1. What is **Bridge**?
2. Why would you use **Bridge** in JavaScript?
3. How does **Bridge** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Bridge**?
5. Where does **Bridge** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Bridge**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
