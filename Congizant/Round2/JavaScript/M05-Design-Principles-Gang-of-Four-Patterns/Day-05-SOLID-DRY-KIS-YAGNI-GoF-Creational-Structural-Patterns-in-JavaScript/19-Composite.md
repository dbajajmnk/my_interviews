...
**Composite** is treats individual objects and groups uniformly through a shared operation.

---

# 2. Why

Interviewers use **Composite** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Composite means treats individual objects and groups uniformly through a shared operation. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

a composite recursively delegates to child components A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
const all = rules => ctx => rules.every(rule => rule(ctx));
```
...
- a composite recursively delegates to child components
...
Use it for validation/rule trees and hierarchical UI/domain structures; reject it when a simpler function/object/module solves the same problem.
...
- Use **Composite** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Composite** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Composite**.
...
- Does **Composite** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Composite** is valid for the target runtime and module system.
...
1. What is **Composite**?
2. Why would you use **Composite** in JavaScript?
3. How does **Composite** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Composite**?
5. Where does **Composite** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Composite**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
