...
**Decorator** is adds behavior by wrapping a function/object while preserving the contract.

---

# 2. Why

Interviewers use **Decorator** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Decorator means adds behavior by wrapping a function/object while preserving the contract. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

wrapper executes before/after behavior and delegates to the original A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
const withMetrics = fn => async (...args) => {
...
- wrapper executes before/after behavior and delegates to the original
...
Use it for metrics, retries, caching, authorization, tracing; reject it when a simpler function/object/module solves the same problem.
...
- Use **Decorator** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Decorator** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Decorator**.
...
- Does **Decorator** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Decorator** is valid for the target runtime and module system.
...
1. What is **Decorator**?
2. Why would you use **Decorator** in JavaScript?
3. How does **Decorator** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Decorator**?
5. Where does **Decorator** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Decorator**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
