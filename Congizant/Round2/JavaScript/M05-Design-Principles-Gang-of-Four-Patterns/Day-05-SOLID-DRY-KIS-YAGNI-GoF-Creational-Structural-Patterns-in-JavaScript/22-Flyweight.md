...
Interviewers use **Flyweight** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Flyweight means shares reusable intrinsic state across many logical objects to reduce memory cost. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

canonical immutable shared objects are referenced by many consumers while per-use state stays external A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
const getType = code => productTypes.get(code) ?? productTypes.set(code, Object.freeze({code})).get(code);
```
...
- canonical immutable shared objects are referenced by many consumers while per-use state stays external
...
Use it for high-volume repeated metadata when memory evidence justifies it; reject it when a simpler function/object/module solves the same problem.
...
- Use **Flyweight** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Flyweight** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Flyweight**.
...
- Does **Flyweight** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Flyweight** is valid for the target runtime and module system.
...
1. What is **Flyweight**?
2. Why would you use **Flyweight** in JavaScript?
3. How does **Flyweight** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Flyweight**?
5. Where does **Flyweight** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Flyweight**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
