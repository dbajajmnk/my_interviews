...
**Command** is represents an action/request as data or an executable object/function.

---

# 2. Why

Interviewers use **Command** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Command means represents an action/request as data or an executable object/function. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

commands can be queued, logged, retried, audited, or scheduled A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
const command = { type: "CreateOrder", payload: { customerId } };
```
...
- commands can be queued, logged, retried, audited, or scheduled
...
Use it for job queues, CQRS-style commands, undo/audit workflows; reject it when a simpler function/object/module solves the same problem.
...
- Use **Command** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Command** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Command**.
...
- Does **Command** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Command** is valid for the target runtime and module system.
...
1. What is **Command**?
2. Why would you use **Command** in JavaScript?
3. How does **Command** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Command**?
5. Where does **Command** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Command**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
