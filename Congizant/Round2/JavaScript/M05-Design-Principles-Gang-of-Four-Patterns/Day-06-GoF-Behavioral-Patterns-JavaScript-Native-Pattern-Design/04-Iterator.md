...
Interviewers use **Iterator** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Iterator means provides sequential traversal without exposing collection representation. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

objects implement `Symbol.iterator` and return `{value, done}` from `next()` A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
const range = {
  *[Symbol.iterator]() { for (let i=1;i<=3;i++) yield i; }
};
```
...
- objects implement `Symbol.iterator` and return `{value, done}` from `next()`
...
Use it for custom lazy sequences and traversal; reject it when a simpler function/object/module solves the same problem.
...
- Use **Iterator** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Iterator** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Iterator**.
...
- Does **Iterator** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Iterator** is valid for the target runtime and module system.
...
1. What is **Iterator**?
2. Why would you use **Iterator** in JavaScript?
3. How does **Iterator** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Iterator**?
5. Where does **Iterator** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Iterator**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
