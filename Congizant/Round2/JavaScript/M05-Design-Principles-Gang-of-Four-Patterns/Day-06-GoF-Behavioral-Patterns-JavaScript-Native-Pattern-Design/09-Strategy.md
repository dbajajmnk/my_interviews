...
**Strategy** is encapsulates interchangeable algorithms behind a stable contract.

---

# 2. Why

Interviewers use **Strategy** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Strategy means encapsulates interchangeable algorithms behind a stable contract. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

JavaScript functions often make Strategy almost ceremony-free A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
const price = (order, strategy) => strategy(order);
const premium = order => order.total * 0.9;
```
...
- JavaScript functions often make Strategy almost ceremony-free
...
Use it for pricing, routing, retry policy, tax, validation; reject it when a simpler function/object/module solves the same problem.
...
- Use **Strategy** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Strategy** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Strategy**.
...
- Does **Strategy** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Strategy** is valid for the target runtime and module system.
...
1. What is **Strategy**?
2. Why would you use **Strategy** in JavaScript?
3. How does **Strategy** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Strategy**?
5. Where does **Strategy** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Strategy**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Strategy** in plain English.
