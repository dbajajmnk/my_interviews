...
**Template Method** is defines a fixed algorithm skeleton with customizable steps.

---

# 2. Why

Interviewers use **Template Method** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Template Method means defines a fixed algorithm skeleton with customizable steps. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

class inheritance can provide hooks, though function composition is often clearer in JavaScript A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
class ImportJob {
  async run(){ await this.read(); await this.validate(); await this.persist(); }
}
```
...
- class inheritance can provide hooks, though function composition is often clearer in JavaScript
...
Use it for framework-like processing when ordered inheritance hooks are justified; reject it when a simpler function/object/module solves the same problem.
...
- Use **Template Method** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Template Method** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Template Method**.
...
- Does **Template Method** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Template Method** is valid for the target runtime and module system.
...
2. Why would you use **Template Method** in JavaScript?
3. How does **Template Method** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Template Method**?
5. Where does **Template Method** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Template Method**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
