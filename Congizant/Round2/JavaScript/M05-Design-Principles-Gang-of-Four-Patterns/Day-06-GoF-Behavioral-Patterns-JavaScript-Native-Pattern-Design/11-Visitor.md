...
Interviewers use **Visitor** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Visitor means adds operations across a stable object structure without modifying every element type. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

dispatch uses element type/tag plus visitor operation A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
const visit = (node, visitor) => visitor[node.type](node);
```
...
- dispatch uses element type/tag plus visitor operation
...
Use it for ASTs/compilers and stable node hierarchies with many added operations; reject it when a simpler function/object/module solves the same problem.
...
- Use **Visitor** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Visitor** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Visitor**.
...
- Does **Visitor** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Visitor** is valid for the target runtime and module system.
...
1. What is **Visitor**?
2. Why would you use **Visitor** in JavaScript?
3. How does **Visitor** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Visitor**?
5. Where does **Visitor** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Visitor**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
