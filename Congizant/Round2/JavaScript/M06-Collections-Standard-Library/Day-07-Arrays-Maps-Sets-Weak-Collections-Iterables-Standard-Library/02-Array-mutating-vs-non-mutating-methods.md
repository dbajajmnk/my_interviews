# Topic 02 — Array mutating vs non-mutating methods
...
**Array mutating vs non-mutating methods** is distinguishing in-place Array operations from value-producing operations.

---

# 2. Why

Interviewers use **Array mutating vs non-mutating methods** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Array mutating vs non-mutating methods means distinguishing in-place Array operations from value-producing operations. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

push/splice/sort mutate; map/filter/toSorted-style APIs create new results A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- push/splice/sort mutate; map/filter/toSorted-style APIs create new results
...
In real projects, this matters for state-management/code review.
...
- Use **Array mutating vs non-mutating methods** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Array mutating vs non-mutating methods** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Array mutating vs non-mutating methods**.
...
- Does **Array mutating vs non-mutating methods** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Array mutating vs non-mutating methods** is valid for the target runtime and module system.
...
1. What is **Array mutating vs non-mutating methods**?
2. Why would you use **Array mutating vs non-mutating methods** in JavaScript?
3. How does **Array mutating vs non-mutating methods** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Array mutating vs non-mutating methods**?
5. Where does **Array mutating vs non-mutating methods** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Array mutating vs non-mutating methods**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Array mutating vs non-mutating methods** in plain English.
