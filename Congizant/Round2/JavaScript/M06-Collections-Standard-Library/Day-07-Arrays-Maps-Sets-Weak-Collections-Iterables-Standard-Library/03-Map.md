...
**Map** is key/value collection supporting arbitrary key types and insertion order.

---

# 2. Why

Interviewers use **Map** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Map means key/value collection supporting arbitrary key types and insertion order. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
- Use **Map** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Map** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Map**.
...
- Does **Map** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Map** is valid for the target runtime and module system.
...
1. What is **Map**?
2. Why would you use **Map** in JavaScript?
3. How does **Map** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Map**?
5. Where does **Map** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Map**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Map** in plain English.
