...
**Iterator protocol** is standard pull-based iteration contract.

---

# 2. Why

Interviewers use **Iterator protocol** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Iterator protocol means standard pull-based iteration contract. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

iterator `next()` returns `{value, done}` and iterables expose `Symbol.iterator` A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- iterator `next()` returns `{value, done}` and iterables expose `Symbol.iterator`
...
In real projects, this matters for language iteration.
...
- Use **Iterator protocol** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Iterator protocol** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Iterator protocol**.
...
- Does **Iterator protocol** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Iterator protocol** is valid for the target runtime and module system.
...
1. What is **Iterator protocol**?
2. Why would you use **Iterator protocol** in JavaScript?
3. How does **Iterator protocol** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Iterator protocol**?
5. Where does **Iterator protocol** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Iterator protocol**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Iterator protocol** in plain English.
