...
**Promise rejection** is asynchronous failure state.

---

# 2. Why

Interviewers use **Promise rejection** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Promise rejection means asynchronous failure state. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

a rejected Promise propagates until handled by catch/await try-catch A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- a rejected Promise propagates until handled by catch/await try-catch
...
In real projects, this matters for async errors.
...
- Use **Promise rejection** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Promise rejection** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Promise rejection**.
...
- Does **Promise rejection** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Promise rejection** is valid for the target runtime and module system.
...
1. What is **Promise rejection**?
2. Why would you use **Promise rejection** in JavaScript?
3. How does **Promise rejection** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Promise rejection**?
5. Where does **Promise rejection** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Promise rejection**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Promise rejection** in plain English.
