# Topic 10 — Dependencies vs devDependencies
...
**Dependencies vs devDependencies** is the JavaScript concept **Dependencies vs devDependencies** within ES Modules, CommonJS, npm, package.json, SemVer, Lockfiles & Build Tooling.

---

# 2. Why

Interviewers use **Dependencies vs devDependencies** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Dependencies vs devDependencies means the JavaScript concept **Dependencies vs devDependencies** within ES Modules, CommonJS, npm, package.json, SemVer, Lockfiles & Build Tooling. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
In JavaScript applications, **Dependencies vs devDependencies** should be connected to a concrete runtime, API, correctness, debugging, performance, or maintainability requirement.
...
- Use **Dependencies vs devDependencies** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Dependencies vs devDependencies** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Dependencies vs devDependencies**.
...
- Does **Dependencies vs devDependencies** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Dependencies vs devDependencies** is valid for the target runtime and module system.
...
1. What is **Dependencies vs devDependencies**?
2. Why would you use **Dependencies vs devDependencies** in JavaScript?
3. How does **Dependencies vs devDependencies** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Dependencies vs devDependencies**?
5. Where does **Dependencies vs devDependencies** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Dependencies vs devDependencies**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Dependencies vs devDependencies** in plain English.
