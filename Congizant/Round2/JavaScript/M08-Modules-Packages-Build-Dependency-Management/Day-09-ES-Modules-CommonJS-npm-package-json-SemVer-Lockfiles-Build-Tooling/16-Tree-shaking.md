...
**Tree shaking** is build-time removal of unused statically analyzable exports.

---

# 2. Why

Interviewers use **Tree shaking** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Tree shaking means build-time removal of unused statically analyzable exports. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

ESM's static structure helps bundlers eliminate unreachable code, subject to side effects/dynamic behavior A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- ESM's static structure helps bundlers eliminate unreachable code, subject to side effects/dynamic behavior
...
In real projects, this matters for browser bundle optimization.
...
- Use **Tree shaking** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Tree shaking** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Tree shaking**.
...
- Does **Tree shaking** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Tree shaking** is valid for the target runtime and module system.
...
2. Why would you use **Tree shaking** in JavaScript?
3. How does **Tree shaking** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Tree shaking**?
5. Where does **Tree shaking** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Tree shaking**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Tree shaking** in plain English.
