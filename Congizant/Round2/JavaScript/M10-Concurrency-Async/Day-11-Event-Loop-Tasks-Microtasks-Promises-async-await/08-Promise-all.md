...
**Promise.all** is fail-fast aggregation of multiple input promises.

---

# 2. Why

Interviewers use **Promise.all** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Promise.all means fail-fast aggregation of multiple input promises. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

starts from already-created promises and fulfills when all fulfill; rejects on first observed rejection A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- starts from already-created promises and fulfills when all fulfill; rejects on first observed rejection
...
In real projects, this matters for parallel independent work.
...
- Use **Promise.all** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Promise.all** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Promise.all**.
...
- Does **Promise.all** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Promise.all** is valid for the target runtime and module system.
...
2. Why would you use **Promise.all** in JavaScript?
3. How does **Promise.all** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Promise.all**?
5. Where does **Promise.all** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Promise.all**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Promise.all** in plain English.
