...
**Promise.allSettled** is unique-value collection.

---

# 2. Why

Interviewers use **Promise.allSettled** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Promise.allSettled means unique-value collection. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
- Use **Promise.allSettled** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Promise.allSettled** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Promise.allSettled**.
...
- Does **Promise.allSettled** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Promise.allSettled** is valid for the target runtime and module system.
...
2. Why would you use **Promise.allSettled** in JavaScript?
3. How does **Promise.allSettled** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Promise.allSettled**?
5. Where does **Promise.allSettled** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Promise.allSettled**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Promise.allSettled** in plain English.
