...
**async/await** is syntax for Promise-based asynchronous control flow.

---

# 2. Why

Interviewers use **async/await** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: async/await means syntax for Promise-based asynchronous control flow. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
- Use **async/await** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **async/await** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **async/await**.
...
- Does **async/await** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **async/await** is valid for the target runtime and module system.
...
2. Why would you use **async/await** in JavaScript?
3. How does **async/await** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **async/await**?
5. Where does **async/await** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **async/await**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **async/await** in plain English.
