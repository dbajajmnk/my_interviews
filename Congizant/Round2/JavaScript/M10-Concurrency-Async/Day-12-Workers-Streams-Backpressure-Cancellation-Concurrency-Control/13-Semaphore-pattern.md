...
**Semaphore pattern** is key/value collection supporting arbitrary key types and insertion order.

---

# 2. Why

Interviewers use **Semaphore pattern** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Semaphore pattern means key/value collection supporting arbitrary key types and insertion order. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

Map uses key identity/SameValueZero-like semantics and explicit collection APIs A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- Map uses key identity/SameValueZero-like semantics and explicit collection APIs
...
In real projects, this matters for caches/indexes.
...
- Use **Semaphore pattern** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Semaphore pattern** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Semaphore pattern**.
...
- Does **Semaphore pattern** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Semaphore pattern** is valid for the target runtime and module system.
...
1. What is **Semaphore pattern**?
2. Why would you use **Semaphore pattern** in JavaScript?
3. How does **Semaphore pattern** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Semaphore pattern**?
5. Where does **Semaphore pattern** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Semaphore pattern**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Semaphore pattern** in plain English.
