...
**Generator functions** is pausable function producing iterator results lazily.

---

# 2. Why

Interviewers use **Generator functions** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Generator functions means pausable function producing iterator results lazily. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

calling generator returns iterator; `yield` suspends/resumes execution state A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- calling generator returns iterator; `yield` suspends/resumes execution state
...
In real projects, this matters for lazy sequences.
...
- Use **Generator functions** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Generator functions** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Generator functions**.
...
- Does **Generator functions** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Generator functions** is valid for the target runtime and module system.
...
1. What is **Generator functions**?
2. Why would you use **Generator functions** in JavaScript?
3. How does **Generator functions** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Generator functions**?
5. Where does **Generator functions** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Generator functions**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Generator functions** in plain English.
