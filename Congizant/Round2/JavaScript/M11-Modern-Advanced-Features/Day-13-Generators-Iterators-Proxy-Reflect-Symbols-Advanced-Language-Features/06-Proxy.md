...
**Proxy** is controls or observes access to another object while keeping a compatible usage surface.

---

# 2. Why

Interviewers use **Proxy** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Proxy means controls or observes access to another object while keeping a compatible usage surface. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

JavaScript `Proxy` can intercept fundamental operations dynamically; explicit wrappers are often easier to reason about A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
const secured = new Proxy(service, {
  get(target, prop) {
    authorize(prop);
...
- JavaScript `Proxy` can intercept fundamental operations dynamically; explicit wrappers are often easier to reason about
...
Use it for access control, instrumentation, lazy behavior, framework internals; reject it when a simpler function/object/module solves the same problem.
...
- Use **Proxy** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Proxy** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Proxy**.
...
- Does **Proxy** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Proxy** is valid for the target runtime and module system.
...
1. What is **Proxy**?
2. Why would you use **Proxy** in JavaScript?
3. How does **Proxy** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Proxy**?
5. Where does **Proxy** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Proxy**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
