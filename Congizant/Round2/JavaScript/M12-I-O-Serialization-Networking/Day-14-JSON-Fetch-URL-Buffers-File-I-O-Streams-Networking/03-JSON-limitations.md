...
**JSON limitations** is the JavaScript concept **JSON limitations** within JSON, Fetch, URL, Buffers, File I/O, Streams & Networking.

---

# 2. Why

Interviewers use **JSON limitations** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: JSON limitations means the JavaScript concept **JSON limitations** within JSON, Fetch, URL, Buffers, File I/O, Streams & Networking. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
In JavaScript applications, **JSON limitations** should be connected to a concrete runtime, API, correctness, debugging, performance, or maintainability requirement.
...
- Use **JSON limitations** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **JSON limitations** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **JSON limitations**.
...
- Does **JSON limitations** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **JSON limitations** is valid for the target runtime and module system.
...
1. What is **JSON limitations**?
2. Why would you use **JSON limitations** in JavaScript?
3. How does **JSON limitations** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **JSON limitations**?
5. Where does **JSON limitations** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **JSON limitations**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **JSON limitations** in plain English.
