...
**fetch** is Promise-based HTTP request API.

---

# 2. Why

Interviewers use **fetch** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: fetch means Promise-based HTTP request API. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
- Use **fetch** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **fetch** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **fetch**.
...
- Does **fetch** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **fetch** is valid for the target runtime and module system.
...
1. What is **fetch**?
2. Why would you use **fetch** in JavaScript?
3. How does **fetch** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **fetch**?
5. Where does **fetch** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **fetch**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **fetch** in plain English.
