...
**WebSocket awareness** is full-duplex message-oriented persistent connection.

---

# 2. Why

Interviewers use **WebSocket awareness** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: WebSocket awareness means full-duplex message-oriented persistent connection. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

connection stays open for bidirectional frames and requires heartbeat/backpressure/reconnect design A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- connection stays open for bidirectional frames and requires heartbeat/backpressure/reconnect design
...
In real projects, this matters for real-time applications.
...
- Use **WebSocket awareness** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **WebSocket awareness** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **WebSocket awareness**.
...
- Does **WebSocket awareness** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **WebSocket awareness** is valid for the target runtime and module system.
...
1. What is **WebSocket awareness**?
2. Why would you use **WebSocket awareness** in JavaScript?
3. How does **WebSocket awareness** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **WebSocket awareness**?
5. Where does **WebSocket awareness** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **WebSocket awareness**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **WebSocket awareness** in plain English.
