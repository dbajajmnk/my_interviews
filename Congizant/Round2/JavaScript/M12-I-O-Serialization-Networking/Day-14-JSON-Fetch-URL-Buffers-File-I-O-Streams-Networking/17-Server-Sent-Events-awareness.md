# Topic 17 — Server-Sent Events awareness
...
**Server-Sent Events awareness** is server-to-browser one-way event stream over HTTP.

---

# 2. Why

Interviewers use **Server-Sent Events awareness** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Server-Sent Events awareness means server-to-browser one-way event stream over HTTP. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

EventSource receives text events with browser reconnect behavior A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- EventSource receives text events with browser reconnect behavior
...
In real projects, this matters for notifications/streaming updates.
...
- Use **Server-Sent Events awareness** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Server-Sent Events awareness** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Server-Sent Events awareness**.
...
- Does **Server-Sent Events awareness** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Server-Sent Events awareness** is valid for the target runtime and module system.
...
1. What is **Server-Sent Events awareness**?
2. Why would you use **Server-Sent Events awareness** in JavaScript?
3. How does **Server-Sent Events awareness** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Server-Sent Events awareness**?
5. Where does **Server-Sent Events awareness** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Server-Sent Events awareness**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Server-Sent Events awareness** in plain English.
