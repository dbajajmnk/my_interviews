...
**Call stack depth** is explicitly invoking a function with a chosen `this` and arguments.

---

# 2. Why

Interviewers use **Call stack depth** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Call stack depth means explicitly invoking a function with a chosen `this` and arguments. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
- Use **Call stack depth** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Call stack depth** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Call stack depth**.
...
- Does **Call stack depth** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Call stack depth** is valid for the target runtime and module system.
...
1. What is **Call stack depth**?
2. Why would you use **Call stack depth** in JavaScript?
3. How does **Call stack depth** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Call stack depth**?
5. Where does **Call stack depth** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Call stack depth**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Call stack depth** in plain English.
