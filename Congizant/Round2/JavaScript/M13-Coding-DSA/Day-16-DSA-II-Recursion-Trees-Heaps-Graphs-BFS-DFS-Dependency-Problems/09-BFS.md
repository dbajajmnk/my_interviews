...
**BFS** is breadth-first traversal using a queue.

---

# 2. Why

Interviewers use **BFS** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: BFS means breadth-first traversal using a queue. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

process nodes in distance layers and track visited to avoid repeats A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
const q=[start]; let head=0; while(head<q.length){const n=q[head++];}
```
...
- process nodes in distance layers and track visited to avoid repeats
...
In real projects, this matters for graphs/trees.
...
- Use **BFS** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **BFS** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **BFS**.
...
- Does **BFS** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **BFS** is valid for the target runtime and module system.
...
1. What is **BFS**?
2. Why would you use **BFS** in JavaScript?
3. How does **BFS** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **BFS**?
5. Where does **BFS** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **BFS**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
