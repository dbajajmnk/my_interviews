...
**DFS** is depth-first traversal using recursion/stack.

---

# 2. Why

Interviewers use **DFS** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: DFS means depth-first traversal using recursion/stack. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

follow a branch then backtrack; track visited for graphs A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
const dfs=n=>{ if(seen.has(n))return; seen.add(n); };
```
...
- follow a branch then backtrack; track visited for graphs
...
- Use **DFS** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **DFS** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **DFS**.
...
- Does **DFS** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **DFS** is valid for the target runtime and module system.
...
1. What is **DFS**?
2. Why would you use **DFS** in JavaScript?
3. How does **DFS** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **DFS**?
5. Where does **DFS** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **DFS**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **DFS** in plain English.
