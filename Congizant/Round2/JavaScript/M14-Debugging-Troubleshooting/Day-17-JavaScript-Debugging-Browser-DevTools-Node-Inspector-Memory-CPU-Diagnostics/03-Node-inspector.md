...
**Node inspector** is Chrome DevTools-compatible debugging protocol for Node.

---

# 2. Why

Interviewers use **Node inspector** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Node inspector means Chrome DevTools-compatible debugging protocol for Node. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

`--inspect` exposes debugger/profiler hooks A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- `--inspect` exposes debugger/profiler hooks
...
In real projects, this matters for server debugging.
...
- Use **Node inspector** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Node inspector** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Node inspector**.
...
- Does **Node inspector** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Node inspector** is valid for the target runtime and module system.
...
2. Why would you use **Node inspector** in JavaScript?
3. How does **Node inspector** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Node inspector**?
5. Where does **Node inspector** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Node inspector**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Node inspector** in plain English.
