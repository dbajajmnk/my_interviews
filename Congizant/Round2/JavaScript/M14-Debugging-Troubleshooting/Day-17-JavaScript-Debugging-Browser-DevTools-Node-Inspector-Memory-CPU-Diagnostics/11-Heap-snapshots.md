...
**Heap snapshots** is runtime memory for objects/functions/closures.

---

# 2. Why

Interviewers use **Heap snapshots** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Heap snapshots means runtime memory for objects/functions/closures. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
- Use **Heap snapshots** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Heap snapshots** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Heap snapshots**.
...
- Does **Heap snapshots** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Heap snapshots** is valid for the target runtime and module system.
...
1. What is **Heap snapshots**?
2. Why would you use **Heap snapshots** in JavaScript?
3. How does **Heap snapshots** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Heap snapshots**?
5. Where does **Heap snapshots** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Heap snapshots**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Heap snapshots** in plain English.
