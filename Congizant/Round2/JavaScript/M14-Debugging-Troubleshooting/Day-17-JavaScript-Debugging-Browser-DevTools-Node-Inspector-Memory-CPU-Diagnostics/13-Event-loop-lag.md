...
**Event-loop lag** is delay between expected and actual event-loop progress.

---

# 2. Why

Interviewers use **Event-loop lag** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Event-loop lag means delay between expected and actual event-loop progress. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

CPU blocking or overloaded callbacks delay timers/I/O callbacks despite low average throughput metrics A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- CPU blocking or overloaded callbacks delay timers/I/O callbacks despite low average throughput metrics
...
In real projects, this matters for Node latency diagnosis.
...
- Use **Event-loop lag** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Event-loop lag** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Event-loop lag**.
...
- Does **Event-loop lag** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Event-loop lag** is valid for the target runtime and module system.
...
1. What is **Event-loop lag**?
2. Why would you use **Event-loop lag** in JavaScript?
3. How does **Event-loop lag** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Event-loop lag**?
5. Where does **Event-loop lag** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Event-loop lag**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Event-loop lag** in plain English.
