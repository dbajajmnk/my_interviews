# Topic 01 — Risk-first code review
...
**Risk-first code review** is reviewing highest-impact correctness/security/data risks before style.

---

# 2. Why

Interviewers use **Risk-first code review** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Risk-first code review means reviewing highest-impact correctness/security/data risks before style. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

human attention is limited; prioritize defects that can break users or systems A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
// correctness → security → async/data → failures → performance → tests
```
...
- human attention is limited; prioritize defects that can break users or systems
...
In real projects, this matters for senior code review.
...
- Use **Risk-first code review** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Risk-first code review** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Risk-first code review**.
...
- Does **Risk-first code review** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Risk-first code review** is valid for the target runtime and module system.
...
1. What is **Risk-first code review**?
2. Why would you use **Risk-first code review** in JavaScript?
3. How does **Risk-first code review** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Risk-first code review**?
5. Where does **Risk-first code review** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Risk-first code review**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Risk-first code review** in plain English.
