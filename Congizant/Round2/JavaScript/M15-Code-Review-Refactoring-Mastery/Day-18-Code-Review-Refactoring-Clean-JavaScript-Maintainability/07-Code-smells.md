...
**Code smells** is the JavaScript concept **Code smells** within Code Review, Refactoring, Clean JavaScript & Maintainability.

---

# 2. Why

Interviewers use **Code smells** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Code smells means the JavaScript concept **Code smells** within Code Review, Refactoring, Clean JavaScript & Maintainability. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
In JavaScript applications, **Code smells** should be connected to a concrete runtime, API, correctness, debugging, performance, or maintainability requirement.
...
- Use **Code smells** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Code smells** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Code smells**.
...
- Does **Code smells** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Code smells** is valid for the target runtime and module system.
...
1. What is **Code smells**?
2. Why would you use **Code smells** in JavaScript?
3. How does **Code smells** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Code smells**?
5. Where does **Code smells** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Code smells**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
