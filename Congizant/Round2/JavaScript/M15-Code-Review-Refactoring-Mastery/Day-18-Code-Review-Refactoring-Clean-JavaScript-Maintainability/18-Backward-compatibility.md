# Topic 18 — Backward compatibility
...
**Backward compatibility** is the JavaScript concept **Backward compatibility** within Code Review, Refactoring, Clean JavaScript & Maintainability.

---

# 2. Why

Interviewers use **Backward compatibility** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Backward compatibility means the JavaScript concept **Backward compatibility** within Code Review, Refactoring, Clean JavaScript & Maintainability. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
In JavaScript applications, **Backward compatibility** should be connected to a concrete runtime, API, correctness, debugging, performance, or maintainability requirement.
...
- Use **Backward compatibility** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Backward compatibility** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Backward compatibility**.
...
- Does **Backward compatibility** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Backward compatibility** is valid for the target runtime and module system.
...
2. Why would you use **Backward compatibility** in JavaScript?
3. How does **Backward compatibility** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Backward compatibility**?
5. Where does **Backward compatibility** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Backward compatibility**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Backward compatibility** in plain English.
