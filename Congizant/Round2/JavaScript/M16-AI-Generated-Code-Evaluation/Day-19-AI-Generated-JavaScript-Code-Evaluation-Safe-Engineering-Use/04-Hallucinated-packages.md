# Topic 04 — Hallucinated packages
...
**Hallucinated packages** is detecting nonexistent or wrong third-party dependencies.

---

# 2. Why

Interviewers use **Hallucinated packages** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Hallucinated packages means detecting nonexistent or wrong third-party dependencies. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

model may invent names/versions/APIs; package install is a supply-chain action A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- model may invent names/versions/APIs; package install is a supply-chain action
...
In real projects, this matters for AI dependency safety.
...
- Use **Hallucinated packages** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Hallucinated packages** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Hallucinated packages**.
...
- Does **Hallucinated packages** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Hallucinated packages** is valid for the target runtime and module system.
...
1. What is **Hallucinated packages**?
2. Why would you use **Hallucinated packages** in JavaScript?
3. How does **Hallucinated packages** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Hallucinated packages**?
5. Where does **Hallucinated packages** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Hallucinated packages**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Hallucinated packages** in plain English.
