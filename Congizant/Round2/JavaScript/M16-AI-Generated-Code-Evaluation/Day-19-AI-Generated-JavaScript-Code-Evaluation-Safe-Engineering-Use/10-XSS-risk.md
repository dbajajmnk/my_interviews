...
**XSS risk** is untrusted content executing in browser origin.

---

# 2. Why

Interviewers use **XSS risk** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: XSS risk means untrusted content executing in browser origin. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

unsafe HTML/DOM sinks can interpret attacker data as markup/script A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- unsafe HTML/DOM sinks can interpret attacker data as markup/script
...
- Use **XSS risk** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **XSS risk** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **XSS risk**.
...
- Does **XSS risk** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **XSS risk** is valid for the target runtime and module system.
...
1. What is **XSS risk**?
2. Why would you use **XSS risk** in JavaScript?
3. How does **XSS risk** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **XSS risk**?
5. Where does **XSS risk** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **XSS risk**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **XSS risk** in plain English.
