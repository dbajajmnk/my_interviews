...
**HTTP/API tests** is the JavaScript concept **HTTP/API tests** within Testing: Node Test Runner, Jest/Vitest Awareness, Mocks, Integration, Contract & E2E.

---

# 2. Why

Interviewers use **HTTP/API tests** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: HTTP/API tests means the JavaScript concept **HTTP/API tests** within Testing: Node Test Runner, Jest/Vitest Awareness, Mocks, Integration, Contract & E2E. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
In JavaScript applications, **HTTP/API tests** should be connected to a concrete runtime, API, correctness, debugging, performance, or maintainability requirement.
...
- Use **HTTP/API tests** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **HTTP/API tests** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **HTTP/API tests**.
...
- Does **HTTP/API tests** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **HTTP/API tests** is valid for the target runtime and module system.
...
1. What is **HTTP/API tests**?
2. Why would you use **HTTP/API tests** in JavaScript?
3. How does **HTTP/API tests** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **HTTP/API tests**?
5. Where does **HTTP/API tests** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **HTTP/API tests**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **HTTP/API tests** in plain English.
