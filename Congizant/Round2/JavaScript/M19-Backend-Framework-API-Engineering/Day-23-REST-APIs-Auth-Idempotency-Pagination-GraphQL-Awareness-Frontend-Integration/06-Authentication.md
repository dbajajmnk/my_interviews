...
**Authentication** is proving caller identity.

---

# 2. Why

Interviewers use **Authentication** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Authentication means proving caller identity. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
- Use **Authentication** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Authentication** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Authentication**.
...
- Does **Authentication** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Authentication** is valid for the target runtime and module system.
...
2. Why would you use **Authentication** in JavaScript?
3. How does **Authentication** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Authentication**?
5. Where does **Authentication** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Authentication**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
