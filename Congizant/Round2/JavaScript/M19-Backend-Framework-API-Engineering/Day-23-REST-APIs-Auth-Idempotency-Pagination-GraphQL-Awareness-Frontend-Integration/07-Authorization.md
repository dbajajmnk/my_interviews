...
**Authorization** is deciding whether a principal may perform an action on a resource.

---

# 2. Why

Interviewers use **Authorization** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Authorization means deciding whether a principal may perform an action on a resource. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

server policy checks role/ownership/tenant/context for every sensitive operation A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- server policy checks role/ownership/tenant/context for every sensitive operation
...
- Use **Authorization** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Authorization** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Authorization**.
...
- Does **Authorization** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Authorization** is valid for the target runtime and module system.
...
2. Why would you use **Authorization** in JavaScript?
3. How does **Authorization** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Authorization**?
5. Where does **Authorization** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Authorization**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
