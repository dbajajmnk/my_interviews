# Topic 17 — Cookies vs bearer tokens
...
**Cookies vs bearer tokens** is browser-managed credential/state storage with automatic request sending rules.

---

# 2. Why

Interviewers use **Cookies vs bearer tokens** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Cookies vs bearer tokens means browser-managed credential/state storage with automatic request sending rules. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

SameSite/Secure/HttpOnly attributes affect CSRF/XSS exposure A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- SameSite/Secure/HttpOnly attributes affect CSRF/XSS exposure
...
In real projects, this matters for web auth.
...
- Use **Cookies vs bearer tokens** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Cookies vs bearer tokens** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Cookies vs bearer tokens**.
...
- Does **Cookies vs bearer tokens** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Cookies vs bearer tokens** is valid for the target runtime and module system.
...
1. What is **Cookies vs bearer tokens**?
2. Why would you use **Cookies vs bearer tokens** in JavaScript?
3. How does **Cookies vs bearer tokens** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Cookies vs bearer tokens**?
5. Where does **Cookies vs bearer tokens** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Cookies vs bearer tokens**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Cookies vs bearer tokens** in plain English.
