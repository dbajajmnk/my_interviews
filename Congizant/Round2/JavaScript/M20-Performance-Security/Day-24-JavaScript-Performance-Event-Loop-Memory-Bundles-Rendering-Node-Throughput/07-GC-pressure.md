...
**GC pressure** is the JavaScript concept **GC pressure** within JavaScript Performance: Event Loop, Memory, Bundles, Rendering & Node Throughput.

---

# 2. Why

Interviewers use **GC pressure** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: GC pressure means the JavaScript concept **GC pressure** within JavaScript Performance: Event Loop, Memory, Bundles, Rendering & Node Throughput. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
In JavaScript applications, **GC pressure** should be connected to a concrete runtime, API, correctness, debugging, performance, or maintainability requirement.
...
- Use **GC pressure** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **GC pressure** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **GC pressure**.
...
- Does **GC pressure** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **GC pressure** is valid for the target runtime and module system.
...
1. What is **GC pressure**?
2. Why would you use **GC pressure** in JavaScript?
3. How does **GC pressure** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **GC pressure**?
5. Where does **GC pressure** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **GC pressure**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **GC pressure** in plain English.
