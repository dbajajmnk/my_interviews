...
**Browser bundle size** is amount of browser JavaScript downloaded/parsed/executed.

---

# 2. Why

Interviewers use **Browser bundle size** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Browser bundle size means amount of browser JavaScript downloaded/parsed/executed. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

dependency size/code splitting/cache affect startup performance A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- dependency size/code splitting/cache affect startup performance
...
In real projects, this matters for frontend performance.
...
- Use **Browser bundle size** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Browser bundle size** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Browser bundle size**.
...
- Does **Browser bundle size** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Browser bundle size** is valid for the target runtime and module system.
...
1. What is **Browser bundle size**?
2. Why would you use **Browser bundle size** in JavaScript?
3. How does **Browser bundle size** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Browser bundle size**?
5. Where does **Browser bundle size** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Browser bundle size**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Browser bundle size** in plain English.
