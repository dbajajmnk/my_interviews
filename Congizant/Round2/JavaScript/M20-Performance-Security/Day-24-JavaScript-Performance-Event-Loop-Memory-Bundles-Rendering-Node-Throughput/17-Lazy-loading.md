...
**Lazy loading** is the JavaScript concept **Lazy loading** within JavaScript Performance: Event Loop, Memory, Bundles, Rendering & Node Throughput.

---

# 2. Why

Interviewers use **Lazy loading** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Lazy loading means the JavaScript concept **Lazy loading** within JavaScript Performance: Event Loop, Memory, Bundles, Rendering & Node Throughput. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
In JavaScript applications, **Lazy loading** should be connected to a concrete runtime, API, correctness, debugging, performance, or maintainability requirement.
...
- Use **Lazy loading** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Lazy loading** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Lazy loading**.
...
- Does **Lazy loading** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Lazy loading** is valid for the target runtime and module system.
...
2. Why would you use **Lazy loading** in JavaScript?
3. How does **Lazy loading** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Lazy loading**?
5. Where does **Lazy loading** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Lazy loading**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Lazy loading** in plain English.
