# Topic 02 — Authentication vs authorization
...
**Authentication vs authorization** is proving caller identity.

---

# 2. Why

Interviewers use **Authentication vs authorization** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Authentication vs authorization means proving caller identity. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

session/token/identity-provider result establishes principal; it does not grant every permission A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- session/token/identity-provider result establishes principal; it does not grant every permission
...
In real projects, this matters for security.
...
- Use **Authentication vs authorization** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Authentication vs authorization** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Authentication vs authorization**.
...
- Does **Authentication vs authorization** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Authentication vs authorization** is valid for the target runtime and module system.
...
2. Why would you use **Authentication vs authorization** in JavaScript?
3. How does **Authentication vs authorization** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Authentication vs authorization**?
5. Where does **Authentication vs authorization** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Authentication vs authorization**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Authentication vs authorization** in plain English.
