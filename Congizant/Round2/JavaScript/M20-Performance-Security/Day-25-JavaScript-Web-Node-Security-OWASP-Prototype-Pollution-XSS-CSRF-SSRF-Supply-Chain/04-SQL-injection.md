...
**SQL injection** is the JavaScript concept **SQL injection** within JavaScript/Web/Node Security: OWASP, Prototype Pollution, XSS, CSRF, SSRF & Supply Chain.

---

# 2. Why

Interviewers use **SQL injection** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: SQL injection means the JavaScript concept **SQL injection** within JavaScript/Web/Node Security: OWASP, Prototype Pollution, XSS, CSRF, SSRF & Supply Chain. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
In JavaScript applications, **SQL injection** should be connected to a concrete runtime, API, correctness, debugging, performance, or maintainability requirement.
...
- Use **SQL injection** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **SQL injection** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **SQL injection**.
...
- Does **SQL injection** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **SQL injection** is valid for the target runtime and module system.
...
2. Why would you use **SQL injection** in JavaScript?
3. How does **SQL injection** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **SQL injection**?
5. Where does **SQL injection** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **SQL injection**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
