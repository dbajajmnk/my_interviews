...
**XSS** is untrusted content executing in browser origin.

---

# 2. Why

Interviewers use **XSS** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: XSS means untrusted content executing in browser origin. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
- Use **XSS** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **XSS** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **XSS**.
...
- Does **XSS** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **XSS** is valid for the target runtime and module system.
...
1. What is **XSS**?
2. Why would you use **XSS** in JavaScript?
3. How does **XSS** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **XSS**?
5. Where does **XSS** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **XSS**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **XSS** in plain English.
