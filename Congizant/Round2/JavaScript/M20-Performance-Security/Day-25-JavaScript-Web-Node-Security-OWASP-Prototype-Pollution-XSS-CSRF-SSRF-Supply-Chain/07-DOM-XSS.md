...
**DOM XSS** is untrusted content executing in browser origin.

---

# 2. Why

Interviewers use **DOM XSS** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: DOM XSS means untrusted content executing in browser origin. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
- Use **DOM XSS** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **DOM XSS** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **DOM XSS**.
...
- Does **DOM XSS** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **DOM XSS** is valid for the target runtime and module system.
...
1. What is **DOM XSS**?
2. Why would you use **DOM XSS** in JavaScript?
3. How does **DOM XSS** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **DOM XSS**?
5. Where does **DOM XSS** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **DOM XSS**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **DOM XSS** in plain English.
