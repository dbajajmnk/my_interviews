...
**CSRF** is cross-site request forgery against automatically authenticated browser requests.

---

# 2. Why

Interviewers use **CSRF** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: CSRF means cross-site request forgery against automatically authenticated browser requests. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

attacker site triggers state change while browser sends credentials automatically A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
// SameSite + CSRF token/origin checks as appropriate
```
...
- attacker site triggers state change while browser sends credentials automatically
...
In real projects, this matters for web security.
...
- Use **CSRF** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **CSRF** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **CSRF**.
...
- Does **CSRF** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **CSRF** is valid for the target runtime and module system.
...
1. What is **CSRF**?
2. Why would you use **CSRF** in JavaScript?
3. How does **CSRF** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **CSRF**?
5. Where does **CSRF** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **CSRF**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **CSRF** in plain English.
