...
Interviewers use **SSRF** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: SSRF means server-side request forgery. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

server fetches attacker-selected internal/link-local destinations A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- server fetches attacker-selected internal/link-local destinations
...
In real projects, this matters for Node integrations.
...
- Use **SSRF** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **SSRF** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **SSRF**.
...
- Does **SSRF** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **SSRF** is valid for the target runtime and module system.
...
1. What is **SSRF**?
2. Why would you use **SSRF** in JavaScript?
3. How does **SSRF** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **SSRF**?
5. Where does **SSRF** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **SSRF**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
