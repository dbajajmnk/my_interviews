...
**Mass assignment** is blindly copying client fields into privileged domain objects.

---

# 2. Why

Interviewers use **Mass assignment** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Mass assignment means blindly copying client fields into privileged domain objects. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

untrusted properties such as role/tenant/status can be set unless allowlisted A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- untrusted properties such as role/tenant/status can be set unless allowlisted
...
In real projects, this matters for API security.
...
- Use **Mass assignment** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Mass assignment** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Mass assignment**.
...
- Does **Mass assignment** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Mass assignment** is valid for the target runtime and module system.
...
2. Why would you use **Mass assignment** in JavaScript?
3. How does **Mass assignment** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Mass assignment**?
5. Where does **Mass assignment** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Mass assignment**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Mass assignment** in plain English.
