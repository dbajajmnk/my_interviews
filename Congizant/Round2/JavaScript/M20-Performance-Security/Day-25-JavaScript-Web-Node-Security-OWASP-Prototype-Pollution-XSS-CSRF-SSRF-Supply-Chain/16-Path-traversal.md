...
**Path traversal** is escaping intended filesystem directory through attacker-controlled paths.

---

# 2. Why

Interviewers use **Path traversal** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Path traversal means escaping intended filesystem directory through attacker-controlled paths. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

`../`/absolute/symlink behavior can access unexpected files A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
const safe=path.join(base, path.basename(input));
```
...
- `../`/absolute/symlink behavior can access unexpected files
...
In real projects, this matters for file APIs.
...
- Use **Path traversal** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Path traversal** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Path traversal**.
...
- Does **Path traversal** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Path traversal** is valid for the target runtime and module system.
...
2. Why would you use **Path traversal** in JavaScript?
3. How does **Path traversal** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Path traversal**?
5. Where does **Path traversal** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Path traversal**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Path traversal** in plain English.
