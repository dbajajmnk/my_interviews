...
**Command injection** is untrusted data becoming shell syntax.

---

# 2. Why

Interviewers use **Command injection** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Command injection means untrusted data becoming shell syntax. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

shell execution interprets metacharacters; prefer direct process APIs with argument arrays and allowlists A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
spawn('tool',[safeArg],{shell:false});
```
...
- shell execution interprets metacharacters; prefer direct process APIs with argument arrays and allowlists
...
In real projects, this matters for server security.
...
- Use **Command injection** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Command injection** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Command injection**.
...
- Does **Command injection** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Command injection** is valid for the target runtime and module system.
...
2. Why would you use **Command injection** in JavaScript?
3. How does **Command injection** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Command injection**?
5. Where does **Command injection** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Command injection**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Command injection** in plain English.
