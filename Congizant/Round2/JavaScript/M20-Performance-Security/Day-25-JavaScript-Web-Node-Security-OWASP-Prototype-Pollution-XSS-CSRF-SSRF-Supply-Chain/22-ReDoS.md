...
**ReDoS** is regular-expression denial of service.

---

# 2. Why

Interviewers use **ReDoS** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: ReDoS means regular-expression denial of service. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

catastrophic backtracking can consume event-loop CPU on crafted input A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
// prefer linear-time patterns/limits
```
...
- catastrophic backtracking can consume event-loop CPU on crafted input
...
In real projects, this matters for web/API security.
...
- Use **ReDoS** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **ReDoS** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **ReDoS**.
...
- Does **ReDoS** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **ReDoS** is valid for the target runtime and module system.
...
1. What is **ReDoS**?
2. Why would you use **ReDoS** in JavaScript?
3. How does **ReDoS** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **ReDoS**?
5. Where does **ReDoS** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **ReDoS**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **ReDoS** in plain English.
