# Topic 05 — Stateless Node services
...
**Stateless Node services** is request handlers without process-local durable/session state.

---

# 2. Why

Interviewers use **Stateless Node services** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Stateless Node services means request handlers without process-local durable/session state. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

instances become interchangeable behind a load balancer; shared state lives externally A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
```javascript
// no user session in module-global mutable object
```
...
- instances become interchangeable behind a load balancer; shared state lives externally
...
In real projects, this matters for scaling.
...
- Use **Stateless Node services** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Stateless Node services** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Stateless Node services**.
...
- Does **Stateless Node services** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Stateless Node services** is valid for the target runtime and module system.
...
1. What is **Stateless Node services**?
2. Why would you use **Stateless Node services** in JavaScript?
3. How does **Stateless Node services** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Stateless Node services**?
5. Where does **Stateless Node services** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Stateless Node services**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Stateless Node services** in plain English.
