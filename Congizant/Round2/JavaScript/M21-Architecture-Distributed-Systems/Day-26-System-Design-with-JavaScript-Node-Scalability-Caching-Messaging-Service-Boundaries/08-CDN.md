...
**CDN** is edge caching/distribution for static/cacheable content.

---

# 2. Why

Interviewers use **CDN** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: CDN means edge caching/distribution for static/cacheable content. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

CDN serves content closer to users and offloads origin subject to cache rules A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- CDN serves content closer to users and offloads origin subject to cache rules
...
In real projects, this matters for web scalability.
...
- Use **CDN** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **CDN** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **CDN**.
...
- Does **CDN** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **CDN** is valid for the target runtime and module system.
...
1. What is **CDN**?
2. Why would you use **CDN** in JavaScript?
3. How does **CDN** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **CDN**?
5. Where does **CDN** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **CDN**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **CDN** in plain English.
