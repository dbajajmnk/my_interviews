...
**Retry** is repeating transiently failed operations with limits.

---

# 2. Why

Interviewers use **Retry** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Retry means repeating transiently failed operations with limits. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.
...
- Use **Retry** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Retry** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Retry**.
...
- Does **Retry** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Retry** is valid for the target runtime and module system.
...
1. What is **Retry**?
2. Why would you use **Retry** in JavaScript?
3. How does **Retry** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Retry**?
5. Where does **Retry** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Retry**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Retry** in plain English.
