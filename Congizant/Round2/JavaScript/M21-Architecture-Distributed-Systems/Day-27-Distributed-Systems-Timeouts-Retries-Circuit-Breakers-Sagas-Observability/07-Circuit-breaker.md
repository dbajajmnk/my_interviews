...
**Circuit breaker** is temporarily stopping calls to unhealthy dependency.

---

# 2. Why

Interviewers use **Circuit breaker** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Circuit breaker means temporarily stopping calls to unhealthy dependency. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

breaker transitions closed/open/half-open based on failures/recovery probes A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- breaker transitions closed/open/half-open based on failures/recovery probes
...
In real projects, this matters for failure isolation.
...
- Use **Circuit breaker** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Circuit breaker** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Circuit breaker**.
...
- Does **Circuit breaker** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Circuit breaker** is valid for the target runtime and module system.
...
2. Why would you use **Circuit breaker** in JavaScript?
3. How does **Circuit breaker** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Circuit breaker**?
5. Where does **Circuit breaker** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Circuit breaker**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
