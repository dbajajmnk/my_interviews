...
**Load shedding** is rejecting lower-priority work before overload collapses the system.

---

# 2. Why

Interviewers use **Load shedding** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Load shedding means rejecting lower-priority work before overload collapses the system. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

fast 429/503 can be safer than letting everything timeout A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- fast 429/503 can be safer than letting everything timeout
...
In real projects, this matters for overload control.
...
- Use **Load shedding** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Load shedding** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Load shedding**.
...
- Does **Load shedding** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Load shedding** is valid for the target runtime and module system.
...
2. Why would you use **Load shedding** in JavaScript?
3. How does **Load shedding** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Load shedding**?
5. Where does **Load shedding** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Load shedding**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **Load shedding** in plain English.
