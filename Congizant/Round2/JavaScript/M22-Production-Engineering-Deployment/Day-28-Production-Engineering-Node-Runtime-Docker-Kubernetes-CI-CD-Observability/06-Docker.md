...
**Docker** is containerized application packaging/runtime.

---

# 2. Why

Interviewers use **Docker** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: Docker means containerized application packaging/runtime. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

container image bundles runtime/app while resource limits constrain the process A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- container image bundles runtime/app while resource limits constrain the process
...
In real projects, this matters for production deployment.
...
- Use **Docker** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **Docker** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **Docker**.
...
- Does **Docker** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **Docker** is valid for the target runtime and module system.
...
1. What is **Docker**?
2. Why would you use **Docker** in JavaScript?
3. How does **Docker** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **Docker**?
5. Where does **Docker** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **Docker**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
