...
**RPO/RTO** is disaster recovery objectives.

---

# 2. Why

Interviewers use **RPO/RTO** to test whether you understand actual JavaScript semantics, host/runtime boundaries, failure modes, and engineering trade-offs—not only syntax.

---

# 3. Plain English

Plain English: RPO/RTO means disaster recovery objectives. Use it when it solves the stated problem; do not add complexity simply because the feature or pattern exists.

---

# 4. Engineering Explanation

RPO is acceptable data loss window; RTO is acceptable restoration time A senior answer separates **ECMAScript language guarantees**, **browser/Node host behavior**, and **engine/framework implementation details**.
...
- RPO is acceptable data loss window; RTO is acceptable restoration time
...
In real projects, this matters for DR.
...
- Use **RPO/RTO** because a requirement needs it, not because it is fashionable.
...
1. Memorizing **RPO/RTO** as trivia without understanding its language/runtime semantics.
...
- Reproduce the smallest failing case involving **RPO/RTO**.
...
- Does **RPO/RTO** improve correctness or does it hide behavior?
...
- Verify that AI-generated use of **RPO/RTO** is valid for the target runtime and module system.
...
1. What is **RPO/RTO**?
2. Why would you use **RPO/RTO** in JavaScript?
3. How does **RPO/RTO** work internally at the level relevant to an engineer?
4. What is one common bug or misconception with **RPO/RTO**?
5. Where does **RPO/RTO** appear in a real browser/Node system?
...
Create the smallest runnable JavaScript example that demonstrates **RPO/RTO**. Then deliberately introduce one realistic misuse, reproduce it, fix it, and explain the review rule that would have caught it.
...
- [ ] I can define **RPO/RTO** in plain English.
