...
**if expression** is the Kotlin engineering concept **if expression**.

---

# 2. Why

Interviewers use **if expression** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **if expression** is about the Kotlin engineering concept **if expression**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **if expression** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **if expression**.
...
- Does **if expression** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **if expression**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **if expression** in plain English.
