...
**when expression** is the Kotlin engineering concept **when expression**.

---

# 2. Why

Interviewers use **when expression** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **when expression** is about the Kotlin engineering concept **when expression**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **when expression** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **when expression**.
...
- Does **when expression** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **when expression**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **when expression** in plain English.
