...
**Type inference** is the Kotlin engineering concept **Type inference**.

---

# 2. Why

Interviewers use **Type inference** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Type inference** is about the Kotlin engineering concept **Type inference**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Type inference** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Type inference**.
...
- Does **Type inference** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Type inference**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
