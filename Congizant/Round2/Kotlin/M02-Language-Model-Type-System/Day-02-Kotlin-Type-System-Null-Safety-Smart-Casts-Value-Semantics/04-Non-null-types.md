...
**Non-null types** is the Kotlin engineering concept **Non-null types**.

---

# 2. Why

Interviewers use **Non-null types** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Non-null types** is about the Kotlin engineering concept **Non-null types**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Non-null types** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Non-null types**.
...
- Does **Non-null types** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Non-null types**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Non-null types** in plain English.
