...
**Safe-call operator** is calling/accessing only when receiver is non-null.

---

# 2. Why

Interviewers use **Safe-call operator** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Safe-call operator** is about calling/accessing only when receiver is non-null. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

`?.` short-circuits to null instead of throwing NPE
...
In a Kotlin/JVM production system, **Safe-call operator** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Safe-call operator**.
...
- Does **Safe-call operator** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Safe-call operator**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Safe-call operator** in plain English.
