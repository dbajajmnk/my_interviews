...
**Elvis operator** is providing a fallback for a nullable expression.

---

# 2. Why

Interviewers use **Elvis operator** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Elvis operator** is about providing a fallback for a nullable expression. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Elvis operator** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Elvis operator**.
...
- Does **Elvis operator** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Elvis operator**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Elvis operator** in plain English.
