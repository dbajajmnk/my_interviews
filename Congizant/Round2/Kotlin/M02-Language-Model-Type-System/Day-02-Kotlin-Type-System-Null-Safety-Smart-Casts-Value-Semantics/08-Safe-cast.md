...
**Safe cast** is the Kotlin engineering concept **Safe cast**.

---

# 2. Why

Interviewers use **Safe cast** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Safe cast** is about the Kotlin engineering concept **Safe cast**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Safe cast** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Safe cast**.
...
- Does **Safe cast** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Safe cast**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Safe cast** in plain English.
