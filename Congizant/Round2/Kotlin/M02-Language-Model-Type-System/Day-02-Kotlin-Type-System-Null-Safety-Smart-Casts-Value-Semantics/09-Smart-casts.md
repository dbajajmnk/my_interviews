...
**Smart casts** is compiler narrowing after a proven type/null check.

---

# 2. Why

Interviewers use **Smart casts** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Smart casts** is about compiler narrowing after a proven type/null check. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

compiler tracks stability/control flow and inserts no runtime wrapper beyond the actual check/cast
...
In a Kotlin/JVM production system, **Smart casts** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Smart casts**.
...
- Does **Smart casts** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Smart casts**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Smart casts** in plain English.
