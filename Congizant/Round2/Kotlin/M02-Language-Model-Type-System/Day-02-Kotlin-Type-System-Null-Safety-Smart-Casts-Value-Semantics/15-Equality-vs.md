...
**Equality == vs ===** is distinguishing structural equality from reference identity.

---

# 2. Why

Interviewers use **Equality == vs ===** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Equality == vs ===** is about distinguishing structural equality from reference identity. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

`==` delegates to equals with null safety; `===` checks identity
...
In a Kotlin/JVM production system, **Equality == vs ===** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Equality == vs ===**.
...
- Does **Equality == vs ===** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Equality == vs ===**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Equality == vs ===** in plain English.
