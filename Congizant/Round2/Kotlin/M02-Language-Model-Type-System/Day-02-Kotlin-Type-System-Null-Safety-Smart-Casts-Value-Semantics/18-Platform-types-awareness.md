# Topic 18 — Platform types awareness
...
**Platform types awareness** is Java-interoperability type with relaxed nullability information.

---

# 2. Why

Interviewers use **Platform types awareness** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Platform types awareness** is about Java-interoperability type with relaxed nullability information. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

compiler may allow both nullable/non-null usage because Java signature lacks precise nullability
...
```kotlin
val value = javaApi.value // String! conceptually
```
...
In a Kotlin/JVM production system, **Platform types awareness** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Platform types awareness**.
...
- Does **Platform types awareness** solve a real requirement?
...
1. What is **Platform types awareness**?
...
Create the smallest runnable Kotlin example demonstrating **Platform types awareness**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Platform types awareness** in plain English.
