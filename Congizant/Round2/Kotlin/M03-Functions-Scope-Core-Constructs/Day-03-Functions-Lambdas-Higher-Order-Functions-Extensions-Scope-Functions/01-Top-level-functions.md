...
**Top-level functions** is the Kotlin engineering concept **Top-level functions**.

---

# 2. Why

Interviewers use **Top-level functions** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Top-level functions** is about the Kotlin engineering concept **Top-level functions**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Top-level functions** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Top-level functions**.
...
- Does **Top-level functions** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Top-level functions**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Top-level functions** in plain English.
