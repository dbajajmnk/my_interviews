...
**Default arguments** is the Kotlin engineering concept **Default arguments**.

---

# 2. Why

Interviewers use **Default arguments** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Default arguments** is about the Kotlin engineering concept **Default arguments**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Default arguments** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Default arguments** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Default arguments**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
