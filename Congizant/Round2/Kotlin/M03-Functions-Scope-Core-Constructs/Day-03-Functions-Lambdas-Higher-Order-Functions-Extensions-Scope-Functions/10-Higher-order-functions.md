# Topic 10 — Higher-order functions
...
**Higher-order functions** is the Kotlin engineering concept **Higher-order functions**.

---

# 2. Why

Interviewers use **Higher-order functions** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Higher-order functions** is about the Kotlin engineering concept **Higher-order functions**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Higher-order functions** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Higher-order functions**.
...
- Does **Higher-order functions** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Higher-order functions**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
