...
**Function references** is the Kotlin engineering concept **Function references**.

---

# 2. Why

Interviewers use **Function references** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Function references** is about the Kotlin engineering concept **Function references**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Function references** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Function references**.
...
- Does **Function references** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Function references**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Function references** in plain English.
