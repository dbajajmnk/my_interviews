...
**Extension functions** is statically resolved function callable with receiver syntax.

---

# 2. Why

Interviewers use **Extension functions** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Extension functions** is about statically resolved function callable with receiver syntax. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

extensions do not modify the class or participate in virtual override dispatch
...
In a Kotlin/JVM production system, **Extension functions** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Extension functions**.
...
- Does **Extension functions** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Extension functions**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Extension functions** in plain English.
