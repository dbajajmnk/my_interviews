# Topic 21 — Non-local returns awareness
...
**Non-local returns awareness** is the Kotlin engineering concept **Non-local returns awareness**.

---

# 2. Why

Interviewers use **Non-local returns awareness** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Non-local returns awareness** is about the Kotlin engineering concept **Non-local returns awareness**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Non-local returns awareness** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Non-local returns awareness**.
...
- Does **Non-local returns awareness** solve a real requirement?
...
1. What is **Non-local returns awareness**?
...
Create the smallest runnable Kotlin example demonstrating **Non-local returns awareness**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Non-local returns awareness** in plain English.
