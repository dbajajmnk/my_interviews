...
**init blocks** is the Kotlin engineering concept **init blocks**.

---

# 2. Why

Interviewers use **init blocks** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **init blocks** is about the Kotlin engineering concept **init blocks**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **init blocks** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **init blocks**.
...
- Does **init blocks** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **init blocks**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **init blocks** in plain English.
