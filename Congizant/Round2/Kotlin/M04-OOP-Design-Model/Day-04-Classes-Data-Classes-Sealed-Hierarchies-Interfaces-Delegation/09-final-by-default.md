...
**final by default** is the Kotlin engineering concept **final by default**.

---

# 2. Why

Interviewers use **final by default** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **final by default** is about the Kotlin engineering concept **final by default**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **final by default** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **final by default**.
...
- Does **final by default** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **final by default**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **final by default** in plain English.
