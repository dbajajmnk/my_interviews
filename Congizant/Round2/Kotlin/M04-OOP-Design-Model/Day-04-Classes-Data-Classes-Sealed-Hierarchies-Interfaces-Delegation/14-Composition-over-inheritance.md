# Topic 14 — Composition over inheritance
...
**Composition over inheritance** is the Kotlin engineering concept **Composition over inheritance**.

---

# 2. Why

Interviewers use **Composition over inheritance** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Composition over inheritance** is about the Kotlin engineering concept **Composition over inheritance**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Composition over inheritance** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Composition over inheritance** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Composition over inheritance**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
