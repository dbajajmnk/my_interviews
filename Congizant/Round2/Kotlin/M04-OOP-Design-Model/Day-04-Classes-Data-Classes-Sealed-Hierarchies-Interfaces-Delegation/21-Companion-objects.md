...
**Companion objects** is object associated with a class rather than Java static members.

---

# 2. Why

Interviewers use **Companion objects** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Companion objects** is about object associated with a class rather than Java static members. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Companion objects** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Companion objects**.
...
- Does **Companion objects** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Companion objects**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Companion objects** in plain English.
