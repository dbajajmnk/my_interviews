...
**Inner classes** is the Kotlin engineering concept **Inner classes**.

---

# 2. Why

Interviewers use **Inner classes** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Inner classes** is about the Kotlin engineering concept **Inner classes**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Inner classes** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Inner classes**.
...
- Does **Inner classes** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Inner classes**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Inner classes** in plain English.
