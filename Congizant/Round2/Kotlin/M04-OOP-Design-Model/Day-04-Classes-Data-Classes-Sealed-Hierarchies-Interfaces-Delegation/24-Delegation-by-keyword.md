# Topic 24 — Delegation by keyword
...
**Delegation by keyword** is compiler-generated forwarding implementation.

---

# 2. Why

Interviewers use **Delegation by keyword** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Delegation by keyword** is about compiler-generated forwarding implementation. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

`by` forwards interface members to a delegate while allowing overrides
...
```kotlin
class LoggingRepo(private val d:Repo):Repo by d
```
...
In a Kotlin/JVM production system, **Delegation by keyword** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Delegation by keyword**.
...
- Does **Delegation by keyword** solve a real requirement?
...
1. What is **Delegation by keyword**?
...
Create the smallest runnable Kotlin example demonstrating **Delegation by keyword**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Delegation by keyword** in plain English.
