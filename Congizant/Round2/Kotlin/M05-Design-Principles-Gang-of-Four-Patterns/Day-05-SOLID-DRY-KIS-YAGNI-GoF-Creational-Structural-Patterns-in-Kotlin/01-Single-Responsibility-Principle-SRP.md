...
Interviewers use **Single Responsibility Principle (SRP)** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Single Responsibility Principle (SRP)** is about one primary reason to change. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

separate policy, transport, persistence and formatting
...
In a Kotlin/JVM production system, **Single Responsibility Principle (SRP)** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Single Responsibility Principle (SRP)** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Single Responsibility Principle (SRP)**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
