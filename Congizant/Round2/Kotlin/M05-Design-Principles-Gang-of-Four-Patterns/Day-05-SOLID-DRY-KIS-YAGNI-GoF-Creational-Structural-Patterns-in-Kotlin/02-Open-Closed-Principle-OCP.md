...
**Open/Closed Principle (OCP)** is extend expected variation without invasive edits.

---

# 2. Why

Interviewers use **Open/Closed Principle (OCP)** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Open/Closed Principle (OCP)** is about extend expected variation without invasive edits. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

use functions/interfaces/sealed variants where the variation is real
...
In a Kotlin/JVM production system, **Open/Closed Principle (OCP)** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Open/Closed Principle (OCP)** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Open/Closed Principle (OCP)**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
