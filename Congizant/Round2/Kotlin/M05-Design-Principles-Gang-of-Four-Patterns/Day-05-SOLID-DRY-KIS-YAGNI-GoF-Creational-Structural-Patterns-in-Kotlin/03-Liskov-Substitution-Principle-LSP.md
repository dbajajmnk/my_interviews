...
**Day:** 05 — SOLID, DRY, KIS, YAGNI & GoF Creational/Structural Patterns in Kotlin  
...
**Liskov Substitution Principle (LSP)** is subtypes preserve behavioral expectations.

---

# 2. Why

Interviewers use **Liskov Substitution Principle (LSP)** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Liskov Substitution Principle (LSP)** is about subtypes preserve behavioral expectations. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

final-by-default Kotlin discourages accidental inheritance, but semantic substitutability still matters
...
In a Kotlin/JVM production system, **Liskov Substitution Principle (LSP)** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Liskov Substitution Principle (LSP)**.
...
- Does **Liskov Substitution Principle (LSP)** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Liskov Substitution Principle (LSP)**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
