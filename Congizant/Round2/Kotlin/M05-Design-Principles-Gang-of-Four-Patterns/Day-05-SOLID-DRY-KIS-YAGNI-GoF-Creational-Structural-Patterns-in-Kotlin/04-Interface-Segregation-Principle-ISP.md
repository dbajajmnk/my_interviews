...
**Interface Segregation Principle (ISP)** is clients depend on narrow cohesive contracts.

---

# 2. Why

Interviewers use **Interface Segregation Principle (ISP)** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Interface Segregation Principle (ISP)** is about clients depend on narrow cohesive contracts. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

split broad service interfaces into capability-specific ports
...
In a Kotlin/JVM production system, **Interface Segregation Principle (ISP)** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Interface Segregation Principle (ISP)**.
...
- Does **Interface Segregation Principle (ISP)** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Interface Segregation Principle (ISP)**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
