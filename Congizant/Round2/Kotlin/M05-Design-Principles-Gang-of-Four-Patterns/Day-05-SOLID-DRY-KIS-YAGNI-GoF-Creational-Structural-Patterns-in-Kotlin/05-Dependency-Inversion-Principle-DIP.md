...
Interviewers use **Dependency Inversion Principle (DIP)** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Dependency Inversion Principle (DIP)** is about high-level policy depends on abstractions. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

inject repositories/clients instead of constructing them in domain/application logic
...
In a Kotlin/JVM production system, **Dependency Inversion Principle (DIP)** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Dependency Inversion Principle (DIP)** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Dependency Inversion Principle (DIP)**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
