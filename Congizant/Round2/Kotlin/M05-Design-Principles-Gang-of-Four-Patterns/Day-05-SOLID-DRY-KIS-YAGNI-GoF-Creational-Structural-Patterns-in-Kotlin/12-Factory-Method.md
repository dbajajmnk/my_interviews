...
**Factory Method** is delegates creation while preserving a stable product contract.

---

# 2. Why

Interviewers use **Factory Method** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Factory Method** is about delegates creation while preserving a stable product contract. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

use a function/object/class factory when creation varies by input/environment
...
```kotlin
interface Parser { fun parse(s:String): Any }
fun parser(kind:String): Parser = TODO()
```
...
In a Kotlin/JVM production system, **Factory Method** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Factory Method**.
...
- Does **Factory Method** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Factory Method**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
