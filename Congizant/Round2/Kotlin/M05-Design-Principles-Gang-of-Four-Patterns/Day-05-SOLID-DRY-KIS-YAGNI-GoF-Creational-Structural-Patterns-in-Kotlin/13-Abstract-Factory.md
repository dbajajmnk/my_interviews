...
**Abstract Factory** is creates compatible families of related collaborators.

---

# 2. Why

Interviewers use **Abstract Factory** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Abstract Factory** is about creates compatible families of related collaborators. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

use one family contract for provider/environment-specific implementations
...
```kotlin
interface CloudFactory { fun storage(): Storage; fun queue(): Queue }
```
...
In a Kotlin/JVM production system, **Abstract Factory** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Abstract Factory**.
...
- Does **Abstract Factory** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Abstract Factory**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
