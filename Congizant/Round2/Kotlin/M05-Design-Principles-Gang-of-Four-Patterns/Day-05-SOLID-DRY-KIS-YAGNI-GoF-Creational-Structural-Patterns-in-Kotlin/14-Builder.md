...
**Builder** is constructs complex values step by step.

---

# 2. Why

Interviewers use **Builder** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Builder** is about constructs complex values step by step. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

prefer named/default parameters first; use Builder when staged construction/interop needs it
...
```kotlin
val request = RequestBuilder().url(url).timeout(3000).build()
```
...
In a Kotlin/JVM production system, **Builder** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Builder**.
...
- Does **Builder** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Builder**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
