...
**Prototype** is creates variants from existing configured values.

---

# 2. Why

Interviewers use **Prototype** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Prototype** is about creates variants from existing configured values. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

data-class copy often provides a Kotlin-native alternative
...
```kotlin
val next = config.copy(timeoutMs = 5000)
```
...
In a Kotlin/JVM production system, **Prototype** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Prototype** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Prototype**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
