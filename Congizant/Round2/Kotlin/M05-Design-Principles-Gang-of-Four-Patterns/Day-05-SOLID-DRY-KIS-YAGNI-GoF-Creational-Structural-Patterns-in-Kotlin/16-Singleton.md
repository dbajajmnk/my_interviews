...
Interviewers use **Singleton** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Singleton** is about provides one shared logical instance. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

Kotlin `object` gives thread-safe JVM initialization semantics for the object declaration
...
```kotlin
object AppClock { fun now() = Instant.now() }
```
...
In a Kotlin/JVM production system, **Singleton** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Singleton** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Singleton**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
