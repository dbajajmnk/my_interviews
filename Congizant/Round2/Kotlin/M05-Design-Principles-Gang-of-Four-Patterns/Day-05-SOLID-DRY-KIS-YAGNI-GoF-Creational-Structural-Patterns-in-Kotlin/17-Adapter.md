...
**Adapter** is translates an incompatible dependency to your domain port.

---

# 2. Why

Interviewers use **Adapter** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Adapter** is about translates an incompatible dependency to your domain port. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

wrap vendor SDK types and map methods/results/errors at the boundary
...
```kotlin
class LegacyPaymentAdapter(private val client: LegacyClient): PaymentPort { override suspend fun charge(a:Amount)=TODO() }
```
...
In a Kotlin/JVM production system, **Adapter** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Adapter**.
...
- Does **Adapter** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Adapter**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
