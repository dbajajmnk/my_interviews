...
**Bridge** is separates abstraction from implementation dimension.

---

# 2. Why

Interviewers use **Bridge** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Bridge** is about separates abstraction from implementation dimension. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

inject an implementation interface instead of multiplying subclasses
...
```kotlin
abstract class Notification(protected val sender: Sender)
```
...
In a Kotlin/JVM production system, **Bridge** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Bridge**.
...
- Does **Bridge** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Bridge**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
