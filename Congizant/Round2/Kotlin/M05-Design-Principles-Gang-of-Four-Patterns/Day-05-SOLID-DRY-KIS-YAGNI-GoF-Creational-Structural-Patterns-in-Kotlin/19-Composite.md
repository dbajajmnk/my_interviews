...
**Composite** is treats leaves and groups uniformly.

---

# 2. Why

Interviewers use **Composite** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Composite** is about treats leaves and groups uniformly. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

a composite implements the same contract and delegates recursively
...
```kotlin
interface Rule { fun valid(c:Context): Boolean }
class All(private val rules:List<Rule>):Rule { override fun valid(c:Context)=rules.all{it.valid(c)} }
```
...
In a Kotlin/JVM production system, **Composite** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Composite**.
...
- Does **Composite** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Composite**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
