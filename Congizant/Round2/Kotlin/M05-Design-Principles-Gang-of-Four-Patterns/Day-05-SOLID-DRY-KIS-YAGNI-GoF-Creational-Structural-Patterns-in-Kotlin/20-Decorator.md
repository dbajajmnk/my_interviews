...
**Decorator** is adds behavior by wrapping the same contract.

---

# 2. Why

Interviewers use **Decorator** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Decorator** is about adds behavior by wrapping the same contract. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

delegation can make decorators concise
...
```kotlin
class MetricsStorage(private val delegate:Storage): Storage by delegate
```
...
In a Kotlin/JVM production system, **Decorator** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Decorator** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Decorator**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
