...
**Facade** is offers one cohesive entry point over a complex subsystem.

---

# 2. Why

Interviewers use **Facade** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Facade** is about offers one cohesive entry point over a complex subsystem. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

coordinate lower-level collaborators behind an application service
...
```kotlin
class CheckoutFacade(...) { suspend fun checkout(order:Order):Receipt = TODO() }
```
...
In a Kotlin/JVM production system, **Facade** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Facade**.
...
- Does **Facade** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Facade**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
