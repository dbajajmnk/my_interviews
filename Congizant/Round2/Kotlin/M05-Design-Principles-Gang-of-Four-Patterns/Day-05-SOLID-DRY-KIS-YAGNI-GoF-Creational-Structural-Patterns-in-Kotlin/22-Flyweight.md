...
**Flyweight** is shares reusable intrinsic immutable state.

---

# 2. Why

Interviewers use **Flyweight** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Flyweight** is about shares reusable intrinsic immutable state. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

cache canonical metadata only when measurements justify it
...
```kotlin
val productTypes = mutableMapOf<String, ProductType>()
```
...
In a Kotlin/JVM production system, **Flyweight** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Flyweight** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Flyweight**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
