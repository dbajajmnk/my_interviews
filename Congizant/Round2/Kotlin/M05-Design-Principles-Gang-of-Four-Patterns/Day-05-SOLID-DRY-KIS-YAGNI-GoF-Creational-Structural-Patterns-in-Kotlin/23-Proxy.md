...
**Proxy** is controls access while preserving the service contract.

---

# 2. Why

Interviewers use **Proxy** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Proxy** is about controls access while preserving the service contract. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

explicit wrapper/delegation is usually clearer than dynamic proxy unless framework needs one
...
```kotlin
class Secured(private val delegate:ReportService):ReportService by delegate
```
...
In a Kotlin/JVM production system, **Proxy** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Proxy**.
...
- Does **Proxy** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Proxy**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
