...
**Chain of Responsibility** is passes input through ordered handlers.

---

# 2. Why

Interviewers use **Chain of Responsibility** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Chain of Responsibility** is about passes input through ordered handlers. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

model handlers as functions or objects and keep termination/order explicit
...
```kotlin
typealias Handler = suspend (Request, suspend () -> Response) -> Response
```
...
In a Kotlin/JVM production system, **Chain of Responsibility** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Chain of Responsibility** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Chain of Responsibility**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
