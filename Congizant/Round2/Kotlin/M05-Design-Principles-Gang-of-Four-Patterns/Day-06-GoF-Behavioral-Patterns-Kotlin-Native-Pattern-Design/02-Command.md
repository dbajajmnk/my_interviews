...
**Command** is represents an action as a first-class object/value.

---

# 2. Why

Interviewers use **Command** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Command** is about represents an action as a first-class object/value. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

sealed command types enable exhaustive handling and queue/audit use
...
data class CreateOrder(val id:String):Command
```
...
In a Kotlin/JVM production system, **Command** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Command** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Command**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
