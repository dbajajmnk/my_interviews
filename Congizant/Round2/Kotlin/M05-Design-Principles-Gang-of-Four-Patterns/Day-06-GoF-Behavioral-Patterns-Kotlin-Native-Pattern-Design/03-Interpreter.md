...
**Interpreter** is models/evaluates a small grammar or expression language.

---

# 2. Why

Interviewers use **Interpreter** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Interpreter** is about models/evaluates a small grammar or expression language. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

sealed AST plus exhaustive `when` is Kotlin-native
...
data class Eq(val field:String,val value:String):Expr
```
...
In a Kotlin/JVM production system, **Interpreter** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Interpreter** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Interpreter**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
