...
Interviewers use **Iterator** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Iterator** is about provides sequential traversal without exposing representation. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

Kotlin Iterable/Iterator/Sequence already implement the core protocol
...
In a Kotlin/JVM production system, **Iterator** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Iterator** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Iterator**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
