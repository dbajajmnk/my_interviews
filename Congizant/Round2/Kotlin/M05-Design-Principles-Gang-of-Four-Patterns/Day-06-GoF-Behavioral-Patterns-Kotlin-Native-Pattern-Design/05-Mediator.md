...
**Mediator** is centralizes complex collaboration.

---

# 2. Why

Interviewers use **Mediator** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Mediator** is about centralizes complex collaboration. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

use an application coordinator when peer-to-peer dependencies explode
...
```kotlin
interface CheckoutMediator { suspend fun checkout(order:Order):Receipt }
```
...
In a Kotlin/JVM production system, **Mediator** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Mediator** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Mediator**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
