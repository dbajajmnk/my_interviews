...
Interviewers use **Memento** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Memento** is about captures restorable state snapshots. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

data classes are natural immutable snapshot carriers
...
In a Kotlin/JVM production system, **Memento** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Memento** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Memento**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
