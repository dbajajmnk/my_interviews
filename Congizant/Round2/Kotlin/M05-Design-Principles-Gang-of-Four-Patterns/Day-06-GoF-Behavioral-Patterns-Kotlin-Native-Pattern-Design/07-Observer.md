...
**Observer** is notifies subscribers about changes/events.

---

# 2. Why

Interviewers use **Observer** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Observer** is about notifies subscribers about changes/events. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

Flow/SharedFlow or explicit listener sets often implement this intent
...
```kotlin
val events = MutableSharedFlow<OrderEvent>()
```
...
In a Kotlin/JVM production system, **Observer** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Observer** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Observer**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
