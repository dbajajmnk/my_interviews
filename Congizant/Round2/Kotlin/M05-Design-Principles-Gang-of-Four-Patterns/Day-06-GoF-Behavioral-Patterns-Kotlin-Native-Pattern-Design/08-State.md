...
**State** is encapsulates behavior by explicit state.

---

# 2. Why

Interviewers use **State** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **State** is about encapsulates behavior by explicit state. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

sealed classes/interfaces plus exhaustive when make state models clear
...
```kotlin
sealed interface OrderState { data object Pending:OrderState; data class Paid(val receipt:String):OrderState }
```
...
In a Kotlin/JVM production system, **State** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **State** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **State**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
