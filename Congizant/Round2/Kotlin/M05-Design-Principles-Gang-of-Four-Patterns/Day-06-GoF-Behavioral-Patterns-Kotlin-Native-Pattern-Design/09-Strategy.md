...
Interviewers use **Strategy** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Strategy** is about encapsulates interchangeable algorithms. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
```kotlin
typealias PricingStrategy = (Order) -> Money
```
...
In a Kotlin/JVM production system, **Strategy** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Strategy** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Strategy**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
