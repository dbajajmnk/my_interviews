...
**Template Method** is fixes algorithm skeleton with customizable hooks.

---

# 2. Why

Interviewers use **Template Method** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Template Method** is about fixes algorithm skeleton with customizable hooks. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

abstract class works, but composition/higher-order functions may be simpler
...
```kotlin
abstract class ImportJob { suspend fun run(){ read(); validate(); persist() }; protected abstract suspend fun read() }
```
...
In a Kotlin/JVM production system, **Template Method** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Template Method** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Template Method**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
