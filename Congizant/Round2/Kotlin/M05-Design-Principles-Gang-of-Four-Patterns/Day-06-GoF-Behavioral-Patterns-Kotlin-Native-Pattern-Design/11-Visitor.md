...
**Visitor** is adds operations across a stable object structure.

---

# 2. Why

Interviewers use **Visitor** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Visitor** is about adds operations across a stable object structure. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

often compare classic Visitor with exhaustive sealed `when` in Kotlin
...
```kotlin
fun render(node:Node)=when(node){ is TextNode->node.text; is ImageNode->node.url }
```
...
In a Kotlin/JVM production system, **Visitor** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Visitor** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Visitor**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
