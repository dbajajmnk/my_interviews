...
**Function types as Strategy** is the Kotlin engineering concept **Function types as Strategy**.

---

# 2. Why

Interviewers use **Function types as Strategy** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Function types as Strategy** is about the Kotlin engineering concept **Function types as Strategy**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Function types as Strategy** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Function types as Strategy**.
...
- Does **Function types as Strategy** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Function types as Strategy**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
