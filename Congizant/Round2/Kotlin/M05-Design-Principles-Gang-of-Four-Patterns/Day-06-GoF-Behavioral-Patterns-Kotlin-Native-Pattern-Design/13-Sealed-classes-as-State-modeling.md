# Topic 13 — Sealed classes as State modeling
...
**Sealed classes as State modeling** is closed hierarchy known to compiler within allowed scope.

---

# 2. Why

Interviewers use **Sealed classes as State modeling** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Sealed classes as State modeling** is about closed hierarchy known to compiler within allowed scope. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Sealed classes as State modeling** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Sealed classes as State modeling**.
...
- Does **Sealed classes as State modeling** solve a real requirement?
...
1. What is **Sealed classes as State modeling**?
...
Create the smallest runnable Kotlin example demonstrating **Sealed classes as State modeling**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Sealed classes as State modeling** in plain English.
