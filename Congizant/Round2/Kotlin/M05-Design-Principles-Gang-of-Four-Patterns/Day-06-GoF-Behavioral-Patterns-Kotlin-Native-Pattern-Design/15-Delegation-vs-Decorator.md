# Topic 15 — Delegation vs Decorator
...
**Delegation vs Decorator** is the Kotlin engineering concept **Delegation vs Decorator**.

---

# 2. Why

Interviewers use **Delegation vs Decorator** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Delegation vs Decorator** is about the Kotlin engineering concept **Delegation vs Decorator**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Delegation vs Decorator** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Delegation vs Decorator**.
...
- Does **Delegation vs Decorator** solve a real requirement?
...
1. What is **Delegation vs Decorator**?
...
Create the smallest runnable Kotlin example demonstrating **Delegation vs Decorator**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Delegation vs Decorator** in plain English.
