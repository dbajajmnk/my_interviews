...
**associateBy** is the Kotlin engineering concept **associateBy**.

---

# 2. Why

Interviewers use **associateBy** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **associateBy** is about the Kotlin engineering concept **associateBy**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **associateBy** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **associateBy**.
...
- Does **associateBy** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **associateBy**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **associateBy** in plain English.
