...
**distinct** is the Kotlin engineering concept **distinct**.

---

# 2. Why

Interviewers use **distinct** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **distinct** is about the Kotlin engineering concept **distinct**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **distinct** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **distinct**.
...
- Does **distinct** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **distinct**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **distinct** in plain English.
