...
**Sequence** is lazy pull-based transformation pipeline.

---

# 2. Why

Interviewers use **Sequence** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Sequence** is about lazy pull-based transformation pipeline. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Sequence** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Sequence**.
...
- Does **Sequence** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Sequence**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Sequence** in plain English.
