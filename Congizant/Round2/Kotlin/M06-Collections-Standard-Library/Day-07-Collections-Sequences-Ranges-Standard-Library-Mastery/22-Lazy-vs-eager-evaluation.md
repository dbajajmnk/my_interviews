# Topic 22 — Lazy vs eager evaluation
...
**Lazy vs eager evaluation** is the Kotlin engineering concept **Lazy vs eager evaluation**.

---

# 2. Why

Interviewers use **Lazy vs eager evaluation** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Lazy vs eager evaluation** is about the Kotlin engineering concept **Lazy vs eager evaluation**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Lazy vs eager evaluation** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Lazy vs eager evaluation**.
...
- Does **Lazy vs eager evaluation** solve a real requirement?
...
1. What is **Lazy vs eager evaluation**?
...
Create the smallest runnable Kotlin example demonstrating **Lazy vs eager evaluation**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Lazy vs eager evaluation** in plain English.
