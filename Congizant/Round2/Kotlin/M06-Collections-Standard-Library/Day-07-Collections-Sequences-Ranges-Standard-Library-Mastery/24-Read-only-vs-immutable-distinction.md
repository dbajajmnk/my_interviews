# Topic 24 — Read-only vs immutable distinction
...
**Read-only vs immutable distinction** is the Kotlin engineering concept **Read-only vs immutable distinction**.

---

# 2. Why

Interviewers use **Read-only vs immutable distinction** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Read-only vs immutable distinction** is about the Kotlin engineering concept **Read-only vs immutable distinction**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Read-only vs immutable distinction** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Read-only vs immutable distinction**.
...
- Does **Read-only vs immutable distinction** solve a real requirement?
...
1. What is **Read-only vs immutable distinction**?
...
Create the smallest runnable Kotlin example demonstrating **Read-only vs immutable distinction**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Read-only vs immutable distinction** in plain English.
