...
**throw expression** is the Kotlin engineering concept **throw expression**.

---

# 2. Why

Interviewers use **throw expression** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **throw expression** is about the Kotlin engineering concept **throw expression**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **throw expression** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **throw expression**.
...
- Does **throw expression** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **throw expression**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **throw expression** in plain English.
