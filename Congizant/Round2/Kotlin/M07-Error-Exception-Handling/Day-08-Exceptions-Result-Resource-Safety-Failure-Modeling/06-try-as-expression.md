...
**try as expression** is the Kotlin engineering concept **try as expression**.

---

# 2. Why

Interviewers use **try as expression** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **try as expression** is about the Kotlin engineering concept **try as expression**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **try as expression** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **try as expression**.
...
- Does **try as expression** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **try as expression**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **try as expression** in plain English.
