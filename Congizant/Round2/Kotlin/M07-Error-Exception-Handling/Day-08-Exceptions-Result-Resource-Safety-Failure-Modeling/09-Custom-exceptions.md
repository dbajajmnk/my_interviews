...
**Custom exceptions** is the Kotlin engineering concept **Custom exceptions**.

---

# 2. Why

Interviewers use **Custom exceptions** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Custom exceptions** is about the Kotlin engineering concept **Custom exceptions**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Custom exceptions** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Custom exceptions** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Custom exceptions**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
