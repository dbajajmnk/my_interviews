...
**runCatching** is the Kotlin engineering concept **runCatching**.

---

# 2. Why

Interviewers use **runCatching** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **runCatching** is about the Kotlin engineering concept **runCatching**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **runCatching** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **runCatching**.
...
- Does **runCatching** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **runCatching**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **runCatching** in plain English.
