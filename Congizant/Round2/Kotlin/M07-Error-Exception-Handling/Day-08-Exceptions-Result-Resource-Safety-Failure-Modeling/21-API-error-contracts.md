...
**API error contracts** is the Kotlin engineering concept **API error contracts**.

---

# 2. Why

Interviewers use **API error contracts** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **API error contracts** is about the Kotlin engineering concept **API error contracts**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **API error contracts** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **API error contracts** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **API error contracts**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
