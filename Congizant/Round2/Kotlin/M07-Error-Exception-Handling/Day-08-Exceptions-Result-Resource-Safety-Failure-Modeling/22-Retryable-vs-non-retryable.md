# Topic 22 — Retryable vs non-retryable
...
**Retryable vs non-retryable** is the Kotlin engineering concept **Retryable vs non-retryable**.

---

# 2. Why

Interviewers use **Retryable vs non-retryable** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Retryable vs non-retryable** is about the Kotlin engineering concept **Retryable vs non-retryable**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Retryable vs non-retryable** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Retryable vs non-retryable**.
...
- Does **Retryable vs non-retryable** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Retryable vs non-retryable**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
