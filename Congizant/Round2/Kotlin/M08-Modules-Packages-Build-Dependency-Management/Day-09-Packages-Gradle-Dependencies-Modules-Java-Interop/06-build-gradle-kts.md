...
**build.gradle.kts** is build/dependency automation.

---

# 2. Why

Interviewers use **build.gradle.kts** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **build.gradle.kts** is about build/dependency automation. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **build.gradle.kts** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **build.gradle.kts**.
...
- Does **build.gradle.kts** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **build.gradle.kts**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **build.gradle.kts** in plain English.
