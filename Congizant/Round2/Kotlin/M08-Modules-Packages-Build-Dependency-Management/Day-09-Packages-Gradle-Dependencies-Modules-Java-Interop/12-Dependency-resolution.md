# Topic 12 — Dependency resolution
...
**Dependency resolution** is the Kotlin engineering concept **Dependency resolution**.

---

# 2. Why

Interviewers use **Dependency resolution** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Dependency resolution** is about the Kotlin engineering concept **Dependency resolution**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Dependency resolution** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Dependency resolution**.
...
- Does **Dependency resolution** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Dependency resolution**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Dependency resolution** in plain English.
