...
**Kotlin JVM target** is the Kotlin engineering concept **Kotlin JVM target**.

---

# 2. Why

Interviewers use **Kotlin JVM target** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Kotlin JVM target** is about the Kotlin engineering concept **Kotlin JVM target**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Kotlin JVM target** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Kotlin JVM target**.
...
- Does **Kotlin JVM target** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Kotlin JVM target**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Kotlin JVM target** in plain English.
