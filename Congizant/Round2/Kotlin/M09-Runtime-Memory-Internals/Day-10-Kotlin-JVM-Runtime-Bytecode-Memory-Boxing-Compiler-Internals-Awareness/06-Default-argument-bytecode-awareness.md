# Topic 06 — Default argument bytecode awareness
...
**Default argument bytecode awareness** is JVM instruction output produced by Kotlin compiler.

---

# 2. Why

Interviewers use **Default argument bytecode awareness** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Default argument bytecode awareness** is about JVM instruction output produced by Kotlin compiler. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

Kotlin/JVM source is compiled to class files with generated bridges/helpers depending on language feature
...
In a Kotlin/JVM production system, **Default argument bytecode awareness** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Default argument bytecode awareness**.
...
- Does **Default argument bytecode awareness** solve a real requirement?
...
1. What is **Default argument bytecode awareness**?
...
Create the smallest runnable Kotlin example demonstrating **Default argument bytecode awareness**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Default argument bytecode awareness** in plain English.
