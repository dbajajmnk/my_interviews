...
**Garbage collection** is the Kotlin engineering concept **Garbage collection**.

---

# 2. Why

Interviewers use **Garbage collection** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Garbage collection** is about the Kotlin engineering concept **Garbage collection**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Garbage collection** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Garbage collection**.
...
- Does **Garbage collection** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Garbage collection**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
