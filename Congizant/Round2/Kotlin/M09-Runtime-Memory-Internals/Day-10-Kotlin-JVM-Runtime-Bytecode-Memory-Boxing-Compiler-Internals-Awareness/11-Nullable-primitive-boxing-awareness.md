# Topic 11 — Nullable primitive boxing awareness
...
**Nullable primitive boxing awareness** is representing possible absence explicitly in the type system.

---

# 2. Why

Interviewers use **Nullable primitive boxing awareness** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Nullable primitive boxing awareness** is about representing possible absence explicitly in the type system. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

`T?` permits null while `T` does not; Java/platform boundaries can weaken this guarantee
...
In a Kotlin/JVM production system, **Nullable primitive boxing awareness** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Nullable primitive boxing awareness**.
...
- Does **Nullable primitive boxing awareness** solve a real requirement?
...
1. What is **Nullable primitive boxing awareness**?
...
Create the smallest runnable Kotlin example demonstrating **Nullable primitive boxing awareness**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Nullable primitive boxing awareness** in plain English.
