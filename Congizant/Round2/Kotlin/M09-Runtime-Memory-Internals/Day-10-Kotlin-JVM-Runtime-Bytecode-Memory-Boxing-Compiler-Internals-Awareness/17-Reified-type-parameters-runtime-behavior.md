# Topic 17 — Reified type parameters runtime behavior
...
**Reified type parameters runtime behavior** is retaining an inline type argument enough for certain runtime class checks/access.

---

# 2. Why

Interviewers use **Reified type parameters runtime behavior** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Reified type parameters runtime behavior** is about retaining an inline type argument enough for certain runtime class checks/access. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

works only on inline type parameters because compiler substitutes type at call site
...
```kotlin
inline fun <reified T> isType(x:Any)=x is T
```
...
In a Kotlin/JVM production system, **Reified type parameters runtime behavior** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Reified type parameters runtime behavior**.
...
- Does **Reified type parameters runtime behavior** solve a real requirement?
...
1. What is **Reified type parameters runtime behavior**?
...
Create the smallest runnable Kotlin example demonstrating **Reified type parameters runtime behavior**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Reified type parameters runtime behavior** in plain English.
