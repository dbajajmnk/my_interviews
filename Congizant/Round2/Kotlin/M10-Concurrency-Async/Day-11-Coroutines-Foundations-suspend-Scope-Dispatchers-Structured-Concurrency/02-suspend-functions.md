...
**suspend functions** is compiler-supported function that may suspend without blocking a thread.

---

# 2. Why

Interviewers use **suspend functions** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **suspend functions** is about compiler-supported function that may suspend without blocking a thread. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

compiler transforms suspend function into continuation/state-machine style JVM code
...
```kotlin
suspend fun load(): User = repo.load()
```
...
In a Kotlin/JVM production system, **suspend functions** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **suspend functions**.
...
- Does **suspend functions** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **suspend functions**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **suspend functions** in plain English.
