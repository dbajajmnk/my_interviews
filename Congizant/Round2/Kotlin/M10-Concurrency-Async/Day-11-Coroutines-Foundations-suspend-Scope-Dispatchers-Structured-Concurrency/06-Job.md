...
**Job** is the Kotlin engineering concept **Job**.

---

# 2. Why

Interviewers use **Job** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Job** is about the Kotlin engineering concept **Job**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Job** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Job**.
...
- Does **Job** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Job**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Job** in plain English.
