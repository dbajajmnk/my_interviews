# Topic 09 — runBlocking awareness
...
**runBlocking awareness** is the Kotlin engineering concept **runBlocking awareness**.

---

# 2. Why

Interviewers use **runBlocking awareness** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **runBlocking awareness** is about the Kotlin engineering concept **runBlocking awareness**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **runBlocking awareness** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **runBlocking awareness**.
...
- Does **runBlocking awareness** solve a real requirement?
...
1. What is **runBlocking awareness**?
...
Create the smallest runnable Kotlin example demonstrating **runBlocking awareness**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **runBlocking awareness** in plain English.
