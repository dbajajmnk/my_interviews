...
**Dispatchers.Default** is coroutine context element selecting execution resources.

---

# 2. Why

Interviewers use **Dispatchers.Default** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Dispatchers.Default** is about coroutine context element selecting execution resources. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Dispatchers.Default** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Dispatchers.Default**.
...
- Does **Dispatchers.Default** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Dispatchers.Default**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Dispatchers.Default** in plain English.
