...
**Dispatchers.IO** is coroutine context element selecting execution resources.

---

# 2. Why

Interviewers use **Dispatchers.IO** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Dispatchers.IO** is about coroutine context element selecting execution resources. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Dispatchers.IO** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Dispatchers.IO**.
...
- Does **Dispatchers.IO** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Dispatchers.IO**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Dispatchers.IO** in plain English.
