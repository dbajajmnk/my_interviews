# Topic 13 — Main dispatcher awareness
...
**Main dispatcher awareness** is coroutine context element selecting execution resources.

---

# 2. Why

Interviewers use **Main dispatcher awareness** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Main dispatcher awareness** is about coroutine context element selecting execution resources. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Main dispatcher awareness** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Main dispatcher awareness**.
...
- Does **Main dispatcher awareness** solve a real requirement?
...
1. What is **Main dispatcher awareness**?
...
Create the smallest runnable Kotlin example demonstrating **Main dispatcher awareness**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Main dispatcher awareness** in plain English.
