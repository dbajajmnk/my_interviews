# Topic 15 — Parent-child hierarchy
...
**Parent-child hierarchy** is the Kotlin engineering concept **Parent-child hierarchy**.

---

# 2. Why

Interviewers use **Parent-child hierarchy** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Parent-child hierarchy** is about the Kotlin engineering concept **Parent-child hierarchy**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Parent-child hierarchy** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Parent-child hierarchy**.
...
- Does **Parent-child hierarchy** solve a real requirement?
...
1. What is **Parent-child hierarchy**?
...
Create the smallest runnable Kotlin example demonstrating **Parent-child hierarchy**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Parent-child hierarchy** in plain English.
