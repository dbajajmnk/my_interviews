...
**Cancellation** is the Kotlin engineering concept **Cancellation**.

---

# 2. Why

Interviewers use **Cancellation** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Cancellation** is about the Kotlin engineering concept **Cancellation**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Cancellation** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Cancellation**.
...
- Does **Cancellation** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Cancellation**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
