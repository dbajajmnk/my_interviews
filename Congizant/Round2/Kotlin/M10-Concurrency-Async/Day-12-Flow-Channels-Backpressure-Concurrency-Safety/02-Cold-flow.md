...
**Cold flow** is cold asynchronous stream abstraction.

---

# 2. Why

Interviewers use **Cold flow** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Cold flow** is about cold asynchronous stream abstraction. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Cold flow** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Cold flow**.
...
- Does **Cold flow** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Cold flow**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Cold flow** in plain English.
