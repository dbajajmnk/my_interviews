...
**map** is the Kotlin engineering concept **map**.

---

# 2. Why

Interviewers use **map** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **map** is about the Kotlin engineering concept **map**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **map** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **map**.
...
- Does **map** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **map**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
