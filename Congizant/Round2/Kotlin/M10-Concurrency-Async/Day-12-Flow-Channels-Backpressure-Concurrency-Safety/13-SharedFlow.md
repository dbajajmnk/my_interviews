...
**SharedFlow** is cold asynchronous stream abstraction.

---

# 2. Why

Interviewers use **SharedFlow** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **SharedFlow** is about cold asynchronous stream abstraction. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **SharedFlow** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **SharedFlow**.
...
- Does **SharedFlow** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **SharedFlow**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **SharedFlow** in plain English.
