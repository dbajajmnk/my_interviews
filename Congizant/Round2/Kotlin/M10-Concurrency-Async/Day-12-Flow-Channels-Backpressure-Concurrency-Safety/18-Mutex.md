...
**Mutex** is coroutine-friendly mutual exclusion.

---

# 2. Why

Interviewers use **Mutex** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Mutex** is about coroutine-friendly mutual exclusion. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

Mutex suspends waiters rather than blocking OS threads; still protect only the minimal critical section
...
In a Kotlin/JVM production system, **Mutex** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Mutex**.
...
- Does **Mutex** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Mutex**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Mutex** in plain English.
