...
**Semaphore** is bounded concurrency primitive.

---

# 2. Why

Interviewers use **Semaphore** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Semaphore** is about bounded concurrency primitive. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

permits limit concurrent access to a scarce downstream resource
...
In a Kotlin/JVM production system, **Semaphore** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Semaphore**.
...
- Does **Semaphore** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Semaphore**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Semaphore** in plain English.
