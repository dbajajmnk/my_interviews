...
**Thread confinement** is the Kotlin engineering concept **Thread confinement**.

---

# 2. Why

Interviewers use **Thread confinement** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Thread confinement** is about the Kotlin engineering concept **Thread confinement**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Thread confinement** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Thread confinement**.
...
- Does **Thread confinement** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Thread confinement**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Thread confinement** in plain English.
