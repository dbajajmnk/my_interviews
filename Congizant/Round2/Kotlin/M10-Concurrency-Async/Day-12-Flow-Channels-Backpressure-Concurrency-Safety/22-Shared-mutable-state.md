...
**Shared mutable state** is the Kotlin engineering concept **Shared mutable state**.

---

# 2. Why

Interviewers use **Shared mutable state** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Shared mutable state** is about the Kotlin engineering concept **Shared mutable state**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Shared mutable state** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Shared mutable state**.
...
- Does **Shared mutable state** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Shared mutable state**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Shared mutable state** in plain English.
