# Topic 25 — Coroutine dispatcher choice
...
**Coroutine dispatcher choice** is coroutine context element selecting execution resources.

---

# 2. Why

Interviewers use **Coroutine dispatcher choice** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Coroutine dispatcher choice** is about coroutine context element selecting execution resources. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

Default is CPU-oriented shared pool; IO expands for blocking I/O; framework context may supply dispatcher
...
In a Kotlin/JVM production system, **Coroutine dispatcher choice** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Coroutine dispatcher choice**.
...
- Does **Coroutine dispatcher choice** solve a real requirement?
...
1. What is **Coroutine dispatcher choice**?
...
Create the smallest runnable Kotlin example demonstrating **Coroutine dispatcher choice**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Coroutine dispatcher choice** in plain English.
