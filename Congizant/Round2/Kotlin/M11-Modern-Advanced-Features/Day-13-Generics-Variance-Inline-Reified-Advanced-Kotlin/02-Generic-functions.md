...
**Generic functions** is the Kotlin engineering concept **Generic functions**.

---

# 2. Why

Interviewers use **Generic functions** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Generic functions** is about the Kotlin engineering concept **Generic functions**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Generic functions** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Generic functions** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Generic functions**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
