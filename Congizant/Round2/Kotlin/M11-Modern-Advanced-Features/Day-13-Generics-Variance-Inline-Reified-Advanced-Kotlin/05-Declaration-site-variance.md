# Topic 05 — Declaration-site variance
...
**Declaration-site variance** is controlling subtype relationships for generic type constructors.

---

# 2. Why

Interviewers use **Declaration-site variance** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Declaration-site variance** is about controlling subtype relationships for generic type constructors. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

`out` makes producer covariant; `in` makes consumer contravariant
...
In a Kotlin/JVM production system, **Declaration-site variance** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Declaration-site variance**.
...
- Does **Declaration-site variance** solve a real requirement?
...
1. What is **Declaration-site variance**?
...
Create the smallest runnable Kotlin example demonstrating **Declaration-site variance**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Declaration-site variance** in plain English.
