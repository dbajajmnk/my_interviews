...
**Type erasure on JVM** is the Kotlin engineering concept **Type erasure on JVM**.

---

# 2. Why

Interviewers use **Type erasure on JVM** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Type erasure on JVM** is about the Kotlin engineering concept **Type erasure on JVM**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Type erasure on JVM** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Type erasure on JVM**.
...
- Does **Type erasure on JVM** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Type erasure on JVM**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Type erasure on JVM** in plain English.
