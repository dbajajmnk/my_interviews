...
**Inline functions** is request to inline function body at call sites subject to compiler constraints.

---

# 2. Why

Interviewers use **Inline functions** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Inline functions** is about request to inline function body at call sites subject to compiler constraints. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Inline functions** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Inline functions**.
...
- Does **Inline functions** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Inline functions**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Inline functions** in plain English.
