...
**noinline** is request to inline function body at call sites subject to compiler constraints.

---

# 2. Why

Interviewers use **noinline** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **noinline** is about request to inline function body at call sites subject to compiler constraints. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **noinline** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **noinline**.
...
- Does **noinline** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **noinline**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **noinline** in plain English.
