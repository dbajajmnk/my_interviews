...
**crossinline** is request to inline function body at call sites subject to compiler constraints.

---

# 2. Why

Interviewers use **crossinline** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **crossinline** is about request to inline function body at call sites subject to compiler constraints. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **crossinline** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **crossinline**.
...
- Does **crossinline** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **crossinline**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **crossinline** in plain English.
