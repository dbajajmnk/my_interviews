# Topic 14 — Reified type parameters
...
**Reified type parameters** is retaining an inline type argument enough for certain runtime class checks/access.

---

# 2. Why

Interviewers use **Reified type parameters** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Reified type parameters** is about retaining an inline type argument enough for certain runtime class checks/access. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Reified type parameters** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Reified type parameters**.
...
- Does **Reified type parameters** solve a real requirement?
...
1. What is **Reified type parameters**?
...
Create the smallest runnable Kotlin example demonstrating **Reified type parameters**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Reified type parameters** in plain English.
