...
**lazy** is the Kotlin engineering concept **lazy**.

---

# 2. Why

Interviewers use **lazy** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **lazy** is about the Kotlin engineering concept **lazy**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **lazy** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **lazy**.
...
- Does **lazy** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **lazy**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **lazy** in plain English.
