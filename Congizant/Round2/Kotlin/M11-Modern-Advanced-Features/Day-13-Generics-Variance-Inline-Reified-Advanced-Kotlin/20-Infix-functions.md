...
**Infix functions** is the Kotlin engineering concept **Infix functions**.

---

# 2. Why

Interviewers use **Infix functions** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Infix functions** is about the Kotlin engineering concept **Infix functions**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Infix functions** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Infix functions**.
...
- Does **Infix functions** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Infix functions**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Infix functions** in plain English.
