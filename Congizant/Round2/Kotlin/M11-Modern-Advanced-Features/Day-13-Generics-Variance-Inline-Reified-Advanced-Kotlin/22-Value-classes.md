...
**Value classes** is the Kotlin engineering concept **Value classes**.

---

# 2. Why

Interviewers use **Value classes** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Value classes** is about the Kotlin engineering concept **Value classes**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Value classes** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Value classes**.
...
- Does **Value classes** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Value classes**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Value classes** in plain English.
