# Topic 23 — Sealed exhaustive when
...
**Sealed exhaustive when** is closed hierarchy known to compiler within allowed scope.

---

# 2. Why

Interviewers use **Sealed exhaustive when** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Sealed exhaustive when** is about closed hierarchy known to compiler within allowed scope. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Sealed exhaustive when** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Sealed exhaustive when**.
...
- Does **Sealed exhaustive when** solve a real requirement?
...
1. What is **Sealed exhaustive when**?
...
Create the smallest runnable Kotlin example demonstrating **Sealed exhaustive when**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Sealed exhaustive when** in plain English.
