# Topic 03 — JSON encoding/decoding
...
**JSON encoding/decoding** is the Kotlin engineering concept **JSON encoding/decoding**.

---

# 2. Why

Interviewers use **JSON encoding/decoding** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **JSON encoding/decoding** is about the Kotlin engineering concept **JSON encoding/decoding**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **JSON encoding/decoding** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **JSON encoding/decoding**.
...
- Does **JSON encoding/decoding** solve a real requirement?
...
1. What is **JSON encoding/decoding**?
...
Create the smallest runnable Kotlin example demonstrating **JSON encoding/decoding**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **JSON encoding/decoding** in plain English.
