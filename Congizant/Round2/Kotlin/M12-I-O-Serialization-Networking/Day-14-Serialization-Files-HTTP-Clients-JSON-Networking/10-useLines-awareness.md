...
**useLines awareness** is the Kotlin engineering concept **useLines awareness**.

---

# 2. Why

Interviewers use **useLines awareness** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **useLines awareness** is about the Kotlin engineering concept **useLines awareness**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **useLines awareness** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **useLines awareness**.
...
- Does **useLines awareness** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **useLines awareness**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **useLines awareness** in plain English.
