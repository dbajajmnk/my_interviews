# Topic 18 — Cancellation with HTTP calls
...
**Cancellation with HTTP calls** is remote request/response boundary.

---

# 2. Why

Interviewers use **Cancellation with HTTP calls** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Cancellation with HTTP calls** is about remote request/response boundary. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Cancellation with HTTP calls** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Cancellation with HTTP calls**.
...
- Does **Cancellation with HTTP calls** solve a real requirement?
...
1. What is **Cancellation with HTTP calls**?
...
Create the smallest runnable Kotlin example demonstrating **Cancellation with HTTP calls**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Cancellation with HTTP calls** in plain English.
