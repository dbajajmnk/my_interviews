# Topic 03 — IntArray vs Array<Int> awareness
...
**IntArray vs Array<Int> awareness** is primitive-specialized array representation.

---

# 2. Why

Interviewers use **IntArray vs Array<Int> awareness** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **IntArray vs Array<Int> awareness** is about primitive-specialized array representation. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

`IntArray` avoids boxing that can occur with generic `Array<Int>` in JVM representation
...
In a Kotlin/JVM production system, **IntArray vs Array<Int> awareness** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **IntArray vs Array<Int> awareness**.
...
- Does **IntArray vs Array<Int> awareness** solve a real requirement?
...
1. What is **IntArray vs Array<Int> awareness**?
...
Create the smallest runnable Kotlin example demonstrating **IntArray vs Array<Int> awareness**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **IntArray vs Array<Int> awareness** in plain English.
