...
**HashMap** is hash-based key/value collection.

---

# 2. Why

Interviewers use **HashMap** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **HashMap** is about hash-based key/value collection. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

hashCode/equals contract drives key lookup; mutable keys are dangerous
...
In a Kotlin/JVM production system, **HashMap** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **HashMap**.
...
- Does **HashMap** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **HashMap**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **HashMap** in plain English.
