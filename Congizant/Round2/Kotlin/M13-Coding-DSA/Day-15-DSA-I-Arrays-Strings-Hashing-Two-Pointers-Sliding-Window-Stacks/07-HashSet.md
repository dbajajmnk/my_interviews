...
**HashSet** is hash-based uniqueness collection.

---

# 2. Why

Interviewers use **HashSet** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **HashSet** is about hash-based uniqueness collection. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

membership uses hashCode/equals; complexity average O(1) under usual assumptions
...
In a Kotlin/JVM production system, **HashSet** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **HashSet**.
...
- Does **HashSet** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **HashSet**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
