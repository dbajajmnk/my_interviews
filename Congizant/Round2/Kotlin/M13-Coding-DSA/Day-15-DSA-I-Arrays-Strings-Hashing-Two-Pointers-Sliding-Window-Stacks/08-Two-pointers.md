...
Interviewers use **Two pointers** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Two pointers** is about two-index algorithm pattern. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

monotonic pointer movement can reduce repeated scanning to O(n)
...
In a Kotlin/JVM production system, **Two pointers** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Two pointers** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Two pointers**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
