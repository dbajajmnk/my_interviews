...
Interviewers use **Sliding window** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Sliding window** is about incremental contiguous-range algorithm. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

maintain counts/sum while moving left/right boundaries
...
```kotlin
var left=0; for(right in s.indices){ }
```
...
In a Kotlin/JVM production system, **Sliding window** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Sliding window**.
...
- Does **Sliding window** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Sliding window**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
