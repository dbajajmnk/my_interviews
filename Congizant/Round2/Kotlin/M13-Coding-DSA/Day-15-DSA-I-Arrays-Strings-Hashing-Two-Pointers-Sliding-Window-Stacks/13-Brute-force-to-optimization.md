# Topic 13 — Brute force to optimization
...
**Brute force to optimization** is the Kotlin engineering concept **Brute force to optimization**.

---

# 2. Why

Interviewers use **Brute force to optimization** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Brute force to optimization** is about the Kotlin engineering concept **Brute force to optimization**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Brute force to optimization** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Brute force to optimization** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Brute force to optimization**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
