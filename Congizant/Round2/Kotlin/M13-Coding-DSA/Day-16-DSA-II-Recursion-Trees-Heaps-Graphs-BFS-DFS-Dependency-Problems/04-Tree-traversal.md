...
**Tree traversal** is the Kotlin engineering concept **Tree traversal**.

---

# 2. Why

Interviewers use **Tree traversal** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Tree traversal** is about the Kotlin engineering concept **Tree traversal**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Tree traversal** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Tree traversal** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Tree traversal**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
