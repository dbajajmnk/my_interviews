...
Interviewers use **BFS** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **BFS** is about breadth-first traversal. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

ArrayDeque can implement queue and visited set prevents reprocessing
...
In a Kotlin/JVM production system, **BFS** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **BFS** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **BFS**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
