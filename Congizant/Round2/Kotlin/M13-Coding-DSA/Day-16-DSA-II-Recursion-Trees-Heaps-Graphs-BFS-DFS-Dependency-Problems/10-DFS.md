...
Interviewers use **DFS** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **DFS** is about depth-first traversal. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

recursion or explicit stack explores branch then backtracks
...
```kotlin
fun dfs(n:Node){ if(!seen.add(n)) return }
```
...
In a Kotlin/JVM production system, **DFS** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **DFS**.
...
- Does **DFS** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **DFS**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
