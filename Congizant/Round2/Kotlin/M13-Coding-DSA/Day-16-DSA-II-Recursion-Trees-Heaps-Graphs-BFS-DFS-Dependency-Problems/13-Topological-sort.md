...
**Topological sort** is DAG dependency ordering.

---

# 2. Why

Interviewers use **Topological sort** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Topological sort** is about DAG dependency ordering. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

in-degree or DFS detects cycles and emits order in O(V+E)
...
In a Kotlin/JVM production system, **Topological sort** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Topological sort**.
...
- Does **Topological sort** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Topological sort**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
