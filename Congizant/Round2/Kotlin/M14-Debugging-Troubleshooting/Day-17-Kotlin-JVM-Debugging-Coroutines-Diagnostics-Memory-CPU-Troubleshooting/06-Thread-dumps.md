...
**Thread dumps** is snapshot of JVM thread states/stacks.

---

# 2. Why

Interviewers use **Thread dumps** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Thread dumps** is about snapshot of JVM thread states/stacks. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

useful for deadlocks, blocking and CPU stack correlation
...
In a Kotlin/JVM production system, **Thread dumps** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Thread dumps**.
...
- Does **Thread dumps** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Thread dumps**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
