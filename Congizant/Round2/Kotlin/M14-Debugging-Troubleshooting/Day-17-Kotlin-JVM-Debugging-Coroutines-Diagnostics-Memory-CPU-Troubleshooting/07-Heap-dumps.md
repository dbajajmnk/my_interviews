...
**Heap dumps** is snapshot of heap reachability.

---

# 2. Why

Interviewers use **Heap dumps** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Heap dumps** is about snapshot of heap reachability. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

retained-size/dominator analysis identifies why objects remain reachable
...
In a Kotlin/JVM production system, **Heap dumps** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Heap dumps**.
...
- Does **Heap dumps** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Heap dumps**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
