...
**JFR awareness** is JVM event recorder.

---

# 2. Why

Interviewers use **JFR awareness** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **JFR awareness** is about JVM event recorder. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

captures CPU, allocation, GC, locks and other runtime events with low overhead
...
In a Kotlin/JVM production system, **JFR awareness** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **JFR awareness**.
...
- Does **JFR awareness** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **JFR awareness**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **JFR awareness** in plain English.
