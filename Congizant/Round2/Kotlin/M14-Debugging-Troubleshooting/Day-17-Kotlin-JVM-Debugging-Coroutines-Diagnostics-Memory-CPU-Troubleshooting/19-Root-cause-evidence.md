...
**Root-cause evidence** is the Kotlin engineering concept **Root-cause evidence**.

---

# 2. Why

Interviewers use **Root-cause evidence** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Root-cause evidence** is about the Kotlin engineering concept **Root-cause evidence**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Root-cause evidence** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Root-cause evidence** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Root-cause evidence**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
