...
**Risk-first code review** is risk-based examination of change intent and behavior.

---

# 2. Why

Interviewers use **Risk-first code review** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Risk-first code review** is about risk-based examination of change intent and behavior. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

review correctness/security/concurrency/failure before style
...
```kotlin
// correctness -> security -> concurrency -> failure -> performance -> tests
```
...
In a Kotlin/JVM production system, **Risk-first code review** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Risk-first code review** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Risk-first code review**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
