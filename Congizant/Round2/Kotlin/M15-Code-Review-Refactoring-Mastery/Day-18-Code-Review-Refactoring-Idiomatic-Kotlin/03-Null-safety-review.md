...
**Null-safety review** is the Kotlin engineering concept **Null-safety review**.

---

# 2. Why

Interviewers use **Null-safety review** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Null-safety review** is about the Kotlin engineering concept **Null-safety review**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Null-safety review** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Null-safety review**.
...
- Does **Null-safety review** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Null-safety review**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Null-safety review** in plain English.
