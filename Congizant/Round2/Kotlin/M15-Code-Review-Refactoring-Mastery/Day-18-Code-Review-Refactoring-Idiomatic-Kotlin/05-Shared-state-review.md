...
**Shared-state review** is the Kotlin engineering concept **Shared-state review**.

---

# 2. Why

Interviewers use **Shared-state review** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Shared-state review** is about the Kotlin engineering concept **Shared-state review**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Shared-state review** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Shared-state review**.
...
- Does **Shared-state review** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Shared-state review**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Shared-state review** in plain English.
