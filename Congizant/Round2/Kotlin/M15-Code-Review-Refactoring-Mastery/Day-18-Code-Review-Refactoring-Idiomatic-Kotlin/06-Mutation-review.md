...
**Mutation review** is the Kotlin engineering concept **Mutation review**.

---

# 2. Why

Interviewers use **Mutation review** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Mutation review** is about the Kotlin engineering concept **Mutation review**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Mutation review** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Mutation review**.
...
- Does **Mutation review** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Mutation review**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Mutation review** in plain English.
