...
**API contracts** is the Kotlin engineering concept **API contracts**.

---

# 2. Why

Interviewers use **API contracts** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **API contracts** is about the Kotlin engineering concept **API contracts**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **API contracts** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **API contracts** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **API contracts**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
