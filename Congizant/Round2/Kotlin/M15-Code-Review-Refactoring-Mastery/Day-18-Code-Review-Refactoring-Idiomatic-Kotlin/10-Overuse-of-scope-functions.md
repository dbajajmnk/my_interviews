# Topic 10 — Overuse of scope functions
...
**Overuse of scope functions** is standard higher-order helpers differing by receiver (`this`/`it`) and return value.

---

# 2. Why

Interviewers use **Overuse of scope functions** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Overuse of scope functions** is about standard higher-order helpers differing by receiver (`this`/`it`) and return value. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

let/run/with/apply/also are library functions; choose by intent/clarity
...
In a Kotlin/JVM production system, **Overuse of scope functions** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Overuse of scope functions**.
...
- Does **Overuse of scope functions** solve a real requirement?
...
1. What is **Overuse of scope functions**?
...
Create the smallest runnable Kotlin example demonstrating **Overuse of scope functions**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Overuse of scope functions** in plain English.
