...
**Extension abuse** is the Kotlin engineering concept **Extension abuse**.

---

# 2. Why

Interviewers use **Extension abuse** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Extension abuse** is about the Kotlin engineering concept **Extension abuse**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Extension abuse** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Extension abuse**.
...
- Does **Extension abuse** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Extension abuse**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Extension abuse** in plain English.
