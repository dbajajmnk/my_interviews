...
**runCatching abuse** is the Kotlin engineering concept **runCatching abuse**.

---

# 2. Why

Interviewers use **runCatching abuse** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **runCatching abuse** is about the Kotlin engineering concept **runCatching abuse**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **runCatching abuse** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **runCatching abuse**.
...
- Does **runCatching abuse** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **runCatching abuse**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **runCatching abuse** in plain English.
