...
**Dependency direction** is the Kotlin engineering concept **Dependency direction**.

---

# 2. Why

Interviewers use **Dependency direction** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Dependency direction** is about the Kotlin engineering concept **Dependency direction**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Dependency direction** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Dependency direction**.
...
- Does **Dependency direction** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Dependency direction**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
