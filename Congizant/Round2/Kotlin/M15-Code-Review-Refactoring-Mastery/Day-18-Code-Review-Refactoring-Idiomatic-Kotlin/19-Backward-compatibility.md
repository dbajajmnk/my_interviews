# Topic 19 — Backward compatibility
...
**Backward compatibility** is the Kotlin engineering concept **Backward compatibility**.

---

# 2. Why

Interviewers use **Backward compatibility** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Backward compatibility** is about the Kotlin engineering concept **Backward compatibility**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Backward compatibility** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Backward compatibility** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Backward compatibility**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
