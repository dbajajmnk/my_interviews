...
**GlobalScope misuse** is the Kotlin engineering concept **GlobalScope misuse**.

---

# 2. Why

Interviewers use **GlobalScope misuse** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **GlobalScope misuse** is about the Kotlin engineering concept **GlobalScope misuse**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **GlobalScope misuse** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **GlobalScope misuse**.
...
- Does **GlobalScope misuse** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **GlobalScope misuse**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **GlobalScope misuse** in plain English.
