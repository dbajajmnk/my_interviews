...
**Unsafe !!** is the Kotlin engineering concept **Unsafe !!**.

---

# 2. Why

Interviewers use **Unsafe !!** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Unsafe !!** is about the Kotlin engineering concept **Unsafe !!**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Unsafe !!** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Unsafe !!**.
...
- Does **Unsafe !!** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Unsafe !!**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Unsafe !!** in plain English.
