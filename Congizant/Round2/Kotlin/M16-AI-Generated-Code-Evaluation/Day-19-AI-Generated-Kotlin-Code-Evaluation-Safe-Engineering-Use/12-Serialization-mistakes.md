# Topic 12 — Serialization mistakes
...
**Serialization mistakes** is encoding/decoding typed data to external representation.

---

# 2. Why

Interviewers use **Serialization mistakes** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Serialization mistakes** is about encoding/decoding typed data to external representation. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Serialization mistakes** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Serialization mistakes**.
...
- Does **Serialization mistakes** solve a real requirement?
...
1. What is **Serialization mistakes**?
...
Create the smallest runnable Kotlin example demonstrating **Serialization mistakes**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Serialization mistakes** in plain English.
