# Topic 18 — Accept repair reject rubric
...
**Accept repair reject rubric** is the Kotlin engineering concept **Accept repair reject rubric**.

---

# 2. Why

Interviewers use **Accept repair reject rubric** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Accept repair reject rubric** is about the Kotlin engineering concept **Accept repair reject rubric**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Accept repair reject rubric** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Accept repair reject rubric** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Accept repair reject rubric**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
