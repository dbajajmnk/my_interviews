...
**JUnit 5** is JVM unit testing framework.

---

# 2. Why

Interviewers use **JUnit 5** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **JUnit 5** is about JVM unit testing framework. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

JUnit discovers and runs test methods/lifecycle; Kotlin assertions/lambdas integrate naturally
...
```kotlin
@Test fun works(){ assertEquals(2,1+1) }
```
...
In a Kotlin/JVM production system, **JUnit 5** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **JUnit 5**.
...
- Does **JUnit 5** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **JUnit 5**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
