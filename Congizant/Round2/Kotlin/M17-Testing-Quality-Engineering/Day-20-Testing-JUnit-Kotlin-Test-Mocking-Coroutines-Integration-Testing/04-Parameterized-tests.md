...
**Parameterized tests** is the Kotlin engineering concept **Parameterized tests**.

---

# 2. Why

Interviewers use **Parameterized tests** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Parameterized tests** is about the Kotlin engineering concept **Parameterized tests**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Parameterized tests** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Parameterized tests** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Parameterized tests**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
