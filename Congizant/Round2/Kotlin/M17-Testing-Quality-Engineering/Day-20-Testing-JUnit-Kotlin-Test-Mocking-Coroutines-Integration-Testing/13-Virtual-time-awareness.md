# Topic 13 — Virtual time awareness
...
**Virtual time awareness** is the Kotlin engineering concept **Virtual time awareness**.

---

# 2. Why

Interviewers use **Virtual time awareness** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Virtual time awareness** is about the Kotlin engineering concept **Virtual time awareness**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Virtual time awareness** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Virtual time awareness**.
...
- Does **Virtual time awareness** solve a real requirement?
...
1. What is **Virtual time awareness**?
...
Create the smallest runnable Kotlin example demonstrating **Virtual time awareness**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Virtual time awareness** in plain English.
