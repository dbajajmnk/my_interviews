...
**HTTP/API tests** is remote request/response boundary.

---

# 2. Why

Interviewers use **HTTP/API tests** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **HTTP/API tests** is about remote request/response boundary. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **HTTP/API tests** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **HTTP/API tests**.
...
- Does **HTTP/API tests** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **HTTP/API tests**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
