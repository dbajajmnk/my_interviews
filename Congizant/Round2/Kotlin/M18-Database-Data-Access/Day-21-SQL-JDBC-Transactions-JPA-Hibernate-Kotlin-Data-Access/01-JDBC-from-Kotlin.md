...
**JDBC from Kotlin** is direct JVM database API.

---

# 2. Why

Interviewers use **JDBC from Kotlin** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **JDBC from Kotlin** is about direct JVM database API. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

connections/statements/results are runtime resources that should use use/close and parameterized SQL
...
In a Kotlin/JVM production system, **JDBC from Kotlin** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **JDBC from Kotlin**.
...
- Does **JDBC from Kotlin** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **JDBC from Kotlin**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **JDBC from Kotlin** in plain English.
