...
**Kotlin JPA pitfalls** is ORM persistence abstraction.

---

# 2. Why

Interviewers use **Kotlin JPA pitfalls** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Kotlin JPA pitfalls** is about ORM persistence abstraction. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

Kotlin final classes/nullability/data classes can interact poorly with proxy/no-arg expectations unless designed/plugins used
...
In a Kotlin/JVM production system, **Kotlin JPA pitfalls** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Kotlin JPA pitfalls**.
...
- Does **Kotlin JPA pitfalls** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Kotlin JPA pitfalls**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Kotlin JPA pitfalls** in plain English.
