...
**N+1** is the Kotlin engineering concept **N+1**.

---

# 2. Why

Interviewers use **N+1** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **N+1** is about the Kotlin engineering concept **N+1**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **N+1** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **N+1**.
...
- Does **N+1** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **N+1**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
