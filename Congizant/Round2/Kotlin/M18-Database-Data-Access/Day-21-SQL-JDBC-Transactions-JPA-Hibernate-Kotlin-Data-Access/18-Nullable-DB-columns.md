...
**Nullable DB columns** is representing possible absence explicitly in the type system.

---

# 2. Why

Interviewers use **Nullable DB columns** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Nullable DB columns** is about representing possible absence explicitly in the type system. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Nullable DB columns** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Nullable DB columns**.
...
- Does **Nullable DB columns** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Nullable DB columns**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Nullable DB columns** in plain English.
