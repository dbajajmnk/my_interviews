# Topic 22 — Remote calls inside transactions
...
**Remote calls inside transactions** is database atomicity boundary.

---

# 2. Why

Interviewers use **Remote calls inside transactions** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Remote calls inside transactions** is about database atomicity boundary. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

DB isolation/locking/MVCC determine concurrency semantics; Kotlin does not change them
...
In a Kotlin/JVM production system, **Remote calls inside transactions** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Remote calls inside transactions**.
...
- Does **Remote calls inside transactions** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Remote calls inside transactions**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
