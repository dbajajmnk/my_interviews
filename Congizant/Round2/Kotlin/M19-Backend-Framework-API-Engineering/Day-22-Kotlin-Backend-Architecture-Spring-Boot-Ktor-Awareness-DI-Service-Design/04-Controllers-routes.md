...
**Controllers/routes** is the Kotlin engineering concept **Controllers/routes**.

---

# 2. Why

Interviewers use **Controllers/routes** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Controllers/routes** is about the Kotlin engineering concept **Controllers/routes**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Controllers/routes** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Controllers/routes**.
...
- Does **Controllers/routes** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Controllers/routes**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Controllers/routes** in plain English.
