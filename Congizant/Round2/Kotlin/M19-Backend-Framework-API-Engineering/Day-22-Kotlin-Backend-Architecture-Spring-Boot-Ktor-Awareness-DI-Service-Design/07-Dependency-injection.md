...
**Dependency injection** is the Kotlin engineering concept **Dependency injection**.

---

# 2. Why

Interviewers use **Dependency injection** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Dependency injection** is about the Kotlin engineering concept **Dependency injection**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Dependency injection** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Dependency injection** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Dependency injection**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
