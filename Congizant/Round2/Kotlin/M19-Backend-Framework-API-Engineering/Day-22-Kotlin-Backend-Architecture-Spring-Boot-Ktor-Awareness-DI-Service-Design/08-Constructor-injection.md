# Topic 08 — Constructor injection
...
**Constructor injection** is the Kotlin engineering concept **Constructor injection**.

---

# 2. Why

Interviewers use **Constructor injection** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Constructor injection** is about the Kotlin engineering concept **Constructor injection**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Constructor injection** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Constructor injection**.
...
- Does **Constructor injection** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Constructor injection**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
