...
**REST resource design** is the Kotlin engineering concept **REST resource design**.

---

# 2. Why

Interviewers use **REST resource design** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **REST resource design** is about the Kotlin engineering concept **REST resource design**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **REST resource design** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **REST resource design**.
...
- Does **REST resource design** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **REST resource design**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
