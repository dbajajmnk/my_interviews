# Topic 06 — Stable error contracts
...
**Stable error contracts** is the Kotlin engineering concept **Stable error contracts**.

---

# 2. Why

Interviewers use **Stable error contracts** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Stable error contracts** is about the Kotlin engineering concept **Stable error contracts**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Stable error contracts** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Stable error contracts** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Stable error contracts**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
