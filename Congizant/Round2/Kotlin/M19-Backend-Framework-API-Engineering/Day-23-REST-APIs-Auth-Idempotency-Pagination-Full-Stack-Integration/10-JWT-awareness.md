...
**JWT awareness** is the Kotlin engineering concept **JWT awareness**.

---

# 2. Why

Interviewers use **JWT awareness** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **JWT awareness** is about the Kotlin engineering concept **JWT awareness**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **JWT awareness** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **JWT awareness** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **JWT awareness**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
