# Topic 13 — Cursor pagination awareness
...
**Cursor pagination awareness** is the Kotlin engineering concept **Cursor pagination awareness**.

---

# 2. Why

Interviewers use **Cursor pagination awareness** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Cursor pagination awareness** is about the Kotlin engineering concept **Cursor pagination awareness**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Cursor pagination awareness** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Cursor pagination awareness** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Cursor pagination awareness**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
