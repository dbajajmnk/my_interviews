...
**Idempotency keys** is duplicate-safe business operation.

---

# 2. Why

Interviewers use **Idempotency keys** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Idempotency keys** is about duplicate-safe business operation. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

persistence/unique key/state machine enforces idempotency beyond coroutine retries
...
In a Kotlin/JVM production system, **Idempotency keys** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Idempotency keys**.
...
- Does **Idempotency keys** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Idempotency keys**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
