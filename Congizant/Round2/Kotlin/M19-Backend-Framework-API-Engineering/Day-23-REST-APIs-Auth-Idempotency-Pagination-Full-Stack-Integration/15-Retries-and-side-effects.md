...
**Retries and side effects** is the Kotlin engineering concept **Retries and side effects**.

---

# 2. Why

Interviewers use **Retries and side effects** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Retries and side effects** is about the Kotlin engineering concept **Retries and side effects**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Retries and side effects** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Retries and side effects** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Retries and side effects**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
