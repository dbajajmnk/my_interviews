...
**Boxing cost** is wrapping primitive-like values in objects when generics/nullability/runtime representation requires it.

---

# 2. Why

Interviewers use **Boxing cost** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Boxing cost** is about wrapping primitive-like values in objects when generics/nullability/runtime representation requires it. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Boxing cost** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Boxing cost**.
...
- Does **Boxing cost** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Boxing cost**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Boxing cost** in plain English.
