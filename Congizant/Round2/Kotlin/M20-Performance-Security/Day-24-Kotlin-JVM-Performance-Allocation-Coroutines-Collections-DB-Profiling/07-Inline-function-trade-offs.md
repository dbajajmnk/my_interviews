# Topic 07 — Inline function trade-offs
...
**Inline function trade-offs** is request to inline function body at call sites subject to compiler constraints.

---

# 2. Why

Interviewers use **Inline function trade-offs** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Inline function trade-offs** is about request to inline function body at call sites subject to compiler constraints. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Inline function trade-offs** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Inline function trade-offs**.
...
- Does **Inline function trade-offs** solve a real requirement?
...
1. What is **Inline function trade-offs**?
...
Create the smallest runnable Kotlin example demonstrating **Inline function trade-offs**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Inline function trade-offs** in plain English.
