...
**Authentication vs authorization** is runtime identity establishment.

---

# 2. Why

Interviewers use **Authentication vs authorization** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Authentication vs authorization** is about runtime identity establishment. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

types do not authenticate; server/framework must verify token/session and create principal
...
In a Kotlin/JVM production system, **Authentication vs authorization** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Authentication vs authorization** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Authentication vs authorization**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
