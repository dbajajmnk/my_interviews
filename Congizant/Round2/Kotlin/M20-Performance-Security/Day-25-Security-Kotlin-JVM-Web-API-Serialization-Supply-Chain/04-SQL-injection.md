...
**SQL injection** is untrusted data becoming SQL syntax.

---

# 2. Why

Interviewers use **SQL injection** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **SQL injection** is about untrusted data becoming SQL syntax. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

use prepared statements/ORM parameters rather than string concatenation
...
In a Kotlin/JVM production system, **SQL injection** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **SQL injection** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **SQL injection**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
