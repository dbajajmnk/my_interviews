...
Interviewers use **SSRF** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **SSRF** is about server-side request forgery. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

user-controlled destination can reach internal/link-local services; validate and constrain egress
...
In a Kotlin/JVM production system, **SSRF** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **SSRF** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **SSRF**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
