# Topic 15 — Unsafe Java serialization
...
**Unsafe Java serialization** is encoding/decoding typed data to external representation.

---

# 2. Why

Interviewers use **Unsafe Java serialization** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Unsafe Java serialization** is about encoding/decoding typed data to external representation. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Unsafe Java serialization** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Unsafe Java serialization**.
...
- Does **Unsafe Java serialization** solve a real requirement?
...
1. What is **Unsafe Java serialization**?
...
Create the smallest runnable Kotlin example demonstrating **Unsafe Java serialization**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Unsafe Java serialization** in plain English.
