...
**Deserialization risk** is encoding/decoding typed data to external representation.

---

# 2. Why

Interviewers use **Deserialization risk** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Deserialization risk** is about encoding/decoding typed data to external representation. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Deserialization risk** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Deserialization risk** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Deserialization risk**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
