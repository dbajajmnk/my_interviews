# Topic 18 — Gradle supply-chain risk
...
**Gradle supply-chain risk** is build/dependency automation.

---

# 2. Why

Interviewers use **Gradle supply-chain risk** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Gradle supply-chain risk** is about build/dependency automation. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Gradle supply-chain risk** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Gradle supply-chain risk**.
...
- Does **Gradle supply-chain risk** solve a real requirement?
...
1. What is **Gradle supply-chain risk**?
...
Create the smallest runnable Kotlin example demonstrating **Gradle supply-chain risk**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Gradle supply-chain risk** in plain English.
