# Topic 22 — Rate limiting awareness
...
**Rate limiting awareness** is the Kotlin engineering concept **Rate limiting awareness**.

---

# 2. Why

Interviewers use **Rate limiting awareness** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Rate limiting awareness** is about the Kotlin engineering concept **Rate limiting awareness**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Rate limiting awareness** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Rate limiting awareness**.
...
- Does **Rate limiting awareness** solve a real requirement?
...
1. What is **Rate limiting awareness**?
...
Create the smallest runnable Kotlin example demonstrating **Rate limiting awareness**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Rate limiting awareness** in plain English.
