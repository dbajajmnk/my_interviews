...
**Non-functional requirements** is the Kotlin engineering concept **Non-functional requirements**.

---

# 2. Why

Interviewers use **Non-functional requirements** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Non-functional requirements** is about the Kotlin engineering concept **Non-functional requirements**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Non-functional requirements** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Non-functional requirements** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Non-functional requirements**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
