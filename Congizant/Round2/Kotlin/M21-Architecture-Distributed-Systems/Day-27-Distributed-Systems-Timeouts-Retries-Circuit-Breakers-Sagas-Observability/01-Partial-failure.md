...
**Partial failure** is the Kotlin engineering concept **Partial failure**.

---

# 2. Why

Interviewers use **Partial failure** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Partial failure** is about the Kotlin engineering concept **Partial failure**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Partial failure** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Partial failure** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Partial failure**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
