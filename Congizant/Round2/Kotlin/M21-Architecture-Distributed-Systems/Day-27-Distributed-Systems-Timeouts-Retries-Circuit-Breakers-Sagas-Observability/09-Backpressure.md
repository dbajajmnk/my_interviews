...
**Backpressure** is the Kotlin engineering concept **Backpressure**.

---

# 2. Why

Interviewers use **Backpressure** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Backpressure** is about the Kotlin engineering concept **Backpressure**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Backpressure** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Backpressure** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Backpressure**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
