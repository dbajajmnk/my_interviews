...
**Idempotency** is duplicate-safe business operation.

---

# 2. Why

Interviewers use **Idempotency** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Idempotency** is about duplicate-safe business operation. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Idempotency** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Idempotency** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Idempotency**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
