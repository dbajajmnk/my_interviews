...
**Outbox** is reliable event publication intent tied to DB commit.

---

# 2. Why

Interviewers use **Outbox** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Outbox** is about reliable event publication intent tied to DB commit. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Outbox** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Outbox** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Outbox**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
