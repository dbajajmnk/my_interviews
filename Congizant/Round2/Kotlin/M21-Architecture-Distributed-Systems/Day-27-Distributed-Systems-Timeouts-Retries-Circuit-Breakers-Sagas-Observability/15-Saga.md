...
**Saga** is distributed workflow with local transactions and compensation.

---

# 2. Why

Interviewers use **Saga** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Saga** is about distributed workflow with local transactions and compensation. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

explicit state/compensation handles partial success without global DB transaction
...
In a Kotlin/JVM production system, **Saga** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Saga** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Saga**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
