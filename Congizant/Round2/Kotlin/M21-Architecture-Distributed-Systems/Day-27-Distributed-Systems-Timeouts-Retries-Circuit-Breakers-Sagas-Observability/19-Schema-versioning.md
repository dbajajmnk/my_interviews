...
**Schema versioning** is the Kotlin engineering concept **Schema versioning**.

---

# 2. Why

Interviewers use **Schema versioning** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Schema versioning** is about the Kotlin engineering concept **Schema versioning**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Schema versioning** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Schema versioning** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Schema versioning**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
