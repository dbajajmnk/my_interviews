...
**Distributed tracing** is the Kotlin engineering concept **Distributed tracing**.

---

# 2. Why

Interviewers use **Distributed tracing** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Distributed tracing** is about the Kotlin engineering concept **Distributed tracing**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Distributed tracing** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Distributed tracing**.
...
- Does **Distributed tracing** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Distributed tracing**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
