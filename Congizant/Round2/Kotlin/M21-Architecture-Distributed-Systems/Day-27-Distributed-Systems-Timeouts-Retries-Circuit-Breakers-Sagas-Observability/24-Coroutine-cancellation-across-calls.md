# Topic 24 — Coroutine cancellation across calls
...
**Coroutine cancellation across calls** is the Kotlin engineering concept **Coroutine cancellation across calls**.

---

# 2. Why

Interviewers use **Coroutine cancellation across calls** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Coroutine cancellation across calls** is about the Kotlin engineering concept **Coroutine cancellation across calls**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Coroutine cancellation across calls** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Coroutine cancellation across calls**.
...
- Does **Coroutine cancellation across calls** solve a real requirement?
...
1. What is **Coroutine cancellation across calls**?
...
Create the smallest runnable Kotlin example demonstrating **Coroutine cancellation across calls**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Coroutine cancellation across calls** in plain English.
