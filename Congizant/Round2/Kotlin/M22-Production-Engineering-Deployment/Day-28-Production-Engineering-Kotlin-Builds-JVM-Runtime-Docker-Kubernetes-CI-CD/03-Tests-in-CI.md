...
**Tests in CI** is the Kotlin engineering concept **Tests in CI**.

---

# 2. Why

Interviewers use **Tests in CI** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Tests in CI** is about the Kotlin engineering concept **Tests in CI**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Tests in CI** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Tests in CI**.
...
- Does **Tests in CI** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Tests in CI**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Tests in CI** in plain English.
