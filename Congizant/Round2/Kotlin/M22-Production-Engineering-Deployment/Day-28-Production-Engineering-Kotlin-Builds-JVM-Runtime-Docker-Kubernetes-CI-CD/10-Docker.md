...
**Docker** is containerized JVM application runtime.

---

# 2. Why

Interviewers use **Docker** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Docker** is about containerized JVM application runtime. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

container limits interact with JVM heap/native/thread memory and startup/shutdown
...
In a Kotlin/JVM production system, **Docker** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Docker** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Docker**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
