...
**Kubernetes awareness** is orchestration of replicas/lifecycle/traffic.

---

# 2. Why

Interviewers use **Kubernetes awareness** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Kubernetes awareness** is about orchestration of replicas/lifecycle/traffic. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.

---

# 4. Engineering Explanation

readiness controls routing, liveness restart, resources/probes must reflect JVM behavior
...
In a Kotlin/JVM production system, **Kubernetes awareness** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Kubernetes awareness**.
...
- Does **Kubernetes awareness** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Kubernetes awareness**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
