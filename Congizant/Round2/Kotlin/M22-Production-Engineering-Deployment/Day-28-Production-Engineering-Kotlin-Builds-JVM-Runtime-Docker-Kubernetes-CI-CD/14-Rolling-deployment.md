...
**Rolling deployment** is the Kotlin engineering concept **Rolling deployment**.

---

# 2. Why

Interviewers use **Rolling deployment** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Rolling deployment** is about the Kotlin engineering concept **Rolling deployment**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Rolling deployment** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Rolling deployment** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Rolling deployment**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
