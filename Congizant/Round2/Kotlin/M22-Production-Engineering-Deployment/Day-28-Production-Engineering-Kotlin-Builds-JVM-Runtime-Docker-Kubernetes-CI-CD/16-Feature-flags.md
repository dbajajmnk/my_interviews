...
**Feature flags** is the Kotlin engineering concept **Feature flags**.

---

# 2. Why

Interviewers use **Feature flags** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Feature flags** is about the Kotlin engineering concept **Feature flags**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Feature flags** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Feature flags** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Feature flags**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
