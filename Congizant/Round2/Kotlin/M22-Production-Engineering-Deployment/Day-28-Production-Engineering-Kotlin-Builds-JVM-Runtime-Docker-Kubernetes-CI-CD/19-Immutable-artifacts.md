...
**Immutable artifacts** is the Kotlin engineering concept **Immutable artifacts**.

---

# 2. Why

Interviewers use **Immutable artifacts** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Immutable artifacts** is about the Kotlin engineering concept **Immutable artifacts**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Immutable artifacts** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Immutable artifacts**.
...
- Does **Immutable artifacts** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Immutable artifacts**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
