...
**RPO/RTO** is the Kotlin engineering concept **RPO/RTO**.

---

# 2. Why

Interviewers use **RPO/RTO** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **RPO/RTO** is about the Kotlin engineering concept **RPO/RTO**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **RPO/RTO** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **RPO/RTO** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **RPO/RTO**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
