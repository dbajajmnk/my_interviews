# Topic 05 — Legacy thread model migration
...
**Legacy thread model migration** is incremental system evolution.

---

# 2. Why

Interviewers use **Legacy thread model migration** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Legacy thread model migration** is about incremental system evolution. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Legacy thread model migration** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Legacy thread model migration**.
...
- Does **Legacy thread model migration** solve a real requirement?
...
1. What is **Legacy thread model migration**?
...
Create the smallest runnable Kotlin example demonstrating **Legacy thread model migration**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Legacy thread model migration** in plain English.
