...
**Coroutine leak** is the Kotlin engineering concept **Coroutine leak**.

---

# 2. Why

Interviewers use **Coroutine leak** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Coroutine leak** is about the Kotlin engineering concept **Coroutine leak**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Coroutine leak** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Coroutine leak**.
...
- Does **Coroutine leak** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Coroutine leak**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Coroutine leak** in plain English.
