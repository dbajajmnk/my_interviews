# Topic 15 — Framework migration decision
...
**Framework migration decision** is incremental system evolution.

---

# 2. Why

Interviewers use **Framework migration decision** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Framework migration decision** is about incremental system evolution. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Framework migration decision** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Framework migration decision**.
...
- Does **Framework migration decision** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Framework migration decision**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
