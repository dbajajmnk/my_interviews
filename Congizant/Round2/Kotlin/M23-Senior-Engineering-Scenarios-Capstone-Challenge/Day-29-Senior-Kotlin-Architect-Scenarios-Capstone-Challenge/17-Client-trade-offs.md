...
**Client trade-offs** is the Kotlin engineering concept **Client trade-offs**.

---

# 2. Why

Interviewers use **Client trade-offs** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Client trade-offs** is about the Kotlin engineering concept **Client trade-offs**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Client trade-offs** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Client trade-offs** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Client trade-offs**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
