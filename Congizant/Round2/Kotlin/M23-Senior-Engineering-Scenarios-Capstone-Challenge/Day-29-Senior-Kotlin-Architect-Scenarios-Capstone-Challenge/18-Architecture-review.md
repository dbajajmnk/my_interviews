...
**Architecture review** is the Kotlin engineering concept **Architecture review**.

---

# 2. Why

Interviewers use **Architecture review** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Architecture review** is about the Kotlin engineering concept **Architecture review**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Architecture review** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **Architecture review** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Architecture review**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
