...
**Variance questions** is controlling subtype relationships for generic type constructors.

---

# 2. Why

Interviewers use **Variance questions** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Variance questions** is about controlling subtype relationships for generic type constructors. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Variance questions** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Variance questions**.
...
- Does **Variance questions** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Variance questions**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Variance questions** in plain English.
