...
**Reified questions** is retaining an inline type argument enough for certain runtime class checks/access.

---

# 2. Why

Interviewers use **Reified questions** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **Reified questions** is about retaining an inline type argument enough for certain runtime class checks/access. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **Reified questions** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Reproduce the smallest failure involving **Reified questions**.
...
- Does **Reified questions** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **Reified questions**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
...
- [ ] I can define **Reified questions** in plain English.
