...
**No Bluff** is the Kotlin engineering concept **No Bluff**.

---

# 2. Why

Interviewers use **No Bluff** to test whether you understand Kotlin semantics, JVM/platform behavior, failure modes, and engineering trade-offs rather than only syntax.

---

# 3. Plain English

Plain English: **No Bluff** is about the Kotlin engineering concept **No Bluff**. Use it when the requirement matches the problem; prefer simpler Kotlin when extra abstraction is not justified.
...
In a Kotlin/JVM production system, **No Bluff** should connect to a real domain, coroutine, JVM, API, data, Android/backend, or architecture requirement.
...
- Does **No Bluff** solve a real requirement?
...
Create the smallest runnable Kotlin example demonstrating **No Bluff**. Then introduce one realistic misuse, reproduce the failure, fix it, and explain the review rule that should catch it.
