...
**Closure capture** is anonymous callable with environment capture.

# 2. Why

Interviewers use **Closure capture** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.
...
Use **Closure capture** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
Create a minimal runnable Rust example for **Closure capture**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
