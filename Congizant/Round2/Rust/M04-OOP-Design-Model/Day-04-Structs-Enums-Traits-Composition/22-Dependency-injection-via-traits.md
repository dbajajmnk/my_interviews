# Topic 22 — Dependency injection via traits
...
**Dependency injection via traits** is the Rust engineering concept **Dependency injection via traits**.

# 2. Why

Interviewers use **Dependency injection via traits** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.

# 3. Plain English

Plain English: the Rust engineering concept **Dependency injection via traits**. Make ownership and invalid states explicit first; optimize only after correctness is proven and measured.
...
Use **Dependency injection via traits** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
1. What is **Dependency injection via traits**?
...
Create a minimal runnable Rust example for **Dependency injection via traits**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
