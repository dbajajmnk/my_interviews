...
**Day:** 05 — SOLID, DRY, KIS, YAGNI & GoF Creational/Structural  
**Module:** M05 — Design Principles & Gang of Four Patterns  
**Priority:** P0

# 1. What

**Liskov Substitution Principle (LSP)** is implementations preserve semantic trait contracts.

# 2. Why

Interviewers use **Liskov Substitution Principle (LSP)** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.

# 3. Plain English

Plain English: implementations preserve semantic trait contracts. Make ownership and invalid states explicit first; optimize only after correctness is proven and measured.

# 4. Engineering Explanation

type compatibility does not excuse behavioral contract violations
...
```rust
struct OrderService<R: OrderRepository> { repo: R }
```
...
Use **Liskov Substitution Principle (LSP)** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
Create a minimal runnable Rust example for **Liskov Substitution Principle (LSP)**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
