...
**Interface Segregation Principle (ISP)** is consumers depend on focused behavior.

# 2. Why

Interviewers use **Interface Segregation Principle (ISP)** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.

# 3. Plain English

Plain English: consumers depend on focused behavior. Make ownership and invalid states explicit first; optimize only after correctness is proven and measured.

# 4. Engineering Explanation

prefer small traits over broad service traits
...
Use **Interface Segregation Principle (ISP)** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
Create a minimal runnable Rust example for **Interface Segregation Principle (ISP)**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
