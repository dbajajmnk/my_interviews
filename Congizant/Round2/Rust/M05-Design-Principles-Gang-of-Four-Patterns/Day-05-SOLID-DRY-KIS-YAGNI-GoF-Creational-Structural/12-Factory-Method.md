...
**Factory Method** is delegates creation behind a stable function or associated function.

# 2. Why

Interviewers use **Factory Method** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.

# 3. Plain English

Plain English: delegates creation behind a stable function or associated function. Make ownership and invalid states explicit first; optimize only after correctness is proven and measured.

# 4. Engineering Explanation

associated/free factories avoid subclass-style ceremony
...
```rust
fn make_parser(kind: ParserKind) -> Box<dyn Parser> { todo!() }
```
...
Use **Factory Method** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
Create a minimal runnable Rust example for **Factory Method**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
