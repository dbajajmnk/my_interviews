...
**Prototype** is creates a new value from an existing one.

# 2. Why

Interviewers use **Prototype** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.

# 3. Plain English

Plain English: creates a new value from an existing one. Make ownership and invalid states explicit first; optimize only after correctness is proven and measured.

# 4. Engineering Explanation

Clone is the natural mechanism when clone semantics are valid
...
Use **Prototype** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
Create a minimal runnable Rust example for **Prototype**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
