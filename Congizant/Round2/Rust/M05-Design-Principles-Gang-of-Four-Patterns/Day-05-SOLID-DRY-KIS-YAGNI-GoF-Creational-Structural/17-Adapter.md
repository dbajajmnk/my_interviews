...
**Adapter** is translates an incompatible dependency into a domain trait.

# 2. Why

Interviewers use **Adapter** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.

# 3. Plain English

Plain English: translates an incompatible dependency into a domain trait. Make ownership and invalid states explicit first; optimize only after correctness is proven and measured.

# 4. Engineering Explanation

wrapper maps vendor calls and errors
...
```rust
struct LegacyAdapter { client: LegacyClient }
```
...
Use **Adapter** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
Create a minimal runnable Rust example for **Adapter**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
