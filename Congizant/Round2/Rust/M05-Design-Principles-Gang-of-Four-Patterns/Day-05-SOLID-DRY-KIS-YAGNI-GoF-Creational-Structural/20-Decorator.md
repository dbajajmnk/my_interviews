...
**Decorator** is adds behavior around the same contract.

# 2. Why

Interviewers use **Decorator** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.

# 3. Plain English

Plain English: adds behavior around the same contract. Make ownership and invalid states explicit first; optimize only after correctness is proven and measured.

# 4. Engineering Explanation

generic wrappers/trait objects can add logging, metrics or retry
...
Use **Decorator** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
Create a minimal runnable Rust example for **Decorator**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
