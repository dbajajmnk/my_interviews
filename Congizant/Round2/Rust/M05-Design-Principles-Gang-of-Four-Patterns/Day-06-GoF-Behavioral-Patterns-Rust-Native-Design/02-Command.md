...
**Command** is represents an action as data.

# 2. Why

Interviewers use **Command** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.

# 3. Plain English

Plain English: represents an action as data. Make ownership and invalid states explicit first; optimize only after correctness is proven and measured.

# 4. Engineering Explanation

structs/enums make commands queueable and exhaustively matchable
...
```rust
enum Command { CreateOrder{customer_id:String}, CancelOrder{order_id:String} }
```
...
Use **Command** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
Create a minimal runnable Rust example for **Command**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
