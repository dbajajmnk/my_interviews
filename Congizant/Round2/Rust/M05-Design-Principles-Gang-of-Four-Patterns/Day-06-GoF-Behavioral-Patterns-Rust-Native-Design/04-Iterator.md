...
Interviewers use **Iterator** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.

# 3. Plain English

Plain English: provides sequential traversal. Make ownership and invalid states explicit first; optimize only after correctness is proven and measured.

# 4. Engineering Explanation

Iterator is a core standard trait
...
```rust
impl Iterator for MyIter { type Item=i32; fn next(&mut self)->Option<i32>{None} }
```
...
Use **Iterator** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
Create a minimal runnable Rust example for **Iterator**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
