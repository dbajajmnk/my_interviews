...
**Template Method** is defines an algorithm skeleton with customizable steps.

# 2. Why

Interviewers use **Template Method** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.

# 3. Plain English

Plain English: defines an algorithm skeleton with customizable steps. Make ownership and invalid states explicit first; optimize only after correctness is proven and measured.

# 4. Engineering Explanation

trait default methods can define the skeleton
...
```rust
trait ImportJob { fn read(&self); fn run(&self){ self.read(); } }
```
...
Use **Template Method** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
Create a minimal runnable Rust example for **Template Method**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
