...
**Vec<T>** is contiguous growable collection.

# 2. Why

Interviewers use **Vec<T>** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.
...
Use **Vec<T>** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
Create a minimal runnable Rust example for **Vec<T>**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
