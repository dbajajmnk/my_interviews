...
**LockResult awareness** is recoverable success/error enum.

# 2. Why

Interviewers use **LockResult awareness** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.
...
Use **LockResult awareness** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
1. What is **LockResult awareness**?
...
Create a minimal runnable Rust example for **LockResult awareness**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
