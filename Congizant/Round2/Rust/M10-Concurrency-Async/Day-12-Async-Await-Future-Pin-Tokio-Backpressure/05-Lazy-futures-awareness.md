# Topic 05 — Lazy futures awareness
...
**Lazy futures awareness** is lazy asynchronous computation.

# 2. Why

Interviewers use **Lazy futures awareness** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.
...
Use **Lazy futures awareness** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
1. What is **Lazy futures awareness**?
...
Create a minimal runnable Rust example for **Lazy futures awareness**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
