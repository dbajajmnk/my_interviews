# Topic 13 — tokio::spawn awareness
...
**tokio::spawn awareness** is popular Rust async runtime.

# 2. Why

Interviewers use **tokio::spawn awareness** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.
...
Use **tokio::spawn awareness** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
1. What is **tokio::spawn awareness**?
...
Create a minimal runnable Rust example for **tokio::spawn awareness**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
