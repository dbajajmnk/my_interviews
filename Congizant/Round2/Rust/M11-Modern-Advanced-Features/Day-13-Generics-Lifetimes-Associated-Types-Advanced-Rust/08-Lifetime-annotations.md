...
**Lifetime annotations** is compile-time relationship among references.

# 2. Why

Interviewers use **Lifetime annotations** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.
...
Use **Lifetime annotations** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
1. What is **Lifetime annotations**?
...
Create a minimal runnable Rust example for **Lifetime annotations**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
