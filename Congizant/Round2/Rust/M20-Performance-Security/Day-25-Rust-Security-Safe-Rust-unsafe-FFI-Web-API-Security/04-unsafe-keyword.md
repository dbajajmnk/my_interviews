...
**unsafe keyword** is trusted boundary for operations compiler cannot prove safe.

# 2. Why

Interviewers use **unsafe keyword** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.
...
Use **unsafe keyword** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
Create a minimal runnable Rust example for **unsafe keyword**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
