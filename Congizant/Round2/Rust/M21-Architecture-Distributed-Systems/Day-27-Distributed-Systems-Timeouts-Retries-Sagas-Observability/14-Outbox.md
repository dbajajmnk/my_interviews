...
**Outbox** is reliable event publication in same local transaction.

# 2. Why

Interviewers use **Outbox** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.
...
Use **Outbox** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
Create a minimal runnable Rust example for **Outbox**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
