...
**Clippy in CI** is Rust lint suite.

# 2. Why

Interviewers use **Clippy in CI** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.
...
Use **Clippy in CI** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
Create a minimal runnable Rust example for **Clippy in CI**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
