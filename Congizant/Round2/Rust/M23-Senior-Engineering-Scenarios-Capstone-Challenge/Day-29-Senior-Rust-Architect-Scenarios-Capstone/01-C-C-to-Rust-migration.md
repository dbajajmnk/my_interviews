# Topic 01 — C/C++ to Rust migration
...
**C/C++ to Rust migration** is the Rust engineering concept **C/C++ to Rust migration**.

# 2. Why

Interviewers use **C/C++ to Rust migration** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.

# 3. Plain English

Plain English: the Rust engineering concept **C/C++ to Rust migration**. Make ownership and invalid states explicit first; optimize only after correctness is proven and measured.
...
Use **C/C++ to Rust migration** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
1. What is **C/C++ to Rust migration**?
...
Create a minimal runnable Rust example for **C/C++ to Rust migration**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
