# Topic 08 — Unsafe code elimination
...
**Unsafe code elimination** is trusted boundary for operations compiler cannot prove safe.

# 2. Why

Interviewers use **Unsafe code elimination** to test Rust ownership/type guarantees, runtime costs, concurrency/async behavior, error modeling, security and production trade-offs.
...
Use **Unsafe code elimination** where it solves a real Rust systems, backend, async, data, safety or performance problem. **Evidence First:** conceptual examples are not project claims.
...
1. What is **Unsafe code elimination**?
...
Create a minimal runnable Rust example for **Unsafe code elimination**, introduce one realistic misuse, reproduce it with evidence, repair it and state the review rule.
