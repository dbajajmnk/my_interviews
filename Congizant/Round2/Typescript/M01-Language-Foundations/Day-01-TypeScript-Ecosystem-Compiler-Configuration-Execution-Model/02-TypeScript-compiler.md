...
**TypeScript compiler** is the `tsc` type-checking and emit pipeline.

---

# 2. Why

Interviewers use **TypeScript compiler** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **TypeScript compiler** means the `tsc` type-checking and emit pipeline. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

the compiler parses source, resolves modules/types, checks, transforms/prints JavaScript/declarations according to config
...
In real TypeScript systems, this matters for build/type-check workflows.
...
- Reproduce the smallest case involving **TypeScript compiler**.
...
1. What is **TypeScript compiler**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **TypeScript compiler**?
5. Where would you use **TypeScript compiler** in a real system?
...
Create the smallest strict TypeScript example demonstrating **TypeScript compiler**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **TypeScript compiler** in plain English.
