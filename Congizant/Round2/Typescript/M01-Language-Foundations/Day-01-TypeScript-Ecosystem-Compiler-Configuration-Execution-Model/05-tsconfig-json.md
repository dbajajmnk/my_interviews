...
**tsconfig.json** is project-level compiler contract.

---

# 2. Why

Interviewers use **tsconfig.json** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **tsconfig.json** means project-level compiler contract. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

config controls files, strictness, module resolution, emit and project references; inherited configs merge by TypeScript rules
...
In real TypeScript systems, this matters for reproducible project behavior.
...
- Reproduce the smallest case involving **tsconfig.json**.
...
4. What is a common misuse of **tsconfig.json**?
5. Where would you use **tsconfig.json** in a real system?
...
Create the smallest strict TypeScript example demonstrating **tsconfig.json**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **tsconfig.json** in plain English.
