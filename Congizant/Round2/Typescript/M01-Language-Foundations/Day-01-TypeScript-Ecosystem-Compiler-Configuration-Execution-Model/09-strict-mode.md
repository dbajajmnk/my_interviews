...
**strict mode** is enabling TypeScript's strict family of checks.

---

# 2. Why

Interviewers use **strict mode** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **strict mode** means enabling TypeScript's strict family of checks. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

`strict` enables a coordinated set of stricter type-system options whose exact members can evolve by compiler version
...
In real TypeScript systems, this matters for safety baseline.
...
- Reproduce the smallest case involving **strict mode**.
...
4. What is a common misuse of **strict mode**?
5. Where would you use **strict mode** in a real system?
...
Create the smallest strict TypeScript example demonstrating **strict mode**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
