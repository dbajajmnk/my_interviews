...
**source maps** is mapping emitted/transformed JavaScript back to TypeScript source.

---

# 2. Why

Interviewers use **source maps** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **source maps** means mapping emitted/transformed JavaScript back to TypeScript source. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

debuggers use map metadata to reconstruct original source positions
...
In real TypeScript systems, this matters for production debugging.
...
- Reproduce the smallest case involving **source maps**.
...
4. What is a common misuse of **source maps**?
5. Where would you use **source maps** in a real system?
...
Create the smallest strict TypeScript example demonstrating **source maps**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
