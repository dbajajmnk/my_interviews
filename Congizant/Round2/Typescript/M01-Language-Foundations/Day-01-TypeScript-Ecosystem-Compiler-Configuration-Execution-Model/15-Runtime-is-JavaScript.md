# Topic 15 — Runtime is JavaScript
...
**Runtime is JavaScript** is separating compile-time TypeScript from runtime JavaScript.

---

# 2. Why

Interviewers use **Runtime is JavaScript** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Runtime is JavaScript** means separating compile-time TypeScript from runtime JavaScript. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

most TypeScript types are erased; the JavaScript runtime receives emitted JS and untrusted input remains unchecked unless validated
...
```typescript
interface User { id:string } // no runtime object emitted
```
...
In real TypeScript systems, this matters for trust-boundary reasoning.
...
- Reproduce the smallest case involving **Runtime is JavaScript**.
...
1. What is **Runtime is JavaScript**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Runtime is JavaScript**?
5. Where would you use **Runtime is JavaScript** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Runtime is JavaScript**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Runtime is JavaScript** in plain English.
