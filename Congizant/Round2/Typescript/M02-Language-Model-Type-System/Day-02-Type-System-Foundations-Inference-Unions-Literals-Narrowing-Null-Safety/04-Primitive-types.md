...
**Primitive types** is the TypeScript concept **Primitive types**.

---

# 2. Why

Interviewers use **Primitive types** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Primitive types** means the TypeScript concept **Primitive types**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Primitive types** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Primitive types**.
...
4. What is a common misuse of **Primitive types**?
5. Where would you use **Primitive types** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Primitive types**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
