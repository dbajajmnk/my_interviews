...
**Literal types** is type representing an exact literal value.

---

# 2. Why

Interviewers use **Literal types** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Literal types** means type representing an exact literal value. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
- Reproduce the smallest case involving **Literal types**.
...
1. What is **Literal types**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Literal types**?
5. Where would you use **Literal types** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Literal types**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Literal types** in plain English.
