...
**Optional properties** is properties that may be absent.

---

# 2. Why

Interviewers use **Optional properties** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Optional properties** means properties that may be absent. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

`x?: T` models absence and interacts with exactOptionalPropertyTypes when enabled
...
In real TypeScript systems, this matters for partial input/config.
...
- Reproduce the smallest case involving **Optional properties**.
...
1. What is **Optional properties**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Optional properties**?
5. Where would you use **Optional properties** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Optional properties**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Optional properties** in plain English.
