...
**Readonly properties** is compile-time prevention of assignment through that typed reference.

---

# 2. Why

Interviewers use **Readonly properties** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Readonly properties** means compile-time prevention of assignment through that typed reference. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

readonly does not make nested runtime objects deeply immutable by itself
...
```typescript
type Config = { readonly timeout:number };
```
...
In real TypeScript systems, this matters for immutable APIs.
...
- Reproduce the smallest case involving **Readonly properties**.
...
1. What is **Readonly properties**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Readonly properties**?
5. Where would you use **Readonly properties** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Readonly properties**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Readonly properties** in plain English.
