# Topic 17 — in operator narrowing
...
**in operator narrowing** is narrowing object unions by property presence.

---

# 2. Why

Interviewers use **in operator narrowing** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **in operator narrowing** means narrowing object unions by property presence. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

`'prop' in obj` can refine types that may/may not declare the property
...
In real TypeScript systems, this matters for variant narrowing.
...
- Reproduce the smallest case involving **in operator narrowing**.
...
4. What is a common misuse of **in operator narrowing**?
5. Where would you use **in operator narrowing** in a real system?
...
Create the smallest strict TypeScript example demonstrating **in operator narrowing**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **in operator narrowing** in plain English.
