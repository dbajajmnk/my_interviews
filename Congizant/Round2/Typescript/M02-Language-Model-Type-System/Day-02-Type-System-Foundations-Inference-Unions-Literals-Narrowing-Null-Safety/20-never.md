...
**never** is type representing values that cannot occur.

---

# 2. Why

Interviewers use **never** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **never** means type representing values that cannot occur. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
- Reproduce the smallest case involving **never**.
...
4. What is a common misuse of **never**?
5. Where would you use **never** in a real system?
...
Create the smallest strict TypeScript example demonstrating **never**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **never** in plain English.
