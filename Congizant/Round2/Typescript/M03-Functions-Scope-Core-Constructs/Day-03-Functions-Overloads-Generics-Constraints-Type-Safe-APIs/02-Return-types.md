...
**Return types** is declared or inferred function output contracts.

---

# 2. Why

Interviewers use **Return types** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Return types** means declared or inferred function output contracts. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

explicit public return types can stabilize APIs while local inference reduces noise
...
```typescript
function parse(): Result { /*...*/ throw new Error(); }
```
...
In real TypeScript systems, this matters for public APIs.
...
- Reproduce the smallest case involving **Return types**.
...
1. What is **Return types**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Return types**?
5. Where would you use **Return types** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Return types**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Return types** in plain English.
