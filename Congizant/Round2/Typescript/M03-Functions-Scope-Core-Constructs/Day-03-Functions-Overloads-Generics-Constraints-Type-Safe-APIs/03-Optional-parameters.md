...
**Optional parameters** is parameters callers may omit.

---

# 2. Why

Interviewers use **Optional parameters** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Optional parameters** means parameters callers may omit. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

`x?: T` gives `T | undefined` behavior inside the function under strict null checks
...
```typescript
function f(x?:string){ return x?.length ?? 0; }
```
...
In real TypeScript systems, this matters for flexible APIs.
...
- Reproduce the smallest case involving **Optional parameters**.
...
1. What is **Optional parameters**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Optional parameters**?
5. Where would you use **Optional parameters** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Optional parameters**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Optional parameters** in plain English.
