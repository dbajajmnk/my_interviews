...
**keyof constraints** is restricting generic type parameters to required capabilities.

---

# 2. Why

Interviewers use **keyof constraints** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **keyof constraints** means restricting generic type parameters to required capabilities. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

`T extends U` lets generic code safely use members guaranteed by U
...
In real TypeScript systems, this matters for generic APIs.
...
- Reproduce the smallest case involving **keyof constraints**.
...
1. What is **keyof constraints**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **keyof constraints**?
5. Where would you use **keyof constraints** in a real system?
...
Create the smallest strict TypeScript example demonstrating **keyof constraints**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **keyof constraints** in plain English.
