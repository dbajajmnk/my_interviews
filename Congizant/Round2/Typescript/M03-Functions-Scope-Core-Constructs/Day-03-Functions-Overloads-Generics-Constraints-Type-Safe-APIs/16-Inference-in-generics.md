# Topic 16 — Inference in generics
...
**Inference in generics** is introducing a type variable inside conditional matching.

---

# 2. Why

Interviewers use **Inference in generics** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Inference in generics** means introducing a type variable inside conditional matching. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
- Reproduce the smallest case involving **Inference in generics**.
...
1. What is **Inference in generics**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Inference in generics**?
5. Where would you use **Inference in generics** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Inference in generics**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Inference in generics** in plain English.
