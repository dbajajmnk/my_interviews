# Topic 20 — never returning functions
...
**never returning functions** is type representing values that cannot occur.

---

# 2. Why

Interviewers use **never returning functions** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **never returning functions** means type representing values that cannot occur. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

appears for unreachable branches, functions that never return, and exhaustiveness checks
...
In real TypeScript systems, this matters for exhaustive design.
...
- Reproduce the smallest case involving **never returning functions**.
...
1. What is **never returning functions**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **never returning functions**?
5. Where would you use **never returning functions** in a real system?
...
Create the smallest strict TypeScript example demonstrating **never returning functions**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **never returning functions** in plain English.
