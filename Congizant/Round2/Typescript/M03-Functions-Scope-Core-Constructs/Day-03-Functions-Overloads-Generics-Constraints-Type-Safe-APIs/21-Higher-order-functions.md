# Topic 21 — Higher-order functions
...
**Higher-order functions** is the TypeScript concept **Higher-order functions**.

---

# 2. Why

Interviewers use **Higher-order functions** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Higher-order functions** means the TypeScript concept **Higher-order functions**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Higher-order functions** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Higher-order functions**.
...
4. What is a common misuse of **Higher-order functions**?
5. Where would you use **Higher-order functions** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Higher-order functions**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
