...
**Implements** is compile-time check that a class instance satisfies an interface.

---

# 2. Why

Interviewers use **Implements** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Implements** means compile-time check that a class instance satisfies an interface. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

implements checks instance shape but does not change emitted inheritance/runtime behavior
...
In real TypeScript systems, this matters for contract conformance.
...
- Reproduce the smallest case involving **Implements**.
...
4. What is a common misuse of **Implements**?
5. Where would you use **Implements** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Implements**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Implements** in plain English.
