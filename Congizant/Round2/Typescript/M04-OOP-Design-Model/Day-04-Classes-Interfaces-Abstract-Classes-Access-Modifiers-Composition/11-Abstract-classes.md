...
**Abstract classes** is partially implemented class contract.

---

# 2. Why

Interviewers use **Abstract classes** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Abstract classes** means partially implemented class contract. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

abstract classes/classes can include runtime implementation plus erased abstract declarations
...
```typescript
abstract class Store { abstract save(x:Item):Promise<void>; }
```
...
In real TypeScript systems, this matters for shared lifecycle/behavior.
...
- Reproduce the smallest case involving **Abstract classes**.
...
4. What is a common misuse of **Abstract classes**?
5. Where would you use **Abstract classes** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Abstract classes**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
