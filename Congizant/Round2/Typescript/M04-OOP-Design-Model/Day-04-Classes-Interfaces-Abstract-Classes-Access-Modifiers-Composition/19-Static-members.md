...
**Static members** is the TypeScript concept **Static members**.

---

# 2. Why

Interviewers use **Static members** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Static members** means the TypeScript concept **Static members**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Static members** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Static members**.
...
4. What is a common misuse of **Static members**?
5. Where would you use **Static members** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Static members**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Static members** in plain English.
