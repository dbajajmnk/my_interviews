...
Interviewers use **Single Responsibility Principle (SRP)** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Single Responsibility Principle (SRP)** means one cohesive reason to change. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

separate unrelated change axes even when TypeScript can express them in one class/module
...
In production TypeScript, apply it to split transport, policy, persistence, and formatting.
...
- Reproduce the smallest case involving **Single Responsibility Principle (SRP)**.
...
4. What is a common misuse of **Single Responsibility Principle (SRP)**?
5. Where would you use **Single Responsibility Principle (SRP)** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Single Responsibility Principle (SRP)**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
