...
**Open/Closed Principle (OCP)** is support expected extension through stable typed contracts.

---

# 2. Why

Interviewers use **Open/Closed Principle (OCP)** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Open/Closed Principle (OCP)** means support expected extension through stable typed contracts. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

union/interfaces/generics can isolate variation without editing giant conditionals
...
In production TypeScript, apply it to inject a pricing strategy.
...
- Reproduce the smallest case involving **Open/Closed Principle (OCP)**.
...
4. What is a common misuse of **Open/Closed Principle (OCP)**?
5. Where would you use **Open/Closed Principle (OCP)** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Open/Closed Principle (OCP)**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
