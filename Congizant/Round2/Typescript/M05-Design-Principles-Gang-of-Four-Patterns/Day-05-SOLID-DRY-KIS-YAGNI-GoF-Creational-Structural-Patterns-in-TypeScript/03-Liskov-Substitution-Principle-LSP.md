...
**Liskov Substitution Principle (LSP)** is substitutable implementations preserve behavior.

---

# 2. Why

Interviewers use **Liskov Substitution Principle (LSP)** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Liskov Substitution Principle (LSP)** means substitutable implementations preserve behavior. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

structural compatibility does not guarantee semantic substitutability
...
In production TypeScript, apply it to all repository adapters preserve the same contract.
...
- Reproduce the smallest case involving **Liskov Substitution Principle (LSP)**.
...
4. What is a common misuse of **Liskov Substitution Principle (LSP)**?
5. Where would you use **Liskov Substitution Principle (LSP)** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Liskov Substitution Principle (LSP)**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
