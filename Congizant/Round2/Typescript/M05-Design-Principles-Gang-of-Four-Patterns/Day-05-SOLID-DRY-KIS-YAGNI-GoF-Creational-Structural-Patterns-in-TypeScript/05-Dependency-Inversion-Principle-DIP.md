...
**Dependency Inversion Principle (DIP)** is high-level policy depends on typed abstractions.

---

# 2. Why

Interviewers use **Dependency Inversion Principle (DIP)** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Dependency Inversion Principle (DIP)** means high-level policy depends on typed abstractions. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

constructor/function injection keeps runtime infrastructure replaceable and testable
...
```typescript
interface Repository<T> { find(id:string): Promise<T | undefined> }
```
...
In production TypeScript, apply it to inject repository/http client ports.
...
- Reproduce the smallest case involving **Dependency Inversion Principle (DIP)**.
...
4. What is a common misuse of **Dependency Inversion Principle (DIP)**?
5. Where would you use **Dependency Inversion Principle (DIP)** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Dependency Inversion Principle (DIP)**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
