...
**Factory Method** is delegates creation through a typed factory operation.

---

# 2. Why

Interviewers use **Factory Method** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Factory Method** means delegates creation through a typed factory operation. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

the client depends on a creation contract and not a concrete constructor
...
```typescript
interface Parser { parse(s:string): unknown };
const createParser = (kind:'json'|'text'): Parser => kind==='json' ? {parse: JSON.parse} : {parse:s=>s};
```
...
In production TypeScript, use it for provider/parser/client creation; reject it when it only adds ceremony.
...
- Reproduce the smallest case involving **Factory Method**.
...
4. What is a common misuse of **Factory Method**?
5. Where would you use **Factory Method** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Factory Method**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
