...
**Prototype** is creates variants by copying or delegating from an existing value.

---

# 2. Why

Interviewers use **Prototype** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Prototype** means creates variants by copying or delegating from an existing value. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

TypeScript types can describe copy operations but JavaScript determines shallow/deep runtime semantics
...
```typescript
type Config = Readonly<{retries:number;timeout:number}>;
const next: Config = {...base, timeout:5000};
```
...
In production TypeScript, use it for configuration templates; reject it when it only adds ceremony.
...
- Reproduce the smallest case involving **Prototype**.
...
4. What is a common misuse of **Prototype**?
5. Where would you use **Prototype** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Prototype**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
