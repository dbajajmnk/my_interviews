...
**Adapter** is translates an incompatible API into your application's typed port.

---

# 2. Why

Interviewers use **Adapter** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Adapter** means translates an incompatible API into your application's typed port. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

wrapper maps arguments/results/errors and prevents vendor types leaking inward
...
```typescript
class LegacyAdapter implements PaymentPort { async charge(a:Amount):Promise<Receipt>{ /* map */ throw new Error(); } }
```
...
In production TypeScript, use it for third-party integrations; reject it when it only adds ceremony.
...
- Reproduce the smallest case involving **Adapter**.
...
4. What is a common misuse of **Adapter**?
5. Where would you use **Adapter** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Adapter**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
