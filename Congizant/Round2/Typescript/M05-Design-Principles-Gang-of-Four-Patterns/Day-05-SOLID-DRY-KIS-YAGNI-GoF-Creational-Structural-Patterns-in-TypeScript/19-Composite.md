...
**Composite** is treats leaves and groups through one typed interface.

---

# 2. Why

Interviewers use **Composite** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Composite** means treats leaves and groups through one typed interface. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

composite recursively delegates the same operation to children
...
```typescript
interface Rule { valid(c:Context): boolean }
class AllRules implements Rule { constructor(private rules:Rule[]){} valid(c:Context){ return this.rules.every(r=>r.valid(c)); } }
```
...
In production TypeScript, use it for rule trees/hierarchies; reject it when it only adds ceremony.
...
- Reproduce the smallest case involving **Composite**.
...
4. What is a common misuse of **Composite**?
5. Where would you use **Composite** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Composite**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
