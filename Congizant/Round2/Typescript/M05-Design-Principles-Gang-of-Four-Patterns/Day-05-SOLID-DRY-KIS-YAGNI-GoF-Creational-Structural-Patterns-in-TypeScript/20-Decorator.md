...
**Decorator** is adds behavior by wrapping an object/function with the same typed contract.

---

# 2. Why

Interviewers use **Decorator** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Decorator** means adds behavior by wrapping an object/function with the same typed contract. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

decorator delegates while adding metrics/cache/auth/retry around it
...
```typescript
const withMetrics = <A extends unknown[],R>(fn:(...a:A)=>Promise<R>) => async(...a:A):Promise<R> => fn(...a);
```
...
In production TypeScript, use it for cross-cutting behavior; reject it when it only adds ceremony.
...
- Reproduce the smallest case involving **Decorator**.
...
4. What is a common misuse of **Decorator**?
5. Where would you use **Decorator** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Decorator**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
