...
**Facade** is provides a simpler typed entry point over a subsystem.

---

# 2. Why

Interviewers use **Facade** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Facade** means provides a simpler typed entry point over a subsystem. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

facade coordinates internal collaborators while exposing a cohesive use-case contract
...
```typescript
class CheckoutFacade { async checkout(order:Order):Promise<Receipt>{ /* coordinate */ throw new Error(); } }
```
...
In production TypeScript, use it for application/service orchestration; reject it when it only adds ceremony.
...
- Reproduce the smallest case involving **Facade**.
...
4. What is a common misuse of **Facade**?
5. Where would you use **Facade** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Facade**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
