...
**Flyweight** is shares reusable intrinsic state to reduce repeated memory.

---

# 2. Why

Interviewers use **Flyweight** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Flyweight** means shares reusable intrinsic state to reduce repeated memory. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

shared immutable typed values are reused while extrinsic state remains outside
...
```typescript
type ProductType = Readonly<{code:string}>;
...
In production TypeScript, use it for high-volume repeated metadata; reject it when it only adds ceremony.
...
- Reproduce the smallest case involving **Flyweight**.
...
4. What is a common misuse of **Flyweight**?
5. Where would you use **Flyweight** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Flyweight**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
