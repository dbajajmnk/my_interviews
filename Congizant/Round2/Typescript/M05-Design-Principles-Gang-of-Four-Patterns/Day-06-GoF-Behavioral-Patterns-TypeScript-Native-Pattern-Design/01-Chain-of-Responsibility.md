...
**Chain of Responsibility** is passes typed input through ordered handlers.

---

# 2. Why

Interviewers use **Chain of Responsibility** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Chain of Responsibility** means passes typed input through ordered handlers. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

each handler processes/rejects/enriches then delegates or returns
...
```typescript
type Handler = (req:Req,next:()=>Promise<Res>)=>Promise<Res>;
```
...
In production TypeScript, use it for middleware/validation pipelines; reject it when it only adds ceremony.
...
- Reproduce the smallest case involving **Chain of Responsibility**.
...
4. What is a common misuse of **Chain of Responsibility**?
5. Where would you use **Chain of Responsibility** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Chain of Responsibility**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
