...
**Command** is represents an action as a typed request object.

---

# 2. Why

Interviewers use **Command** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Command** means represents an action as a typed request object. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

discriminated command types enable routing, audit, queueing, and exhaustive handling
...
```typescript
type Command = {type:'CreateOrder';payload:CreateOrder} | {type:'CancelOrder';payload:{id:OrderId}};
```
...
In production TypeScript, use it for application commands/jobs; reject it when it only adds ceremony.
...
- Reproduce the smallest case involving **Command**.
...
4. What is a common misuse of **Command**?
5. Where would you use **Command** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Command**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
