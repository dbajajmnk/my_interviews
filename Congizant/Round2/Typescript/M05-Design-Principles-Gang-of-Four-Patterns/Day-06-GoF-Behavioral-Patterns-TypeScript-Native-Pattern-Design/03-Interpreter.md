...
**Interpreter** is models and evaluates a small typed expression language.

---

# 2. Why

Interviewers use **Interpreter** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Interpreter** means models and evaluates a small typed expression language. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

a discriminated AST plus exhaustive evaluator is a TypeScript-native implementation
...
```typescript
type Expr = {kind:'eq';field:string;value:unknown} | {kind:'and';items:Expr[]};
```
...
In production TypeScript, use it for small rule/filter DSLs; reject it when it only adds ceremony.
...
- Reproduce the smallest case involving **Interpreter**.
...
4. What is a common misuse of **Interpreter**?
5. Where would you use **Interpreter** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Interpreter**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
