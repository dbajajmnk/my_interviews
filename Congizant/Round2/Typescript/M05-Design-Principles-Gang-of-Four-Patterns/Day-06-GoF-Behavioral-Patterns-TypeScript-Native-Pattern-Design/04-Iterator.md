...
**Iterator** is provides typed sequential traversal.

---

# 2. Why

Interviewers use **Iterator** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Iterator** means provides typed sequential traversal. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

Iterable<T>/Iterator<T> describe the runtime iterator protocol
...
```typescript
function* range():Iterable<number>{ yield 1; yield 2; }
```
...
In production TypeScript, use it for lazy/custom traversal; reject it when it only adds ceremony.
...
- Reproduce the smallest case involving **Iterator**.
...
4. What is a common misuse of **Iterator**?
5. Where would you use **Iterator** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Iterator**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
