...
**State** is models state-dependent behavior explicitly.

---

# 2. Why

Interviewers use **State** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **State** means models state-dependent behavior explicitly. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

discriminated unions enable exhaustive state transitions without class hierarchy
...
```typescript
type OrderState={kind:'pending'}|{kind:'paid';receiptId:string}|{kind:'cancelled'};
```
...
In production TypeScript, use it for workflow/order lifecycle; reject it when it only adds ceremony.
...
- Reproduce the smallest case involving **State**.
...
4. What is a common misuse of **State**?
5. Where would you use **State** in a real system?
...
Create the smallest strict TypeScript example demonstrating **State**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
