...
**Strategy** is encapsulates interchangeable algorithms behind a typed function/interface.

---

# 2. Why

Interviewers use **Strategy** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Strategy** means encapsulates interchangeable algorithms behind a typed function/interface. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

function types make Strategy lightweight and inference-friendly
...
```typescript
type PricingStrategy=(o:Order)=>Money;
```
...
In production TypeScript, use it for pricing/routing/validation; reject it when it only adds ceremony.
...
- Reproduce the smallest case involving **Strategy**.
...
4. What is a common misuse of **Strategy**?
5. Where would you use **Strategy** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Strategy**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
