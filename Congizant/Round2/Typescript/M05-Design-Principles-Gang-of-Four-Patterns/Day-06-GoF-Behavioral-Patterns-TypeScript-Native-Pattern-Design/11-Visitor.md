...
**Visitor** is adds operations across a stable typed structure.

---

# 2. Why

Interviewers use **Visitor** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Visitor** means adds operations across a stable typed structure. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

discriminated unions or visitor interfaces can express exhaustive operations; choose the simpler model
...
```typescript
type Node={kind:'text';value:string}|{kind:'image';url:string};
```
...
In production TypeScript, use it for ASTs/stable hierarchies; reject it when it only adds ceremony.
...
- Reproduce the smallest case involving **Visitor**.
...
4. What is a common misuse of **Visitor**?
5. Where would you use **Visitor** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Visitor**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
