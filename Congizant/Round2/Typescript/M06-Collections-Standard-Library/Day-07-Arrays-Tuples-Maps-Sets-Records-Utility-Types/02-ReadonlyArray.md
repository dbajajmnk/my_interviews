...
**ReadonlyArray** is the TypeScript concept **ReadonlyArray**.

---

# 2. Why

Interviewers use **ReadonlyArray** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **ReadonlyArray** means the TypeScript concept **ReadonlyArray**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **ReadonlyArray** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **ReadonlyArray**.
...
1. What is **ReadonlyArray**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **ReadonlyArray**?
5. Where would you use **ReadonlyArray** in a real system?
...
Create the smallest strict TypeScript example demonstrating **ReadonlyArray**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **ReadonlyArray** in plain English.
