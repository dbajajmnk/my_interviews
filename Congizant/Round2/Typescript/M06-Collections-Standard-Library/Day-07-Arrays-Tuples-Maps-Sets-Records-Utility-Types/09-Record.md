...
**Record** is mapping key union to a value type.

---

# 2. Why

Interviewers use **Record** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Record** means mapping key union to a value type. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

`Record<K,V>` is a mapped utility type and does not create a runtime Map
...
In real TypeScript systems, this matters for dictionary-like object shapes.
...
- Reproduce the smallest case involving **Record**.
...
4. What is a common misuse of **Record**?
5. Where would you use **Record** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Record**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Record** in plain English.
