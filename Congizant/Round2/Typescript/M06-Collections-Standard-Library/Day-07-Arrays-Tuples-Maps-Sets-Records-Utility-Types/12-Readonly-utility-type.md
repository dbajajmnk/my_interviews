# Topic 12 — Readonly utility type
...
**Readonly utility type** is making properties readonly at one mapped level.

---

# 2. Why

Interviewers use **Readonly utility type** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Readonly utility type** means making properties readonly at one mapped level. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

compile-time readonly does not recursively freeze runtime objects
...
In real TypeScript systems, this matters for read-only API surfaces.
...
- Reproduce the smallest case involving **Readonly utility type**.
...
1. What is **Readonly utility type**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Readonly utility type**?
5. Where would you use **Readonly utility type** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Readonly utility type**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Readonly utility type** in plain English.
