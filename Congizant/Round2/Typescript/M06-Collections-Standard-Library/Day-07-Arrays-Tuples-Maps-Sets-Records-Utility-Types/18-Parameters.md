...
**Parameters** is extracting a function's parameter tuple.

---

# 2. Why

Interviewers use **Parameters** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Parameters** means extracting a function's parameter tuple. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
- Reproduce the smallest case involving **Parameters**.
...
4. What is a common misuse of **Parameters**?
5. Where would you use **Parameters** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Parameters**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Parameters** in plain English.
