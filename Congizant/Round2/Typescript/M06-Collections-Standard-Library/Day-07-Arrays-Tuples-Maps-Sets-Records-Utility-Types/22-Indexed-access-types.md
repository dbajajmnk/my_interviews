...
**Indexed access types** is looking up a property value type by key type.

---

# 2. Why

Interviewers use **Indexed access types** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Indexed access types** means looking up a property value type by key type. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

`T[K]` relates keys to exact property types
...
- Reproduce the smallest case involving **Indexed access types**.
...
1. What is **Indexed access types**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Indexed access types**?
5. Where would you use **Indexed access types** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Indexed access types**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Indexed access types** in plain English.
