...
**Optional chaining** is the TypeScript concept **Optional chaining**.

---

# 2. Why

Interviewers use **Optional chaining** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Optional chaining** means the TypeScript concept **Optional chaining**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Optional chaining** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Optional chaining**.
...
4. What is a common misuse of **Optional chaining**?
5. Where would you use **Optional chaining** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Optional chaining**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
