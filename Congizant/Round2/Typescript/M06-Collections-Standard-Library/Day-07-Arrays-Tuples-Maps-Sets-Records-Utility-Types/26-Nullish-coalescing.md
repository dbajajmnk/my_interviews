...
**Nullish coalescing** is the TypeScript concept **Nullish coalescing**.

---

# 2. Why

Interviewers use **Nullish coalescing** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Nullish coalescing** means the TypeScript concept **Nullish coalescing**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Nullish coalescing** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Nullish coalescing**.
...
4. What is a common misuse of **Nullish coalescing**?
5. Where would you use **Nullish coalescing** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Nullish coalescing**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
