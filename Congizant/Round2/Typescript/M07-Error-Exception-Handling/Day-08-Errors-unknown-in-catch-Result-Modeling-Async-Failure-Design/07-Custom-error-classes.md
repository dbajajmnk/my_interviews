...
**Custom error classes** is typing JavaScript error handling without assuming every thrown value is Error.

---

# 2. Why

Interviewers use **Custom error classes** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Custom error classes** means typing JavaScript error handling without assuming every thrown value is Error. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
- Reproduce the smallest case involving **Custom error classes**.
...
1. What is **Custom error classes**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Custom error classes**?
5. Where would you use **Custom error classes** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Custom error classes**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Custom error classes** in plain English.
