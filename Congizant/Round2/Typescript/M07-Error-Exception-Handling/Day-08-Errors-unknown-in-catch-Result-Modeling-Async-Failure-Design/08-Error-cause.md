...
**Error cause** is typing JavaScript error handling without assuming every thrown value is Error.

---

# 2. Why

Interviewers use **Error cause** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Error cause** means typing JavaScript error handling without assuming every thrown value is Error. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
- Reproduce the smallest case involving **Error cause**.
...
4. What is a common misuse of **Error cause**?
5. Where would you use **Error cause** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Error cause**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
