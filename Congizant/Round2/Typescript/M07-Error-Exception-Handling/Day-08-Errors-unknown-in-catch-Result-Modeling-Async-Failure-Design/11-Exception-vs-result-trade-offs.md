# Topic 11 — Exception vs result trade-offs
...
**Exception vs result trade-offs** is the TypeScript concept **Exception vs result trade-offs**.

---

# 2. Why

Interviewers use **Exception vs result trade-offs** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Exception vs result trade-offs** means the TypeScript concept **Exception vs result trade-offs**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Exception vs result trade-offs** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Exception vs result trade-offs**.
...
1. What is **Exception vs result trade-offs**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Exception vs result trade-offs**?
5. Where would you use **Exception vs result trade-offs** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Exception vs result trade-offs**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Exception vs result trade-offs** in plain English.
