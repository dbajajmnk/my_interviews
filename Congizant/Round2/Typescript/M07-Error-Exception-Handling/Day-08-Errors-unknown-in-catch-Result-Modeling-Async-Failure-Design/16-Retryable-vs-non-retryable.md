# Topic 16 — Retryable vs non-retryable
...
**Retryable vs non-retryable** is bounded repeated attempts for transient failure.

---

# 2. Why

Interviewers use **Retryable vs non-retryable** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Retryable vs non-retryable** means bounded repeated attempts for transient failure. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

typed errors can classify retryability, but idempotency and backoff are runtime/business concerns
...
In real TypeScript systems, this matters for resilience.
...
- Reproduce the smallest case involving **Retryable vs non-retryable**.
...
1. What is **Retryable vs non-retryable**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Retryable vs non-retryable**?
5. Where would you use **Retryable vs non-retryable** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Retryable vs non-retryable**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Retryable vs non-retryable** in plain English.
