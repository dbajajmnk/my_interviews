# Topic 17 — Logging errors safely
...
**Logging errors safely** is typing JavaScript error handling without assuming every thrown value is Error.

---

# 2. Why

Interviewers use **Logging errors safely** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Logging errors safely** means typing JavaScript error handling without assuming every thrown value is Error. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
- Reproduce the smallest case involving **Logging errors safely**.
...
4. What is a common misuse of **Logging errors safely**?
5. Where would you use **Logging errors safely** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Logging errors safely**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
