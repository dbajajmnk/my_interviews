...
**ES modules** is the TypeScript concept **ES modules**.

---

# 2. Why

Interviewers use **ES modules** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **ES modules** means the TypeScript concept **ES modules**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **ES modules** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **ES modules**.
...
4. What is a common misuse of **ES modules**?
5. Where would you use **ES modules** in a real system?
...
Create the smallest strict TypeScript example demonstrating **ES modules**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
