...
**Path aliases** is compiler-side module specifier mapping.

---

# 2. Why

Interviewers use **Path aliases** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Path aliases** means compiler-side module specifier mapping. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

paths influence type/module resolution but runtime must have matching resolver/bundler/package mapping
...
In real TypeScript systems, this matters for project ergonomics.
...
- Reproduce the smallest case involving **Path aliases**.
...
1. What is **Path aliases**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Path aliases**?
5. Where would you use **Path aliases** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Path aliases**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Path aliases** in plain English.
