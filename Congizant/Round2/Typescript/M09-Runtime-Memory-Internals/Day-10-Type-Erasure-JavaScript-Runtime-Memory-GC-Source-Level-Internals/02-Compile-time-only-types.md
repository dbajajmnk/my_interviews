# Topic 02 — Compile-time only types
...
**Compile-time only types** is the TypeScript concept **Compile-time only types**.

---

# 2. Why

Interviewers use **Compile-time only types** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Compile-time only types** means the TypeScript concept **Compile-time only types**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Compile-time only types** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Compile-time only types**.
...
1. What is **Compile-time only types**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Compile-time only types**?
5. Where would you use **Compile-time only types** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Compile-time only types**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Compile-time only types** in plain English.
