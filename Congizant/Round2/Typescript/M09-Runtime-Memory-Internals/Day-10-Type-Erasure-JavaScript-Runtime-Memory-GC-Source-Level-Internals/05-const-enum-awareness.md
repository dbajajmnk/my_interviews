...
**const enum awareness** is TypeScript enum with runtime representation unless const-erased/configured.

---

# 2. Why

Interviewers use **const enum awareness** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **const enum awareness** means TypeScript enum with runtime representation unless const-erased/configured. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

regular enums emit JavaScript objects; string/numeric behavior differs and may affect bundles/interoperability
...
In real TypeScript systems, this matters for runtime constants with trade-offs.
...
- Reproduce the smallest case involving **const enum awareness**.
...
1. What is **const enum awareness**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **const enum awareness**?
5. Where would you use **const enum awareness** in a real system?
...
Create the smallest strict TypeScript example demonstrating **const enum awareness**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **const enum awareness** in plain English.
