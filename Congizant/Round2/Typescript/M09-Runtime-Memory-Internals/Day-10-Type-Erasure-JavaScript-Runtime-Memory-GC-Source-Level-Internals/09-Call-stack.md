...
**Call stack** is runtime stack of active synchronous JavaScript calls.

---

# 2. Why

Interviewers use **Call stack** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Call stack** means runtime stack of active synchronous JavaScript calls. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

deep recursion or synchronous blocking behaves exactly as JavaScript after emit
...
In real TypeScript systems, this matters for runtime failures.
...
- Reproduce the smallest case involving **Call stack**.
...
4. What is a common misuse of **Call stack**?
5. Where would you use **Call stack** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Call stack**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
