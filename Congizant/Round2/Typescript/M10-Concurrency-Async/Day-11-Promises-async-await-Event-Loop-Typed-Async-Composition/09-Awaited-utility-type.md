...
**Awaited utility type** is recursively unwrapping Promise-like result types.

---

# 2. Why

Interviewers use **Awaited utility type** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Awaited utility type** means recursively unwrapping Promise-like result types. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

utility models await resolution at type level
...
```typescript
type Value=Awaited<Promise<Promise<number>>>;
```
...
In real TypeScript systems, this matters for async helper types.
...
- Reproduce the smallest case involving **Awaited utility type**.
...
1. What is **Awaited utility type**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Awaited utility type**?
5. Where would you use **Awaited utility type** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Awaited utility type**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Awaited utility type** in plain English.
