# Topic 11 — Async error typing limitations
...
**Async error typing limitations** is typing JavaScript error handling without assuming every thrown value is Error.

---

# 2. Why

Interviewers use **Async error typing limitations** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Async error typing limitations** means typing JavaScript error handling without assuming every thrown value is Error. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

JavaScript permits throwing any value; safe catch code starts from unknown and narrows
...
```typescript
catch (err: unknown) { if (err instanceof Error) console.error(err.message); }
```
...
In real TypeScript systems, this matters for robust failures.
...
- Reproduce the smallest case involving **Async error typing limitations**.
...
1. What is **Async error typing limitations**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Async error typing limitations**?
5. Where would you use **Async error typing limitations** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Async error typing limitations**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Async error typing limitations** in plain English.
