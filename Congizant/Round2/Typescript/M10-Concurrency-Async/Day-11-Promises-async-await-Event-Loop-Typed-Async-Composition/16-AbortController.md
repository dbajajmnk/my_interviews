...
**AbortController** is runtime cooperative cancellation API with TypeScript declarations.

---

# 2. Why

Interviewers use **AbortController** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **AbortController** means runtime cooperative cancellation API with TypeScript declarations. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

AbortSignal is a real host object; types only describe it
...
In real TypeScript systems, this matters for request cancellation.
...
- Reproduce the smallest case involving **AbortController**.
...
4. What is a common misuse of **AbortController**?
5. Where would you use **AbortController** in a real system?
...
Create the smallest strict TypeScript example demonstrating **AbortController**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
