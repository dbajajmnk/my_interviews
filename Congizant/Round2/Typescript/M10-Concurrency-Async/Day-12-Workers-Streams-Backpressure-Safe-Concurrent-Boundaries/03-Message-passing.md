...
**Message passing** is the TypeScript concept **Message passing**.

---

# 2. Why

Interviewers use **Message passing** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Message passing** means the TypeScript concept **Message passing**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Message passing** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Message passing**.
...
4. What is a common misuse of **Message passing**?
5. Where would you use **Message passing** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Message passing**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Message passing** in plain English.
