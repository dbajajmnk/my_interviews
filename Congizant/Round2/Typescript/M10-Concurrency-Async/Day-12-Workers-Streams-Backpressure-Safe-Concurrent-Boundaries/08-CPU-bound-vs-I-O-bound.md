# Topic 08 — CPU-bound vs I/O-bound
...
**CPU-bound vs I/O-bound** is the TypeScript concept **CPU-bound vs I/O-bound**.

---

# 2. Why

Interviewers use **CPU-bound vs I/O-bound** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **CPU-bound vs I/O-bound** means the TypeScript concept **CPU-bound vs I/O-bound**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **CPU-bound vs I/O-bound** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **CPU-bound vs I/O-bound**.
...
4. What is a common misuse of **CPU-bound vs I/O-bound**?
5. Where would you use **CPU-bound vs I/O-bound** in a real system?
...
Create the smallest strict TypeScript example demonstrating **CPU-bound vs I/O-bound**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
