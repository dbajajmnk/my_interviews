...
**Backpressure** is bounding production of work against consumer capacity.

---

# 2. Why

Interviewers use **Backpressure** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Backpressure** means bounding production of work against consumer capacity. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

types can model stream chunks/messages but runtime capacity must be controlled with streams/queues/semaphores
...
```typescript
// await drain / limit in-flight work
```
...
In real TypeScript systems, this matters for memory/reliability.
...
- Reproduce the smallest case involving **Backpressure**.
...
4. What is a common misuse of **Backpressure**?
5. Where would you use **Backpressure** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Backpressure**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
