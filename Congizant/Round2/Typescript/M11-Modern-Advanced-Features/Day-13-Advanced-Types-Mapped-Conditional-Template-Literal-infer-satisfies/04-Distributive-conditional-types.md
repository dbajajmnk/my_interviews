# Topic 04 — Distributive conditional types
...
**Distributive conditional types** is type-level branch based on assignability.

---

# 2. Why

Interviewers use **Distributive conditional types** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Distributive conditional types** means type-level branch based on assignability. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

`T extends U ? X : Y` can distribute over naked type parameters in unions
...
In real TypeScript systems, this matters for generic libraries.
...
- Reproduce the smallest case involving **Distributive conditional types**.
...
1. What is **Distributive conditional types**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Distributive conditional types**?
5. Where would you use **Distributive conditional types** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Distributive conditional types**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Distributive conditional types** in plain English.
