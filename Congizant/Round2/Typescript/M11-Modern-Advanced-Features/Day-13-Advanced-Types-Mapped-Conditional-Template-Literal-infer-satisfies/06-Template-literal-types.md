# Topic 06 — Template literal types
...
**Template literal types** is type representing an exact literal value.

---

# 2. Why

Interviewers use **Template literal types** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Template literal types** means type representing an exact literal value. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

literal types combine into discriminated unions and precise configuration contracts
...
In real TypeScript systems, this matters for state/config modeling.
...
- Reproduce the smallest case involving **Template literal types**.
...
1. What is **Template literal types**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Template literal types**?
5. Where would you use **Template literal types** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Template literal types**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Template literal types** in plain English.
