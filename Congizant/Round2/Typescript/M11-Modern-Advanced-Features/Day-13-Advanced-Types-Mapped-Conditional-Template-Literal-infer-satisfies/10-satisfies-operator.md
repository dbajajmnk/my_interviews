...
**satisfies operator** is checking assignability while preserving a more specific inferred type.

---

# 2. Why

Interviewers use **satisfies operator** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **satisfies operator** means checking assignability while preserving a more specific inferred type. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

`expr satisfies T` validates compatibility without replacing the expression's inferred type with T
...
```typescript
const cfg={mode:'prod'} satisfies Config;
```
...
In real TypeScript systems, this matters for config correctness.
...
- Reproduce the smallest case involving **satisfies operator**.
...
1. What is **satisfies operator**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **satisfies operator**?
5. Where would you use **satisfies operator** in a real system?
...
Create the smallest strict TypeScript example demonstrating **satisfies operator**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **satisfies operator** in plain English.
