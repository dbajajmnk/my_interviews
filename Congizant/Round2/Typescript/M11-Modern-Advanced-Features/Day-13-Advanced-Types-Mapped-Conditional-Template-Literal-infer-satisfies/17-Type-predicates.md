...
**Type predicates** is user-defined function that narrows a parameter.

---

# 2. Why

Interviewers use **Type predicates** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Type predicates** means user-defined function that narrows a parameter. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

`x is T` is trusted by compiler; implementation must truly validate or it becomes an unsound escape hatch
...
```typescript
function isUser(x:unknown): x is User { return typeof x==='object' && x!==null && 'id' in x; }
```
...
In real TypeScript systems, this matters for runtime guards.
...
- Reproduce the smallest case involving **Type predicates**.
...
1. What is **Type predicates**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Type predicates**?
5. Where would you use **Type predicates** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Type predicates**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Type predicates** in plain English.
