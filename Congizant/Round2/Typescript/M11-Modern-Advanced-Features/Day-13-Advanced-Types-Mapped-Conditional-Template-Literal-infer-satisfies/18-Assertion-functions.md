...
**Assertion functions** is function declaring that a condition/value holds after successful return.

---

# 2. Why

Interviewers use **Assertion functions** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Assertion functions** means function declaring that a condition/value holds after successful return. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

`asserts` updates control-flow analysis but implementation is responsible for runtime checking
...
```typescript
function assertUser(x:unknown): asserts x is User { if(!isUser(x)) throw new Error('bad'); }
```
...
In real TypeScript systems, this matters for validated boundaries.
...
- Reproduce the smallest case involving **Assertion functions**.
...
1. What is **Assertion functions**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Assertion functions**?
5. Where would you use **Assertion functions** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Assertion functions**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Assertion functions** in plain English.
