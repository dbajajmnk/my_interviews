...
**Declaration merging** is describing public TypeScript types in `.d.ts` output.

---

# 2. Why

Interviewers use **Declaration merging** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Declaration merging** means describing public TypeScript types in `.d.ts` output. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
- Reproduce the smallest case involving **Declaration merging**.
...
4. What is a common misuse of **Declaration merging**?
5. Where would you use **Declaration merging** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Declaration merging**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Declaration merging** in plain English.
