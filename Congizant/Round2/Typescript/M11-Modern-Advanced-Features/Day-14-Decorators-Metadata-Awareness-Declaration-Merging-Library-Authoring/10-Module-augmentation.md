...
**Module augmentation** is adding static declarations to an existing module's types.

---

# 2. Why

Interviewers use **Module augmentation** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Module augmentation** means adding static declarations to an existing module's types. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

compiler merges augmentations; runtime implementation must actually provide the behavior
...
```typescript
declare module 'lib' { interface Options { traceId?: string } }
```
...
In real TypeScript systems, this matters for library extension.
...
- Reproduce the smallest case involving **Module augmentation**.
...
1. What is **Module augmentation**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Module augmentation**?
5. Where would you use **Module augmentation** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Module augmentation**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Module augmentation** in plain English.
