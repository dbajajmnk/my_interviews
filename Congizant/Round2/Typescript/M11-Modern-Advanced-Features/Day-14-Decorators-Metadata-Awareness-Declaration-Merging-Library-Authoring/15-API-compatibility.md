...
**API compatibility** is the TypeScript concept **API compatibility**.

---

# 2. Why

Interviewers use **API compatibility** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **API compatibility** means the TypeScript concept **API compatibility**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **API compatibility** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **API compatibility**.
...
1. What is **API compatibility**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **API compatibility**?
5. Where would you use **API compatibility** in a real system?
...
Create the smallest strict TypeScript example demonstrating **API compatibility**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **API compatibility** in plain English.
