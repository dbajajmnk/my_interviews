# Topic 01 — JSON.parse returns runtime values
...
**JSON.parse returns runtime values** is runtime parsing that does not validate domain shape.

---

# 2. Why

Interviewers use **JSON.parse returns runtime values** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **JSON.parse returns runtime values** means runtime parsing that does not validate domain shape. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

JSON.parse returns JavaScript values; TypeScript annotation/assertion after parse does not prove structure
...
In real TypeScript systems, this matters for trust boundaries.
...
- Reproduce the smallest case involving **JSON.parse returns runtime values**.
...
1. What is **JSON.parse returns runtime values**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **JSON.parse returns runtime values**?
5. Where would you use **JSON.parse returns runtime values** in a real system?
...
Create the smallest strict TypeScript example demonstrating **JSON.parse returns runtime values**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **JSON.parse returns runtime values** in plain English.
