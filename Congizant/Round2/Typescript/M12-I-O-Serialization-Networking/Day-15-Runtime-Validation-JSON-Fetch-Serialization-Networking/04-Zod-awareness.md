...
**Zod awareness** is runtime schema validation library awareness.

---

# 2. Why

Interviewers use **Zod awareness** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Zod awareness** means runtime schema validation library awareness. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

Zod schemas execute at runtime and can infer TypeScript static types; package/version APIs must be verified
...
```typescript
const User=z.object({id:z.string()}); type User=z.infer<typeof User>;
```
...
In real TypeScript systems, this matters for validated APIs.
...
- Reproduce the smallest case involving **Zod awareness**.
...
1. What is **Zod awareness**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Zod awareness**?
5. Where would you use **Zod awareness** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Zod awareness**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Zod awareness** in plain English.
