...
**AbortSignal** is the TypeScript concept **AbortSignal**.

---

# 2. Why

Interviewers use **AbortSignal** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **AbortSignal** means the TypeScript concept **AbortSignal**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **AbortSignal** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **AbortSignal**.
...
1. What is **AbortSignal**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **AbortSignal**?
5. Where would you use **AbortSignal** in a real system?
...
Create the smallest strict TypeScript example demonstrating **AbortSignal**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **AbortSignal** in plain English.
