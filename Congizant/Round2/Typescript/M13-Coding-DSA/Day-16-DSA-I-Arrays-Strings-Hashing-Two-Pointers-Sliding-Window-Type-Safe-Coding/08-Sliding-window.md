...
**Sliding window** is incremental contiguous-range algorithm.

---

# 2. Why

Interviewers use **Sliding window** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Sliding window** means incremental contiguous-range algorithm. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

maintains counts/sums between left/right positions rather than recomputing each window
...
- Reproduce the smallest case involving **Sliding window**.
...
4. What is a common misuse of **Sliding window**?
5. Where would you use **Sliding window** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Sliding window**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
