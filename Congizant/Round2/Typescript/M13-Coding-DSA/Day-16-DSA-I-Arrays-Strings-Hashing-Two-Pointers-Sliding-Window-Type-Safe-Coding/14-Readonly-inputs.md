...
**Readonly inputs** is the TypeScript concept **Readonly inputs**.

---

# 2. Why

Interviewers use **Readonly inputs** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Readonly inputs** means the TypeScript concept **Readonly inputs**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Readonly inputs** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Readonly inputs**.
...
1. What is **Readonly inputs**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Readonly inputs**?
5. Where would you use **Readonly inputs** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Readonly inputs**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Readonly inputs** in plain English.
