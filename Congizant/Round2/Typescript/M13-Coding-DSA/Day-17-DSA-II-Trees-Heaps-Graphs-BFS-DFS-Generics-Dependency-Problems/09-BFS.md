...
Interviewers use **BFS** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **BFS** means breadth-first traversal. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

queue + visited processes graph/tree by layers; TypeScript can preserve node ID types
...
- Reproduce the smallest case involving **BFS**.
...
4. What is a common misuse of **BFS**?
5. Where would you use **BFS** in a real system?
...
Create the smallest strict TypeScript example demonstrating **BFS**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
