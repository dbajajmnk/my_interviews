...
**Compiler diagnostics** is the `tsc` type-checking and emit pipeline.

---

# 2. Why

Interviewers use **Compiler diagnostics** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Compiler diagnostics** means the `tsc` type-checking and emit pipeline. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
- Reproduce the smallest case involving **Compiler diagnostics**.
...
1. What is **Compiler diagnostics**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Compiler diagnostics**?
5. Where would you use **Compiler diagnostics** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Compiler diagnostics**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Compiler diagnostics** in plain English.
