# Topic 12 — Declaration conflicts
...
**Declaration conflicts** is describing public TypeScript types in `.d.ts` output.

---

# 2. Why

Interviewers use **Declaration conflicts** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Declaration conflicts** means describing public TypeScript types in `.d.ts` output. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
- Reproduce the smallest case involving **Declaration conflicts**.
...
1. What is **Declaration conflicts**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Declaration conflicts**?
5. Where would you use **Declaration conflicts** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Declaration conflicts**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Declaration conflicts** in plain English.
