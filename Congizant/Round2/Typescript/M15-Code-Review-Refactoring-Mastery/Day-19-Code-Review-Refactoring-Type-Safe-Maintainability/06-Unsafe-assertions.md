...
**Unsafe assertions** is casts/non-null assertions that suppress compiler evidence.

---

# 2. Why

Interviewers use **Unsafe assertions** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Unsafe assertions** means casts/non-null assertions that suppress compiler evidence. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

`as` and `!` do not add runtime checks; repeated use often marks a missing invariant/validator
...
In real TypeScript systems, this matters for code review.
...
- Reproduce the smallest case involving **Unsafe assertions**.
...
1. What is **Unsafe assertions**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Unsafe assertions**?
5. Where would you use **Unsafe assertions** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Unsafe assertions**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Unsafe assertions** in plain English.
