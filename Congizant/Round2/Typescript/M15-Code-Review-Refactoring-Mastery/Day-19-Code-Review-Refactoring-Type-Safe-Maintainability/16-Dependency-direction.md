...
**Dependency direction** is the TypeScript concept **Dependency direction**.

---

# 2. Why

Interviewers use **Dependency direction** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Dependency direction** means the TypeScript concept **Dependency direction**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Dependency direction** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Dependency direction**.
...
4. What is a common misuse of **Dependency direction**?
5. Where would you use **Dependency direction** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Dependency direction**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
