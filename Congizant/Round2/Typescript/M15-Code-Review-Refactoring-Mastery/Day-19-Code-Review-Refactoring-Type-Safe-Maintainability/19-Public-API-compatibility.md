# Topic 19 — Public API compatibility
...
**Public API compatibility** is public class member accessibility.

---

# 2. Why

Interviewers use **Public API compatibility** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Public API compatibility** means public class member accessibility. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
- Reproduce the smallest case involving **Public API compatibility**.
...
1. What is **Public API compatibility**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Public API compatibility**?
5. Where would you use **Public API compatibility** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Public API compatibility**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Public API compatibility** in plain English.
