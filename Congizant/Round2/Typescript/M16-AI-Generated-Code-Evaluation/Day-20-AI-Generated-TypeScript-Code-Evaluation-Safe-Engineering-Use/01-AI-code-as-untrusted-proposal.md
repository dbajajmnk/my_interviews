...
**AI code as untrusted proposal** is treating generated TypeScript as untrusted source.

---

# 2. Why

Interviewers use **AI code as untrusted proposal** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **AI code as untrusted proposal** means treating generated TypeScript as untrusted source. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

generated types can look impressive while hiding `any`, assertions, invented APIs, or absent runtime checks
...
```typescript
// run tsc --noEmit + tests + lint/security review
```
...
In real TypeScript systems, this matters for AI engineering.
...
- Reproduce the smallest case involving **AI code as untrusted proposal**.
...
4. What is a common misuse of **AI code as untrusted proposal**?
5. Where would you use **AI code as untrusted proposal** in a real system?
...
Create the smallest strict TypeScript example demonstrating **AI code as untrusted proposal**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
