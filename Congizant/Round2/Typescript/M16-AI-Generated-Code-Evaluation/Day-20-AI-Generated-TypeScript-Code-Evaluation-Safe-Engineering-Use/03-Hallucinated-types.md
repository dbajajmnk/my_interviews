...
**Hallucinated types** is invented interfaces/library types that do not correspond to real runtime contracts.

---

# 2. Why

Interviewers use **Hallucinated types** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Hallucinated types** means invented interfaces/library types that do not correspond to real runtime contracts. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

compiler may accept locally declared fake types even though external data/API does not match
...
```typescript
interface ImaginarySdkResponse { ok:true }
```
...
In real TypeScript systems, this matters for AI review.
...
- Reproduce the smallest case involving **Hallucinated types**.
...
1. What is **Hallucinated types**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Hallucinated types**?
5. Where would you use **Hallucinated types** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Hallucinated types**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Hallucinated types** in plain English.
