# Topic 20 — Accept repair reject rubric
...
**Accept repair reject rubric** is the TypeScript concept **Accept repair reject rubric**.

---

# 2. Why

Interviewers use **Accept repair reject rubric** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Accept repair reject rubric** means the TypeScript concept **Accept repair reject rubric**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Accept repair reject rubric** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Accept repair reject rubric**.
...
1. What is **Accept repair reject rubric**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Accept repair reject rubric**?
5. Where would you use **Accept repair reject rubric** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Accept repair reject rubric**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Accept repair reject rubric** in plain English.
