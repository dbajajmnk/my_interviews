...
**OpenAPI/schema tests** is machine-readable runtime API contract description.

---

# 2. Why

Interviewers use **OpenAPI/schema tests** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **OpenAPI/schema tests** means machine-readable runtime API contract description. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

OpenAPI schema can generate TS types/clients, but server/client runtime behavior still needs validation and compatibility testing
...
```typescript
// generate client from versioned OpenAPI schema
```
...
In real TypeScript systems, this matters for API contracts.
...
- Reproduce the smallest case involving **OpenAPI/schema tests**.
...
1. What is **OpenAPI/schema tests**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **OpenAPI/schema tests**?
5. Where would you use **OpenAPI/schema tests** in a real system?
...
Create the smallest strict TypeScript example demonstrating **OpenAPI/schema tests**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **OpenAPI/schema tests** in plain English.
