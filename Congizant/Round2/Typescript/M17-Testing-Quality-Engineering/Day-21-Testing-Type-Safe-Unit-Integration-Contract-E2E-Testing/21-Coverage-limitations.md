...
**Coverage limitations** is the TypeScript concept **Coverage limitations**.

---

# 2. Why

Interviewers use **Coverage limitations** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Coverage limitations** means the TypeScript concept **Coverage limitations**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Coverage limitations** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Coverage limitations**.
...
4. What is a common misuse of **Coverage limitations**?
5. Where would you use **Coverage limitations** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Coverage limitations**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
