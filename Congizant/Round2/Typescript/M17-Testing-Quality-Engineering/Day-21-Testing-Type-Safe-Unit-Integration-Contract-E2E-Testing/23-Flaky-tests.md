...
**Flaky tests** is the TypeScript concept **Flaky tests**.

---

# 2. Why

Interviewers use **Flaky tests** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Flaky tests** means the TypeScript concept **Flaky tests**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Flaky tests** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Flaky tests**.
...
4. What is a common misuse of **Flaky tests**?
5. Where would you use **Flaky tests** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Flaky tests**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
