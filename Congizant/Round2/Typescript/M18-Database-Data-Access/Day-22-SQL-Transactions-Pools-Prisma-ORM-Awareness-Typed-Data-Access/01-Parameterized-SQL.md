...
**Parameterized SQL** is typed application code still needs parameterized database execution.

---

# 2. Why

Interviewers use **Parameterized SQL** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Parameterized SQL** means typed application code still needs parameterized database execution. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

TypeScript prevents some local mistakes but cannot make string-concatenated SQL safe
...
In real TypeScript systems, this matters for SQL security.
...
- Reproduce the smallest case involving **Parameterized SQL**.
...
1. What is **Parameterized SQL**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Parameterized SQL**?
5. Where would you use **Parameterized SQL** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Parameterized SQL**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Parameterized SQL** in plain English.
