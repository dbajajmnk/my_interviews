# Topic 15 — Database types vs domain types
...
**Database types vs domain types** is separating persistence representation from domain/API contracts.

---

# 2. Why

Interviewers use **Database types vs domain types** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Database types vs domain types** means separating persistence representation from domain/API contracts. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

generated row/entity types reflect storage concerns that may not equal business invariants or public DTOs
...
```typescript
type OrderRow = { id:string; total_cents:number };
```
...
In real TypeScript systems, this matters for layered architecture.
...
- Reproduce the smallest case involving **Database types vs domain types**.
...
1. What is **Database types vs domain types**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Database types vs domain types**?
5. Where would you use **Database types vs domain types** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Database types vs domain types**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Database types vs domain types** in plain English.
