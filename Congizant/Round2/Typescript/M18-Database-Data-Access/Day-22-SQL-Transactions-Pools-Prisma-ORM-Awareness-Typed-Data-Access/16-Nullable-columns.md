...
**Nullable columns** is the TypeScript concept **Nullable columns**.

---

# 2. Why

Interviewers use **Nullable columns** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Nullable columns** means the TypeScript concept **Nullable columns**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Nullable columns** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Nullable columns**.
...
1. What is **Nullable columns**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Nullable columns**?
5. Where would you use **Nullable columns** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Nullable columns**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Nullable columns** in plain English.
