...
**Dependency injection** is supplying dependencies rather than constructing them inside policy code.

---

# 2. Why

Interviewers use **Dependency injection** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Dependency injection** means supplying dependencies rather than constructing them inside policy code. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
- Reproduce the smallest case involving **Dependency injection**.
...
4. What is a common misuse of **Dependency injection**?
5. Where would you use **Dependency injection** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Dependency injection**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Dependency injection** in plain English.
