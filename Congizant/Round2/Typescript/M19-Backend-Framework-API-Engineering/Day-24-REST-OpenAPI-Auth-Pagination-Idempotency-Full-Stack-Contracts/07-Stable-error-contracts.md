# Topic 07 — Stable error contracts
...
**Stable error contracts** is typing JavaScript error handling without assuming every thrown value is Error.

---

# 2. Why

Interviewers use **Stable error contracts** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Stable error contracts** means typing JavaScript error handling without assuming every thrown value is Error. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
- Reproduce the smallest case involving **Stable error contracts**.
...
4. What is a common misuse of **Stable error contracts**?
5. Where would you use **Stable error contracts** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Stable error contracts**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
