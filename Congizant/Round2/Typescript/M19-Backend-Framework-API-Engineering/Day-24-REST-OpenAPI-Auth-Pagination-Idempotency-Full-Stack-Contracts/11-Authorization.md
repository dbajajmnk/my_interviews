...
**Authorization** is checking principal permission on action/resource.

---

# 2. Why

Interviewers use **Authorization** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Authorization** means checking principal permission on action/resource. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
- Reproduce the smallest case involving **Authorization**.
...
4. What is a common misuse of **Authorization**?
5. Where would you use **Authorization** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Authorization**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
