# Topic 12 — Object-level authorization
...
**Object-level authorization** is checking principal permission on action/resource.

---

# 2. Why

Interviewers use **Object-level authorization** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Object-level authorization** means checking principal permission on action/resource. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

authorization is deterministic runtime policy; TypeScript can model roles/resources but cannot enforce by itself
...
```typescript
authorize(principal,'read',order);
```
...
In real TypeScript systems, this matters for security.
...
- Reproduce the smallest case involving **Object-level authorization**.
...
4. What is a common misuse of **Object-level authorization**?
5. Where would you use **Object-level authorization** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Object-level authorization**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
