# Topic 13 — OAuth2/OIDC awareness
...
**OAuth2/OIDC awareness** is the TypeScript concept **OAuth2/OIDC awareness**.

---

# 2. Why

Interviewers use **OAuth2/OIDC awareness** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **OAuth2/OIDC awareness** means the TypeScript concept **OAuth2/OIDC awareness**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **OAuth2/OIDC awareness** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **OAuth2/OIDC awareness**.
...
4. What is a common misuse of **OAuth2/OIDC awareness**?
5. Where would you use **OAuth2/OIDC awareness** in a real system?
...
Create the smallest strict TypeScript example demonstrating **OAuth2/OIDC awareness**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
