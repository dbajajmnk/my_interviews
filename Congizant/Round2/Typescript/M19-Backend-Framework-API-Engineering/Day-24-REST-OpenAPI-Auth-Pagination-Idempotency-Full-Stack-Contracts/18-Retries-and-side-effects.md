# Topic 18 — Retries and side effects
...
**Retries and side effects** is the TypeScript concept **Retries and side effects**.

---

# 2. Why

Interviewers use **Retries and side effects** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Retries and side effects** means the TypeScript concept **Retries and side effects**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Retries and side effects** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Retries and side effects**.
...
4. What is a common misuse of **Retries and side effects**?
5. Where would you use **Retries and side effects** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Retries and side effects**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
