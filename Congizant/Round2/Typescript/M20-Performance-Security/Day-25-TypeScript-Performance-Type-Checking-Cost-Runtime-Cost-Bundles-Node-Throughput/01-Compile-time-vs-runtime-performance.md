# Topic 01 — Compile-time vs runtime performance
...
**Compile-time vs runtime performance** is separating TypeScript checker cost from emitted JavaScript performance.

---

# 2. Why

Interviewers use **Compile-time vs runtime performance** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Compile-time vs runtime performance** means separating TypeScript checker cost from emitted JavaScript performance. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

advanced types can slow tsc/editor while disappearing entirely from runtime; emitted JS/runtime profile is separate
...
In real TypeScript systems, this matters for performance.
...
- Reproduce the smallest case involving **Compile-time vs runtime performance**.
...
1. What is **Compile-time vs runtime performance**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Compile-time vs runtime performance**?
5. Where would you use **Compile-time vs runtime performance** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Compile-time vs runtime performance**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Compile-time vs runtime performance** in plain English.
