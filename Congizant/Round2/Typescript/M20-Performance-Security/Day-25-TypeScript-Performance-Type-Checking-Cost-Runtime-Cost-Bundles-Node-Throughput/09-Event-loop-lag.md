...
**Event-loop lag** is the TypeScript concept **Event-loop lag**.

---

# 2. Why

Interviewers use **Event-loop lag** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Event-loop lag** means the TypeScript concept **Event-loop lag**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Event-loop lag** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Event-loop lag**.
...
4. What is a common misuse of **Event-loop lag**?
5. Where would you use **Event-loop lag** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Event-loop lag**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
