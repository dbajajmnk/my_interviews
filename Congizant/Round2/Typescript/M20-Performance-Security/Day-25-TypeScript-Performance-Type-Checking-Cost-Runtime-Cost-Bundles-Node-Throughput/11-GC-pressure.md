...
**GC pressure** is the TypeScript concept **GC pressure**.

---

# 2. Why

Interviewers use **GC pressure** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **GC pressure** means the TypeScript concept **GC pressure**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **GC pressure** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **GC pressure**.
...
4. What is a common misuse of **GC pressure**?
5. Where would you use **GC pressure** in a real system?
...
Create the smallest strict TypeScript example demonstrating **GC pressure**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
