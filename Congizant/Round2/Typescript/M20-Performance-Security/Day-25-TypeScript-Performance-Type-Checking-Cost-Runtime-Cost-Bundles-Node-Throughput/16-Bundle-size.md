...
**Bundle size** is the TypeScript concept **Bundle size**.

---

# 2. Why

Interviewers use **Bundle size** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Bundle size** means the TypeScript concept **Bundle size**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Bundle size** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Bundle size**.
...
1. What is **Bundle size**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Bundle size**?
5. Where would you use **Bundle size** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Bundle size**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Bundle size** in plain English.
