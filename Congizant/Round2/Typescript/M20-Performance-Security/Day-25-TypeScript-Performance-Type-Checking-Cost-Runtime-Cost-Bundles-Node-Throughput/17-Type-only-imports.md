...
**Type-only imports** is the TypeScript concept **Type-only imports**.

---

# 2. Why

Interviewers use **Type-only imports** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Type-only imports** means the TypeScript concept **Type-only imports**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Type-only imports** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Type-only imports**.
...
1. What is **Type-only imports**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Type-only imports**?
5. Where would you use **Type-only imports** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Type-only imports**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Type-only imports** in plain English.
