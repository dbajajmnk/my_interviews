# Topic 01 — Type safety is not security
...
**Type safety is not security** is distinguishing compile-time developer feedback from runtime trust enforcement.

---

# 2. Why

Interviewers use **Type safety is not security** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Type safety is not security** means distinguishing compile-time developer feedback from runtime trust enforcement. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

attacker input, DB rows, JSON, env vars and network messages can violate declared types; security requires runtime validation/authZ
...
In real TypeScript systems, this matters for security architecture.
...
- Reproduce the smallest case involving **Type safety is not security**.
...
1. What is **Type safety is not security**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Type safety is not security**?
5. Where would you use **Type safety is not security** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Type safety is not security**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Type safety is not security** in plain English.
