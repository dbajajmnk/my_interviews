# Topic 03 — Authentication vs authorization
...
**Authentication vs authorization** is proving caller identity at runtime.

---

# 2. Why

Interviewers use **Authentication vs authorization** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Authentication vs authorization** means proving caller identity at runtime. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

token/session/provider establishes principal; static `User` type does not authenticate anyone
...
- Reproduce the smallest case involving **Authentication vs authorization**.
...
4. What is a common misuse of **Authentication vs authorization**?
5. Where would you use **Authentication vs authorization** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Authentication vs authorization**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
