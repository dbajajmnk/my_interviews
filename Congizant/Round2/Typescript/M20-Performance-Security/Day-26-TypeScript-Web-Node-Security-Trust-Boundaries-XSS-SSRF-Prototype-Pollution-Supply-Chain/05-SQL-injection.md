...
**SQL injection** is the TypeScript concept **SQL injection**.

---

# 2. Why

Interviewers use **SQL injection** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **SQL injection** means the TypeScript concept **SQL injection**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **SQL injection** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **SQL injection**.
...
4. What is a common misuse of **SQL injection**?
5. Where would you use **SQL injection** in a real system?
...
Create the smallest strict TypeScript example demonstrating **SQL injection**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
