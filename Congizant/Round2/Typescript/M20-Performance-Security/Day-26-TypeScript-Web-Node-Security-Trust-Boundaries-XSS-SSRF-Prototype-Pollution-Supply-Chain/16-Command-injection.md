...
**Command injection** is the TypeScript concept **Command injection**.

---

# 2. Why

Interviewers use **Command injection** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Command injection** means the TypeScript concept **Command injection**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Command injection** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Command injection**.
...
4. What is a common misuse of **Command injection**?
5. Where would you use **Command injection** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Command injection**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
