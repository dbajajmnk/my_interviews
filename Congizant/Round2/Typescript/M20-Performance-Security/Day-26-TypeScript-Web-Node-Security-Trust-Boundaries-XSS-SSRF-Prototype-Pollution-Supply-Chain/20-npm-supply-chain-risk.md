...
**npm supply-chain risk** is the TypeScript concept **npm supply-chain risk**.

---

# 2. Why

Interviewers use **npm supply-chain risk** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **npm supply-chain risk** means the TypeScript concept **npm supply-chain risk**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **npm supply-chain risk** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **npm supply-chain risk**.
...
4. What is a common misuse of **npm supply-chain risk**?
5. Where would you use **npm supply-chain risk** in a real system?
...
Create the smallest strict TypeScript example demonstrating **npm supply-chain risk**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
