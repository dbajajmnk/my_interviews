...
**Stateless services** is the TypeScript concept **Stateless services**.

---

# 2. Why

Interviewers use **Stateless services** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Stateless services** means the TypeScript concept **Stateless services**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Stateless services** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Stateless services**.
...
1. What is **Stateless services**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Stateless services**?
5. Where would you use **Stateless services** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Stateless services**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
