# Topic 14 — Typed event contracts
...
**Typed event contracts** is compile-time representation of event payloads.

---

# 2. Why

Interviewers use **Typed event contracts** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Typed event contracts** means compile-time representation of event payloads. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

types help producers/consumers during development but brokers carry bytes; runtime schema/version validation remains required
...
```typescript
type Event={type:'OrderCreated';version:1;payload:OrderCreatedV1};
```
...
In real TypeScript systems, this matters for messaging.
...
- Reproduce the smallest case involving **Typed event contracts**.
...
1. What is **Typed event contracts**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Typed event contracts**?
5. Where would you use **Typed event contracts** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Typed event contracts**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Typed event contracts** in plain English.
