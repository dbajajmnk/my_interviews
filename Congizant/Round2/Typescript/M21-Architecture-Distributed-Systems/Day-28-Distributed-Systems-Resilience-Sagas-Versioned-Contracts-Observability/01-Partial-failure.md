...
**Partial failure** is making all properties optional at type level.

---

# 2. Why

Interviewers use **Partial failure** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Partial failure** means making all properties optional at type level. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

mapped type transforms property modifiers and is erased at runtime
...
In real TypeScript systems, this matters for patch/update DTO types.
...
- Reproduce the smallest case involving **Partial failure**.
...
4. What is a common misuse of **Partial failure**?
5. Where would you use **Partial failure** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Partial failure**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
