...
**Retry budgets** is bounded repeated attempts for transient failure.

---

# 2. Why

Interviewers use **Retry budgets** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Retry budgets** means bounded repeated attempts for transient failure. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
- Reproduce the smallest case involving **Retry budgets**.
...
4. What is a common misuse of **Retry budgets**?
5. Where would you use **Retry budgets** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Retry budgets**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
