...
**Circuit breaker** is runtime failure-isolation state machine.

---

# 2. Why

Interviewers use **Circuit breaker** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Circuit breaker** means runtime failure-isolation state machine. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

static types describe breaker API, while health/failure thresholds/state transitions are runtime signals
...
- Reproduce the smallest case involving **Circuit breaker**.
...
4. What is a common misuse of **Circuit breaker**?
5. Where would you use **Circuit breaker** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Circuit breaker**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
