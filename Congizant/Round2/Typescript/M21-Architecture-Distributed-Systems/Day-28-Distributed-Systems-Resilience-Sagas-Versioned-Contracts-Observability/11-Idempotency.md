...
**Idempotency** is making retries duplicate-safe.

---

# 2. Why

Interviewers use **Idempotency** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Idempotency** means making retries duplicate-safe. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
- Reproduce the smallest case involving **Idempotency**.
...
4. What is a common misuse of **Idempotency**?
5. Where would you use **Idempotency** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Idempotency**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
