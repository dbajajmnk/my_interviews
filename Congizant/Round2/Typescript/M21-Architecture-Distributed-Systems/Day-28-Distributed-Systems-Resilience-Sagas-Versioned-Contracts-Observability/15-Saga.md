...
**Saga** is distributed business workflow with compensation.

---

# 2. Why

Interviewers use **Saga** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Saga** means distributed business workflow with compensation. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

typed step/event states improve exhaustiveness, but compensation and consistency are runtime business behavior
...
```typescript
type SagaState={kind:'reserved'}|{kind:'charged'}|{kind:'failed';reason:string};
```
...
In real TypeScript systems, this matters for distributed workflows.
...
- Reproduce the smallest case involving **Saga**.
...
4. What is a common misuse of **Saga**?
5. Where would you use **Saga** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Saga**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
