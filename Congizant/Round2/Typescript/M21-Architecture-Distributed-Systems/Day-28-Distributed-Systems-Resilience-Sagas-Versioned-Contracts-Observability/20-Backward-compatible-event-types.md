# Topic 20 — Backward-compatible event types
...
**Backward-compatible event types** is the TypeScript concept **Backward-compatible event types**.

---

# 2. Why

Interviewers use **Backward-compatible event types** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Backward-compatible event types** means the TypeScript concept **Backward-compatible event types**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Backward-compatible event types** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Backward-compatible event types**.
...
1. What is **Backward-compatible event types**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Backward-compatible event types**?
5. Where would you use **Backward-compatible event types** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Backward-compatible event types**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Backward-compatible event types** in plain English.
