# Topic 09 — Runtime config validation
...
**Runtime config validation** is validating env/config despite compile-time types.

---

# 2. Why

Interviewers use **Runtime config validation** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Runtime config validation** means validating env/config despite compile-time types. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

process.env and deployment values are runtime strings/undefined; parse/validate before creating trusted config
...
In real TypeScript systems, this matters for production safety.
...
- Reproduce the smallest case involving **Runtime config validation**.
...
1. What is **Runtime config validation**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Runtime config validation**?
5. Where would you use **Runtime config validation** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Runtime config validation**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Runtime config validation** in plain English.
