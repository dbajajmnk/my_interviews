...
**Kubernetes awareness** is orchestrating Node runtime replicas.

---

# 2. Why

Interviewers use **Kubernetes awareness** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Kubernetes awareness** means orchestrating Node runtime replicas. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.

---

# 4. Engineering Explanation

Kubernetes manages process/container lifecycle and traffic; TypeScript types are irrelevant to probe/resource behavior
...
- Reproduce the smallest case involving **Kubernetes awareness**.
...
4. What is a common misuse of **Kubernetes awareness**?
5. Where would you use **Kubernetes awareness** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Kubernetes awareness**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
