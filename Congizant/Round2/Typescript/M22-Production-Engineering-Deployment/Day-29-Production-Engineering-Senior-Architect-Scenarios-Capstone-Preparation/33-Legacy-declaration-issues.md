# Topic 33 — Legacy declaration issues
...
**Legacy declaration issues** is describing public TypeScript types in `.d.ts` output.

---

# 2. Why

Interviewers use **Legacy declaration issues** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Legacy declaration issues** means describing public TypeScript types in `.d.ts` output. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
- Reproduce the smallest case involving **Legacy declaration issues**.
...
1. What is **Legacy declaration issues**?
2. What does TypeScript guarantee here?
3. What remains purely runtime JavaScript behavior?
4. What is a common misuse of **Legacy declaration issues**?
5. Where would you use **Legacy declaration issues** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Legacy declaration issues**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
...
- [ ] I can define **Legacy declaration issues** in plain English.
