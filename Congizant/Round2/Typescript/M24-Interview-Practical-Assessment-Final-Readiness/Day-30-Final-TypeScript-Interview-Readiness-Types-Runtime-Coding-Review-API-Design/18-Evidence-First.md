...
**Evidence First** is the TypeScript concept **Evidence First**.

---

# 2. Why

Interviewers use **Evidence First** to test whether you understand both TypeScript's compile-time guarantees and the emitted JavaScript/runtime behavior. Senior answers must distinguish static safety from runtime truth.

---

# 3. Plain English

Plain English: **Evidence First** means the TypeScript concept **Evidence First**. TypeScript can prove some relationships before execution, but it cannot make unvalidated external data or runtime effects safe by declaration alone.
...
In TypeScript projects, **Evidence First** should be tied to a concrete contract, runtime boundary, maintainability, debugging, performance, or production requirement.
...
- Reproduce the smallest case involving **Evidence First**.
...
4. What is a common misuse of **Evidence First**?
5. Where would you use **Evidence First** in a real system?
...
Create the smallest strict TypeScript example demonstrating **Evidence First**. Inspect or reason about the emitted JavaScript. Then introduce one realistic misuse, reproduce the compiler/runtime failure, fix it, and explain which review rule would have caught it.
